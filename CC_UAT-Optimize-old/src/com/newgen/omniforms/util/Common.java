...
//frameCollapsibleAdjusment(childFrameLRSP, parentFrameLRSP, childFrameToExpLRSP, childFrameToVisLRSP, fragSequenceLRSP, allframesLRSP, formObject);
   ...
   public void frameAdjusment(String[] childFrame, String parentFrame, String[] childFrameToVis, String[] frag, String[] allframes, FormReference formObject) {
       ...
       CreditCard.mLogger.info("In frameAdjusment");
       int prevTop = formObject.getTop(childFrame[0]);
       int prevHeight = 0;
       //int flag = 0;
       CreditCard.mLogger.info(childFrame[0] + " prevTop " + prevTop);
       CreditCard.mLogger.info("Child Frame To Visible " + childFrameToVis.length);

       if (childFrameToVis.length == 0) {
           for (int i = 0; i < childFrame.length; i++) {
               if (formObject.isVisible(childFrame[i])) {
                   CreditCard.mLogger.info("ChildFrame " + childFrame[i] + " is Visible");
                   formObject.setTop(childFrame[i], prevTop + prevHeight);
                   CreditCard.mLogger.info("ChildFrame " + childFrame[i] + " moved up");

                   prevTop = formObject.getTop(childFrame[i]);
                   prevHeight = formObject.getHeight(childFrame[i]) + 2;
                   CreditCard.mLogger.info("ChildFrame " + childFrame[i] + " prevTop " + prevTop + " prevHeight " + prevHeight);

               }
           }

           CreditCard.mLogger.info("parentFrame " + parentFrame + " to be set prevTop " + prevTop + " prevHeight " + prevHeight);
           formObject.setHeight(parentFrame, prevTop + prevHeight + 2);

       } else {
           CreditCard.mLogger.info("In else");
           for (int i = 0; i < childFrame.length; i++) {
               if (formObject.isVisible(childFrame[i])) {
                   CreditCard.mLogger.info("ChildFrame " + childFrame[i] + " is Visible");
                   formObject.setTop(childFrame[i], prevTop + prevHeight);
                   ...
                   CreditCard.mLogger.info("ChildFrame " + childFrame[i] + " is InVisible");
                   for (String childToVis : childFrameToVis) {
   ...
   public static String execBRMSWebService(int rule_id, String inputXml, String inputObj) {
       ...
       try {
           ...
           // CreditCard.mLogger.info("Executing query");
           ...
           CreditCard.mLogger.info("Creating tags from the table");
           ...
           CreditCard.mLogger.info("strResponseMsg" + strResponseMsg);
       } catch (Exception ex) {
           ex.printStackTrace();
       }
