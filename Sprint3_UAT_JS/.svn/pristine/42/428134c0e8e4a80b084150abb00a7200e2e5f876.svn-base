...
function validateApprovedLimitSupplementary(){
	...
	function validateSuppEntry(){
		var lsitViewname = 'SupplementCardDetails_cmplx_SupplementGrid';
		var suppRows = getLVWRowCount(lsitViewname);
		if(suppRows>0){
			var gridValues = [];
			...
			for(var i=0;i<suppRows;i++){
				if(getNGListIndex(lsitViewname)>-1 && i==getNGListIndex(lsitViewname)){
					continue;
				}
				gridValues.push(getLVWAT(lsitViewname,i,3)+':'+getLVWAT(lsitViewname,i,22));
			}
...
function calcAgeAtMaturity(maturityDate){
		var dob=getNGValue('cmplx_Customer_DOb');
		//var maturityDate=getNGValue('cmplx_LoanDetails_maturitydate');
		var today = new Date();
		var parts = dob.split('/');
		var parts2 = maturityDate.split('/');
		var oldDate=new Date(parts[2],parts[1]-1,parts[0]);
		var newDate=new Date(parts2[2],parts2[1]-1,parts2[0]);
		if(oldDate>=today)
			{
			com.newgen.omniforms.util.showError("cmplx_Customer_DOb","Date cannot be Current or future date");
			document.getElementById("cmplx_Customer_DOb").value=null;
			}
	   
	   var age = newDate.getFullYear() - oldDate.getFullYear();
		var m = newDate.getMonth() - oldDate.getMonth();
		if (m < 0 || (m == 0 && today.getDate() < oldDate.getDate())) {
			age--;
			m= oldDate.getMonth() - newDate.getMonth();
		}
		return  age+'.'+m;
}
...
function MaturityDate(FirstRepaymentDate,Tenor)
{
...
{
    //alert('$'+activityName+'$');
    if (activityName == 'CSM' || activityName == 'Interest_Rate_Approval' || activityName == 'Telesales_Agent_Review' || activityName == 'CSM_Review' ||activityName == 'DSA_CSO_Review' || activityName == 'RM_Review' || activityName == 'Collection_Agent_Review' || activityName == 'Relationship_Manager' || activityName == 'Rejected_Application' || activityName == 'Reject_Queue' || activityName == 'SalesCoordinator')
    {
    ...
    if (activityName == 'DDVT_maker' || activityName == 'DDVT_Checker')
    {
        //setSheetVisible("Tab1", 6, false); //System Checks
		setSheetVisible("Tab1", 7, false); // Contact Point Verification
...
//change by saurabh on 19th Dec
   if (activityName == 'Disbursal' || activityName == 'CC_Disbursal')
    {
	setSheetVisible("Tab1", 7, false); // Contact Point Verification
        setSheetVisible("Tab1", 8, false); // Fraud Control Unit
        setSheetVisible("Tab1", 13, false); //CAD
   ...
   if (activityName == 'CAD_Analyst1' || activityName == 'CAD_Analyst2' )
    ...
    if (activityName == 'Relationship_Manager')
    {
        setSheetVisible("Tab1", 4, false); // loan
        setSheetVisible("Tab1", 6, false); //System Checks
        setSheetVisible("Tab1", 7, false); // Contact Point Verification
    ...
    if (activityName == 'SalesCoordinator')
    {
        setSheetVisible("Tab1", 4, false); // loan
        setSheetVisible("Tab1", 6, false); //System Checks
        setSheetVisible("Tab1", 7, false); // Contact Point Verification
...
function uploadDocument(){
				...
				var firstName = getNGValue('FirstName');
				var lastName = getNGValue('LAstName');
	...
	function Fatca_disableCondition(){
			...
			setLocked("cmplx_FATCA_decforIndv", true);
			...
			setLocked("cmplx_FATCA_lossofnationality",true);
			setNGValue("cmplx_FATCA_lossofnationality", false);
	...
	function Fatca_Enable(){
			...
			setNGValue("cmplx_FATCA_iddoc", false);
			setLocked("cmplx_FATCA_decforIndv", false);
			setNGValue("cmplx_FATCA_decforIndv", false);
			setLocked("cmplx_FATCA_w8form",false);
			setNGValue("cmplx_FATCA_w8form", false);
			setLocked("cmplx_FATCA_w9form",false);
			setNGValue("cmplx_FATCA_w9form", false);
			setLocked("cmplx_FATCA_lossofnationality",false);
			setNGValue("cmplx_FATCA_lossofnationality", false);
