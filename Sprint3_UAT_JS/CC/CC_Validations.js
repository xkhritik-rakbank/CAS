...
var Firco_fields= ["cmplx_Customer_FirstNAme","cmplx_Customer_MiddleNAme","cmplx_Customer_LastNAme","cmplx_Customer_DOb","cmplx_Customer_gender","cmplx_Customer_COUNTRYOFRESIDENCE","cmplx_Customer_PAssportNo","cmplx_Customer_PassPortExpiry","cmplx_Customer_Nationality","compName","estbDate"];
...
function checkMandatory_Add()
{
	  ...
	  var activityName=window.parent.stractivityName;
		...
		else if(activityName=='Smart_CPV')
		{
			...
			if(getNGValue('cmplx_DEC_Decision')!='Smart CPV Hold')
			{
			var identifier=false;
			var smart_grid=getLVWRowCount('cmplx_SmartCheck_SmartCheckGrid');
			for(var i=0;i<smart_grid;i++)
	...
	if(n>0)
		{
           for(var i=0;i<n;i++)
			...
			if(getLVWAT('DecisionHistory_Decision_ListView1',n-1,3)=='Refer' && getLVWAT('DecisionHistory_Decision_ListView1',i,12)!='Y' && getLVWAT('DecisionHistory_Decision_ListView1',n-1,2)==activityname && (getNGValue('DecisionHistory_ReferTo')=='Smart CPV' ||getNGValue('DecisionHistory_ReferTo')=='Mail Approval' || getNGValue('DecisionHistory_ReferTo')=='Credit' || getNGValue('DecisionHistory_ReferTo')=='DDVT_Maker'|| getNGValue('DecisionHistory_ReferTo')=='CPV Checker'))
				{
           ...
           for(var i=0;i<n;i++)
			{
				...
				if(getLVWAT('DecisionHistory_Decision_ListView1',n-2,2)==activityname && getLVWAT('DecisionHistory_Decision_ListView1',n-2,3)=='Refer' && getLVWAT('DecisionHistory_Decision_ListView1',n-2,12)!='Y')
				{
					if(getLVWAT('DecisionHistory_Decision_ListView1',parseInt(com.newgen.omniforms.formviewer.getNgSelectedIndices('DecisionHistory_Decision_ListView1')),3)!=getNGValue('cmplx_DEC_Decision')){
					showAlert('','Cannot add two decision values!!!');
					return false;
					}
				}
...
function DMandatoryCC(){
	...
	//Added By nikita for passport No/Mobile No/EID on change CRN/ECRN Reset Start 22 Aug 2021
	/* if(activityName=='DDVT_Maker')
	 {
		 if((PassportNo_old!=getNGValue('cmplx_Customer_PAssportNo') && PassportNo_old=='') || (MobileNo_old!=getNGValue('cmplx_Customer_MobileNo') && MobileNo_old=='') || (EID_old!=getNGValue('cmplx_Customer_EmiratesID') && EID_old==''))
		{
			if(getNGValue('CRN')!="" || getNGValue('ECRN')!="")
			{
				 if(reset_CRN_ECRN_Click==false)
				 {
					 showAlert('',alerts_String_Map['VAL401']);
					 return false;
				 }
				 if(modify_CRN_ECRN_Click==false)
				 {
					 showAlert('',alerts_String_Map['VAL403']);
					 return false;
				 }
				 return true;
			}
	 ...
	 //Added By nikita for passport No/Mobile No/EID on change CRN/ECRN Reset End 22 Aug 2021
	var newAddedrow=0;
	var ReferTo="";
	...
	else
	{
	 ...
	 var n=com.newgen.omniforms.formviewer.getLVWRowCount("cmplx_NotepadDetails_cmplx_notegrid");
			...
			for(var i=0;i<n;i++)
			{
				if(getLVWAT("cmplx_NotepadDetails_cmplx_notegrid",i,11)==activityName || (getLVWAT("cmplx_NotepadDetails_cmplx_notegrid",i,11)=='FCU' && activityName == 'FPU')){
					gridValue.push(getLVWAT("cmplx_NotepadDetails_cmplx_notegrid",i,11));
					var set=i;
				}
	...
	if(((activityName!='FCU'&& activityName!='FPU') || activityName!='Compliance'))
	 {
		...
		/*	if((getNGValue('DecisionHistory_ReferTo')!='Smart CPV' && getNGValue('DecisionHistory_ReferTo')!='Mail Approval') 
			&& (getNGValue('cmplx_DEC_ReferTo')=='' || getNGValue('cmplx_DEC_ReferTo')=='--Select--') && getNGValue('DectechCategory')!='A')
			{
		 ...
		 if (getNGValue('cmplx_DEC_Decision') == 'Reject')
		{
			
		if(update_customer==false)
		{
			showAlert('DecisionHistory_Button3', "Please update customer!");
			return false;
		}
	...
	else if(((activityName=='FCU'||activityName=='FPU') || activityName=='Compliance') && getNGValue('cmplx_DEC_Decision')=='Refer' && validateReferGridonRefer())
	{
	...
	//Added by shivang for PCASP-2719
  ...
  /*  if (getNGValue('cmplx_DEC_Decision') == null || getNGValue('cmplx_DEC_Decision') == '' || getNGValue('cmplx_DEC_Decision') == '--Select--' )
	...
	//done by sagarika for pcsp-360
		else if (activityName=='Original_Validation')
		{
			...
			if(SecurityGridCount==0 && getNGValue('Sub_Product')!='LI' && (getNGValue('Sub_Product')=='IM' && activityName=='Original_Validation') && getNGValue('cmplx_Customer_VIPFlag')==false && getLVWAT("cmplx_Product_cmplx_ProductGrid",0,2)!='SEC' ){
	...
	else if(activityName=='Disbursal')
	{
		...
		if(rows>0){
			for(var i=0;i<rows;i++){
				if(getLVWAT(lvname,i,19)=='Y' && getLVWAT(lvname,i,12)=='Supplement')
				{
					//donothing
				}
				else{
				//if(getLVWAT(lvname,i,8)!='Y' || getLVWAT(lvname,i,9)!='Y' || getLVWAT(lvname,i,10)!='Y'){
	...
	else if(activityName=='DDVT_Maker')
	{
		...
		if((getNGValue('cmplx_IncomeDetails_StatementCycle') == '--Select--' || getNGValue('cmplx_IncomeDetails_StatementCycle') == null)&&(getLVWAT('cmplx_Product_cmplx_ProductGrid',0,6) !='Self Employed')){
			showAlert('cmplx_IncomeDetails_StatementCycle','Statements Cycle can not be blank');
					return false;
		}else if((getNGValue('cmplx_IncomeDetails_StatementCycle2') == '--Select--' || getNGValue('cmplx_IncomeDetails_StatementCycle2') == null)&&(getLVWAT('cmplx_Product_cmplx_ProductGrid',0,6) =='Self Employed')){
			showAlert('cmplx_IncomeDetails_StatementCycle2','Statements Cycle can not be blank');
					return false;
		}
		...
		if(!Customer_Save_Check1())
		{
			...
			//Deepak temp commented for FIRCO removal
		/*if(getNGValue('FIRCO_dec')==null || getNGValue('FIRCO_dec')==''){
			showAlert('Customer_FetchFirco', 'Please Refresh Firco Call Details!');
			return false;
		}
			if(flag_firco!=true )
			{
				showAlert('Customer_FircoStatus', 'Please check Firco Status for primary first.');
				return false;
			}
			*/
		if (getLVWAT('cmplx_Product_cmplx_ProductGrid',0,6)!='Self Employed' && getNGValue('cmplx_Customer_VIPFlag')==false){
			//Changed by Shivang for PCASP-2040
			if(getNGValue('Sub_Product')=='SEC'){
				if(!checkMandatory(CC_EMPLOYMENTDETAILS_SEC)){			
					return false;
				}
			}
			if(getNGValue('Sub_Product')!='SEC'){
		...
		if(!Income_Save_Check())
		 ...
		 if (getNGValue('EmploymentType').indexOf('Self')>-1 ){
	...
	
{
		...
		else if (getNGValue('AlternateContactDetails_Email2')=='' && activityName=='DDVT_Maker')
		{
			showAlert('AlternateContactDetails_Email2', "Secondary Mail Id cannot be blank!");
			return false;
		}
	...
	if(getNGValue('Sub_Product')=='SEC' && getLVWRowCount('ELigibiltyAndProductInfo_ListView5')==0)
	{
	showAlert('','Lien details are mandatory for secured cards!');
	return false;
	}
	//Added by shivang
	if(getLVWAT('cmplx_Product_cmplx_ProductGrid',0,6)=='Salaried'){
	...
	{
	var row_count_lien=getLVWRowCount('cmplx_FinacleCore_liendet_grid');
	var lien_found=false;
	if(row_count_lien>0)
	{
	for(var i=0 ; i<row_count_lien;i++)
	{
		if(getLVWAT('cmplx_FinacleCore_liendet_grid',i,7)=='true')
		{
		lien_found=true;
		}
	}
	...
	if(!lien_found)
	{
		showAlert('', 'Please Select Lien from System check to be considered!');
				return false;
	}
	}
...
if(!checkFor_SupplemntGrid()){
		...
		if(getNGValue('is_cc_waiver_require')!='Y' && getNGValue('Sub_Product')!='PU'){//changed from LI to PU by akshay on 3/5/18 for proc 8978
			...
			if(crngridcount>0){
				for(var i=0;i<crngridcount;i++){
					...
					if((getLVWAT('cmplx_CardDetails_cmplx_CardCRNDetails',i,1)=='' || getLVWAT('cmplx_CardDetails_cmplx_CardCRNDetails',i,2)=='') && getLVWAT('cmplx_CardDetails_cmplx_CardCRNDetails',i,9)!='Supplement'){
			...
			var SecurityGridCount=getLVWRowCount('cmplx_CardDetails_cmpmx_gr_cardDetails');
			if(SecurityGridCount==0 && getNGValue('Sub_Product')!='LI' && getNGValue('Sub_Product')!='IM' && !Check_Elite_Customer() && getNGValue('cmplx_Customer_VIPFlag')==false && getLVWAT("cmplx_Product_cmplx_ProductGrid",0,2)!='SEC' ){
				showAlert('CardDetails_BankName', alerts_String_Map['CC264']);
				return false;
			}
		}
	if(!checkForNA_Grids('cmplx_FATCA_cmplx_FATCAGrid')){
			return false;
		}
   ...
   if(isVisible('cmplx_CompanyDetails_cmplx_CompanyGrid') && activityName=='DDVT_Maker'){
			for(var i=0;i<getLVWRowCount('cmplx_CompanyDetails_cmplx_CompanyGrid');i++){
				if(getLVWAT('cmplx_CompanyDetails_cmplx_CompanyGrid',i,1)=='Business'){
					if(getLVWAT('cmplx_CompanyDetails_cmplx_CompanyGrid',i,40)==''){
						showAlert('cmplx_CompanyDetails_cmplx_CompanyGrid', 'Company Code not Created in ALOC. Kindly Create in Finacle to Proceed!');
						return false;
					}
				}
			}
	...
	else if(activityName=='DDVT_Checker')
	{
		...
		//deepak temp coment for Firco
		/*
		if(flag_supp_firco!=true)
			{
				var alert_flag=false;
				var count=getLVWRowCount("cmplx_DEC_MultipleApplicantsGrid");
				for(var i=0;i<count;i++)
				{
					if(getLVWAT("cmplx_DEC_MultipleApplicantsGrid",i,0)=='Supplement')
					{
						alert_flag=true;
						break;
					}					
				...
				if(alert_flag==true)
				{
				showAlert('DecisionHistory_FircoStatus', 'Please check Firco Status for Supplementary first.');
				return false;
				}
	...
	//below code addded for JIRa Income/employment check
		...
		if(isVisible('cmplx_CompanyDetails_cmplx_CompanyGrid') && activityName=='CAD_Analyst1'){
			for(var i=0;i<getLVWRowCount('cmplx_CompanyDetails_cmplx_CompanyGrid');i++){
				if(getLVWAT('cmplx_CompanyDetails_cmplx_CompanyGrid',i,1)=='Business'){
		...
		if(getNGValue('all_call_status')!='' && getNGValue('cmplx_Customer_EmiratesID')!='')
		{
			...
			showAlert('cmplx_DEC_HighDeligatinAuth','Call failed for : '+fail_status +'  Kindly Re-Run');
				return false;
		}
		...
		else if(desc=='Refer to Smart CPV')
		{
		 ...
		 //validation for customer detail verification CPV changes 16-04 - Changed on 29/7/21 - CPV Changes Req 1 (hidden)
...
*/		
	...
	else if(activityName=='Smart_CPV')
	{
		...
		if(getLVWAT("cmplx_SmartCheck_SmartCheckGrid",i,1)=='' && getNGValue("cmplx_DEC_Decision")!='Smart CPV Hold')
			{
...
//added 6th sept for proc 2021
	...
	//12th september
		...
		if(update_customer==false && getNGValue('cmplx_DEC_Decision') == 'Reject')
		{
			showAlert('DecisionHistory_Button3', "Please update customer!");
			return false;
		}
	...
	else if(activityName=='CPV_Analyst' && getNGValue('EmploymentType')=='Salaried')
	{
	 if (getNGValue('cmplx_OffVerification_Decision') != 'Not Applicable'  && getNGValue('cmplx_DEC_Decision') == 'Waiver')
	 {
		showAlert('cmplx_OffVerification_Decision', "Office Verification Decision must be NA");
			return false;
	 }
	 if (getNGValue('cmplx_OffVerification_reamrks') == ''  && getNGValue('cmplx_DEC_Decision') == 'Waiver')
	 {
		showAlert('cmplx_OffVerification_reamrks', "Office Verification Remarks is Mandatory!");
			return false;
	 }
	}
	...
	if(activityName=='Smart_CPV' && getNGValue("cmplx_DEC_Decision")!='Smart CPV Hold')
	{
...
//below code commented by akshay on 23/11/18
	...
	*/
	...
	if((activityName=='FCU'||activityName=='FPU') || activityName=='Compliance' || activityName=='DSA_CSO_Review' || activityName=='CPV_Analyst' || activityName=='RM_Review' || (activityName=='Telesales_Agent' && getNGValue('cmplx_DEC_Decision')=='Information updated')){
	...
	if(activityName=='Smart_CPV' )
	{
		...
		else if (getNGValue('cmplx_DEC_NoOfAttempts_Smart')!=''&& NoOfAttemptsValue==1){
		var toSet;
...
function checkMandatory_Reference_Details_Save(){
	...
	if(Principal_Approval=='Y' && activityName=='CSM')
	{
		...
		/*if(getLVWAT("cmplx_Product_cmplx_ProductGrid",0,0)=='Islamic'){
			...
			//Deepak changes done to ristrict card embossing name to 19 digit.
			if(getNGValue('cmplx_CardDetails_cardEmbossing_name').length>19 && activityName=='DDVT_Maker' ){
				showAlert('cmplx_CardDetails_cardEmbossing_name','Card Embossing name length cannot be more than 19 digit! Kindly update.');
				return false;
				
			}
			//change by saurabh on 7th Jan
			else if(getLVWAT("cmplx_CardDetails_cmplx_CardCRNDetails",0,0).indexOf('KALYAN')>-1 && com.newgen.omniforms.formviewer.isEnabled('cmplx_CardDetails_CompanyEmbossing_name')){
		...
		/*if((activity=='DDVT_Maker') && getLVWAT("cmplx_Product_cmplx_ProductGrid",0,1)=='') // Added by Rajan
		{
		    showAlert('CardDetails_Save', 'Generate CRN!');
		}
		if((activity=='DDVT_Maker') && getLVWAT("cmplx_Product_cmplx_ProductGrid",0,2)=='')// Added by Rajan
		{
		    showAlert('CardDetails_Save', 'Generate ECRN!');
		}*/
		//--above code added by nikhil for Self-Supp CR
			...
			if(n==0 && getNGValue("cmplx_CardDetails_Security_Check_Held")==true && getLVWAT("cmplx_Product_cmplx_ProductGrid",0,2)!='SEC'){ //Added by Rajan for PCASP-1556
				showAlert('cmplx_CardDetails_cmpmx_gr_cardDetails','Security Cheque Details '+alerts_String_Map['CC097']);
				return false;
				}
			}		
		if(getLVWAT('cmplx_Product_cmplx_ProductGrid',0,2)=='BTC' && getNGValue('cmplx_CardDetails_CompanyEmbossing_name')==''){
		showAlert('cmplx_CardDetails_CompanyEmbossing_name',alerts_String_Map['CC297']);	
		return false;
		}
			//12th september
			return true;
...
{	//code added by saurabh on 7th Nov 2017.
	...
	if(activityName=='CAD_Analyst1' && getNGValue('CompanyDetails_aloc')!='Y')
	{
		showAlert('CompanyDetails_Aloc_search','ALOC Search is Mandatory!');
		return false;
	}
		...
		if(getLVWAT('cmplx_Product_cmplx_ProductGrid',0,2)=='SE'){//pcasp-2385
			 if(getNGValue('TargetSegmentCode')=='DOC' && (getNGValue('headOffice')=='' || getNGValue('headOffice')=='--Select--') && isVisible('headOffice')){
				showAlert('headOffice',alerts_String_Map['VAL231']);	
				return false;
			}else if((getNGValue('CompanyDetails_EffectiveLOB')==null || getNGValue('CompanyDetails_EffectiveLOB')=='')  && isVisible('CompanyDetails_EffectiveLOB') ){
				showAlert('CompanyDetails_EffectiveLOB',alerts_String_Map['PL410']);	
				return false;
			} else if((getNGValue('lob')==null ||  getNGValue('lob')=='') && isVisible('lob')){
	...
	if(getNGValue('CompanyDetails_CheckBox4')==true ||  getLVWAT('cmplx_Product_cmplx_ProductGrid',0,2)=='BTC'){
		...
		//changed by shivang for PCASP-1549
		else if(flag_Company_FetchDetails==false && activityName!='CAD_Analyst1'){
...
function checkMandatory_ContactDetails_save(){
	...
	if (getNGValue('AlternateContactDetails_Email2')=='' && activityName=='DDVT_Maker')
	{
		showAlert('AlternateContactDetails_Email2', "Secondary Mail Id cannot be blank!");
        return false;
	}
...
//++Below code added by nikhil 13/11/2017 for Code merge
	...
	{	
		var n=com.newgen.omniforms.formviewer.getLVWRowCount('cmplx_FATCA_cmplx_FATCAGrid');
			...
			for(var i=0;i<n;i++)
			{
				gridValue.push(getLVWAT("cmplx_FATCA_cmplx_FATCAGrid",i,0));
			}
	...
	function AuthSignatory_checkMandatory()
	{
		...
		{	
			if(getNGValue('AuthorisedSignDetails_Name')=="")
			{
				...
				{if (getLVWAT("cmplx_Product_cmplx_ProductGrid",0,6)!='Self Employed') // Added by Rajan
					{
...
//Changes Done by aman for preferred choice
		...
		for(var i=0; i<gridRowCount;i++)
		{
			...
			for(var i=0; i<gridRowCount;i++){
				...
				if(getLVWAT('cmplx_AddressDetails_cmplx_AddressGrid',i,10) == 'true' ){
						//prefferedFlag = true;
						//added by prabhakar
						if(getLVWAT('cmplx_AddressDetails_cmplx_AddressGrid',i,13).indexOf('P-')!=-1){
							allPreffered++;
						}
	...
	//changed by shivang for PCASP-1967
	else if((getNGValue('cmplx_Customer_NEP')== 'NEWJ' || activityName=='DDVT_Maker') && getNGValue('cmplx_Customer_EmirateIDExpiry')== '')
		{
		   showAlert('cmplx_Customer_EmirateIDExpiry_Cont','Emirates ID Expiry Date cannot be blank') ;
		   return false;
		}
	...
	if(getNGValue('cmplx_Customer_VisaNo')=='' && getNGValue('cmplx_Customer_GCCNational')!='Y' && getNGValue('cmplx_Customer_Nationality')!='AE' && getNGValue('cmplx_Customer_RESIDENTNONRESIDENT')!='N'){
				...
				else if(getNGValue('cmplx_Customer_VisaNo')==''){	//Condition Changed By Shivang as Visa No. is available still alert message is coming
					showAlert('cmplx_Customer_VisaNo',alerts_String_Map['VAL154']);
					return false;
				}	
	...
	if(getNGValue('cmplx_Customer_VisaIssueDate')=='' && getNGValue('cmplx_Customer_GCCNational')!='Y' && getNGValue('cmplx_Customer_Nationality')!='AE' && getNGValue('cmplx_Customer_RESIDENTNONRESIDENT')!='N'){
				showAlert('cmplx_Customer_VisaIssueDate','Visa Issue Date is Mandatory!!');
				return false;
				}				
	//changed by nikhil 25/11/2018 to avoid visa validation for UAE NAtionals			
...
return true;
	...
	// added by abhishek as per CC FSD	
//Changed BY shivang for PCASP-1634	
...
//Added By Shivang for PCASP-1445
...
{	
		if((getNGValue('cmplx_Customer_NTB')=='false' || !getNGValue('cmplx_Customer_NTB') ) && getLVWAT('cmplx_Product_cmplx_ProductGrid',0,2)!='IM' )//PCASP-3191
		{
	...
	/*commented by shivang
	 if((getNGValue('transtype')=='SC' && (getNGValue('transferMode')=='S' || getNGValue('transferMode')=='A'))
		||(getNGValue('transtype')=='CCC' && (getNGValue('transferMode')=='S' || getNGValue('transferMode')=='A'))){
...
function DDS_Save_CC()
	...
	{	
		...
		var value = getNGValue('cmplx_CC_Loan_SI_Percentage');
		
		if((value.length>0) && value<3 || value >100)
		{
		showAlert('cmplx_CC_Loan_SI_Percentage','SI Percentage value should be between 3 and 100!');
			return false;
		}
...
//above code also fix point "30-Service Details#Validations not as per FSD."
 ...
 function ValidateRepeaterAvgNBC(){
	 ...
	 var Currentyear=(new Date()).getFullYear();
		 ...
		 for(var j = 1 ; j<13; j++){
			 ...
			 cellValue = document.getElementById(cellName).value;
		var  cellDisabled=	document.getElementById(cellName).readOnly;
 ...
 function CheckMandatory_Verification(s1,s2,s3)
 {
	 ...
	 {
		 if(getNGValue(ver[i])=='Mismatch' && (getNGValue(update[i]).trim()=='' || getNGValue(update[i])=='--Select--'))//&& getNGValue((ver[i].split('_')[0]+'_val')!=''))
		 {
	...
	function validateReferGrid()
	{
		...
		if(n>0){
			for(var i=0;i<n;i++){
				...
				if( getLVWAT('cmplx_ReferHistory_cmplx_GR_ReferHistory',i,5)==activity && getLVWAT('cmplx_ReferHistory_cmplx_GR_ReferHistory',i,6)=='' && getNGValue("cmplx_DEC_Decision")!="Customer Hold-Document requested"){
...
function checkFor_SupplemntGrid()
	...
	var listViewName = 'SupplementCardDetails_cmplx_supplementGrid';
		...
		for(var i=0;i<getLVWRowCount(listViewName);i++){
			...
			else if(getLVWAT(listViewName,i,7)=='' && getLVWAT(listViewName,i,8)=='AE')//Emirates ID
			{
				showAlert('SupplementCardDetails_Frame1','Please add Emirates ID');
				return false;
			}
			
			else if(getLVWAT(listViewName,i,38)=='' && getLVWAT(listViewName,i,8)=='AE' && getLVWAT(listViewName,i,5)!='AE' )//ID Issue date
			{
			...
			else if(getLVWAT(listViewName,i,37)=='' && getLVWAT(listViewName,i,8)=='AE' &&  getLVWAT(listViewName,i,5)!='AE')//ID Issue date
			{
			...
			else if(getLVWAT(listViewName,i,18)=='' && getLVWAT(listViewName,i,8)=='AE')//ID Issue date
			{
				showAlert('SupplementCardDetails_Frame1','Please add Emirates Expiry Date');
				return false;
			}
			else if(getLVWAT(listViewName,i,23)=='' && getLVWAT(listViewName,i,8)=='AE' &&  getLVWAT(listViewName,i,5)!='AE')//ID Issue date
			{
				showAlert('SupplementCardDetails_Frame1','Please add Visa Expiry Date');
				return false;
			}
			else if(getLVWAT(listViewName,i,22)=='' && getLVWAT(listViewName,i,8)=='AE' && getLVWAT(listViewName,i,5)!='AE' )//ID Issue date
			{
...
case 'PRIMARY': 
				...
				//changed by shivang for PCASP-1354
				else if(!(getNGValue('cmplx_Customer_Nationality')=='AE')){
...
//Below code added by shweta	
function calAgeAtMaturity(){	
	var custAge = getNGValue('cmplx_Customer_Age');
	if(isFieldFilled('cmplx_Customer_Age')==false){
		return "";
	}
	var Tenor = getNGValue('cmplx_EligibilityAndProductInfo_Tenor');
	if(isFieldFilled('cmplx_EligibilityAndProductInfo_Tenor')==false){
	...
	var parts = custAge.split('.');
	var tenorYear=0;
	var tenureMonths=0;
	if(Tenor!=0){
		 //tenorYear= Tenor/12;
		 tenorYear= Math.floor(Tenor/12); //Changed by shivang
		 tenureMonths = Tenor % 12; // 6
	}
	var ageAtMaturity=parseInt(parts[0])+tenorYear;
	var calcYear=0;
	var calcMonth=tenureMonths;
	if(parseInt(tenureMonths)+parseInt(parts[1])>=12){
		calcYear=(tenureMonths+parseInt(parts[1]))/12;
		calcMonth=(tenureMonths+parseInt(parts[1]))%12;
	}
	if(calcMonth==0)
		ageAtMaturity =ageAtMaturity+calcYear+'.00';
	else if(calcMonth<10){
		 ageAtMaturity =ageAtMaturity+calcYear+'.0'+calcMonth;
	}
	else
	    ageAtMaturity =ageAtMaturity+calcYear+'.'+calcMonth;
...
function Income_Save_Check()
	...
	if(Principal_Approval=='Y' && activityName=='CSM')
	{
		if(getNGValue('cmplx_IncomeDetails_totSal')=="" && getNGValue("Sub_Product")!='SEC')
		{
			showAlert("cmplx_IncomeDetails_totSal","Total Salary is mandatory");
			return false;
		}
		else if(isFieldFilled('cmplx_IncomeDetails_grossSal')==false && getNGValue("Sub_Product")!='SEC')
		{
			showAlert('cmplx_IncomeDetails_grossSal',alerts_String_Map['CAS005']);
			return false;
		}
		//Added by shivang for PCASP-2440,FLV
		else if(isFieldFilled('cmplx_IncomeDetails_netSal1')==false && getNGValue("Sub_Product")!='SEC' && getNGValue("Sub_Product")!='BPA')
		{
			showAlert('cmplx_IncomeDetails_netSal1',alerts_String_Map['CAS006']);
			return false;
		}
		else if(isFieldFilled('cmplx_IncomeDetails_Accomodation')==false && getNGValue("Sub_Product")!='SEC')
		{
		...
		//Changed by Shivang for PCASP-2440
	    else if(isFieldFilled('cmplx_IncomeDetails_SalaryDay')==false && getNGValue("Sub_Product")!='SEC')
		{
		...
		var AppType=getNGValue('Application_Type');
			if(EmpType=='Salaried' || EmpType=='Salaried Pensioner'){
				if(getNGValue("Sub_Product")=='SEC'){
				 if(!checkMandatory(CC_INCOME_SEC_SALARIED)){
					return false;}
				}
				else{
				...
				//below changes done by shivang for PCASP-2440
				if(getNGValue('cmplx_IncomeDetails_grossSal')=="" && getNGValue("Sub_Product")=='BPA')
					{
						showAlert("cmplx_IncomeDetails_grossSal","Gross Salary cannot be blank");
						return false;
					}
					if(getNGValue('cmplx_IncomeDetails_totSal')=="" && getNGValue("Sub_Product")!='SEC' && getNGValue("Sub_Product")!='BPA')
					{
						showAlert("cmplx_IncomeDetails_totSal","Total Salary is mandatory");
						return false;
					}
					if(getNGValue('cmplx_IncomeDetails_Basic')=="" && getNGValue("Sub_Product")!='SEC' && getNGValue("Sub_Product")!='BPA')
					{
					...
					//Changed by shivang for PCASP-2440
					if(getNGValue('cmplx_IncomeDetails_AvgNetSal')=="" && getNGValue("Sub_Product")!='SEC' && getNGValue("Sub_Product")!='BPA')
					{
						showAlert("cmplx_IncomeDetails_AvgNetSal","Average Net Salary is mandatory");
						return false;
					}
					//Done by aman for JIRA 020818
					//below code added by nikhil PCSP-89
					//Changed by shivang for PCASP-2440,2479
					if(getNGValue('cmplx_IncomeDetails_netSal1')=="" && getNGValue("Sub_Product")!='SEC' && getNGValue("Sub_Product")!='BPA')
					{
						showAlert('cmplx_IncomeDetails_netSal1',alerts_String_Map['VAL373']);
						return false;
					}
					//done by sagarika for PCSP-209
					if(getNGValue("Sub_Product")!='SEC' && (getNGValue('cmplx_IncomeDetails_SalaryXferToBank')=="" || getNGValue('cmplx_IncomeDetails_SalaryXferToBank')=="--Select--"))
					{
	 ...
	 if((EmpType=='Self Employed'  || AppType=='RSEL' || AppType=='RELT' || AppType=='RELTN') && (getLVWAT('cmplx_CompanyDetails_cmplx_CompanyGrid',1,24)!='Surrogate' && getLVWAT('cmplx_CompanyDetails_cmplx_CompanyGrid',1,25)!='DOC')){
				...
				if(activityName == 'CAD_Analyst1')
				 {
					/*commented by shivang for PCASP-1912
					if(AppType=='RELTN' && getNGValue('cmplx_IncomeDetails_NoOfMonthsOtherbankStat2')==""){
				 		showAlert('cmplx_IncomeDetails_NoOfMonthsOtherbankStat2','No Of Months Other bank Statements Is Mandatory');
						return false;
				 	}*/
	...
	if(EmpType=='Self Employed' && activityName == 'CAD_Analyst1'){
		if((getNGValue("Sub_Product")=="SE"&&getLVWAT('cmplx_CompanyDetails_cmplx_CompanyGrid',1,21)=="BAU")||getNGValue("Sub_Product")=="BTC" || (getNGValue("Sub_Product")=="PA" &&(AppType!="RELT"&& AppType!="RELTN" && AppType!="RFL" && AppType!="RFLP" && AppType!="MOR")))
		{
			if(getNGValue('cmplx_IncomeDetails_NoOfMonthsOtherbankStat2')==""){
				showAlert('cmplx_IncomeDetails_NoOfMonthsOtherbankStat2','No Of Months of bank Statements Is Mandatory');
				return false;
			}
		}
	}
	if(EmpType=='Salaried' && activityName == 'CAD_Analyst1'){
		if(getNGValue("Sub_Product")=="BTC" || (getNGValue("Sub_Product")=="PA" &&(AppType!="RELT"&& AppType!="RELTN" && AppType!="RFL" && AppType!="RFLP" && AppType!="MOR")))
		{
			if(getNGValue('cmplx_IncomeDetails_NoOfMonthsOtherbankStat')==""){
				showAlert('cmplx_IncomeDetails_NoOfMonthsOtherbankStat','No Of Months of bank Statements Is Mandatory');
				return false;
			}
		}
	}
...
{
	if(getLVWAT(Grid_name,i,2)=='Secondary')
	{
	count++;
	break;
	}
