function FPUTemplate_PL(){
	...
	attrbList += "&<cust_visemi>&" + get_ngformValue_template('cmplx_CustDetailverification1_EID_ID');
	//added by pooja on 8 sept 2021
	attrbList += "&<Application_type>&" + get_ngformValue_template('cmplx_cust_ver_sp2_application_type');
	...
	//pooja for 3503 Getting the value
	attrbList += "&<emp_verification>&" + get_ngformValue_template('cmplx_emp_ver_sp2_off_drop');
	// end here
	...
	attrbList += "&<field_visited>&" + get_ngformValue_template('cmplx_fieldvisit_sp2_drop2');
...
function FPUTemplate(){
	...
	//attrbList = getVerificationData(attrbList,'cust_visemi','cmplx_CustDetailverification1_EID_ID','cmplx_CustDetailverification1_cmplx_CustDetailverification1_EID_ID_drop','cmplx_CustDetailverification1_cmplx_CustDetailverification1_EID_ID_remarks');
	//Added By pooja 
		attrbList = getVerificationData(attrbList,'Application_type','cmplx_cust_ver_sp2_application_type','cmplx_cust_ver_sp2_application_type_drop','cmplx_cust_ver_sp2_application_type_drop_remarks');
    //Ended here
	...
	attrbList += "&<los_at_prev_emp_upd>&" + get_ngformValue_template('cmplx_cust_ver_sp2_prevemp_los_remark');
	...
	attrbList += "&<authorized_person_desi>&" + get_ngformValue_template('EmploymentVerification_s2_auth_name');//pooja 3503
	attrbList += "&<office_address_type>&" + get_ngformValue_template('EmploymentVerification_s2_off_add');//pooja 3503
	...
	attrbList += "&<empl_fpu_remarks>&" + get_ngformValue_template('cmplx_emp_ver_sp2_FPU_remarks_comp_mismatch');//pooja3503
	if(getLVWAT('cmplx_Product_cmplx_ProductGrid',0,6)=='Self Employed' )
	{
		//Customer section
		attrbList += "&<SE_NumEmp>&" + get_ngformValue_template('cmplx_cust_ver_sp2_No_of_employee');
	...
	var propertyValue='';
	  var applType=get_ngformGridColumn_template("cmplx_Product_cmplx_ProductGrid",0,4);
	   ...
	   attrbList += "&<Segment>&" + "NA"+"@10";//get_ngformGridColumn_template("cmplx_FinacleCore_FinaclecoreGrid",0,14);//PCASP-2834
	    attrbList += "&<FD_Amount>&" + get_ngformValue_template('cmplx_FinacleCore_tot_fd_amt');// by jahnavi for FD changes in SE
	    var propValueSE='';
	   ...
	   attrbList += "&<TotalCrTO3>&" + totCrTurnover3 + "@10";
		attrbList += "&<FDAmountValue>&" + get_ngformValue_template('cmplx_FinacleCore_tot_fd_amt'); // by jahnavi for RAK elite changes
		attrbList += "&<AUM>&" + get_ngformValue_template('cmplx_FinacleCore_Total_AUM');
	var Query = "TotalAUM";
	wparams="WiName=="+com.newgen.omniforms.formviewer.getNGValue('WiLabel');
	var jspName="/webdesktop/custom/CustomJSP/CustomSelect.jsp";
	var params="Query="+Query+"&wparams="+wparams+"&pname="+pname;
...
//attrbList += "&<EMI>&" + get_ngformValue_template('cmplx_LoanDetails_loanemi');
	if(getLVWAT("cmplx_Product_cmplx_ProductGrid",0,2)=='IM'){
...
{ 
	...
	if(docName=="Application_Form_Islamic" || docName=="Application_Form" || docName=="Application_Form_IM"){
	...
	attrbList += "&<Passportexp>&" + emirateIDExpiry;
	...
	var ccn='';
	for(var i=0;i< getLVWRowCount('cmplx_CC_Loan_cmplx_btc');i++){
		if(get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_btc',i,0)=='LOC')
			ccn=get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_btc',i,1);
		
	}
			
	attrbList += "&<CreditCardNo>&" + ccn; 
	attrbList += "&<CompName>&" + get_ngformValue_template('cmplx_EmploymentDetails_EmpName');
	attrbList += "&<grossSalinc>&" + get_ngformValue_template('cmplx_IncomeDetails_totSal');
	attrbList += "&<Passportexp>&" + get_ngformValue_template('cmplx_Customer_PassPortExpiry');
	attrbList += "&<Visaexp>&" + get_ngformValue_template('cmplx_Customer_VIsaExpiry');
	attrbList += "&<Totfinlimfi>&" + get_ngformValue_template('cmplx_EligibilityAndProductInfo_FinalLimit');
	var s=getNGValue('cmplx_EligibilityAndProductInfo_FinalLimit');
	 s = s.toString();
    s = s.replace(/[\, ]/g,'');
...
var dg = ['zero','one','two','three','four', 'five','six','seven','eight','nine'];
 ...
 var tw = ['twenty','thirty','forty','fifty', 'sixty','seventy','eighty','ninety'];
    ...
    str.replace(/\s+/g,' ');
	//above function by jahnavi to convert to words
attrbList += "&<Totfinlimwo>&" +str;
attrbList += "&<othertenor>&"+ get_ngformValue_template('cmplx_EligibilityAndProductInfo_Tenor');
attrbList += "&<othercb>&"+"true@10";

var Acc_no='';
for(var i=0;i< getLVWRowCount('cmplx_CC_Loan_cmplx_btc');i++){
		if(get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_btc',i,0)=='LOC' && get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_btc',i,2)=='A' && get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_btc',i,3)=='RAKBANK')
			Acc_no=get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_btc',i,23);
		
	}
	attrbList += "&<RakAccountno>&"+Acc_no;
	var iban='';
	var banknameim='';
	for(var i=0;i< getLVWRowCount('cmplx_CC_Loan_cmplx_btc');i++){
		if(get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_btc',i,0)=='LOC' && get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_btc',i,2)=='A' && get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_btc',i,3)!='RAKBANK')
			iban=get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_btc',i,9);
		banknameim=get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_btc',i,3);
		
	}
	attrbList += "&<othercb>&"+"Yes@10";
	attrbList += "&<IBANnumber>&"+iban;
	attrbList += "&<Benbankname>&"+ banknameim;
	var si_count=getLVWRowCount('cmplx_CC_Loan_cmplx_si');
	attrbList += "&<SIAccountno>&" + get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_si',si_count-1,0);
	var due_or_si_day='';
attrbList += "&<SIDay>&"+ get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_si',si_count-1,6);
if(getLVWAT('cmplx_CC_Loan_cmplx_si',si_count-1,2)=='Yes')
	attrbList += "&<duedatecb>&"+ 'Yes@10';
else
	attrbList += "&<duedatecb>&"+ 'No@10';
/*---------------LOAN GRID FOR IM BY JAHNAVI------------------------------------------------------------------------------------*/

var Query="LoanDeatilsForIMFormEMI";
var wparams="Wi_Name=="+com.newgen.omniforms.formviewer.getNGValue('WI_name');
var params="Query="+Query+"&wparams="+wparams+"&pname="+pname;
...
if(fields[0]!='' && lf>=1)
attrbList += "&<Monthly_Installment_EMI>&" + retVal +"@10";
if(fields[1]!='' && lf>1)
attrbList += "&<EMI2>&" + fields[1] +"@10";
if(fields[2]!='' && lf>2)
attrbList += "&<EMI3>&" + fields[2] +"@10";
if(fields[3]!='' && lf>3)
attrbList += "&<EMI4>&" + fields[3] +"@10";
if(fields[4]!='' && lf>4)
attrbList += "&<EMI5>&" + fields[4] +"@10";
if(fields[5]!='' && lf>5)
attrbList += "&<EMI6>&" + fields[5] +"@10";


var Query="LoanDeatilsForIMFormOUTSTA";
var wparams="Wi_Name=="+com.newgen.omniforms.formviewer.getNGValue('WI_name');
var params="Query="+Query+"&wparams="+wparams+"&pname="+pname;
var DB_Query = CallAjax(jspName,params);
DB_Query = trimStr(DB_Query);
...
var lf=fields.length;
if(fields[0]!='' && lf>=1)
attrbList += "&<Monthly_Installment_Outstanding>&" + retVal +"@10";
if(fields[1]!='' && lf>1)
attrbList += "&<OUTSTANDAMT2>&" + fields[1] +"@10";
if(fields[2]!='' && lf>2)
attrbList += "&<OUTSTANDAMT3>&" + fields[2] +"@10";
if(fields[3]!='' && lf>3)
attrbList += "&<OUTSTANDAMT4>&" + fields[3] +"@10";
if(fields[4]!='' && lf>3)
attrbList += "&<OUTSTANDAMT5>&" + fields[4] +"@10";
if(fields[5]!='' && lf>5)
attrbList += "&<OUTSTANDAMT6>&" + fields[5] +"@10";
...
attrbList += "&<Company>&"+ get_ngformValue_template('cmplx_EmploymentDetails_EmpName');
...
//changes done by Gaurav for IM App form changes starts

if(docName=="Application_Form_IM" )
{
	attrbList += "&<Passportexp>&" + emirateIDExpiry;

	if(com.newgen.omniforms.formviewer.getNGValue('cmplx_EmploymentDetails_Emp_Type')=='S')
	{
	attrbList += "&<EMploymentDetails_DeptName>&" + com.newgen.omniforms.formviewer.getNGValue('cmplx_EmploymentDetails_DepartmentName');
	}
	
    attrbList += "&<ProcessingFee>&" + com.newgen.omniforms.formviewer.getNGValue('cmplx_EligibilityAndProductInfo_LPFAmount');
	
	attrbList += "&<FinalInteresrRate>&" + com.newgen.omniforms.formviewer.getNGValue('cmplx_EligibilityAndProductInfo_InterestRate');
	
	// for getting the description of IM code *****************************
	
	var IM_Desc ='';
	var instantMoneyValue = get_ngformGridColumn_template('cmplx_Product_cmplx_ProductGrid',0,2);
	if (instantMoneyValue == 'IM@10'){
		 IM_Desc = 'Instant Money';
	}


	attrbList += "&<SubProduct>&" + IM_Desc;
	
	// end here ***********************************
	attrbList += IM_Additional_Liability();
    ...
    for(var i=0;i< getLVWRowCount('cmplx_CC_Loan_cmplx_btc'); i++)
	{
		if(get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_btc',i,0)=='LOC@10' && get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_btc',i,2)=='A@10' ){
			Acc_no=get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_btc',i,23);
		    attrbList += "&<RakAccountno>&"+Acc_no;
	        attrbList += "&<RakAccountnoChec>&"+'true@10';
		}else{
			Acc_no='';
		    attrbList += "&<RakAccountno>&"+Acc_no;
	        attrbList += "&<RakAccountnoChec>&"+'false@10';
		}
	}
	...
	var ir_count = getLVWRowCount('cmplx_CC_Loan_cmplx_btc')
		if(get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_btc',ir_count-1,0)=='LOC@10' && get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_btc',ir_count-1,2)=='S@10' ){
			iban=get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_btc',ir_count-1,9);
			banknameim=get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_btc',ir_count-1,3);
			attrbList += "&<IBANnumberChec>&"+'true@10';
			attrbList += "&<BenbanknameChec>&"+'true@10';
			attrbList += "&<IBANnumber>&"+iban;
			attrbList += "&<Benbankname>&"+ banknameim;
		}
		else{
		iban='';
		banknameim='';
	    attrbList += "&<IBANnumberChec>&"+'false@10';
		attrbList += "&<BenbanknameChec>&"+'false@10';
	    attrbList += "&<IBANnumber>&"+iban;
	    attrbList += "&<Benbankname>&"+ banknameim;
		}

  // for first point 
		var creditCardNo_count = getLVWRowCount('cmplx_Product_cmplx_ProductGrid')
		if(get_ngformGridColumn_template('cmplx_Product_cmplx_ProductGrid',creditCardNo_count-1,2)=='IM@10')
			{
				var cc = getLVWAT('cmplx_CC_Loan_cmplx_btc',0,1)
				var masked = 'XXXXXXXXXXXX'+cc.substr(-4);
				attrbList += "&<CreditCardNo>&"+ masked;
			}
		// second point 
		attrbList += "&<em_id>&"+ 'true@10';
		
		
	// for tenor start ********************************************
	
	var tenour =  get_ngformValue_template('cmplx_EligibilityAndProductInfo_Tenor');
	if (tenour == '6@10'){
		attrbList += "&<6_months>&"+ 'true@10';
		
	} 
	if(tenour == '12@10'){
		attrbList += "&<12_months>&"+ 'true@10';
		
	}
	if(tenour == '18@10'){
		attrbList += "&<18_months>&"+ 'true@10';
		
	}
	if(tenour == '24@10'){
		attrbList += "&<24_months>&"+ 'true@10';
		
	}
	if(tenour == '36@10'){
		attrbList += "&<36_months>&"+ 'true@10';
		
	}
	if (tenour > '36@10'){
		
		attrbList += "&<Other>&"+ 'true@10';
	}
//**************************************** tenor end *************

//******If new AED in bank check click*******************
	if(com.newgen.omniforms.formviewer.getNGValue('cmplx_customerConsents_Account_open_consent_for_security_check') ==true ){
		
		attrbList += "&<current_account_AED>&" +"true@10";
		
	}else{
		attrbList += "&<current_account_AED>&" +"false@10";
	}
	// *********************end here **************************
		
  
	var si_count=getLVWRowCount('cmplx_CC_Loan_cmplx_si');
	var accountNoValue = get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_si',si_count-1,0);
	attrbList += "&<SIAccountno>&" + accountNoValue;
	if (accountNoValue!= '@10'){
		attrbList += "&<SIAccChec>&"+ "true@10";
	}else{
		attrbList += "&<SIAccChec>&"+ "false@10";
	}
	
	
	var due_or_si_day='';
	var si_due_date = getLVWAT('cmplx_CC_Loan_cmplx_si',si_count-1,2);
	
          if(si_due_date=='Yes') 
		  {
			attrbList += "&<duedatecb>&"+ "true@10";
			attrbList += "&<sidaycb>&"+ "true@10";
	        attrbList += "&<SIDay>&"+ get_ngformGridColumn_template('cmplx_CC_Loan_cmplx_si',si_count-1,6);
          }else {
			attrbList += "&<duedatecb>&"+ "false@10";
			attrbList += "&<sidaycb>&"+ "false@10";
		    attrbList += "&<SIDay>&"+ due_or_si_day;
          }
		
	if(com.newgen.omniforms.formviewer.getNGValue('cmplx_customerConsents_Consent_for_Release_of_Statement_account') ==true ){
		attrbList += "&<DocType>&" +"true@10";
		attrbList += "&<Consent_release_statement>&" +"true@10";
		attrbList += "&<id_doc>&" + com.newgen.omniforms.formviewer.getNGValue('cmplx_Customer_EmiratesID');	
		attrbList += "&<CustName>&" + com.newgen.omniforms.formviewer.getNGValue('cmplx_Customer_FIrstNAme')+' '+com.newgen.omniforms.formviewer.getNGValue('cmplx_Customer_MiddleName')+' '+com.newgen.omniforms.formviewer.getNGValue('cmplx_Customer_LAstNAme');
		attrbList += "&<Consent_Iban>&" + com.newgen.omniforms.formviewer.getNGValue('cmplx_customerConsents_iban_number');
		attrbList += "&<Consent_StatementPer>&" + com.newgen.omniforms.formviewer.getNGValue('cmplx_customerConsents_StatementPeriod');
     }
	 
		var contactMe = com.newgen.omniforms.formviewer.getNGValue('cmplx_customerConsents_ContactMe');
	 	var emailClick  = com.newgen.omniforms.formviewer.getNGValue('cmplx_customerConsents_Email');
		var smsClicks  = com.newgen.omniforms.formviewer.getNGValue('cmplx_customerConsents_Sms');
		var telephoneClick = com.newgen.omniforms.formviewer.getNGValue('cmplx_customerConsents_TelephoneCalls');

		if(contactMe == true || contactMe=='true'){
			attrbList += "&<contactMeClick>&"+ 'true@10';
			
			if (smsClicks == true || smsClicks = 'true'){
				
				attrbList += "&<Consent_Sms>&"+ 'true@10';
			}
			if (emailClick== true || emailClick = 'true'){
				
				attrbList += "&<Consent_email>&"+ 'true@10';

			}
			if (telephoneClick == true || telephoneClick =='true'){
				
				attrbList += "&<Consent_TeleCalls>&"+ 'true@10';
			}
		
			
		}else{
			attrbList += "&<contactMeClick>&"+ 'false@10';
			attrbList += "&<Consent_email>&"+ 'false@10';
			attrbList += "&<Consent_Sms>&"+ 'false@10';
			attrbList += "&<Consent_TeleCalls>&"+ 'false@10';
		}
		
		var contactMeCoolingOf = com.newgen.omniforms.formviewer.getNGValue('cmplx_customerConsents_Cooling_Of_Period_Waiver_Form');
		if(contactMeCoolingOf =='true' || contactMeCoolingOf == true ){
			attrbList += "&<coolingChec>&"+ "true@10";
		}else{
			attrbList += "&<coolingChec>&"+ "false@10";
		}
		
		var consesntcoolingper = com.newgen.omniforms.formviewer.getNGValue('cmplx_customerConsents_ContactMeColingOf');
		if(consesntcoolingper =='Yes' || consesntcoolingper == 'Y' ){
			attrbList += "&<Consent_CoolingPer>&"+ "true@10";
		}else{
			attrbList += "&<Consent_CoolingPer>&"+ "false@10";
		}
}
//changes done by Gaurav for IM App form changes end
//changes done by shivang for Islamic App form changes starts
if(docName=="Application_Form_Islamic" || docName=="Application_Form"){
	if(mobileNo1!=''){
...
//Deepak below function added for Internal Liability section in IM form

function IM_Additional_Liability()
	 {
		 var attrbList_new = "";
		 if (com.newgen.omniforms.formviewer.getLVWRowCount("cmplx_Liability_New_cmplx_LiabilityAdditionGrid") > 0){
               for(var i=0; i <com.newgen.omniforms.formviewer.getLVWRowCount("cmplx_Liability_New_cmplx_LiabilityAdditionGrid"); i++){
					var TOC = com.newgen.omniforms.formviewer.getLVWAT("cmplx_Liability_New_cmplx_LiabilityAdditionGrid",i,0);
					var EMI = com.newgen.omniforms.formviewer.getLVWAT("cmplx_Liability_New_cmplx_LiabilityAdditionGrid",i,2);
                    var OutStanding = com.newgen.omniforms.formviewer.getLVWAT("cmplx_Liability_New_cmplx_LiabilityAdditionGrid",i,12);
					
					var Query = "Contract_type_Desc"
					wparams="Code=="+TOC;
					var jspName="/webdesktop/custom/CustomJSP/CustomSelect.jsp";
					var params="Query="+Query+"&wparams="+wparams+"&pname="+'CCTemplateData';
					var Contract_type_Desc_arr_Str = CallAjax(jspName,params);
					var Contract_type_Desc_arr = Contract_type_Desc_arr_Str.split('#');
					if(Contract_type_Desc_arr[0].indexOf("NODATA")>-1 || Contract_type_Desc_arr[0].indexOf("ERROR")>-1){
						Contract_type_Desc_arr[0]='';
					}
					TOC=Contract_type_Desc_arr[0];
                    
					
					attrbList_new = attrbList_new+"&<TOC"+(i+1)+">&"+TOC+"@10"+"&<EMI"+(i+1)+">&"+EMI+"@10"+"&<OUTSTANDAMT"+(i+1)+">&"+OutStanding+"@10";
					
			}
		 }
		 
		 return attrbList_new;
	 }
...
attrbList+= getAECBSummary(attrbList);
attrbList += "&<CAD_Decision_app>&" + get_ngformText_template('cmplx_Decision_Decision');
...
attrbList += "&<EMI>&" + get_ngformValue_template('cmplx_LoanDetails_loanemi'); //hritik
...
function RiskRatingTemplate(){
	var attrbList="";
	var pname="RiskRatingtemp";
	attrbList += "&<CIFNo>&" + get_ngformValue_template('cmplx_Customer_CIFNO');
	attrbList += "&<CustName>&" + get_ngformValue_template('cmplx_Customer_FIrstNAme')+' '+get_ngformValue_template('cmplx_Customer_MiddleName')+' '+get_ngformValue_template('cmplx_Customer_LAstNAme');
	attrbList += "&<DsaName>&" + get_ngformValue_template('cmplx_Customer_DSA_NAme');
	attrbList += "&<EmpType>&" + get_ngformValue_template('cmplx_EmploymentDetails_Emp_Type');
	attrbList += "&<BusinessSeg>&" + get_ngformValue_template('cmplx_RiskRating_BusinessSeg');
	attrbList += "&<SubSeg>&" + get_ngformValue_template('cmplx_RiskRating_SubSeg');
	attrbList += "&<demographics>&" + get_ngformValue_template('cmplx_RiskRating_Demographics');
	attrbList += "&<Industry>&" + get_ngformValue_template('cmplx_RiskRating_Industry');
	
	attrbList += "&<CUSTOMERTYPE>&" + get_ngformValue_template('cmplx_Customer_NEP');
	attrbList += "&<Currency>&" + get_ngformValue_template('ReqProd') +",Current Account,"+"AED";
	var Query = "CustomerCategory";
	var wparams="";
wparams="Code=="+getNGValue('cmplx_Customer_CustomerCategory');//by jahnavi
var jspName="/webdesktop/custom/CustomJSP/CustomSelect.jsp";
var params="Query="+Query+"&wparams="+wparams+"&pname="+pname;
var custcatquery = CallAjax(jspName,params);
var fields = custcatquery.split('#');

if(fields[0].indexOf("ERROR")>-1 || fields[0].indexOf("NODATAFOUND")>-1)
{
	fields[0]=getNGValue('cmplx_Customer_CustomerCategory');
}
	attrbList += "&<CustomerCategory>&" + fields[0]+'@10';
var Query = "Nationality";
	var wparams="";
wparams="Code=="+getNGValue('cmplx_Customer_Nationality');//by jahnavi
var jspName="/webdesktop/custom/CustomJSP/CustomSelect.jsp";
var params="Query="+Query+"&wparams="+wparams+"&pname="+pname;
var custcatquery = CallAjax(jspName,params);
var fields = custcatquery.split('#');
if(fields[0].indexOf("ERROR")>-1 || fields[0].indexOf("NODATAFOUND")>-1)
{
	fields[0]=getNGValue('cmplx_Customer_Nationality');
}
attrbList += "&<Nationality>&" +  fields[0]+'@10';	
	var pep=getLVWAT('cmplx_KYC_cmplx_KYCGrid',0,1);
	attrbList += "&<PEP>&" + pep;
	attrbList += "&<Channel>&" + get_ngformValue_template('InitChannelLabel');
	attrbList += "&<RiskScore>&" + get_ngformValue_template('cmplx_RiskRating_Total_riskScore');
	var risktype="";
	var score=parseInt(get_ngformValue_template('cmplx_RiskRating_Total_riskScore'));
	if( score>0 || score< 3)
		risktype="Low";
		...
		risktype="High";
	attrbList += "&<RiskType>&" +risktype;
	attrbList += "&<DDVTMaker>&" +get_ngformValue_template('UserLabel');
	attrbList += "&<DDVTChecker>&";
	attrbList += "&<Current_date_time>&" + getNGValue('CurrentDateLabel')+ ' '+new Date().getHours()+':'+new Date().getMinutes() +'@10';
	console.log("final attrbList for Risk Rating :: "+attrbList);
	return attrbList;
}
