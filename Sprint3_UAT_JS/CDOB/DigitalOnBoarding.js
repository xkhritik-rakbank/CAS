...
function focus_DigitalOnBoarding(pId) {
...
function click_DigitalOnBoarding(pId, frameState) {
    ...
    if (pId == 'SecNationality_Button_View' || pId == 'Nationality_Button_View' || pId == 'DesignationAsPerVisa_button_View' || pId == 'Designation_button_View' ||
    ...
    if (pId == 'Designation_button3_View' || pId == 'Designation_button4_View' || pId == 'Designation_button1' || pId == 'Designation_button2' || pId == 'Designation_button5_View' || pId == 'Designation_button6_View' || pId == 'Designation_button8_View') {
        return true;
    }
	...
	if(pId=='OriginalValidation_Customer_Update'){
		return true;
	}
    //sagarika on 17/06
    if (pId == 'cmplx_fieldvisit_sp2_drop1') { //alert("value");
        if (getNGValue(pId) == "Yes") { //alert("yes");
        ...
        } else if (getNGValue(pId) == "No") { //alert("no");
    ...
    if (pId == 'DecisionHistory_ADD') {
        ...
        if (activityName == 'CAD_Analyst1' && (getNGValue('Eligibility_Trigger') == 'Y' || getNGValue('Eligibility_Trigger') == '')) {
		    var man_dev = getNGValue('cmplx_DEC_Manual_Deviation')
             if (man_dev == true) {
			   showAlert('DecisionHistory_calReElig', alerts_String_Map['VAL388']);
			 }
			 else{
        ...
        if ((activityName == 'CPV' || activityName=='CPV_Analyst') && getNGValue('cmplx_DEC_Decision') == 'CPV Hold') //PCAS-1868 sagarika
        {
      ...
      /*  else if (activityName == 'Smart_CPV') {
        ...
        if (Add_Validate()) {
            if (checkMandatory_Add()) {
                ...
                //changed by nikhil for CAD1 only
                if ((getNGValue('cmplx_DEC_Weakness').indexOf(userid) == -1 || getNGValue('cmplx_DEC_Weakness') == '') && activityName == 'CAD_Analyst1') {
    ...
    //Below Code added by prabhakar for crn generation and kalayan
        ...
        return true;
    } else if (pId == 'Customer_CreateCIF') {
        //alert("Create CIf button clicked");
        return true;
	...
	//code added by bandana for CDOB application form starts
	 else if (pId == 'DecisionHistory_ApplicationGenerate'){
		...
		if(field_string_value.indexOf(value)==-1){
			...
			var docName = "";
			docName="Application_Form_CDOB";
			var attrbList = "";		
			attrbList += CDOBTemplateData(docName); 	
			...
			var ajaxReq;
				...
				if (ajaxReq.status == 200 && ajaxReq.readyState == 4)
				{
					...
					{
						alert("Application Form generated successfully");
					}
				}
				else
				{
	 ...
	 //code added by bandana for CDOB application form ends
    ...
    //Above Code added by prabhakar for crn generation and kalyan		
        ...
        if (checkMandatory(CDOB_LIABILITY)) //name change by isha
        {
            removeFrame(pId);
            return true;
        }
    ...
    //var flag1 = 'false';
        ...
        if (Customer_Save_Check1()) {
            removeFrame(pId);
			setNGValueCustom('is_Customer_Saved','Y');
            return true;
        } else
            return false;
    ...
    // added by akshay on 15/9/17 to add bt details in grid as per  FSD
        ...
        if (getNGValue('cmplx_EmploymentDetails_Freezone') == true) {
			...
			setNGValueCustom("cmplx_EmploymentDetails_FreezoneName", "");//changed 
    ...
    // Changed by aman for CR 1112
        ...
        if (checkMandatory_SupplementGrid()) {
            if (validateSuppEntry('add')) {
                ...
                /*if(age<18.6 && getNGValue("SupplementCardDetails_Nationality")=='AE')
                {
                   showAlert(pId,'Minor Customer not Applicable for Credit Card');
                	//setNGValueCustom(pId,'');
                	return false;
                }
                else if(age<21 && getNGValue("SupplementCardDetails_Nationality")!='AE')
                {
                	showAlert(pId,'Minor Customer not Applicable for Credit Card');
                	//setNGValueCustom(pId,'');
                	return false;
    ...
    if (pId == 'NotepadDetails_savebutton') {
        ...
        if (activityName == 'CPV' || activityName == 'FCU' || activityName == 'CPV_Analyst') {
    ...
    //++below code added by nikhil for PCAS-1212 CR
		...
		var selectedRow=com.newgen.omniforms.formviewer.getNGListIndex('cmplx_AddressDetails_cmplx_AddressGrid');
		       if(getLVWAT('cmplx_AddressDetails_cmplx_AddressGrid',selectedRow,0)=='Home' && (getLVWAT('cmplx_AddressDetails_cmplx_AddressGrid',selectedRow,5)=='OT' || getLVWAT('cmplx_AddressDetails_cmplx_AddressGrid',selectedRow,6)=='OT'))
{
	setEnabled("AddressDetails_city",true);
	setEnabled("AddressDetails_state",true);
	setEnabled("AddressDetails_addr_Modify",true);
	setEnabled("Button_City",true);
	setEnabled("Button_State",true);
	setEnabled("AddressDetails_Save",true);//  
}
else
{
	setEnabled("AddressDetails_city",false);
	setEnabled("AddressDetails_state",false);
	setEnabled("AddressDetails_addr_Modify",false);
	setEnabled("Button_City",false);
	setEnabled("Button_State",false);
	setEnabled("AddressDetails_Save",false);
}
    ...
    //change by saurabh on 3 aug.
        ...
        var n = getLVWRowCount("cmplx_Product_cmplx_ProductGrid");
	/*	   if(n==0 && activityName=='OV')
      {
      	showAlert('cmplx_Product_cmplx_ProductGrid', 'Please expand Product Details.');
      	return false;
      }*/
        if (n == 0 && activityName!='EFMS_Hold')//added by saurabh1 for pcasf-800 
		{
            showAlert('cmplx_Product_cmplx_ProductGrid', alerts_String_Map['VAL100']);
            setNGFrameState('Finacle_Core', 1);
            return false;
        }//modified by saurabh1 for pcasf-67
        else if(!isVisible('FinacleCore_Frame1')){
    ...
    //below code added by nikhil 16/1/17
        ...
        if (Income_Save_Check()) {
            removeFrame(pId);
			setNGValueCustom('is_Income_Saved','Y');
            return true;
        }
    }
    ...
    /* else if(pId=='IncomeDetails_SelfEmployed_Save'){
        ...
        if (checkMandatory(CC_ELIGIBILITY)) {
            ...
            */
            ...
            if ((getNGValue('Sub_Product') == 'IM' && validatePastDate('cmplx_EligibilityAndProductInfo_FirstRepayDate', 'Repayment')) || getNGValue('Sub_Product') != 'IM') {
                removeFrame(pId);
                return true;
    ...
    //sagarika
        ...
        if (!checkMandatory_Frames(getNGValue('Mandatory_Frames'))) {
		   ...
		   if(getNGValue("cmplx_Liability_New_Range")=='')
        {
        	showAlert('','AEBCB range is blank');
        }
        if(getNGValue("cmplx_Liability_New_ReferenceNo")=='')
        {
        	showAlert('','Bureau Reference number is blank');
        }
          if(getNGValue("cmplx_Liability_New_AECBScore")=='')
        {
        	showAlert('','Aecb Score is blank');
        }
		var ftsReceived=getNGValue('cmplx_Liability_New_isFTSReceived');
		//alert('fts received is '+ftsReceived);
		//if(ftsReceived=='Y'){
        ...
        if (getNGValue('FinacleCoreSave') != 'Y' && (getNGValue('Sub_Product') == 'BTC' || getNGValue('Sub_Product') == 'SE')) { //changed by akshay for proc 12490
        ...
        //Commented by Sajan as overrite not there in FALCON
        /*if(isVisible('Liability_New_Overwrite')==true && getNGValue('Liability_New_overwrite_flag')=='N')
        	{
        		showAlert('InternalExternalLiability',alerts_String_Map['VAL390']);
        		return false;
        ...
        if (getNGValue('IS_AECB') != 'Y' && getNGValue('cmplx_Customer_NTB') == true) {
            showAlert('InternalExternalLiability', alerts_String_Map['VAL106']);
            return false;
        }
		if (getNGValue('is_Customer_Saved') != 'Y'){
			showAlert('Customer_save','Please save Customer Details to run Dectech');
            return false;
		}
		if(getNGValue('is_Income_Saved') != 'Y'){
			showAlert('IncomeDetails_Salaried_Save','Please save Income Details to run Dectech');
            return false;
		}
        if (getNGValue('IS_Employment_Saved') != 'Y' && getLVWAT('cmplx_Product_cmplx_ProductGrid', 0, 6) != 'Self Employed') {
			...
			//alert('Final limit is '+getNGValue('ELigibiltyAndProductInfo_Text7'));
			//alert('Final limit IM is '+getNGValue('cmplx_EligibilityAndProductInfo_FinalLimit'));
            return false;
        }
		if(getNGValue('cmplx_EligibilityAndProductInfo_FinalLimit')!='' && getNGValue('cmplx_EligibilityAndProductInfo_FinalLimit')!=null)
		{
			//alert('the final limit is not zero or null');
		}
		else {
			//alert('final limit is null');
			showAlert('', 'Final Limit/ IM Amount cannot be null or zero');
			return false;
		}
		if(getNGValue('is_CA_Hold_Done')=='Y' && ftsReceived=='N' && parseInt(getNGValue('cmplx_EligibilityAndProductInfo_FinalLimit'))>1){
			showAlert('', 'As FTS is not received Final limit cannot be more than 1 AED');
			return false;
		}
		if(ftsReceived=='Y' && (getNGValue('cmplx_IncomeDetails_netSal1')=='' || getNGValue('cmplx_IncomeDetails_netSal1')==null || getNGValue('cmplx_IncomeDetails_netSal2')=='' || getNGValue('cmplx_IncomeDetails_netSal2')==null || getNGValue('cmplx_IncomeDetails_netSal3')=='' || getNGValue('cmplx_IncomeDetails_netSal3')==null)){
			setNGValueCustom('is_Income_Saved','N');
			showAlert('IncomeDetails_Salaried_Save', 'Net Salary 1,2 and 3 cannot be null to run Dectech');
            return false;
		}
        if (getLVWAT('cmplx_Product_cmplx_ProductGrid', 0, 6) == 'Self Employed' && (getLVWRowCount('cmplx_FinacleCore_FinaclecoreGrid') != 0 || getLVWRowCount('cmplx_FinacleCore_FinaclecoreGrid') != -1) && getLVWAT('cmplx_CompanyDetails_cmplx_CompanyGrid', 1, 22) != 'CAC' && getLVWAT('cmplx_CompanyDetails_cmplx_CompanyGrid', 1, 22) != 'DOC') {
            ...
            if (getNGValue('cmplx_FinacleCore_avg_credit_3month') == '' || getNGValue('cmplx_FinacleCore_avg_credit_6month') == '') {
        ...
        if (pId == 'ELigibiltyAndProductInfo_Button1' || pId == 'DecisionHistory_calReElig' ) {
    ...
    //change by saurabh on 6th Dec
        ...
        // ++ below code already present - 06-10-2017 below line of onsite commented
        //if(checkMandatory(CC_SI))
        if (SI_Save_CC()) {
            removeFrame(pId);
            return true;
        }
    } else if (pId == 'RVC_Save') {
        if (checkMandatory(CC_RVC)) {
            removeFrame(pId);
            return true;
        }
        // added by abhishek as per CC FSd
    ...
    /*else if(pId=='PartMatch_Search'){
        ...
        if (checkMandatory(CC_FINACLECORE_DDS)) {
         ...
         if (checkMandatory(CC_FINACLECORE_DDS)) //sagarika for jira-696
		 {
            var new_value = 'Finacle_Core';
            var field_string_value = getNGValue('FrameName');
            if (field_string_value.indexOf(new_value) == -1) {
    ...
    if (pId == 'DecisionHistory_save') {
        ...
        if (activityName == 'CAD_Analyst1') {
           ...
           // var cad_decision = getNGValue("cmplx_DEC_ReferTo");  pcasf-122,135
            if (decision == "REJECT") {
    ...
    /* combining it with NotepadDetails_Add1
        ...
        if (getNGValue('SmartCheck1_FCURemarks') != '' && activityName == 'FPU') {
      ...
      /*  if (activityName == 'CAD_Analyst1' && visitSystemChecks_Flag == false) { //ADDED BY AKSHAY ON 20/12/17
        ...
        if (parseInt(counter_dec) < parseInt(alerts_String_Map['VAL323'])) {
         //   setNGValueCustom('reEligibility_CC_counter', counter_elig + ';' + (parseInt(counter_dec) + 1));
		  setNGValueCustom('reEligibility_CC_counter', (parseInt(counter_elig) + 1).toString() + ';' + counter_dec);
            return true;
        } else {
        ...
        if (getNGValue('cmplx_Customer_VIPFlag') == false) {
            ...
            if (Principal_Approval == 'Y' && activityName == 'CSM') {
                if (getNGValue('cmplx_EmploymentDetails_EmpIndusSector') == '' || getNGValue('cmplx_EmploymentDetails_EmpIndusSector') == '--Select--') {
                    ...
                    return false;
                } else if (getNGValue('cmplx_EmploymentDetails_LengthOfBusiness') == '') {
            ...
            if (checkMandatory(CC_EMPLOYMENTDETAILS)) {
                ...
                if (getNGValue('cmplx_EmploymentDetails_EMpCode') == '') {
                    ...
                    return false;
                } //Arun (11/12/17)   Commented by Sajan for FALCON CDOB
                ...
                else if (getNGValue('cmplx_EmploymentDetails_targetSegCode') == 'CAC' && getNGValue('cmplx_EmploymentDetails_OtherBankCAC') == '') { //below changed for Wrong alert
                ...
                /*else if (getNGValue('cmplx_EmploymentDetails_LengthOfBusiness') == '') {     Commented by Sajan for FALCOn CDOB
                ...
                else if (getNGValue('cmplx_EmploymentDetails_targetSegCode') == 'EMPID' && (getNGValue('cmplx_EmploymentDetails_IndusSeg') == '' || getNGValue('cmplx_EmploymentDetails_IndusSeg') == '--Select--')) {
    ...
    if (pId == 'ExtLiability_Button1') {
        ...
        if (getNGValue('cmplx_Customer_VIPFlag') == true) {
            if (getNGValue('cmplx_Customer_NTB') == true) {
                ...
                showAlert('ExtLiability_Button1', alerts_String_Map['CAS026']);
                return false;
            } else {
    ...
    */
        ...
        if (getNGValue('IS_AECB') == 'Y') {
            ...
            //alert('url 3 ' + url);
            window.open_(url, "ACCOUNT_SUMMARY", "width=500,height=300");
            ...
            window.open_(url, "CARD_SUMMARY", "width=500,height=300");
        ...
        if (getNGValue('cmplx_CustDetailVerification_Decision') == 'Positive' || getNGValue('cmplx_CustDetailVerification_Decision') == 'Approve Sub to CIF') //changed by akshay on 25/6/18 for proc 9815
        {
            ...
            var name = "Mobile No. 1:Mobile No. 2:Date of Birth:P.O Box No.:Emirates:Personal / Company P.O Box No.:Office Telephone No.:Email Address 1:Email Address 2";
    ...
    // added by abhishek as per CC FSD
        ...
        if (getNGValue('cmplx_BussVerification_Decision') == 'Positive') //changed by akshay on 25/6/18 for proc 9815
        {
    ...
    //commented by abhishek as per CC FSD
		...
		if(getNGValue('cmplx_OffVerification_fxdsal_val')!='' && getNGValue('cmplx_OffVerification_fxdsal_val')==getNGValue('cmplx_OffVerification_fxdsal_upd')){
				showAlert('cmplx_OffVerification_fxdsal_upd', 'Updated and Old value cannot be same');
						return false;
			}
			
			if(getNGValue('cmplx_OffVerification_desig_val')!='' && getNGValue('cmplx_OffVerification_desig_val').toUpperCase()==getNGValue('cmplx_OffVerification_desig_upd').toUpperCase()){
				showAlert('cmplx_OffVerification_desig_upd', 'Updated and Old value cannot be same');
						return false;
			}
			
			if(getNGValue('cmplx_OffVerification_cnfrminjob_val')!='' && getNGValue('cmplx_OffVerification_cnfrminjob_val')==getNGValue('cmplx_OffVerification_cnfrminjob_upd')){
				showAlert('cmplx_OffVerification_cnfrminjob_upd', 'Updated and Old value cannot be same');
						return false;
			}
			if(getNGValue('cmplx_OffVerification_doj_val')!='' && getNGValue('cmplx_OffVerification_doj_val')==getNGValue('cmplx_OffVerification_doj_upd')){
				showAlert('cmplx_OffVerification_doj_upd', 'Updated and Old value cannot be same');
						return false;
			}
        ...
        if (getNGValue('cmplx_OffVerification_Decision') == 'Positive') //changed by akshay on 25/6/18 for proc 9815
        {
			...
			//Added by Sajan for PCASF-388
			if (getNGValue('cmplx_OffVerification_fxdsal_ver') == 'No' || getNGValue('cmplx_OffVerification_desig_ver') == 'No' || getNGValue('cmplx_OffVerification_cnfminjob_ver') == 'No' || 	getNGValue('cmplx_OffVerification_doj_ver') == 'No') {
						showAlert('cmplx_OffVerification_Decision', 'Decison cannot be positive');
						return false;
			}
            ...
            /*  if(getNGValue("cmplx_OffVerification_doj_ver")=='Yes' && (getNGValue("cmplx_OffVerification_doj_upd")==" " ||getNGValue("cmplx_OffVerification_doj_upd")==""))
				{
				showAlert("cmplx_OffVerification_doj_upd","DOJ update cannot be blank.");	
                return false;				
				}as suggested by Simi*/
	...
	*/
    ...
    //added by yash for CC FSD TEMPLATE
        ...
        /*if (getNGValue('ELigibiltyAndProductInfo_EFMS_Status') != 'Y' && activityName == 'CAD_Analyst1') {   //Commented by Sajan for FALCON CDOB
		...
		if(getNGValue('cmplx_DEC_Decision')=='Approve'){
			for(var i=0;i<getLVWRowCount('cmplx_AddressDetails_cmplx_AddressGrid');i++)
			{
				...
				{
					if(getLVWAT('cmplx_AddressDetails_cmplx_AddressGrid',i,5)=='OT' || (getLVWAT('cmplx_AddressDetails_cmplx_AddressGrid',i,6)=='OT'))
					{
						showAlert("cmplx_AddressDetails_cmplx_AddressGrid","Please modify the Address first!");
						return false;
					}
				}
			}
        ...
        var docName = "";
        var attrbList = "";
        attrbList += FalconTemplateData();
        attrbList = attrbList.replace(/\+/g, "SynPlus");
        attrbList = attrbList.replace(/\%/g, "SynPerc");
        attrbList = encodeURIComponent(attrbList);
		//changes done by Shivang for FALCON CAM
        /*if (activityName == 'CAD_Analyst1') {
            //change by saurabh on 7th Dec
            setNGValueCustom('Is_CAM_generated', 'Y');

            if ((getLVWAT('cmplx_Product_cmplx_ProductGrid', 0, 2) == 'Pre-Approved') || (getLVWAT('cmplx_Product_cmplx_ProductGrid', 0, 2) == 'PA')) {
            ...
            if ((getLVWAT('cmplx_Product_cmplx_ProductGrid', 0, 2) == 'Secured Card') || (getLVWAT('cmplx_Product_cmplx_ProductGrid', 0, 2) == 'SEC')) {
                ...
                //window.parent.executeAction('6', 'Y');
                //return true;
            } else if ((getLVWAT('cmplx_Product_cmplx_ProductGrid', 0, 2) == 'Pre-Approved') || (getLVWAT('cmplx_Product_cmplx_ProductGrid', 0, 2) == 'PA')) {
                docName = "CAM_REPORT_PRE_APPROVED";
                //window.parent.executeAction('5', 'Y');
                //return true;
            } else if ((getLVWAT('cmplx_Product_cmplx_ProductGrid', 0, 2) == 'Product Upgrade') || (getLVWAT('cmplx_Product_cmplx_ProductGrid', 0, 2) == 'PU')) {
		...
		//Below condition added by Sajan for PCASF-424
		if(activityName=='CAD_Analyst1' && getNGValue('cmplx_Customer_CIFNo')=='' && getNGValue('cmplx_DEC_Decision')=='Approve' && getNGValue('cmplx_Customer_Nationality')!='AE' && getNGValue('cmplx_Customer_Nationality')!='BH' && getNGValue('cmplx_Customer_Nationality')!='IQ' && getNGValue('cmplx_Customer_Nationality')!='KW' && getNGValue('cmplx_Customer_Nationality')!='OM' && getNGValue('cmplx_Customer_Nationality')!='QA' && getNGValue('cmplx_Customer_Nationality')!='SA' && (getNGValue('cmplx_Customer_VisaIssueDate')=='' || getNGValue('cmplx_Customer_VisaNo')=='' || getNGValue('cmplx_Customer_VisaExpiry')=='' || getNGValue('cmplx_Customer_PassportIssueDate')=='' || getNGValue('cmplx_Customer_PassPortExpiry')=='')){
			showAlert('','Please fill the VISA and passport details to generate CIF');
			return false;
		}
		else if(activityName=='CAD_Analyst1' && getNGValue('cmplx_Customer_CIFNo')=='' && getNGValue('cmplx_DEC_Decision')=='Approve' && getNGValue('cmplx_Customer_Nationality')=='AE' &&(getNGValue('cmplx_Customer_PassportIssueDate')=='' || getNGValue('cmplx_Customer_PassPortExpiry')=='')){
			showAlert('','Please fill the Passport details to generate CIF');
			return false;
		}
		//Changes End here PCASF-424
		if(getNGValue('cmplx_DEC_Decision')=='Approve' && activityName=='CAD_Analyst1'){
			return true;
		}
...
//ended here
function change_DigitalOnBoarding(pId) {
    setNGValueCustom(pId, getNGValue(pId));
	...
	//Added by Sajan for PCASF-414
	if(pId=='cmplx_Liability_New_isFTSReceived'){
		setNGValueCustom('Eligibility_Trigger','Y');
	}
	...
	//Added by Sajan for FALCON CDOB
        ...
        if(getNGValue('cmplx_Customer_Nationality')!='AE'){
			setNGValueCustom('cmplx_Customer_IdIssueDate',getNGValue('cmplx_Customer_VisaIssueDate'));
		}
    }
	else if (pId == 'cmplx_Customer_VisaExpiry') {
        if(getNGValue('cmplx_Customer_Nationality')!='AE'){
			setNGValueCustom('cmplx_Customer_EmirateIDExpiry',getNGValue('cmplx_Customer_VisaExpiry'));
		}
    }
    ...
    //sagarika CR change
        ...
        if (getNGValue(pId) == 'Yes') {
         ...
         /*   if (getNGValue('cmplx_EmploymentDetails_LOS') >= 0.06) {
           ...
           // }*/
				if(getNGValue('cmplx_OffVerification_cnfrminjob_val')=='true')
			{
        ...
        if (getNGValue(pId) == 'Yes') { //sagarika PCAS-3225
            ...
            setLocked("cmplx_OffVerification_doj_ver", true);
			 setLocked("cmplx_OffVerification_doj_upd", true);
			setEnabled("cmplx_OffVerification_doj_upd",false);
            setLocked("cmplx_OffVerification_cnfminjob_ver", true);
            setLocked("cmplx_OffVerification_desig_ver", true);
  ...
  if (getNGValue(pId) == 'Mismatch') {
			//Added by Sajan for PCASF-388
		if(getNGValue('cmplx_OffVerification_cnfrminjob_upd')=='true')
			{
		//	setEnabled("cmplx_OffVerification_doj_ver", false);
			setLocked("cmplx_OffVerification_doj_ver", true);//pcasf-511
		//	setEnabled("cmplx_OffVerification_doj_upd", false);
			setLocked("cmplx_OffVerification_doj_upd", true);
			}
			else 
		...
		else if(getNGValue('cmplx_OffVerification_cnfrminjob_val')=='false')
		{
				setEnabled("cmplx_OffVerification_doj_ver", true);
			setLocked("cmplx_OffVerification_doj_ver", false);//pcasf-511
				setNGValueCustom("cmplx_OffVerification_cnfrminjob_upd","--Select--");	
			setEnabled("cmplx_OffVerification_doj_upd", true);
			setLocked("cmplx_OffVerification_doj_upd", false);
		...
		//	setEnabled("cmplx_OffVerification_doj_upd", false);
			setLocked("cmplx_OffVerification_doj_upd", true);
			setNGValue("cmplx_OffVerification_cnfrminjob_upd","--Select--");
			setEnabled("cmplx_OffVerification_cnfrminjob_upd", true);
            setLocked("cmplx_OffVerification_cnfrminjob_upd", true);
	...
	if(pId=='cmplx_OffVerification_cnfrminjob_upd'){
		if (getNGValue(pId) == 'true') {
			setNGValueCustom("cmplx_OffVerification_doj_ver","");
			setLocked("cmplx_OffVerification_doj_ver", true);
			setNGValueCustom("cmplx_OffVerification_doj_upd", "");
    ...
    if (pId == 'cmplx_OffVerification_desig_ver') {
		...
		setNGValueCustom("cmplx_OffVerification_desig_upd", "--Select--");
    ...
    //below code added by nikhil 11/12/17
    	...
    	if(isFieldFilled('cmplx_Customer_EmiratesID')==false && getNGValue('cmplx_Customer_NEP')!='NEWC'){
    ...
    /*else if(pId=='cmplx_IncomeDetails_Basic' || pId=='cmplx_IncomeDetails_housing' || pId=='cmplx_IncomeDetails_transport' || pId=='cmplx_IncomeDetails_CostOfLiving' || pId=='cmplx_IncomeDetails_FixedOT' || pId=='cmplx_IncomeDetails_Other'){
        ...
        if (flag) {
            ...
            putComma('cmplx_IncomeDetails_totSal');
			if(getNGValue('is_CPV_Done')=='Y'){
				showAlert('','Mismatch : Please check if CPV details match and If CPV is required');
			}
        }
    }
    ...
    //below code commented by akshay on 17/11/17 as discussed with shashank
        ...
        } else if (getNGValue('cmplx_EmploymentDetails_targetSegCode') == 'MOTSUR') { //console.log("target");
            if (activityName == 'CSM') {
    ...
    //below code added by nikhil PCSP-92
        ...
        //++Below code added by nikhil 13/11/2017 for Code merge
            ...
            } else { //--commented by akshay on 4/1/17
                //setNGValueCustom('DecisionHistory_dec_reason_code',"--Select--");
                //setEnabledCustom('DecisionHistory_dec_reason_code',false);
        ...
        } else if (activityName == 'CPV' || activityName == 'CPV_Analyst') //sagarika for PCAS 2372
        {
			...
			//Added by Sajan for PCASF-295
			if(getNGValue(pId) == 'Approve' && (getNGValue('cmplx_OffVerification_fxdsal_ver')=='Mismatch' || getNGValue('cmplx_OffVerification_accpvded_ver')=='Mismatch' || getNGValue('cmplx_OffVerification_desig_ver')=='Mismatch' || getNGValue('cmplx_OffVerification_doj_ver')=='Mismatch' || getNGValue('cmplx_OffVerification_cnfminjob_ver')=='Mismatch')){
				showAlert(pId, 'Approve Decision is not allowed as there is a Mismatch in one of the Office verification details');
                setNGValue(pId, '--Select--');
                return false;
			}
			
			if(getNGValue(pId) == 'Approve and Send Back' && getNGValue('cmplx_OffVerification_fxdsal_ver')!='Mismatch' && getNGValue('cmplx_OffVerification_accpvded_ver')!='Mismatch' && getNGValue('cmplx_OffVerification_desig_ver')!='Mismatch' && getNGValue('cmplx_OffVerification_doj_ver')!='Mismatch' && getNGValue('cmplx_OffVerification_cnfminjob_ver')!='Mismatch'){
				showAlert(pId, 'Approve and Send Back Decision is not allowed as no Mismatch is selected in any of the Office verification details');
                setNGValue(pId, '--Select--');
                return false;
			}

            //below all code chenged by nikhil fo PCSP-74/75/76
            if (getNGValue(pId) == 'Approve' || getNGValue(pId) == 'Approve Sub to CIF' || getNGValue(pId) == 'Reject' || getNGValue(pId) == 'Refer' || getNGValue(pId) == 'Approve and Send Back') {//PCasf-852 saurabh1
        ...
        // ++ above code already present - 06-10-2017 decision validation
            ...
            if (getNGValue(pId).toUpperCase() == 'REJECT') {
                ...
                else if (getNGValue('cmplx_DEC_Manual_Deviation') == true && (parseInt(getNGValue('reEligibility_CC_counter').split(';')[1]) == 0 && parseInt(getNGValue('reEligibility_CC_counter').split(';')[0]) == 0)) {//added by saurabh1 for pcasf-460
               ...
               // setLockedCustom('cmplx_DEC_ReferTo', true); //pcasf-135,122
                //change by saurabh on 30th Nov for FSD 2.7
                ...
                if (getNGValue(pId) == 'APPROVE' || getNGValue(pId) == 'Approve') {
                    ...
                    //setNGValueCustom('cmplx_DEC_ReferTo', '');//pcasf-135,122
                }
                //--commented by akshay on 4/1/17
                //setNGValueCustom('DecisionHistory_dec_reason_code',"--Select--");
                //setEnabledCustom('DecisionHistory_dec_reason_code',false);
    ...
    //below code also fix point "13-Flat amount to be enabled and mandatory when DDS mode is flat amount"
...

        {
            ...
            setEnabledCustom("cmplx_CC_Loan_Percentage", false);
            setEnabledCustom("cmplx_CC_Loan_DDSAmount", true);
        }
        //below code also fix point "30-Service Details#Validations not as per FSD."
        else if (dds_mode == 'P')
        //above code also fix point "30-Service Details#Validations not as per FSD."
        ...
        {
            setEnabledCustom("cmplx_CC_Loan_DDSAmount", false);
            setEnabledCustom("cmplx_CC_Loan_Percentage", true);
        } else {
            setEnabledCustom("cmplx_CC_Loan_DDSAmount", true);
            setEnabledCustom("cmplx_CC_Loan_Percentage", true);
        }
...
function blur_DigitalOnBoarding(pId) {
...
function SetDisableCustomer1() {
    var fields = "cmplx_Customer_Title,cmplx_Customer_RESIDENTNONRESIDENT,cmplx_Customer_gender,cmplx_Customer_age,cmplx_Customer_MotherName,cmplx_Customer_VisaNo,cmplx_Customer_PassPortExpiry,cmplx_Customer_VIsaExpiry,cmplx_Customer_SecNAtionApplicable,cmplx_Customer_MAritalStatus,cmplx_Customer_COUNTRYOFRESIDENCE,cmplx_Customer_EMirateOfVisa,cmplx_Customer_PassportIssueDate";
    var field_array = fields.split(",");
    for (var i = 0; i < field_array.length; i++)
...
function setBlank_Customer() {
    var fields = "cmplx_Customer_EmiratesID,cmplx_Customer_LastNAme,cmplx_Customer_DOb,cmplx_Customer_MobileNo,cmplx_Customer_PAssportNo,cmplx_Customer_Nationality,cmplx_Customer_RESIDENTNONRESIDENT,cmplx_Customer_CIFNo,cmplx_Customer_masroomID,cmplx_Customer_Title,cmplx_Customer_FirstNAme,cmplx_Customer_MiddleNAme,cmplx_Customer_gender,cmplx_Customer_MotherName,cmplx_Customer_VisaNo,cmplx_Customer_MAritalStatus,cmplx_Customer_COUNTRYOFRESIDENCE,cmplx_Customer_Age,cmplx_Customer_SecNAtionApplicable,cmplx_Customer_PassPortExpiry,cmplx_Customer_VIsaExpiry,cmplx_Customer_EMirateOfVisa,cmplx_Customer_EmirateOfResidence,cmplx_Customer_PassportIssueDate,cmplx_Customer_VisaIssuedate";
    var field_array = fields.split(",");
    for (var i = 0; i < field_array.length; i++)
...
function tabSheetHandling_CC(activityName) {
    ...
    } else if (activityName == 'Fulfillment_RM') {
...
function uploadDocument() {
    ...
    $.ajax({
        ...
        data: {
            CifId: getNGValue('cmplx_Customer_CIFNo'),
            AccountNo: '',
            CustSeqNo: '001',
            ItemIndex: FolderId,
            CustomerName: fullName,
            docIndex: getNGValue('sig_docindex'),
            WIName: wi_name,
...
//Changed by Sajan for FALCON CDOB
function CDOB_postHookDRefresh(controlName) {
...
function checkMandatory_Smartcheck() {
    ...
    if (!getNGValue('SmartCheck_CR_Remarks') && activityName.indexOf('CAD') > -1) {
        ...
        return false;
    } else if (!getNGValue('SmartCheck_CPVRemarks') && activityName == 'CPV') {
...
//Changed by Sajan for FALCON CDOB
function keydown_CDOB(pId, keyCode_str) { //added by nikhil for PCAS-1249
    ...
    //for 18,2 fields
    else if (pId == 'totalBalanceTransfer' || pId == 'cmplx_CC_Loan_FlatAMount' || pId == 'amount' || pId == 'ExtLiability_Limit' || pId == 'cmplx_IncomeDetails_UnderwritingOther' || pId == 'totalBalanceTransfer' || pId == 'InwardTT_amount' || pId == 'cmplx_FinacleCore_total_avg_last13' || pId == 'cmplx_FinacleCore_total_avg_last_16' || pId == 'cmplx_MOL_basic' || pId == 'cmplx_CardDetails_charity_amount' || pId == 'AddressDetails_years' || pId == 'cmplx_MOL_resall' || pId == 'cmplx_MOL_transall') {
        var value = getNGValue(pId);
        if (value.indexOf(',') > -1) {
            value = replaceAll(value, ',', '');
        }
        ...
        return true;
    } else if (pId == 'cmplx_CC_Loan_SI_Percentage') {
        var value = getNGValue(pId);
        if (value.indexOf(',') > -1) {
            value = replaceAll(value, ',', '');
        }
...
function VisitSystemChecks_CAD(sheetid) {
    ...
    if ((pname == 'PersonalLoanS' || pname == 'CreditCard' || pname == 'DigitalOnBoarding') && activityName == 'CAD_Analyst1') {
...
//Changed by Sajan for FALCON CDOB
function cdob_click_parameterlist(pId) {
    var args = '';
    switch (pId) {
        case 'Modify':
            args = 'ReqLimit Priority CardProd';
            break;
        case 'Delete':
            args = 'Product_Frame1';
            break;
        case 'IncomeDetails_Add':
            args = 'IncomeDetails_Frame1';
            break;
        case 'IncomeDetails_Modify':
            args = 'IncomeDetails_Frame1';
            break;
        case 'IncomeDetails_Delete':
            args = 'IncomeDetails_Frame1';
            break;
        case 'IncomeDetails_Salaried_Save':
            args = 'IncomeDetails';
            break;
        case 'IncomeDetails_SelfEmployed_Save':
            args = 'IncomeDetails';
            break;
        case 'CompanyDetails_Add':
            args = 'CompanyDetails_Frame1';
            break;
        case 'CompanyDetails_Modify':
            args = 'CompanyDetails_Frame1';
            break;
        case 'CompanyDetails_delete':
            args = 'CompanyDetails_Frame1';
            break;
        case 'CompanyDetails_Button3':
            args = 'cif';
            break;
        case 'CompanyDetails_Aloc_search':
            args = 'compName';
            break;
        case 'AuthorisedSignDetails_add':
            args = 'CompanyDetails_Frame2';
            break;
        case 'AuthorisedSignDetails_modify':
            args = 'CompanyDetails_Frame2';
            break;
        case 'AuthorisedSignDetails_delete':
            args = 'CompanyDetails_Frame2';
            break;
        case 'AuthorisedSignDetails_FetchDetails':
            args = 'AuthorisedSignDetails_CIFNo';
            break;
        case 'PartnerDetails_add':
            args = 'CompanyDetails_Frame3';
            break;
        case 'PartnerDetails_modify':
            args = 'CompanyDetails_Frame3';
            break;
        case 'PartnerDetails_delete':
            args = 'CompanyDetails_Frame3';
            break;
        case 'CompanyDetails_Save':
            args = 'CompanyDetails_Frame3';
            break;
        case 'ExtLiability_Button2':
            args = 'ExtLiability_Frame4';
            break;
        case 'ExtLiability_Button3':
            args = 'ExtLiability_Frame4';
            break;
        case 'ExtLiability_Button4':
            args = 'ExtLiability_Frame4';
            break;
        case 'ExtLiability_Save':
            args = 'ExtLiability_Frame1';
            break;
        case 'ELigibiltyAndProductInfo_Button1':
            args = 'ELigibiltyAndProductInfo_Frame1 EMploymentDetails_Frame1 DecisionHistory IncomeDetails_Frame1 FrameName';
            break; //changed by nikhil for PCSP-328 and also for PCSP-489 updated 12th jan
        case 'DecisionHistory_calReElig':
            args = 'DecisionHistory_Frame1 cmplx_DEC_Manual_Deviation NotepadDetails_Frame1';
            break; //changed by nikhil for PCSP-483
        case 'ELigibiltyAndProductInfo_Save':
            args = 'ELigibiltyAndProductInfo_Frame1';
            break;
        case 'ELigibiltyAndProductInfo_Refresh':
            args = 'ELigibiltyAndProductInfo_Frame1';
            break;
        case 'Nationality_Button':
            args = 'cmplx_Customer_Nationality';
            break;
        case 'SecNationality_Button':
            args = 'cmplx_Customer_SecNationality';
            break;
        case 'Customer_FetchDetails':
            args = 'CustomerDetails';
            break;
        case 'Customer_Button1':
            args = 'CustomerDetails';
            break;
        case 'Customer_save':
            args = 'CustomerDetails';
        ...
        case 'FinacleCore_AddToAverage':
            args = 'FinacleCore_avgbal';
            break;
        case 'FinacleCore_AddToTurnover':
            args = 'FinacleCore_Frame8';
            break;
        case 'FinacleCore_Button9':
            args = 'FinacleCore_Frame3';
            break;
        case 'Designation_button':
            args = 'cmplx_EmploymentDetails_Designation';
            break;
        case 'Designation_button7':
            args = 'cmplx_Customer_Designation';
            break;
        case 'Designation_button8_view':
            args = 'cmplx_Customer_Designation';
            break;
        case 'DesignationAsPerVisa_button':
            args = 'cmplx_EmploymentDetails_DesigVisa';
            break;
        case 'FreeZone_Button':
            args = 'cmplx_EmploymentDetails_FreezoneName';
            break;
        case 'EMploymentDetails_Save':
            args = 'EmploymentDetails';
            break;
        case 'Button_City':
        ...
        case 'Button_State':
            ...
            break;
        case 'AddressDetails_Button1':
            args = 'AddressDetails_country';
            break;
        case 'AddressDetails_addr_Add':
            args = 'Address_Details_container';
            break;
        case 'AddressDetails_addr_Modify':
            args = 'Address_Details_container';
            break;
        case 'AddressDetails_addr_Delete':
            args = 'Address_Details_container';
            break; //cmplx_EmploymentDetails_remarks
        case 'AddressDetails_Save':
            args = 'Address_Details_container';
            break;
        case 'AltContactDetails_ContactDetails_Save':
            args = 'Alt_Contact_container';
            break;
        case 'CardDetails_Button2':
            args = 'CardDetails_Frame1';
            break;
        case 'CardDetails_Button3':
            args = 'CardDetails_Frame1';
            break;
        case 'CardDetails_Button4':
            args = 'CardDetails_Frame1';
            break;
        case 'CardDetails_Button1':
            args = 'CardDetails_Frame1 AlternateContactDetails_AirArabiaIdentifier';
            break;
        case 'CardDetails_Button5':
            args = 'CardDetails_Frame1';
            break;
        case 'CardDetails_modify':
            args = 'CardDetails_Frame1 AltContactDetails_Frame1';
            break;
        case 'CardDetails_delete':
            args = 'CardDetails_Frame1 AltContactDetails_Frame1';
            break;
        case 'CardDetails_Save':
            args = 'CardDetails_Frame1 AltContactDetails_Frame1';
            break;
        case 'SupplementCardDetails_Button1':
            args = 'Supplementary_Card_Details';
            break;
        case 'SupplementCardDetails_Button2':
            args = 'Supplementary_Card_Details';
            break;
        case 'SupplementCardDetails_Button3':
            args = 'Supplementary_Card_Details';
            break;
        case 'SupplementCardDetails_Button4':
            args = 'Supplementary_Card_Details';
            break;
        case 'SupplementCardDetails_Save':
            args = 'Supplementary_Card_Details';
            break;
        case 'FATCA_Button1':
            args = 'FATCA_ListedReason FATCA_SelectedReason';
            break;
        case 'FATCA_Button2':
            args = 'FATCA_ListedReason FATCA_SelectedReason';
            break;
        ...
        case 'OECD_Save':
            args = 'OECD';
            break;
        case 'ButtonOECD_State':
            args = 'OECD_townBirth';
            break;
        case 'ReferenceDetails_Reference_Add':
            args = 'Reference_Details';
            break;
        case 'ReferenceDetails_Reference__modify':
            args = 'Reference_Details';
            break;
        case 'ReferenceDetails_Reference_delete':
            args = 'Reference_Details';
            break;
        case 'ReferenceDetails_save':
            args = 'Reference_Details';
            break;
        case 'BT_Add':
            args = 'totBalTransfer';
        ...
        case 'RVC_Save':
            args = 'CC_Loan_Frame3';
            break;
        case 'PartMatch_Search':
            args = 'Part_Match';
            break;
        case 'PartMatch_Blacklist':
            args = 'Part_Match';
            break;
        case 'PartMatch_Button1':
            args = 'Part_Match';
            break;
        case 'PartMatch_Save':
            args = 'Part_Match';
            break;
        case 'DecisionHistory_CifLock':
            args = '@this';
            break;
        case 'DecisionHistory_CifUnlock':
            args = '@this';
            break;
        case 'NotepadDetails_Add':
            args = 'Notepad_Values';
            break;
        case 'NotepadDetails_Modify':
            args = 'Notepad_Values';
            break;
        case 'NotepadDetails_savebutton':
            args = 'Notepad_Values';
            break;
        case 'CustDetailVerification1_Button2':
            args = 'CustDetailVerification1_Frame1';
            break; //for fcu changes 21/10/18-Nikhil
        case 'CC_Creation_Button2':
            args = 'CC_Creation';
            break; //changes by nikhil 30/10/18 for Card Creation 
        case 'RejectEnq_Save':
            args = 'RejectEnq_Frame1';
            break;
        case 'WorldCheck1_Add':
            args = 'WorldCheck1_Frame1';
            break;
        case 'WorldCheck1_Modify':
            args = 'WorldCheck1_Frame1';
            break;
        case 'WorldCheck1_Delete':
            args = 'WorldCheck1_Frame1';
            break;
        case 'WorldCheck1_Save':
            args = 'WorldCheck1_Frame1';
            break;
        case 'FinacleCore_Button1':
            args = 'FinacleCore_Frame6';
            break;
        case 'FinacleCore_Button2':
            args = 'FinacleCore_Frame6';
            break;
        case 'FinacleCore_Button3':
            args = 'FinacleCore_Frame6';
            break;
        case 'FinacleCore_Button1':
            args = 'FinacleCore_Frame6';
            break;
        case 'ExternalBlackList_Button1':
            args = 'External_Blacklist';
            break;
        case 'FinacleCRMCustInfo_Add':
            args = 'Finacle_CRM_CustomerInformation';
            break;
        case 'FinacleCRMCustInfo_Modify':
            args = 'Finacle_CRM_CustomerInformation';
            break;
        case 'FinacleCRMCustInfo_Delete':
            args = 'Finacle_CRM_CustomerInformation';
            break;
        case 'FinacleCRMCustInfo_Button1':
            args = 'Finacle_CRM_CustomerInformation';
            break;
        case 'FinacleCRMIncident_Button1':
            args = 'Finacle_CRM_Incidents';
            break;
        case 'FinacleCore_Button4':
            args = 'FinacleCore_Frame5';
            break; //changed by nikhil 01/11/2018 for Grid issues
        case 'FinacleCore_Button5':
            args = 'FinacleCore_Frame5';
            break; //changed by nikhil 01/11/2018 for Grid issues
        case 'SmartCheck_Add':
            args = 'SmartCheck_Frame1';
            break; //changed by nikhil 01/11/2018 for Grid issues
        case 'SmartCheck_Modify':
            args = 'SmartCheck_Frame1';
            break; //changed by nikhil 01/11/2018 for Grid issues
        case 'CardCollection_Button1':
            args = 'CardCollection_Frame1';
            break; //changed by nikhil 01/11/2018 for Grid issues
        case 'ReferHistory_Modify':
            args = 'ReferHistory_Frame1';
            break;
        case 'Compliance_Button2':
            args = 'Compliance_Frame1';
            break;
        case 'DecisionHistory_ADD':
            args = 'DecisionHistory';
            break;
        case 'DecisionHistory_ManualDevReason':
            args = 'DecisionHistory';
            break;
        case 'DecisionHistory_Delete':
            args = 'DecisionHistory';
            break;
        case 'DecisionHistory_Modify':
            args = 'DecisionHistory';
            break;
        case 'EMploymentDetails_Button2':
            args = 'EMploymentDetails_Frame2';
            break; //changed by nikhil for employer Search PCSP-70
        case 'SupplementCardDetails_Modify':
            args = 'SupplementCardDetails_Frame1';
            break; //changed by nikhil 15/12/2018
        case 'SupplementCardDetails_Add':
            args = 'SupplementCardDetails_Frame1';
            break; //changed by nikhil 15/12/2018
        case 'SupplementCardDetails_FetchDetails':
            args = 'SupplementCardDetails_Frame1';
            break; //changed by Deepak Supplymentry fetch details button
        case 'EmploymentVerification_Button1':
            args = 'EmploymentVerification_Frame1';
            break; //added by nikhil for PCSP-158
        case 'BankingCheck_save':
            args = 'BankingCheck_Frame1';
            break; //added by nikhil for PCSP-155
        case 'OfficeandMobileVerification_Button1':
            args = 'OfficeandMobileVerification_Frame1';
            break; //added by nikhil for PCSP-224
        case 'CustDetailVerification1_Button1':
            args = 'CustDetailVerification1_Frame1';
            break; //added by nikhil for PCSP-235
        case 'WorldCheck_fetch':
            args = 'WorldCheck';
            break; //added by nikhil for PCSP-387
        case 'IncomingDocNew_Addbtn':
            args = 'IncomingDocNew_Frame1';
            break; // change by disha for new incoming document CR 04/01/2019
        case 'IncomingDocNew_Modifybtn':
            args = 'IncomingDocNew_Frame1';
            break; // change by disha for new incoming document CR 04/01/2019
        case 'IncomingDocNew_Save':
            args = 'IncomingDocNew_Frame1';
            break; // change by disha for new incoming document CR 04/01/2019
        case 'Designation_button_View':
            args = 'cmplx_EmploymentDetails_Designation';
            break; //code by nikhil for PCSP-503
        case 'DesignationAsPerVisa_button_View':
            args = 'cmplx_EmploymentDetails_DesigVisa';
            break; //code by nikhil for PCSP-503
        case 'KYC_Add':
            args = 'KYC_Frame1';
        ...
        case 'KYC_Modify':
            args = 'KYC_Frame1';
            break;
        case 'Reference_Details_Reference_Add':
            args = 'Reference_Details_Frame1';
            break;
        case 'Reference_Details_Reference__modify':
            args = 'Reference_Details_Frame1';
            break;
        case 'FATCA_Add':
            args = 'Fatca_Frame6';
        ...
        case 'FATCA_Modify':
            ...
            break;
        case 'PartMatch_Search':
            args = 'PartMatch_Frame1';
            break;
        case 'PartMatch_Blacklist':
            args = 'PartMatch_Frame1';
            break;
        case 'NotepadDetails_Add1':
            args = 'NotepadDetails_Frame3';
            break; //code changed by nikhil for PCSP-555
        case 'AltContactDetails_Saved':
            args = 'AltContactDetails_Frame1';
            break; //code changed by nikhil for Card dispatch save
        case 'OriginalValidation_Save':
            args = 'Original_validation';
            break; //code for OV save
        case 'EmploymentVerification_s2_Button1':
            args = 'EmploymentVerification_s2_Frame1';
            break;
        case 'Cust_ver_sp2_Button1':
            args = 'Cust_ver_sp2_Frame1';
            break;
        case 'fieldvisit_sp2_Button1':
            args = 'fieldvisit_sp2_Frame1';
            break;
        case 'checklist_ver_sp2_Button1':
            args = 'checklist_ver_sp2_Frame1';
            break;
        case 'exceptionalCase_sp2_Button2':
            args = 'exceptionalCase_sp2_Frame1';
            break;
        case 'Nationality_Button_View':
            args = 'cmplx_Customer_Nationality';
            break;
        ...
        case 'EmploymentDetails':
            args = 'cmplx_Customer_Designation';
            break;
        case 'Designation_button5_View':
            args = 'cmplx_cust_ver_sp2_desig_remarks';
            break;
        case 'Designation_button6_View':
            args = 'cmplx_emp_ver_sp2_desig_remarks';
            break;
        case 'SecNationality_Button_View':
            args = 'cmplx_Customer_SecNationality';
            break;
        case 'Button_City_View':
            args = 'AddressDetails_city';
            break;
        case 'Button_State_View':
            args = 'AddressDetails_state';
            break;
        case 'AddressDetails_Button1_View':
            args = 'AddressDetails_country';
            break;
        case 'CardDispatchToButton_View':
            args = 'AlternateContactDetails_CardDisp';
            break;
        case 'ButtonOECD_State_View':
            args = 'OECD_townBirth';
            break;
        case 'Designation_button_View':
            args = 'cmplx_EmploymentDetails_Designation';
        ...
        case 'Designation_button8_View':
            args = 'cmplx_Customer_Designation';
            break;
        case 'Nationality_Button1_View':
            args = 'PartMatch_nationality';
            break;
            //++below code added by nikhil for Self-Supp CR
        case 'CardDetails_Self_add':
            args = 'CardDetails_Frame1';
            break;
        case 'CardDetails_Self_remove':
            args = 'CardDetails_Frame1';
            break;
            //--above code added by nikhil for Self-Supp CR
        case 'SmartCheck1_Add':
            args = 'SmartCheck1_Frame1';
            break;
        case 'SmartCheck1_Modify':
            args = 'SmartCheck1_Frame1';
            break;
        default:
            args = '@this';
