...
function Add_Validate()
{
	...
	if(n>0)
		{
          	if(getLVWAT('DecisionHistory_Decision_ListView1',n-1,2)==activityname && getLVWAT('DecisionHistory_Decision_ListView1',n-1,1)==userid){
					if((getLVWAT('DecisionHistory_Decision_ListView1',n-1,3)=='CA_Hold' && getLVWAT('DecisionHistory_Decision_ListView1',n-1,2)=='CAD_Analyst1')||(getLVWAT('DecisionHistory_Decision_ListView1',n-1,2)=='CAD_Analyst1' && getLVWAT('DecisionHistory_Decision_ListView1',n-1,6)=='Customer_Hold')||(getLVWAT('DecisionHistory_Decision_ListView1',n-1,2)=='CPV' && getLVWAT('DecisionHistory_Decision_ListView1',n-1,6)=='Customer_Hold')||(getLVWAT('DecisionHistory_Decision_ListView1',n-1,2)=='CPV_Analyst' && getLVWAT('DecisionHistory_Decision_ListView1',n-1,6)=='Customer_Hold')||(getLVWAT('DecisionHistory_Decision_ListView1',n-1,2)=='Smart_CPV' && getLVWAT('DecisionHistory_Decision_ListView1',n-1,6)=='Customer_Hold') || (getLVWAT('DecisionHistory_Decision_ListView1',n-1,2)=='Cad_Analyst2' && getLVWAT('DecisionHistory_Decision_ListView1',n-1,3)=='Approve'))
					{
						return true;
					}
				//if(count==1)
...
function DMandatoryCDOB(){
	...
	//Added by Sajan for PCASF-	416 FALCON
	if(activityName=='CAD_Analyst1' && (getNGValue('cmplx_Customer_CIFNo')==null || getNGValue('cmplx_Customer_CIFNo')=='') && getNGValue('cmplx_DEC_Decision')=='Approve'){
		showAlert('','CIF generation is mandatory for Workitem to be processed');
		return false;
	}
	...
	if((activityName!='FPU' || activityName!='Compliance'))
	 {
	...
	else if((activityName=='FPU' || activityName=='Compliance') && getNGValue('cmplx_DEC_Decision')=='Refer' && validateReferGridonRefer())
	{
...
if(!checkFor_SupplemntGrid()){
	...
	else if(activityName=='Smart_CPV')
	{
		...
		if(getLVWAT("cmplx_SmartCheck_SmartCheckGrid",i,1)=='' && getLVWAT("cmplx_SmartCheck_SmartCheckGrid",i,0)!='')
			{
...
//added 6th sept for proc 2021
	...
	else if(activityName=='CPV' || activityName=='CPV_Analyst')
	{
...
//below code commented by akshay on 23/11/18
	...
	*/
	...
	if(activityName=='FPU'|| activityName=='DSA_CSO_Review' || activityName=='CPV_Analyst' || activityName=='RM_Review'){
...
//Changes Done by aman for preferred choice
	...
	//Commented by Sajan as the field is disabled for PCASF-424
	...
	if(getNGValue('cmplx_Customer_VisaIssueDate')=='' && getNGValue('cmplx_Customer_GCCNational')!='Y' && getNGValue('cmplx_Customer_Nationality')!='AE' && getNGValue('cmplx_Customer_Nationality')!='BH' && getNGValue('cmplx_Customer_Nationality')!='IQ' && getNGValue('cmplx_Customer_Nationality')!='KW' && getNGValue('cmplx_Customer_Nationality')!='OM' && getNGValue('cmplx_Customer_Nationality')!='QA' && getNGValue('cmplx_Customer_Nationality')!='SA' && getNGValue('cmplx_Customer_RESIDENTNONRESIDENT')!='N'){
				showAlert('cmplx_Customer_VisaIssueDate','Visa issue date cannot be blank');
				return false;
	}
	...
	if(!validateFutureDate('cmplx_Customer_VisaIssueDate')){
		showAlert('cmplx_Customer_VisaIssueDate','Visa issue date cannot be future date')
		return false;
	}
	if(!validatePastDate('cmplx_Customer_VisaExpiry','Emirate ID Expiry')){
		showAlert('cmplx_Customer_VisaExpiry','Visa expiry date cannot be past date')
		return false;
	}
	if(!validateFutureDate('cmplx_Customer_PassportIssueDate')){
		showAlert('cmplx_Customer_PassportIssueDate','Passport issue date cannot be future date')
		return false;
	}
	if(!validatePastDate('cmplx_Customer_PassPortExpiry','Passport ID Expiry')){
		showAlert('cmplx_Customer_PassPortExpiry','Passport expiry date cannot be past date')
		return false;
	}
...
//above code also fix point "30-Service Details#Validations not as per FSD."
 ...
 function CheckMandatory_Verification(s1,s2,s3)
 {
	 ...
	 {
		 if(getNGValue(ver[i])=='Mismatch' && (getNGValue(update[i])=='' || getNGValue(update[i])=='--Select--' || getNGValue(update[i])==' --Select--'))//&& getNGValue((ver[i].split('_')[0]+'_val')!=''))
		 {
	...
	function validateReferGrid()
	{
		...
		if(n>0){
			for(var i=0;i<n;i++){
				...
				//if(activity=='CPV_Analyst'){activity='CPV Checker';}//by shweta for PCASF-294
				//else if(activity=='DSA_CSO_Review' && referFrom=='CSM'){activity='CSO (for documents)';}//Changed by nikhil for Update on refer details on Source
				 if(activity=='DSA_CSO_Review'){activity='Source';}//by shweta for PCASF-294
...
function Income_Save_Check()
	...
	if(Principal_Approval=='Y' && activityName=='CSM')
	{
		...
		if(ftsReceived=='Y'){
			if(getNGValue('cmplx_IncomeDetails_netSal1')=='' || getNGValue('cmplx_IncomeDetails_netSal1')==null || getNGValue('cmplx_IncomeDetails_netSal2')=='' || getNGValue('cmplx_IncomeDetails_netSal2')==null || getNGValue('cmplx_IncomeDetails_netSal3')=='' || getNGValue('cmplx_IncomeDetails_netSal3')==null)
			{
				showAlert('cmplx_IncomeDetails_netSal1','Net Salary 1,2 and 3 cannot be null if FTS is received');
				return false;
			}
			/*else if(isFieldFilled('cmplx_IncomeDetails_netSal2')==false){
				showAlert('cmplx_IncomeDetails_netSal1','Net Salary 2 is Mandatory if FTS is received');
				return false;
			}
			else if(isFieldFilled('cmplx_IncomeDetails_netSal3')==false)
			{
				showAlert('cmplx_IncomeDetails_netSal1','Net Salary 3 is mandatory if FTS is received');
				return false;
			}*/
			
		}
		
		if(true){		//Changed by Sajan for FALCOn CDOB as all the customers are salaried
			if(!checkMandatory(CC_INCOME_SALARIED)){
				return false;}
				//Commented by Sajan for FALCOn CDOB
