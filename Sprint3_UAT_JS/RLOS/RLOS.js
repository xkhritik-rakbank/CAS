...
function click_RLOS(pId,frameState){
		...
		else if(pId=='SupplementCardDetails_FetchDetails')
		{
			...
			if(setFlag==true)
			{
				setNGValue("ConsiderObligations", false);
			}
			else
			{
				setNGValue("ConsiderObligations", true);				
			}
		}
		else if(pId=='ConsiderObligations'){
			var setFlag=getNGValue("ConsiderObligations");
			if(setFlag==true)
			{
		...
		//for optimization, such that on second time fragment click comboload function is not called again and again from java code
			...
			if(Product_type==''){
				...
				//	alert("aa");
				if((getLVWAT('cmplx_Product_cmplx_ProductGrid',0,2))=="PA")//sagarika
				{
			setVisible("IncomeDetails_Frame1", true);
					...
					setVisible("IncomeDetails_Frame3", false);
					com.newgen.omniforms.formviewer.setHeight("Incomedetails", "1500px");
					com.newgen.omniforms.formviewer.setHeight("IncomeDetails_Frame2", "600px");
					com.newgen.omniforms.formviewer.setHeight("IncomeDetails_Frame3", "680px");
		...
		else if(pId=='Designation_button1' || pId=='TLRM_Button' || pId=='Sourcing_Branch_Code_button' || pId=='Customer_Button2' || pId=='DesignationAsPerVisa_button' || pId=='CardDispatchToButton' || pId=='SecNationality_Button' || pId=='Third_Nationality_Button'|| pId=='BirthCountry_Button' || pId=='ResidentCountry_Button' || pId=='AddressDetails_Button1' || pId=='Nationality_ButtonPartMatch' || pId=='MOL_Nationality_Button' || pId=='CardDetails_bankName_Button' || pId=='EmploymentDetails_Bank_Button' || pId=='FreeZone_Button' )
		{
		return true;
		}
		...
		else if(pId=='ReadFromCard')
		{
			...
			EIDA_Visa_date();//Deepak Need to check it's  use
				...
				//	setLockedCustom("cmplx_Customer_CIFNO",false);
					...
					//setEnabledCustom("cmplx_Customer_DOb",true); cmplx_Customer_Title
			...
			setEnabledCustom("cmplx_Customer_VIsaExpiry",true);
				...
				//	setLockedCustom("cmplx_Customer_CIFNO",false);
					...
					setLockedCustom("cmplx_Customer_gender",true);
					//setLockedCustom("Designation_button1",true);
					setLockedCustom("cmplx_Customer_Title",true);
					//setLockedCustom("cmplx_Customer_Email_ID",true);
					setLockedCustom("cmplx_Customer_Designation",true);
        ...
        {    
			...
			if(getNGValue('cmplx_Customer_card_id_available')==true){
					...
					//setEnabledCustom("cmplx_Customer_Basic_Sal",true);
					//setEnabledCustom("cmplx_Customer_Email_ID",true);
					//setEnabledCustom("Designation_button1",true);
					...
					//setEnabledCustom("cmplx_Customer_Basic_Sal",false);
					//setEnabledCustom("cmplx_Customer_Email_ID",false);
					//setEnabledCustom("Designation_button1",false);
				}
				setNGValueCustom(pId,getNGValue(pId));
        }
		...
		else if(pId=='IncomeDetails_FinacialSummarySelf' || pId=='IncomeDetails_Button1')
		{
			...
			*/
				...
				var url='/webdesktop/custom/CustomJSP/integration.jsp?request_name=' + request_name + '&param_json=' + param_json + '&wi_name=' + wi_name + '&activityName=' +activityName+ '&button=' + pId+'&sessionId='+window.parent.sessionId+ '&prod=' + prod + '&subprod=' + subprod ;
		...
		{
			setLockedCustom('Third_Nationality_Button',true);
			setNGValueCustom('cmplx_Customer_Third_Nationaity',"");

		}
		
		else if(pId=='cmplx_Customer_Third_Nationaity_Applicable_0'){
			setLockedCustom('Third_Nationality_Button',false);
			//setEnabledCustom('cmplx_Customer_Third_Nationaity',true);
        }
		...
		else if(pId=='cmplx_Customer_Confirmed_in_Job')
		{
			setNGValueCustom('cmplx_EmploymentDetails_JobConfirmed',getNGValue(pId))
		}
		...
		else if(pId=='cmplx_GuarantorDetails_cmplx_GuarantorGrid')
		{
			/*if(getLVWAT(pId,getNGListIndex(pId),1)!=''){
				...
				if(getNGValue('GuarantorDetails_nationality')=='AE'){
...
//Added by aman 08062018
	else if( pId=='SecNationality_Button'|| pId=='TLRM_Button' || pId=='Sourcing_Branch_Code_button' || pId=='Customer_Button2' || pId=='Third_Nationality_Button'|| pId=='CardDispatchToButton'|| pId=='Designation_button'|| pId=='AddressDetails_Button1'|| pId=='EmploymentDetails_Bank_Button'|| pId=='DesignationAsPerVisa_button'|| pId=='FreeZone_Button')
	{
		return true;
	}
	...
	//Added by aman 08062018
			...
			if(AccountSummary_checkMandatory())
			{
				...
				var trade_lic_no="";
					...
					if(isVisible('GuarantorDetails_Frame1')==true){	
						guarantor_CIF=	getLVWAT('cmplx_GuarantorDetails_cmplx_GuarantorGrid',0,1);
					}
				...
				var url='/webdesktop/custom/CustomJSP/integration.jsp?request_name=' + request_name + '&param_json=' + param_json + '&wi_name=' + wi_name + '&activityName=' +activityName + '&prod=' + prod + '&subprod=' + subprod + '&button=' + pId  +'&sessionId='+window.parent.sessionId;
		...
		//Added Code for Customer Save Check
			...
			else if(getNGValue('GuarantorDetails_guardianCif')=='--Select--'){//name changed by akshay on 12/3/18 as decision was not loading
				showAlert('GuarantorDetails_guardianCif','Please Select Guarantor CIF Available');
				return false;
			}	
			else if(getNGValue('GuarantorDetails_guardianCif')=='Yes' || (getNGValue('GuarantorDetails_guardianCif')=='Same' && getNGValue('cif')!='7777777')){//name changed by akshay on 12/3/18 as decision was not loading
				if(getNGValue('cif')==''){
					showAlert('cif','Please Enter Guarantor CIF');
					return false;
				}
			} else if(!checkMandatory(RLOS_GUARANTOR_FetchDetails)){
				return false;
			}
		...
		else if(pId=='Customer_Check')
		{
				if(!Check_Mandate_CustomerCheck())
				{
					return false;
				}
				if(getNGValue('cmplx_Customer_VIPFlag')==true || getNGValue('Individual_Aecb')=='N')	
				{
					return true;
				}			
				...
				var trade_lic_no="";
					...
					//Changes done by aman for AECB Start
					// acc_id replaced with Account_Number Deepak 12Nov2017
					var CIF_Exposure = getNGValue('cmplx_Customer_CIFNO');
					if(activityName =='Telesales_Init'){
						CIF_Exposure='';
					}
					var param_json=('{"cmplx_customer_FirstNAme":"'+getNGValue('cmplx_Customer_FIrstNAme')+'","cmplx_Customer_LAstNAme":"'+getNGValue('cmplx_Customer_LAstNAme')+'","cmplx_Customer_DOb":"'+getNGValue('cmplx_Customer_DOb')+'","cmplx_Customer_MobNo":"'+getNGValue('cmplx_Customer_MobNo')+'","cmplx_Customer_gender":"'+gender+'","OverridePeriod":"'+OverridePeriod+'","wi_name":"'+wi_name_aecb+'","cmplx_Customer_Nationality":"'+getNGValue('cmplx_Customer_Nationality')+'","cmplx_Customer_PAssportNo":"'+getNGValue('cmplx_Customer_PAssportNo')+'","TxnAmount":"'+TxnAmount+'","NoOfInstallments":"'+NoOfInstallments+'","cmplx_Customer_Passport2":"'+getNGValue('cmplx_Customer_Passport2')+'","cmplx_Customer_Passport3":"'+getNGValue('cmplx_Customer_Passport3')+'","cmplx_Customer_PAssport4":"'+getNGValue('cmplx_Customer_PAssport4')+'","cmplx_Username":"deepak","cmplx_Customer_CIFNO":"'+CIF_Exposure+'","cif":"'+company_cif+'","trade_comp_name":"'+trade_comp_name+'","trade_lic_no":"'+trade_lic_no+'","account_no":"'+getNGValue('Account_Number')+'","cmplx_Customer_EmiratesID":"'+getNGValue('cmplx_Customer_EmiratesID')+'","cmplx_EligibilityAndProductInfo_NumberOfInstallment":"5","cmplx_Customer_short_name":"'+getNGValue('cmplx_Customer_short_name')+'","full_name":"'+full_name+'","cmplx_Customer_NTB":"true"}');
				//Changes done by aman for AECB end
				//alert(param_json);
				param_json = encodeURIComponent(param_json);
				var url='/webdesktop/custom/CustomJSP/integration.jsp?request_name=' + request_name + '&param_json=' + param_json + '&wi_name=' + wi_name + '&activityName=' +activityName + '&prod=' + prod + '&subprod=' + subprod+ '&button=' + pId+'&sessionId='+window.parent.sessionId;
		...
		else if(pId=='Eligibility_Check')
		{
			var NTB_Cust=getNGValue('cmplx_Customer_NTB');
			if(NTB_Cust==true)
			{
				document.getElementById("Eligibility_Emp").click();
			}
			else
			{
			var request_name = "";
				...
				var trade_lic_no="";
					...
					//Changes done by aman for AECB Start
					// acc_id replaced with Account_Number Deepak 12Nov2017
					var param_json=('{"cmplx_customer_FirstNAme":"'+getNGValue('cmplx_Customer_FIrstNAme')+'","cmplx_Customer_LAstNAme":"'+getNGValue('cmplx_Customer_LAstNAme')+'","cmplx_Customer_DOb":"'+getNGValue('cmplx_Customer_DOb')+'","cmplx_Customer_MobNo":"'+getNGValue('cmplx_Customer_MobNo')+'","cmplx_Customer_gender":"'+gender+'","OverridePeriod":"'+OverridePeriod+'","wi_name":"'+wi_name_aecb+'","cmplx_Customer_Nationality":"'+getNGValue('cmplx_Customer_Nationality')+'","cmplx_Customer_PAssportNo":"'+getNGValue('cmplx_Customer_PAssportNo')+'","TxnAmount":"'+TxnAmount+'","NoOfInstallments":"'+NoOfInstallments+'","cmplx_Customer_Passport2":"'+getNGValue('cmplx_Customer_Passport2')+'","cmplx_Customer_Passport3":"'+getNGValue('cmplx_Customer_Passport3')+'","cmplx_Customer_PAssport4":"'+getNGValue('cmplx_Customer_PAssport4')+'","cmplx_Username":"deepak","cmplx_Customer_CIFNO":"'+getNGValue('cmplx_Customer_CIFNO')+'","cif":"'+company_cif+'","trade_comp_name":"'+trade_comp_name+'","trade_lic_no":"'+trade_lic_no+'","account_no":"'+getNGValue('Account_Number')+'","cmplx_Customer_EmiratesID":"'+getNGValue('cmplx_Customer_EmiratesID')+'","cmplx_EligibilityAndProductInfo_NumberOfInstallment":"5","cmplx_Customer_short_name":"'+getNGValue('cmplx_Customer_short_name')+'","full_name":"'+full_name+'","cmplx_Customer_NTB":"'+getNGValue('cmplx_Customer_NTB')+'","sub_product":"'+getLVWAT("cmplx_Product_cmplx_ProductGrid",0,2)+'","emp_type":"'+getLVWAT("cmplx_Product_cmplx_ProductGrid",0,6)+'","application_type":"'+getLVWAT("cmplx_Product_cmplx_ProductGrid",0,4)+'"}');
				//Changes done by aman for AECB end
				//alert(param_json);
				param_json = encodeURIComponent(param_json);
				var url='/webdesktop/custom/CustomJSP/integration.jsp?request_name=' + request_name + '&param_json=' + param_json + '&wi_name=' + wi_name + '&activityName=' +activityName + '&prod=' + prod + '&subprod=' + subprod+ '&button=' + pId+'&sessionId='+window.parent.sessionId;
				...
				*/
				...
				//changes by nikhil for product dedupe functionality wotking
		}
		...
		if(getNGValue('age_gua')<=21)
		{
			...
			if(checkMandatory_ProductGrid()){
					if(Product_add_validate()){
						...
						{
							if(getNGValue('cmplx_Customer_age')!="" && getNGValue('cmplx_Customer_age')!=null && isCustomerMinor()){
				...
				{
							setVisible('cmplx_CardDetails_CompanyEmbossingName',true);
							setLocked('cmplx_CardDetails_CompanyEmbossingName',false);
							setEnabled('cmplx_CardDetails_CompanyEmbossingName',true);
				}
							if(getNGValue("Subproduct_productGrid")!='BTC' )
				{
							setVisible('cmplx_CardDetails_CompanyEmbossingName',false);
							setLocked('cmplx_CardDetails_CompanyEmbossingName',true);
							setEnabled('cmplx_CardDetails_CompanyEmbossingName',false);
				}
		...
		//changed by akshay on 18/1/18
			...
			if(checkMandatory_PartnerGrid()){
				setNGValue('Type_Of_Partner','Manual');
				return true;
			}
		}		
 ...
 //ended By AKshay on 10/10/17 start-----For Point 15 of BTC_NTB_Defects
		...
		if(pId=='AuthorisedSignDetails_FetchDetails')
		{
			...
			if(AuthSignatory_checkMandatory()){
				if(AuthSignatory_validate('Add')){//added by akshay on 23/11/17 for validating sole employee
					...
					if(getNGValue("AuthorisedSignDetails_CheckBox1")==false){
						setNGValue('Type_Of_AuthSign','Manual');
					}
					else{
						setNGValue('Type_Of_AuthSign','');
					}
			...
			if(getLVWAT('cmplx_AuthSignDetails_cmplx_GR_AuthorizedSignDetails',getNGListIndex('cmplx_AuthSignDetails_cmplx_GR_AuthorizedSignDetails'),15)=='Manual'){
				setLockedCustom("AuthorisedSignDetails_delete",false);
			}
			else{
				setLockedCustom("AuthorisedSignDetails_delete",true);
		...
		else if(pId=='cmplx_PartnerDetails_cmplx_partnerGrid'){
			if(getLVWAT('cmplx_PartnerDetails_cmplx_partnerGrid',getNGListIndex('cmplx_PartnerDetails_cmplx_partnerGrid'),8)=='Manual'){
				setLockedCustom("PartnerDetails_delete",false);
			}
			else{
				setLockedCustom("PartnerDetails_delete",true);
			}
		}
...
var counter = getNGValue('reEligbility_init_counter').split(';')[0];
			var eligFlag = getNGValue('reEligbility_init_counter').split(';')[1];
		 if(pId=='Eligibility_Emp' && getNGValue('EmploymentType')!='Self Employed'){
		 setNGValueCustom('reEligbility_init_counter',counter+';Y');
				return true;
			}
		...
		var Atype = getLVWAT("cmplx_Product_cmplx_ProductGrid",0,4);
			if (getNGValue('dedupecheck')=='Y' && (Atype!='TOPN' && Atype!='TOPE') && (getLVWAT('cmplx_Product_cmplx_ProductGrid',0,2)!='IM'))// Added by Rajan for PCASP-1525
			{
				showAlert('',alerts_String_Map['VAL357']);
				return false;
			}
			if(getNGValue('Is_Financial_Summary')!='Y' && getNGValue('cmplx_Customer_VIPFlag')==false ){
				if(getNGValue('cmplx_Customer_NTB')==false ){
				showAlert('IncomeDetails_FinacialSummarySelf',alerts_String_Map['VAL223']);
				return false;
				}
			...
			if(getNGValue('aecb_call_status')=='' && getNGValue('cmplx_Customer_NEP')!='NEWC'){
				showAlert('Liability_container',alerts_String_Map['VAL106']);
				return false;
				}
			...
			else if(getNGValue('cmplx_EligibilityAndProductInfo_FinalLimit')=='' ){
				if(getLVWAT('cmplx_Product_cmplx_ProductGrid',0,1)=='Personal Loan'){
					showAlert('cmplx_EligibilityAndProductInfo_FinalLimit',alerts_String_Map['PL433']);

				}else {
					showAlert('cmplx_EligibilityAndProductInfo_FinalLimit',alerts_String_Map['VAL379']);
				}
				return false;
			}
			else{
			...
			if(pId=='ELigibiltyAndProductInfo_Button1')
			{
				...
				var final=getNGValue('cmplx_EligibilityAndProductInfo_FinalLimit');
				var lpf = (1*final)/100;
				if(lpf<500){
					lpf=500;
				}
				...
				setNGValue('cmplx_EligibilityAndProductInfo_LPFAmount',lpf); //hritik
			}
		...
		// Changed by Rajan for PCASP-1427
			...
			if(checkMandatory_SupplementGrid()){
				...
				var age=calcAge(getNGValue('DOB'),'');
				// Changed by Rajan for PCASP-1427
				if(getNGValue("relationship")=='MOT' ||getNGValue("relationship")=='FAT'||getNGValue("relationship")=='SON'||
						getNGValue("relationship")=='BRO' || getNGValue("relationship")=='DAU' ||getNGValue("relationship")=='HUS'||getNGValue("relationship")=='WIF' ||getNGValue("relationship")=='SIS' ||getNGValue("relationship")=='PAR')
				{
					if(age<15){
				   showAlert(pId,'Minor Customer not Applicable for ' + prod);
					//setNGValueCustom(pId,'');
					return false;
					}
				}
				else if(getNGValue("relationship")!='MOT' ||getNGValue("relationship")!='FAT'||getNGValue("relationship")!='SON'||
						getNGValue("relationship")!='BRO' || getNGValue("relationship")!='DAU' ||getNGValue("relationship")!='HUS'|| getNGValue("relationship")!='WIF'  ||getNGValue("relationship")!='SIS' || getNGValue("relationship")!='PAR')
				{    // changed by shivang for PCASP-3203
					...
					//Deepak Code to restrict supplementrary Skyward 
					...
					if(cardProduct.indexOf('EKTMC')>-1 || cardProduct.indexOf('EKWEC')>-1 || cardProduct.indexOf('EKWEI')>-1){
						showAlert('',"This Supplementary card cannot be added.");
						return false;
					}
			...
			if(checkMandatory_SupplementGrid()){
				...
				//Deepak below code to restrict EK supplementary
					var cardProduct=getNGValue("SupplementCardDetails_CardProduct");
					if(cardProduct.indexOf('EKTMC')>-1 || cardProduct.indexOf('EKWEC')>-1 || cardProduct.indexOf('EKWEI')>-1){
		 ...
		 else if (pId == 'IncomingDoc_PrintButton')
    {
		...
		//modified to download the document 
			...
			if(getNGValue('cmplx_Customer_RM_TL_NAME')=='' && getNGValue("PrimaryProduct")=='Personal Loan'){
				showAlert('cmplx_Customer_RM_TL_NAME','Please select the RM/TL Name.');
				return false;
			}
			if(Customer_Save_Check()){
				//Below added by Rajan for PCASP-1822
				if(getNGValue('cmplx_Customer_NTB')==true && (getNGValue('Subproduct_productGrid')=='SEC' || getNGValue("Subproduct_productGrid")=='BTC'|| getNGValue("Subproduct_productGrid")=='PA'))
			{
				showAlert('Customer_Check',"Credit cards to NTB customer can't be provided for the selected sub-product")
				return false;
			}
			if(getNGValue('Is_Principal_Approval')=='' && getNGValue('EmploymentType')!='Self Employed')
			{
				showAlert('Eligibility_Check','Eligibility Check is Mandatory!');
				return false;
			}
			/*if(getNGValue('cmplx_Customer_CIFNO')!='' && getNGValue('cmplx_Customer_OTPValidation')!=true && activityName!='Telesales_Init)
			{
	...
	return true;
		...
		else if(getNGValue('cmplx_EmploymentDetails_Indus_Micro')=='' || getNGValue('cmplx_EmploymentDetails_Indus_Micro')=='--Select--')
		{
	   ...
	   if(getNGValue('cmplx_EmploymentDetails_Indus_Macro')=='' || getNGValue('cmplx_EmploymentDetails_Indus_Macro')=='--Select--')
		{
		...
		else if(getNGValue('cmplx_EmploymentDetails_EmpStatus')=='' || getNGValue('cmplx_EmploymentDetails_EmpStatus')=='--Select--')
		{
			showAlert('cmplx_EmploymentDetails_EmpStatus',alerts_String_Map['CAS014']);
			return false;
		}
		else if(getNGValue('cmplx_EmploymentDetails_Emp_Type')=='' || getNGValue('cmplx_EmploymentDetails_Emp_Type')=='--Select--')
		{
			showAlert('cmplx_EmploymentDetails_Emp_Type',alerts_String_Map['VAL365']);
			return false;
		}
		...
		setNGValueCustom('Is_employment_save','Y');
			...
			{
			if(getNGValue('cmplx_EmploymentDetails_lifeInsurance')=='')
			{
			showAlert('cmplx_EmploymentDetails_lifeInsurance',alerts_String_Map['CC284']);
			return false;
			}
			...
			if(getNGValue('cmplx_EmploymentDetails_NoPremPaid')=='')
			{
			showAlert('cmplx_EmploymentDetails_NoPremPaid',alerts_String_Map['CC285']);
			return false;
			}
			if(getNGValue('cmplx_EmploymentDetails_PremiumType')=='')
			{
			showAlert('cmplx_EmploymentDetails_PremiumType',alerts_String_Map['CC286']);
			return false;
			}
		...
		return false;}
			...
			else if(getNGValue('cmplx_EmploymentDetails_LengthOfBusiness')=='' && isVisible('cmplx_EmploymentDetails_LengthOfBusiness') && getNGValue('cmplx_Customer_VIPFlag')==false){
				if(getLVWAT('cmplx_Product_cmplx_ProductGrid',0,2)=='SAL' && getLVWAT('cmplx_Product_cmplx_ProductGrid',0,2)!='IM'){
				if((!getNGValue('cmplx_EmploymentDetails_IncInCC') && !getNGValue('cmplx_EmploymentDetails_IncInPL'))
					||(getNGValue('cmplx_EmploymentDetails_CurrEmployer')=='CN' && (getNGValue('cmplx_EmploymentDetails_EmpStatusPL')=='OP'||getNGValue('cmplx_EmploymentDetails_EmpStatusPL')=='RS') 
					&& (getNGValue('cmplx_EmploymentDetails_EmpStatusCC')=='CN'||getNGValue('cmplx_EmploymentDetails_EmpStatusCC')==''||getNGValue('cmplx_EmploymentDetails_EmpStatusCC')=='--Select--'))
		    ...
		    if(getNGValue('cmplx_EmploymentDetails_targetSegCode')=='EMPID' && getNGValue('cmplx_EmploymentDetails_IndusSeg')=='')
			{
		...
		else if(pId=='cmplx_AddressDetails_cmplx_AddressGrid')
		{
			...
			{
			if(getNGValue('Is_Principal_Approval')!='Y')
			{
		...
		var n=com.newgen.omniforms.formviewer.getLVWRowCount("cmplx_FATCA_cmplx_GR_FatcaDetails");
			...
			for(var i=0;i<n;i++)
			{
			apptype+=getLVWAT('cmplx_FATCA_cmplx_GR_FatcaDetails',i,13);
			}
			if(Principal_Approval!='Y')
			{
			...
			{
			if(n==0 ||  apptype.indexOf("P-")==-1)
			{
			showAlert('cmplx_FATCA_CustomerType','Please add FATCA for Primary Applicant type');	
					return false;
			}
			}
			...
			for(var i=0;i<n;i++)
			{
			apptype+=getLVWAT('cmplx_OECD_cmplx_GR_OecdDetails',i,8);
			}
			if(Principal_Approval!='Y')
			{
			if((picklistValues.length)>n || n==0)
			...
			if(n==0 ||  apptype.indexOf("P-")==-1)
			{
			showAlert('OECD_CustomerType','Please add OECD for Primary Applicant type');	
					return false;
			}
			}
			removeFrame(pId);
		...
		if(field_string_value.indexOf(value)==-1){
			...
			else if(getLVWAT("cmplx_Product_cmplx_ProductGrid",0,2)=='IM') //by jahnavi for IM application form
			{
				docName='Application_Form_IM';
			}
			else 
			{
			...
			var ajaxReq;
				...
				var cb=true;
				var url = "/webdesktop/custom/CustomJSP/Generate_Template.jsp?attrbList="+attrbList+"&<othercb>="+cb+"&wi_name="+wi_name+"&docName="+docName+"&sessionId="+window.parent.sessionId;
		...
		else if(pId=='Customer_Consents_container' || pId=='cmplx_customerConsents_Consent_for_Release_of_Statement_account'|| pId=='cmplx_customerConsents_ContactMe'|| pId=='CustomerConsent_save')//Funding
		{
			return true;
		}
...
function RLOS_postHookDRefresh(controlName){
	...
	else if(controlName=='Incomedetails' || controlName=='Customer_Check')
	{
		if(getNGValue('cmplx_Customer_NTB')==true && getNGValue('cmplx_Customer_CIFNO')!='')
		{
			if(getNGValue("PrimaryProduct")=='Personal Loan' && controlName=='Incomedetails' ){
			} else {
				setNGValueCustom('cmplx_Customer_CIFNO','');
			}
		}
	if(getNGValue('cmplx_IncomeDetails_Basic')!=null)
	{
	setNGValueCustom('cmplx_IncomeDetails_Basic',getNGValue('cmplx_Customer_Basic_Sal'));
	...
	else if(controlName=='Alt_Contact_container')
	{
		setNGValueCustom('AlternateContactDetails_Email1',getNGValue('cmplx_Customer_Email_ID'));
		setNGValueCustom('AlternateContactDetails_Email2',getNGValue('cmplx_Customer_Email_ID'));
	}
