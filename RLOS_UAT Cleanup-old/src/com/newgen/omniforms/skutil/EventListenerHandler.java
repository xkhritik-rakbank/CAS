        ...
        if ("EMploymentDetails_Button2".equalsIgnoreCase(m_objPickList.getAssociatedTxtCtrl())) {
            ...
            try{
               ...
               /* if(!m_objPickList.getSelectedValue().get(4).equalsIgnoreCase("")){
                	RLOS.mLogger.info("m_objPickList.getSelectedValue().get(4): "+m_objPickList.getSelectedValue().get(4));
                	new Common().LoadPickList("cmplx_EmploymentDetails_Indus_Macro", "select '--Select--' as Description,'' as code union select Description,code from NG_MASTER_EmpIndusMacro with (nolock)");
                	new Common().LoadPickList("cmplx_EmploymentDetails_Indus_Micro", "select '--Select--' as Description,'' as code union select Description,code from NG_MASTER_EmpIndusMicro with (nolock) order by code");
                }*/
