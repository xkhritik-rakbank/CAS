...
public class RLOS_IntegrationInput extends RLOSCommon implements Serializable
{
	...
	public String InternalBureauPipelineProducts(){
		...
		FormReference formObject = FormContext.getCurrentInstance().getFormReference();
		String sQuery = "SELECT cifid,loantype,custroletype,lastupdatedate,totalamount,totalnoofinstalments,totalloanamount,agreementId,NoOfDaysInPipeline FROM ng_RLOS_CUSTEXPOSE_LoanDetails  with (nolock) where Wi_Name = '"+formObject.getWFWorkitemName()+"' and  LoanStat = 'Pipeline'";
	...
	public String ExternalBureauIndividualProducts(){
		...
		for (int i = 0; i<OutputXML.size();i++){
			...
			if (ReqProd.equalsIgnoreCase(NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_CreditCard"))){
				add_xml_str = add_xml_str + "<qc_amount>"+QC_Amt+"</qc_amount><company_flag>N</company_flag><cac_bank_name></cac_bank_name><consider_for_obligation>"+consider_for_obligation+"</consider_for_obligation><duplicate_flag>"+Duplicate_flag+"</duplicate_flag></ExternalBureauIndividualProducts>";
			}
			else{
		...
		*/
		...
		if (Man_liab_row_count !=0){
			for (int i = 0; i<Man_liab_row_count;i++){
				...
				String dpd_60p_in_last_18_mon = (NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_true").equalsIgnoreCase(formObject.getNGValue("cmplx_Liability_New_cmplx_LiabilityAdditionGrid", i, 15))?"1":"0");
				...
				add_xml_str = add_xml_str + "<cac_indicator>"+cac_Indicator+"</cac_indicator>";
				add_xml_str = add_xml_str + "<qc_emi>"+Qc_Emi+"</qc_emi>";
				add_xml_str = add_xml_str + "<qc_amount>"+Qc_amt+"</qc_amount><company_flag>N</company_flag><cac_bank_name></cac_bank_name><take_over_indicator>"+Take_over_Indicator+"</take_over_indicator><consider_for_obligation>"+consider_for_obligation+"</consider_for_obligation><duplicate_flag>N</duplicate_flag></ExternalBureauIndividualProducts>";
	...
	public String ExternalBureauPipelineProducts(){
		...
		FormReference formObject = FormContext.getCurrentInstance().getFormReference();
		String sQuery = "select AgreementId,ProviderNo,LoanType,LoanDesc,CustRoleType,Datelastupdated,TotalAmt,TotalNoOfInstalments,'' as col1,NoOfDaysInPipeline,isnull(Consider_For_Obligations,'true'),case when IsDuplicate= '1' then 'Y' else 'N' end from ng_rlos_cust_extexpo_LoanDetails where wi_name  =  '"+formObject.getWFWorkitemName()+"' and LoanStat = 'Pipeline' union select CardEmbossNum,ProviderNo,CardTypeDesc,CardType,CustRoleType,LastUpdateDate,'' as col2,NoOfInstallments,TotalAmount,NoOfDaysInPipeLine,isnull(Consider_For_Obligations,'true'),case when IsDuplicate= '1' then 'Y' else 'N' end  from ng_rlos_cust_extexpo_CardDetails where wi_name  =  '"+formObject.getWFWorkitemName()+"' and cardstatus = 'Pipeline'";
...
*/
		...
		for (int i = 0; i<prod_row_count;i++){
			...
			prod_xml_str = prod_xml_str + "<limit_expiry_date>"+limitExpiry+"</limit_expiry_date><final_limit>"+formObject.getNGValue("cmplx_EligibilityAndProductInfo_FinalLimit")+"</final_limit><emi>"+EMI+"</emi><manual_deviation>N</manual_deviation><application_date>"+ApplicationDate+"</application_date></ApplicationDetails>";
			...
			for (List<String> mylist : DB_List) {
				...
				if ("AddressDetails".equalsIgnoreCase(tag_name)	&& int_xml.containsKey(parent_tag)) {
					...
					if(int_xml.containsKey(parent_tag))
					{
						...
						xml_str = xml_str + "<"+tag_name+">"+"<ChildCIFID>"+cif_id+"</ChildCIFID><ContactName>"+Contact_Name+"</ContactName><KnownYears></KnownYears><Relationship></Relationship>"
						+"</"+ tag_name+">";
					...
					if("PersonDetails".equalsIgnoreCase(tag_name)){
						...
						String xml_str = int_xml.get(parent_tag);
						String ph_no="";
							ph_no=formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,10);
						...
						String xml_str = int_xml.get(parent_tag);
						String email=formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,27);
						...
						String status=formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid", 0, 23); 
						//added by aman for Drop4 on 4th April
						...
						String xml_str = int_xml.get(parent_tag);
						//added by aman for Drop4 on 4th April
						String Occupation=formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid", 0, 25);
						//added by aman for Drop4 on 4th April
		...
		try{
			for (List<String> mylist : OutputXML) {
				...
				else if("current_emp_catogery".equalsIgnoreCase(tag_name))
				{
					...
					String squerycurremp="select Description from NG_MASTER_EmployerCategory_PL with (nolock) where isActive='Y' and code='"+current_emp_catogery+"'";
...
*/
					...
					if(int_xml.containsKey(parent_tag))
					{
						...
						String aeQuery = " select (select isNull((Sum(convert(float,replace([TotalOutstandingAmt],'NA','0')))),0) as TotalOutstandingAmt from ng_RLOS_CUSTEXPOSE_loanDetails with (nolock) where Consider_For_Obligations='true' and LoanStat in ('A','ACTIVE') and wi_name ='"+formObject.getWFWorkitemName()+"') + (select isNull((Sum(convert(float,replace([TotalOutstandingAmt],'NA','0')))),0) as TotalOutstandingAmt from ng_RLOS_CUSTEXPOSE_loanDetails with (nolock) where Consider_For_Obligations='true' and LoanStat ='Pipeline' and wi_name ='"+formObject.getWFWorkitemName()+"') +(select isNull((Sum(convert(float,replace([OutstandingAmt],'NA','0')))),0)  from ng_RLOS_CUSTEXPOSE_CardDetails with (nolock) where  Consider_For_Obligations='true' and CardStatus in ('A','ACTIVE') and wi_name  ='"+formObject.getWFWorkitemName()+"' and SchemeCardProd like '%LOC%') + (select isNull((Sum(convert(float,replace([CreditLimit],'NA','0')))),0) as OutstandingAmt from ng_RLOS_CUSTEXPOSE_CardDetails with (nolock) where  Consider_For_Obligations='true' and CardStatus in ('A','ACTIVE') and wi_name  ='"+formObject.getWFWorkitemName()+"' and SchemeCardProd not like '%LOC%')+( select isNull((Sum(convert(float,replace([final_limit],'NA','0')))),0) from ng_rlos_EligAndProdInfo with (nolock) where Wi_Name ='"+formObject.getWFWorkitemName()+"') +  ( select isNull((Sum(convert(float,replace([sanctionLimit],'NA','0')))),0) from ng_RLOS_CUSTEXPOSE_AcctDetails with (nolock) where Wi_Name ='"+formObject.getWFWorkitemName()+"' and ODDesc is not null and AcctStat in ('A','ACTIVE')) as aggregateExposure";
	...
	public String getGuarantor_PersonDetails(FormReference formObject){
		...
		String age=formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,11);
		String MinorFlag="";
		...
		if(formObject.getNGValue("cmplx_DecisionHistory_MultipleApplicantsGrid",formObject.getSelectedIndex("cmplx_DecisionHistory_MultipleApplicantsGrid"),0).equalsIgnoreCase("GUARANTOR") && formObject.getNGValue("cmplx_DecisionHistory_MultipleApplicantsGrid",formObject.getSelectedIndex("cmplx_DecisionHistory_MultipleApplicantsGrid"), 2).equals(formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,5)))
		{
			str=str+"<PersonDetails><TitlePrefix>"+formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,6)+"</TitlePrefix>";
			str=str+"<FirstName>"+formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,7)+"</FirstName>";
			str=str+"<LastName>"+formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,9)+"</LastName>";
			str=str+"<ShortName>"+formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,7).substring(0,1)+formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,9).substring(0,1)+"</ShortName>";
			str=str+"<Gender>"+formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,12)+"</Gender>";
			str=str+"<MotherMaidenName>"+formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,26)+"</MotherMaidenName>";
			str=str+"<MinorFlag>"+MinorFlag+"</MinorFlag>";
			str=str+"<NonResidentFlag>N</NonResidentFlag>";
			str=str+"<ResCountry></ResCountry>";
			str=str+"<MaritalStatus>"+formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,22)+"</MaritalStatus>";
			str=str+"<Nationality>"+formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,4)+"</Nationality>";
			str=str+"<DateOfBirth>"+common.Convert_dateFormat(formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,3), "dd/MM/yyyy","yyyy-MM-dd")+"</DateOfBirth>";
	...
	public String getGuarantor_DocDetails(FormReference formObject){
		...
		if(formObject.getNGValue("cmplx_DecisionHistory_MultipleApplicantsGrid",formObject.getSelectedIndex("cmplx_DecisionHistory_MultipleApplicantsGrid"),0).equalsIgnoreCase("GUARANTOR") && formObject.getNGValue("cmplx_DecisionHistory_MultipleApplicantsGrid",formObject.getSelectedIndex("cmplx_DecisionHistory_MultipleApplicantsGrid"), 2).equals(formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,5)))
		{
			str=str+generateDocDetailsTag(Arrays.asList("PPT",formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,20),formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,16),formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,4),formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,5)));
			str=str+generateDocDetailsTag(Arrays.asList("VISA",formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,21),formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,18),"AE",formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,17)));
			str=str+generateDocDetailsTag(Arrays.asList("EMID",formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,13),formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,14),formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,4),formObject.getNGValue("cmplx_GuarantorDetails_cmplx_GuarantorGrid",0,2)));
	}
