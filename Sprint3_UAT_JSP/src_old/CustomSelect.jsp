...
//		sOutputXML=WFCallBroker.execute(sInputXML.toString(),sJtsIp,iJtsPort, appServerType);
	...
	public static String DocTemplateData_CC(String Query)
	{
		...
		{
			sQry="select stuff ((select ','+AcctId from ng_RLOS_CUSTEXPOSE_AcctDetails where Child_Wi=:WiName and  (isDirect='N' or isDirect=null) and acctstat not in ('C' ,'Closed') and CifId  in (select CIF_ID from NG_RLOS_Customer where wi_name=:WiName)  for xml path('')),1,1,'') as AcctNumber";
		}
	...
	public static String PL_Js(String Query)
	{
		...
		{
			sQry="select top 1 ReportURL from NG_RLOS_CUSTEXPOSE_Derived where child_wi=:Wi_Name and Request_Type='ExternalExposure' ";
		}
