...
<%
	...
	String params="";
	String Query_CIF ="select cifid from "+table+" with (nolock) where child_wi='"+Wi_Name+"' and "+agreement_col+"='"+AgreementId+"'";
...
//	WriteLog("AgreementId jsp: result: "+AgreementId);
	...
	try{
        ...
        sQuery_cc="select top 1 * from ((select (case when (convert(datetime,CardsApplcnRecvdDate))>=dateadd(month, -6,getdate()) then CardsApplcnRecvdDate else CardsApplcnRecvdDate  END) as date_val,(case when (convert(datetime,CardsApplcnRecvdDate))>=dateadd(month, -6,getdate()) then 'yes' else 'no' END)as product_date from ng_RLOS_CUSTEXPOSE_CardDetails with (nolock) where CardType in ('CREDIT CARDS') and Child_Wi='"+Wi_Name+"' union select (case when (convert(datetime,Loan_Start_Date))>=dateadd(month, -6,getdate()) then Loan_Start_Date else Loan_Start_Date END) as date_val,(case when (convert(datetime,Loan_Start_Date))>=dateadd(month, -6,getdate()) then 'yes' else 'no' END)as product_date   from ng_RLOS_CUSTEXPOSE_LoanDetails where Child_Wi='"+Wi_Name+"' and LoanType in ('MORTGAGE','CC','PL','Personal','IM','Instant Money','AUTO' ,'RFL')))as ext where date_val is not null order by date_val asc";
