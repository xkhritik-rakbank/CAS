...
var Cab_Name = '<%=sCabname%>';
	...
	var initiationSteps = ['Branch_Init','CSM','Telesales_Init','Re_Initiate'];
	var pname="ECL";
	//changes done for svt server
	if(activityName=='CAS'){
		activityName='CA';
	}
	else if(activityName=='Cr'|| activityName=='CDOB'){
		activityName='CC';
	}
...
function disable_at_DDVT(){
	...
	if(((activityName=="CC") && window.parent.com.newgen.omniforms.formviewer.getLVWAT('cmplx_Product_cmplx_ProductGrid',0,1)  =='Credit Card' && ( ws_name=='Fulfillment_RM' || ws_name=='Telesales_RM'  || ws_name=='DDVT_Maker' || ws_name=='Cad_Analyst2')) || ((activityName=="PL") && (ws_name=='DDVT_Checker' || ws_name=='Original_Validation'))){
		for (var i=1;i<toal_row_count;i++){
...
function CC_Waiver_Check(id)
{
	...
	if(flag_CC_Waiver==false && total_row_count!=0){
