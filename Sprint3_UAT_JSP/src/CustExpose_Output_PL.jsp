			...
			else
			{
				...
				for (Map.Entry<String, String> entry : map.entrySet())
				{
					...
					//Deepak 23 Dec changes done to save updated Rerport URL in DB.
						...
						columnName = valueArr[0]+",Wi_Name,Request_Type,CifId,FullNm,TotalOutstanding,TotalOverdue,NoOfContracts,ReportURL,Child_Wi,ReferenceNo,AECB_Score,Range,EnquiryDate";
						columnValues = valueArr[1]+",'"+parentWiName+"','"+returnType+"','"+cifId+"','"+FullNm+"','"+TotalOutstanding+"','"+TotalOverdue+"','"+NoOfContracts+"','"+ReportUrl+"','"+wi_name+"','"+referenceNo+"','"+Aecb_Score+"','"+range+"','"+EnquiryDate+"'";
					...
					//above changes Done to save AccountType in ng_RLOS_CUSTEXPOSE_AcctDetails table on 29th Dec by Disha
					  ...
					  String isDirect = (parseXml.contains("<IsDirect>")) ? parseXml.substring(parseXml.indexOf("<IsDirect>")+"</IsDirect>".length()-1,parseXml.indexOf("</IsDirect>")):"";
						  ...
						  columnName = valueArr[0]+",Wi_Name,Child_Wi,Request_Type,CifId";
						   ...
						   sWhere="Wi_Name='"+parentWiName+"' AND Child_Wi='"+wi_name+"' AND CifId = '"+cifId+"'";  
						  }
					//ended here
					else{
						...
						for(int ch_len = 0 ;ch_len< ch_nodeList.getLength(); ch_len++){
							if(sub_tag.contains(ch_nodeList.item(ch_len).getNodeName())){
								...
								if(!sub_ch_nodeList.item(0).getTextContent().equalsIgnoreCase("#text")){
									WriteLog("getTagDataParent_deep jsp: subtag_notused: "+subtag_notused);
									WriteLog("getTagDataParent_deep jsp: getTextContent: "+sub_ch_nodeList.item(0).getTextContent());
									WriteLog("getTagDataParent_deep jsp: getTextvalue: "+sub_ch_nodeList.item(1).getTextContent());
