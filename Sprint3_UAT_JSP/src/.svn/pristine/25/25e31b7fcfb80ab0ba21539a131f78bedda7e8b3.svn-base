...
var Cab_Name = '<%=sCabname%>';
	...
	var pname="FAN";
	var initiationSteps = ['Branch_Init','CSM','Telesales_Init','Re_Initiate'];//pcasp-2373
	...
	if(val==""){
		...
		for(var i=0;i<row_arr.length-1;i++){
			...
			if(col_id_arr[1]=="text"){
				...
				var value = val_new[i];
				if(value == "true"){
					if(initiationSteps.indexOf(ws_name)==-1 && 'CAD_Analyst1'.indexOf(ws_name)==-1 && 'DDVT_maker'.indexOf(ws_name)==-1){
					    table_row = table_row +"<td>"+"<input id = '"+col_id_arr[3]+"_"+row_num+"' type = '"+ col_id_arr[1] +"' name = '"+col_id_arr[3]+"_"+row_num+"' onchange = 'check_change(this.id);' "+col_id_arr[4]+" "+col_id_arr[5]+"  checked disabled>"+"</td>";
					} else {
					...
					if(initiationSteps.indexOf(ws_name)==-1 && 'CAD_Analyst1'.indexOf(ws_name)==-1 && 'DDVT_maker'.indexOf(ws_name)==-1){
					table_row = table_row +"<td>"+"<input id = '"+col_id_arr[3]+"_"+row_num+"' type = '"+ col_id_arr[1] +"' name = '"+col_id_arr[3]+"_"+row_num+"' onchange = 'check_change(this.id);' value='"+col_id_arr[4]+"' "+col_id_arr[5]+" disabled>"+"</td>";
					}else {
...
function check_change(obj_id){
	var totalrowid= $('table#mytab tr:last').index();
		...
		if(is_Account_Checked==true){
			...
			window.parent.com.newgen.omniforms.formviewer.setEnabled('AlternateContactDetails_RetainAccIfLoanReq',false);	
			...
			//if(document.getElementById(acct_no_name).value!=''){
				window.parent.com.newgen.omniforms.formviewer.setNGValue('Funding_Account_no',document.getElementById(acct_no).value);
				window.parent.com.newgen.omniforms.formviewer.setNGValue('Account_Number',document.getElementById(acct_no).value);
			//}
		}
		else{
			window.parent.com.newgen.omniforms.formviewer.setEnabled('AlternateContactDetails_RetainAccIfLoanReq',true);	
			...
			window.parent.com.newgen.omniforms.formviewer.setNGValue('Funding_Account_no','');
...
function disable_at_CA(){

	var toal_row_count= $('table#mytab tr:last').index() + 1;//added by shweta for pcasp-4373
	if((activityName=="PL") && window.parent.com.newgen.omniforms.formviewer.getLVWAT('cmplx_Product_cmplx_ProductGrid',0,1)  =='Personal Loan'){ 
		for (var i=1;i<toal_row_count;i++){
				var AcctId =  "AcctId_"+i;
				var AcctType = "AcctType_"+i;
				var AcctNm = "AcctNm_"+i;
				var AccountOpenDate = "AccountOpenDate_"+i;
				var AcctSegment = "AcctSegment_"+i;
				var AcctSubSegment = "AcctSubSegment_"+i;
				var AcctStat = "AcctStat_"+i;
				var Credit_Grade_Code_ = "Credit_Grade_Code_"+i;

				
				document.getElementById(AcctId).disabled = true;
				document.getElementById(AcctType).disabled = true;
				document.getElementById(AcctNm).disabled = true;
				document.getElementById(AccountOpenDate).disabled = true;
				document.getElementById(AcctSegment).disabled = true;
				document.getElementById(AcctSubSegment).disabled = true;
				document.getElementById(AcctStat).disabled = true;
				document.getElementById(Credit_Grade_Code_).disabled = true;				
				}
		}
