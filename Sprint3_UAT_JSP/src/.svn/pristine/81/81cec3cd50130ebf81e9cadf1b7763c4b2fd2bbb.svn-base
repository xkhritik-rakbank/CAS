...
<%
	...
	String input2 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("req_name"), 1000, true) );
	String req_name = ESAPI.encoder().encodeForSQL(new OracleCodec(), input2);
	//String req_name = request.getParameter("req_name");
	
	String input3 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("activity"), 1000, true));
	String activityName = ESAPI.encoder().encodeForSQL(new OracleCodec(), input3);
	//String activityName = request.getParameter("activity");
	
	String input4 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("sessionId"), 1000, true));
	String sessionId = ESAPI.encoder().encodeForSQL(new OracleCodec(), input4);
	//String sessionId = request.getParameter("sessionId");
	String child_wi = "";
	String params="";
	WriteLog("activityName is: "+activityName);
	if(activityName.indexOf("Init")<0 && !activityName.equalsIgnoreCase("Re_Initiate")){
		String input5=ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("child_wi"), 1000, true));
		child_wi = ESAPI.encoder().encodeForSQL(new OracleCodec(), input5);
	}
	String cabinetName ="";
			String wrapperIP = "";
			String wrapperPort = "";
