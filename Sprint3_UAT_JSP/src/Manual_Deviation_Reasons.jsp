...
<script type="text/javascript" language="JavaScript" src="${pageContext.request.contextPath}/custom/CustomJSP/jquery-latest.js"></script>
<script type="text/javascript" language="JavaScript" src="${pageContext.request.contextPath}/custom/CustomJSP/jquery.autocomplete.js"></script>
 <!-- esapi4js dependencies --><script type="text/javascript" language="JavaScript" src="${pageContext.request.contextPath}/custom/esapi4js/lib/log4js.js"></script>
...
//svt points start
	String input1 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("ProcessName"), 1000, true) );
	String process_Name = ESAPI.encoder().encodeForSQL(new OracleCodec(), input1);
	
	String input2 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("deviationReason"), 1000, true) );
	String deviationReason = ESAPI.encoder().encodeForSQL(new OracleCodec(), input2);
...
//svt points start		
	...
	var deviationReason_encode = '<%=deviationReason%>';
	var deviationReason = ($ESAPI.encoder().decodeForHTML(deviationReason_encode));
//svt points end	
//***********************************************************************************************************//
			...
			function setAutocompleteData() {
				...
				//alert('deviationReason '+ deviationReason);
					...
					for(var i=0;i<deviationReason_array.length;i++)
					{
						...
						if(Val!=null && Val!="null" && Val!="NO_CHANGE" )
					{
...
//********************************************************************************************//			
			function populateListbox(codeString) {
				...
				for (var i=0;i<codesArr.length;i++)	{
					...
					if(fieldVal!=null && fieldVal!="null" && fieldVal!="NO_CHANGE" )
					{
					var opt = document.createElement('option');
					opt.value = fieldVal;
...
//***********************************************************************************//			
			function OKClick()
			{
				...
				//Changes done by deepak for change to display Decision remarks in CAm report.
				...
				*/
				...
				if(process_Name=='PersonalLoanS')
				{
					...
					window.opener.setNGValueCustom('cmplx_Decision_Manual_deviation_reason',rejectreason);
				}
				//saurabh1 end
...
//***********************************************************************************//			
	...
	if("PersonalLoanS".equalsIgnoreCase(process_Name))
	{
		Query="select code,Description from NG_MASTER_PL_ManualDeviation_Reason with(nolock) where Active='Y' order by Sno ASC";
