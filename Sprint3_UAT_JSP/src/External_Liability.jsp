...
function saveDate() {
		...
		for(var row_arr_count= 1;row_arr_count<=totalrowid;row_arr_count++){
				...
				*/	
				...
				else if(stable=="ng_rlos_cust_extexpo_ServicesDetails"){
					if(activityName != 'CA'){
						data_save_var="Settlement_Flag,Consider_For_Obligations,Remarks";
							swhere = "Child_Wi = '"+winame+"' and ServiceID = '"+Provider_Number_val+"'";
							}
							else{
							data_save_var="Settlement_Flag,Consider_For_Obligations,Remarks";
							swhere = "Wi_Name = '"+winame+"' and ServiceID = '"+Provider_Number_val+"'";
							}
					}
				...
				for(var x= 0 ; x<data_save_var_arr.length ; x++){
					...
					if(document.getElementById(variable_id) != null ){
				...
				var params="table="+stable+"&columnsNames="+data_save_var+"&columnsValues="+value+"&swhere="+swhere+"&WD_UID="+WD_UID;//svt points
  ...
  function removeClick(obj) {
    ...
    function loadform() {
		...
		//added by nikhil 
		if(ws_name=="CSM" && activityName!="PL")
		{
			hideatcsm();
		}
		if(activityName=="PL")
	...
	function colorClosedLiab(){
		if(Object.keys(closedOrActive).length>0){
			...
			for(var key in closedOrActive){
					...
					document.getElementById('row_'+key).style.backgroundColor = "coral";
					setDesiredColorinCells(table,key,'coral');
				}
			}
		swaprow(table);
		...
		for(var i=0;i<table.rows.length;i++){
			for(var j=0;j<table.rows[i].cells.length;j++){
				if(table.rows[i].cells[j].childNodes[0].id.indexOf('CAC_Indicator')>-1 && document.getElementById('CAC_Indicator_'+i).checked){
					...
					document.getElementById('QC_Amt_'+i).disabled=false;
					//changed for PCSP-883 28th 
					document.getElementById('QC_Amt_'+i).readOnly=false;
					document.getElementById('QC_EMI_'+i).disabled=false;
					//changed for PCSP-883 28th 
					document.getElementById('QC_EMI_'+i).readOnly=false;
					checkedRownum = i;
					break;
				}
			}
...
function swaprow(table){
	...
	for(var key in closedOrActive){
		...
		if(closedOrActive[key].indexOf('Corporate_CIF')>-1 ||closedOrActive[key].indexOf('Individual_CIF')>-1 || closedOrActive[key].indexOf(',')==0){
...
function disable_postload_PL(){
	var toal_row_count= $('table#mytab tr:last').index() + 1;
	if(activityName=="PL" ){
		for (var i=1;i<toal_row_count;i++){
			var Outstanding_Amount = "Outstanding_Amount_"+i;
			var mob = "MOB_"+i;
			//var Take_Amount = "Take_Amount_"+i;
			var Liability_Type = "Liability_Type_"+i;
			var Limit_Amt_Fin = "Limit_Amt_Fin_"+i;
			var Type_Of_Contact = "Type_Of_Contact_"+i;
			var emi = "EMI_"+i;
			var Remarks='Remarks_'+i;
				document.getElementById(Outstanding_Amount).disabled = true;
				document.getElementById(mob).disabled = true;
				document.getElementById(Liability_Type).disabled = true;
				document.getElementById(Limit_Amt_Fin).disabled = true;
				//document.getElementById(Take_Amount).disabled = true;
				document.getElementById(Type_Of_Contact).disabled = true;
	...
	function disable_postload(){
		...
		if((ws_name=='CAD_Analyst1' || ws_name=='CSM')  && activityName=='PL' && (app_type=='TKOE' || app_type=='TKOE'))
				{
				for (var i=1;i<toal_row_count;i++){
				var table_name = "datatablename_"+i;
				if( (document.getElementById(table_name).value=="ng_rlos_cust_extexpo_loanDetails" || document.getElementById(table_name).value=="ng_rlos_cust_extexpo_CardDetails" )){
						var Take_Over_Indicator = "Take_Over_Indicator_"+i;
						document.getElementById(Take_Over_Indicator).disabled = false;
	...
	if(val==""){
		...
		for(var i=0;i<row_arr.length-1;i++){
			...
			if(col_id_arr[1]=="text"){
				...
				if((activityName=="CC" ||activityName=="Cr" || activityName=="PL" ) && ws_name == "CSM"){
		...
		if(activityName!='CA'){		
			if(val_new[val_new.length-2]=='Active' && activityName=='PL'){
				closedOrActive[row_num]= val_new[3]+','+ val_new[val_new.length-3];//by prabhakar
				
			}else{
				closedOrActive[row_num]= val_new[3]+','+ val_new[val_new.length-2];//by prabhakar
			}		
		}
		else{
		...
		for(var i=0;i<row_arr.length-1;i++){
			...
			var app_type = window.parent.com.newgen.omniforms.formviewer.getLVWAT('cmplx_Product_cmplx_ProductGrid',0,4);
				...
				else if(ws_name=='CAD_Analyst1' && col_id_arr[3]=="FinalcleanfundedAmt" && subproduct!='IM' )
				{
				...
				else if( (col_id_arr[3]=="FinalcleanfundedAmt" || col_id_arr[3]=="Remarks" ) && (subproduct=='IM' || activityName=='PL') )
				{
				table_row = table_row +"<td>"+"<input id = '"+col_id_arr[3]+"_"+row_num+"' type = '"+ col_id_arr[1] +"' name = '"+col_id_arr[3]+"_"+row_num+"' value='"+val_new[i+1]+"' onkeypress='validateNum(this.id)' onchange ='limitDecimal(this.id);'  title= '"+val_new[i+1]+"' maxlength ='"+col_id_arr[2]+"' disabled='disabled'>"+"</td>";
				}
				else{
			...
			//ended by akshay 	
				...
				var targetcode = window.parent.com.newgen.omniforms.formviewer.getNGValue('cmplx_EmploymentDetails_targetSegCode');
		
				if(val_new[0].indexOf('loan')>-1 || subproduct=='IM' || activityName=='PL' ||val_new[0].indexOf('Account')>-1 || targetcode!='CAC'){
					table_row = table_row +"<td>"+"<input id = '"+col_id_arr[3]+"_"+row_num+"' type = '"+ col_id_arr[1] +"' name = '"+col_id_arr[3]+"_"+row_num+"' "+col_id_arr[4]+" "+col_id_arr[5]+" disabled= 'disabled' value=false>"+"</td>";
				}
				else{
			  ...
			  else if((ws_name=='DDVT_Maker' || ws_name=='DDVT_maker'  || ws_name=='DDVT_Checker' || ws_name == "CSM" || ws_name=='CAD_Analyst1' || ws_name=='CAD_Analyst2') && col_id_arr[3]=="CleanFunded")
			  {
...
function openPopup(id)
	{
		...
		var liability_type = document.getElementById('Liability_Type_'+rownum).value;
		
		var url='/webdesktop/custom/CustomJSP/external.jsp?Wi_Name='+winame+'&AgreementId='+agreementval+'&table='+tablename+'&liability_type='+liability_type+'&WD_UID='+WD_UID;
		...
		if (window.showModalDialog) {
			popupWindow = window.showModalDialog(url,null,sOptions);
		} else {
			popupWindow = window.open(url,null,windowParams);
		}
