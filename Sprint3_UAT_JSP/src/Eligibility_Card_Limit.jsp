...
<%
    ...
    String input1 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("ChecklistAbbr"), 1000, true) );
	String ChecklistAbbr = ESAPI.encoder().encodeForSQL(new OracleCodec(), input1);	
    //svt points end
	if (sSessionId == null) {
...
var Cab_Name = '<%=sCabname%>';
	...
	var initiationSteps = ['Branch_Init','CSM','Telesales_Init','Re_Initiate','Paperbased_Init'];//pcasp-2373
	var pname="ECL";
	//changes done for svt server
	if(activityName=='CAS'){
		activityName='CA';
	}
	else if(activityName=='Cr'|| activityName=='CDOB' || activityName=='CreditCard'){
		activityName='CC';
	}
...
function saveDate() {
		...
		for(var i=1;i<=row_id_arr.length;i++){
			...
			/*if(parseInt(document.getElementById('Final_Limit_1').value)<parseInt(window.parent.document.getElementById('ELigibiltyAndProductInfo_Text24').value))
			{
				alert("Final amount cannot be less than existing combined eligibility");
				return false;
			}*/
	...
	for(var row_arr_count= 1;row_arr_count<=totalrowid;row_arr_count++){
				...
				for(var x= 0 ; x<data_save_var_arr.length ; x++){
					...
					if(activityName=='CA'){
					key="ECL_P_"+document.getElementById(Card_Product).value+"_"+document.getElementById(EligibleAmount).value;
					...
					key="ECL_C_"+document.getElementById(Card_Product).value+"_"+document.getElementById(EligibleAmount).value;
				...
				key=key + "_" + winame;
				var params="key="+key+"&Name="+InputField_id+"&Value="+value;
				update_result=update_result+","+CallAjax("CustomUpdate_svt.jsp",params);
			}
...
function limitDecimal(Id){
	...
	//done by isha for JIRA-PCSP-298
	else if (window.parent.com.newgen.omniforms.formviewer.getLVWAT('cmplx_Product_cmplx_ProductGrid',0,1)=='Personal Loan' ){
		window.parent.document.getElementById('cmplx_EligibilityAndProductInfo_FinalLimit').value = parseInt(plHidden)+parseInt(efcHidden);
	}
	else{
...
function isNumberKey(evt,fieldId){
  ...
  function removeClick(obj) {
	...
	if (window.parent.com.newgen.omniforms.formviewer.getLVWAT('cmplx_Product_cmplx_ProductGrid',0,1)!='Personal Loan'){
		disable_CCWaiver();	
		
	}
	if(activityName=='CA' || ws_name=='DDVT_Maker')
	{
	CreditSield_CA();
	}
	    disable_Creditshield();	
		disable_existing();
		listIslamicCards();
		disable_at_DDVT();
		CC_Waiver_Check('');
		if (activityName=='PL' && !(initiationSteps.indexOf(ws_name)==1)){
			disable_CCWaiverPL();	//To disable CC waiver
		}else if(activityName=='CC'){	//Added by shivanshi PPCT-44
			disable_CCWaiverCC();
		}
		...
		if("R"==window.parent.parent.wiViewMode)
		{
			$("*","#mainDivContent").prop('disabled',true); //hritik
			/*var field = document.getElementById('mainDivContent').getElementsByTagName('*');
...
function disable_existing(){
	var toal_row_count= $('table#mytab tr:last').index() + 1;
		for (var i=1;i<toal_row_count;i++){
			...
			if(document.getElementById(exis_flag).value=='E' || document.getElementById(exis_flag).value=='e'){
				...
				var CardShield = "CardShield_"+i;
				//document.getElementById(CardShield).disabled = true;
	...
	//Added by Imran to enable CC_Waiver for PL
	/*if(window.parent.com.newgen.omniforms.formviewer.getLVWAT('cmplx_Product_cmplx_ProductGrid',0,1)  =='Personal Loan')
		document.getElementById("CC_Waiver_1").disabled = false;*/
}
...
function disable_Creditshield(){
	var i=1;

	var CardShield=" " ;
	if(activityName=="CA" || activityName=="PL")
	{
	var age = window.parent.document.getElementById('cmplx_Customer_age').value;
	...
	var toal_row_count= $('table#mytab tr:last').index() + 1;
	if('DDVT_Maker'.indexOf(ws_name)==-1)
	{
	if((activityName=="CC"||(activityName=="CA" && age>65)) && window.parent.com.newgen.omniforms.formviewer.getLVWAT('cmplx_Product_cmplx_ProductGrid',0,1)  =='Credit Card'){
		for (i=1;i<toal_row_count;i++){
				 CardShield = "CardShield_"+i;
		
				document.getElementById("CardShield_"+i).disabled = true;
...
function disable_CCWaiverPL(){
	var toal_row_count= $('table#mytab tr:last').index() + 1;
		for (var i=1;i<toal_row_count;i++){
				var CC_Waiver = "CC_Waiver_"+i;
				var final_limit = "Final_Limit_"+i;
				if('CAD_Analyst1'.indexOf(ws_name)==-1){
...
//Added by shivanshi PPCT-44
function disable_CCWaiverCC(){
 var toal_row_count= $('table#mytab tr:last').index() + 1;
		for (var i=1;i<toal_row_count;i++){
				 var CC_Waiver = "CC_Waiver_"+i;
				 var final_limit = "Final_Limit_"+i;
console.log("Workstep name:-"+ws_name)				 
				 if('Cad_Analyst2'.indexOf(ws_name)>-1){
					 document.getElementById(final_limit).disabled = true;
				}
				 document.getElementById(CC_Waiver).disabled = true;		
		 }
...
function disable_at_DDVT(){
	var toal_row_count= $('table#mytab tr:last').index() + 1;//added by shweta for pcasp-4373
	if(((activityName=="CC") && window.parent.com.newgen.omniforms.formviewer.getLVWAT('cmplx_Product_cmplx_ProductGrid',0,1)  =='Credit Card' && ( ws_name=='Fulfillment_RM' || ws_name=='Telesales_RM'  || ws_name=='DDVT_Maker' || ws_name=='Cad_Analyst2')) || ((activityName=='PL') && ( ws_name=='DDVT_maker' ||ws_name=='DDVT_Checker' || ws_name=='CAD_Analyst2' || ws_name=='Original_Validation'))){
		for (var i=1;i<toal_row_count;i++){
...
function fetchGridData()
{
	...
	//var sQry="select Card_Product,Eligible_Limit,Final_Limit FROM  "+table_name+" with (nolock) WHERE wi_name = '"+winame+"'";
			...
			params="Query="+sQry+"&wparams="+wparams+"&pname="+pname+"&WD_UID="+WD_UID;//svt points
...
function fetchValidQueueData()
{
			...
			if (ws_name=='CAD_Analyst1' || ws_name=='DDVT_Maker'){	
			 dhtml+=/*"<image src='addImage.png' height='15px' width='15px' id='addimage1"+"' style='padding-right:10px' onclick='addClick(this)'/>*/"<input type='button' id='savedata' value='save data' style='background-color:#e01414; color:#ffffff' onclick='saveDate()'/></div></table>";
			}
			else{
		...
		for(var i=0;i<row_arr.length-1;i++){
			...
			if(col_id_arr[1]=="text" && col_id_arr[3]=="Final_Limit"){
				...
				if(initiationSteps.indexOf(ws_name)==-1 && 'CAD_Analyst1'.indexOf(ws_name)==-1){
					...
					table_row = table_row +"<td>"+"<input id = '"+col_id_arr[3]+"_"+row_num+"' type = '"+ col_id_arr[1] +"' name = '"+col_id_arr[3]+"_"+row_num+"' value='"+val_new[i]+"' maxlength ='"+col_id_arr[2]+"' onkeypress='isNumberKey(event,this.id);' onchange ='Final_limitCal(this.id);setCardProdDropdown();limitDecimal_final(this.id);' "+col_id_arr[5]+">"+"</td>";
				}
			}
			else if(col_id_arr[3]=="flag" || col_id_arr[3]=="combined_limit" ){
				table_row = table_row +"<td>"+"<input type='hidden' id = '"+col_id_arr[3]+"_"+row_num+"' type = '"+ col_id_arr[1] +"' name = '"+col_id_arr[3]+"_"+row_num+"' value='"+val_new[i]+"' maxlength ='"+col_id_arr[2]+"' onchange ='Final_limitCal(this.id);limitDecimal_final(this.id);' "+col_id_arr[5]+">"+"</td>";
			}
				...
				if(initiationSteps.indexOf(ws_name)==-1 && 'CAD_Analyst1'.indexOf(ws_name)==-1){
					if(value == "true"){
							if('DDVT_Maker'.indexOf(ws_name)!=-1 && col_id_arr[3] == "CardShield" && row_num==1)
						{
						if(age<65)
						{
						table_row = table_row +"<td>"+"<input id = '"+col_id_arr[3]+"_"+row_num+"' type = '"+ col_id_arr[1] +"' name = '"+col_id_arr[3]+"_"+row_num+"' "+col_id_arr[4]+" "+col_id_arr[5]+"  checked enabled>"+"</td>";
						}
						else{
						table_row = table_row +"<td>"+"<input id = '"+col_id_arr[3]+"_"+row_num+"' type = '"+ col_id_arr[1] +"' name = '"+col_id_arr[3]+"_"+row_num+"' "+col_id_arr[4]+" "+col_id_arr[5]+"  disabled>"+"</td>";
					}
						}
					else{
						...
						if('DDVT_Maker'.indexOf(ws_name)!=-1 && col_id_arr[3] == "CardShield" && row_num==1)
						{
							if(age<65)
						{
						table_row = table_row +"<td>"+"<input id = '"+col_id_arr[3]+"_"+row_num+"' type = '"+ col_id_arr[1] +"' name = '"+col_id_arr[3]+"_"+row_num+"' "+col_id_arr[4]+" "+col_id_arr[5]+"   enabled>"+"</td>";
						}
						else{
					...
					else if(col_id_arr[3] == "CardShield"){
						
					if(value == "true" && age<65){
						if(activityName=="CA")
						{
						table_row = table_row +"<td>"+"<input id = '"+col_id_arr[3]+"_"+row_num+"' type = '"+ col_id_arr[1] +"' name = '"+col_id_arr[3]+"_"+row_num+"' "+col_id_arr[4]+" "+col_id_arr[5]+"  checked enabled' >"+"</td>";	
						}
...
function CC_Waiver_Check(id)
{
	...
	for (var row_count = 1;row_count<= total_row_count ; row_count++){
					...
					document.getElementById('Final_Limit_'+row_count).disabled=true;//by simi
					document.getElementById('Final_Limit_'+row_count).value='';		
		}
		if (window.parent.com.newgen.omniforms.formviewer.getLVWAT('cmplx_Product_cmplx_ProductGrid',0,1)=='Personal Loan' ){
			window.parent.document.getElementById('cmplx_EligibilityAndProductInfo_EFCHidden').value='0';	
			var value = 'EligibilityAndProductInformation';
			var field_string_value = window.parent.document.getElementById('FrameName').value;
			if(field_string_value.indexOf(value)==-1){
				var hiddenString = value+',';
			...
			for (var row_count = 1;row_count<= total_row_count ; row_count++){
				...
				//Imran; below added to enable field if ccwaiver not required
				var exis_flag = "flag_"+row_count;  
				if(!(document.getElementById(exis_flag).value=='E' || document.getElementById(exis_flag).value=='e'))
					document.getElementById('Final_Limit_'+row_count).disabled=false;		
...
function CreditShieldCheck(row,cardprod)
{
	jspName="CustomSelect.jsp";
	var params="";
	var result="";
	var sQry="CardProduct";
	var wparams="Card_Product==" + cardprod;
	//var sQry="select Card_Product,Eligible_Limit,Final_Limit FROM  "+table_name+" with (nolock) WHERE wi_name = '"+winame+"'";
			if(activityName=='CA'){//RLOS
				//sQry="select distinct Cardproductselect,Card_Product,Eligible_Limit,Final_Limit,CC_Waiver,flag,combined_limit FROM  "+table_name+" with (nolock) WHERE wi_name = '"+winame+"' order by Cardproductselect desc";
			//	wparams="Card_Product ="+card_product;
			}
			...
			else
			{
				...
				//for(var i=0;i<result_arr.length-1;i++){
				if(result_arr[0] == 'Y' && age<65)
				{
					document.getElementById('CardShield_'+row).disabled=false;
					//break;
					//document.getElementById('CardShield_'+row_count).checked=true;
				}
				else
				{
					document.getElementById('CardShield_'+row).disabled=true;
				//	document.getElementById('CardShield_'+row_count).checked=false;
				}
				//}
			}
}
...
function SelectCheck(id){
	...
	for (var row_count = 1;row_count<= total_row_count ; row_count++){
		...
		var cardproduct="Card_Product_"+row_count;
		if(document.getElementById(exis_flag).value=='E' || document.getElementById(exis_flag).value=='e'){
			i++;
		}
		...
		{
			i++;
			CreditShieldCheck(row_count,document.getElementById(cardproduct).value);
		}
	}
	if (parseInt(i)>9){
...
if(window.parent.com.newgen.omniforms.formviewer.isVisible('CardDetails_Frame1'))
{
		if(id.indexOf('Cardproductselect')>-1)
		{
			...
			if(!selectedRow[row]){
				...
				{
					selectedRow[row]='VM';
					if(window.parent.com.newgen.omniforms.formviewer.getLVWAT('cmplx_Product_cmplx_ProductGrid',0,0)=='Conventional' &&
							window.parent.com.newgen.omniforms.formviewer.getLVWAT('cmplx_Product_cmplx_ProductGrid',0,2)=='IM'){//pcasp-2363
						setVisibility(false);
					}else{
						setVisibility(true);
			...
			if((window.parent.com.newgen.omniforms.formviewer.getNGValue('cmplx_CardDetails_SuppCardReq')=='Yes' || window.parent.com.newgen.omniforms.formviewer.getNGValue('cmplx_CardDetails_suppcardreq')=='Yes') && cc_waiv){
...
function setVisibility(value){
	...
	window.parent.com.newgen.omniforms.formviewer.setVisible('cmplx_CardDetails_charityamt',value);
...
function setKalyanVisibility(value){
	...
	window.parent.com.newgen.omniforms.formviewer.setVisible('cmplx_CardDetails_cardemboss',value);

}


function limitDecimal_final(Id){
	if(Id.indexOf('final_limit')>-1){
		var value = document.getElementById(Id).value;
		if(isNaN(value) || value.indexOf('.')>-1){
			alert('Please enter only numberic Values only without .00');
			document.getElementById(Id).value = "";
		}
		else{
		if(value.indexOf('.')>-1){
