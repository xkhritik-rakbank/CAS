...
function saveDate() {
		...
		for(var row_arr_count= 1;row_arr_count<=totalrowid;row_arr_count++){
			...
			if(!calcTakeoverAmount(parseFloat(document.getElementById('FinalLoanAmount_1').value))){
				alert('Final loan amount should not be lesser than Takeover amount');
				document.getElementById('FinalLoanAmount_1').value='';
				window.parent.document.getElementById('cmplx_EligibilityAndProductInfo_FinalLimit').value='';
...
function calcTakeoverAmount(LoanLimit){
	var eframeId='';
	var oldVal=0.0 ;
	if(activityName=='CA'){eframeId='Liability_New_IFrame_external';}//by Shweta
	else if(activityName=='PL'){eframeId='ExtLiability_IFrame_external';}
	if(ngFormSyntaxVisible('cmplx_EligibilityAndProductInfo_TakeoverAMount')){
		var takeOverBalance = '';
		if(window.parent.getFromJSPTable(eframeId)){
			var externalLiabTable = window.parent.getFromJSPTable(eframeId);
			for(var i=1;i<externalLiabTable.rows.length;i++){
				if(externalLiabTable.rows[i].cells[0].childNodes[0].value.indexOf('Loan')>-1){
					if(window.parent.isJSPCheckboxChecked(eframeId,'Take_Over_Indicator_'+i)==true){
						takeOverBalance = window.parent.getJSPContextualData(eframeId,'Take_Amount_'+i);
						oldVal =oldVal+ parseFloat(takeOverBalance);
		 ...
		 if(LoanLimit< parseFloat(ngFormSyntaxGetvalue('cmplx_EligibilityAndProductInfo_TakeoverAMount'))){
			return false;
		}
	}	
	return 	true;
...
function setFinalLimit(id){
	...
	if(!calcTakeoverAmount(parseFloat(document.getElementById(id).value))){
		alert('Final loan amount should not be lesser than Takeover amount');
		document.getElementById(id).value='';
		window.parent.setNGValueCustom('cmplx_EligibilityAndProductInfo_FinalLimit',document.getElementById(id).value);
		window.parent.setNGValueCustom('Eligibility_Trigger','Y');
	...
	if(activityName=='PL'){
	window.parent.document.getElementById('LoanLabel').value = document.getElementById(id).value;	//PCASI - 3511
	window.parent.document.getElementById('LoanLabel').innerText = document.getElementById(id).value;	//PCASI - 3511
	}
	...
	if(window.parent.document.getElementById('cmplx_LoanDetails_lonamt')!=null){
		...
		window.parent.document.getElementById('cmplx_LoanDetails_lonamt').value = parseInt(plHidden);//Only loan amount should come under loan details	
	}
	...
	window.parent.setNGValueCustom('Final_Limit',document.getElementById(id).value);

}
function changeNetPayoutOnfinalLimit(newfinalLim,oldfinalLim){
	if(activityName=='CA'){iframeId='Liability_New_IFrame_internal';}//by Shweta
	else if(activityName=='PL'){iframeId='ExtLiability_IFrame_internal';}
	...
	if(ngFormSyntaxVisible('cmplx_EligibilityAndProductInfo_NetPayout')){
		var outBalance = '';
		if(window.parent.getFromJSPTable(iframeId)){
			var internalLiabTable = window.parent.getFromJSPTable(iframeId);
			try{
				for(var i=1;i<internalLiabTable.rows.length;i++){
					if(internalLiabTable.rows[i].cells[0].childNodes[0].value.indexOf('Loan')>-1){
						if(window.parent.isJSPCheckboxChecked(iframeId,'Limit_Increase_'+i)==true){
							outBalance = window.parent.getJSPContextualData(iframeId,'OutstandingAmt_'+i);
							break;
						}
					}	
				}
			...
			if(col_id_arr[1]=="text"){
				if(activityName=='CA'){ 	
					...
					if((row_num!=1)&&(i==5)){
						...
						table_row = table_row +"<td>"+"<input id = '"+col_id_arr[3]+"_"+row_num+"' title='"+val_new[i]+"' type = '"+ col_id_arr[1] +"' name = '"+col_id_arr[3]+"_"+row_num+"' value='"+val_new[i]+"' maxlength ='"+col_id_arr[2]+"'"+col_id_arr[5]+" disabled>"+"</td>";
					}
					}
				}
