...
<!-- esapi4js core -->
	...
	try {
			...
			String query="SELECT '--Select--' as File_Generated_name union SELECT distinct File_Generated_name FROM ng_rlos_Murabha_Warranty with(nolock) WHERE Murhabha_WIName IN(SELECT processinstanceid FROM WFINSTRUMENTTABLE with(nolock) WHERE (activityname='Disbursal' AND (ProcessName='CreditCard' or ProcessName='DigitalOnBoarding' )) OR ActivityName='CC_Disbursal') AND Is_Murhabah_Uploaded is not null and Is_Murhabah_Uploaded!='' and (IS_Murhabah_Confirm!='Y' or IS_Murhabah_Confirm is null)   order by File_Generated_Name desc";
		...
		<div class="form-group">		
			...
			<div class="form-group">
				...
				<label class="control-label col-sm-2">Purchase File Name</label>
					 <div class="col-sm-2">
						<select class="form-control" id="PurchaseFileName"  style='text-transform:uppercase' onchange='setValue(this.id)'>
					...
					xmlReq.onreadystatechange = function()
				{
					...
					case 4: 
						...
						if(trimStr(response)=='SUCCESS'){
							...
							window.opener.location.href="MurhabhaIslamicCSV.jsp?sSessionId="+sessionId + '&WD_UID='+WD_UID;
