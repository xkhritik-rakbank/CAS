...
var Cab_Name = '<%=sCabname%>';
	...
	else if(activityName=='CDOB' || activityName=='CreditCard'){
		activityName='CC';
	}
...
function saveDate() {
		...
		for(var row_arr_count= 1;row_arr_count<=totalrowid;row_arr_count++){
				...
				for(var x= 0 ; x<data_save_var_arr.length ; x++){
					...
					if(stable=="ng_RLOS_CUSTEXPOSE_LoanDetails"){
						...
						key = "IntLiab_Loan_C_"+agreementId_val;
						}
						else{
						key = "IntLiab_Loan_P_"+agreementId_val;
						}
					}
					else if(stable=="ng_RLOS_CUSTEXPOSE_CardDetails"){
						if(activityName=="CC" || activityName=="PL" || activityName=="Cr"){
						key = "IntLiab_Card_C_"+agreementId_val;
						}
						else{
							key = "IntLiab_Card_P_"+agreementId_val;
						}
					}
					else if(stable=="ng_RLOS_CUSTEXPOSE_AcctDetails"){
						if(activityName=="CC" || activityName=="PL" || activityName=="Cr"){
						key = "IntLiab_Acct_C_"+agreementId_val;
						}
						else{
							key = "IntLiab_Acct_P_"+agreementId_val;
						}
					}
				...
				key=key + "_" + winame;
				var params="key="+key+"&Name="+data_save_var+"&Value="+value;
  ...
  function removeClick(obj) {
	...
	var toal_row_count= $('table#mytab tr:last').index() + 1;
		...
		if((window.parent.com.newgen.omniforms.formviewer.getLVWAT('cmplx_Product_cmplx_ProductGrid',0,1)=='Credit Card' && window.parent.com.newgen.omniforms.formviewer.getLVWAT('cmplx_Product_cmplx_ProductGrid',0,4)=='TOPIM'))
		{
			for (var i=1;i<toal_row_count;i++){
				var Settlement_Flag = "Settlement_Flag_"+i;
					document.getElementById(Settlement_Flag).disabled = false;
			} //hritik IM case in cc
	...
	var toal_row_count= $('table#mytab tr:last').index() + 1;
		...
		if((initiationSteps.indexOf(ws_name)>-1 || ws_name.indexOf('CAD_Analyst1')>-1) && ws_name!='Cad_Analyst2' && ws_name!='CAD_Analyst2' ){
		for (var i=1;i<toal_row_count;i++){
				var Settlement_Flag = "Consider_For_Obligations_"+i;
					document.getElementById(Settlement_Flag).disabled = false;
					document.getElementById(Settlement_Flag).readOnly = false;
			}
		}
		if(ws_name.indexOf('Cad_Analyst2')>-1 || ws_name.indexOf('CAD_Analyst2')>-1){
		for (var i=1;i<toal_row_count;i++){
				var Settlement_Flag = "Consider_For_Obligations_"+i;
					document.getElementById(Settlement_Flag).disabled = true;
					document.getElementById(Settlement_Flag).readOnly = true;
			}
		}
...
function swaprow(table){
	...
	for(var key in closedOrActive){
		if(closedOrActive[key].indexOf(',C')>-1){
			var Settlement_Flag = "Consider_For_Obligations_"+(key-count);
					document.getElementById(Settlement_Flag).disabled = false;
					document.getElementById(Settlement_Flag).readonly = false;
		...
		if(((activityName=="CC" || activityName=="PL" || activityName=="Cr" ) && ws_name.indexOf('CAD_Analyst1')<0) && initiationSteps.indexOf(ws_name)==-1){//changed by nikhil for editable at CSM
			for (var i=1;i<toal_row_count;i++){
				...
				var Settlement_Flag = "Settlement_Flag_"+i;
					...
					document.getElementById(Consider_For_Obligations).disabled = false;
					document.getElementById(Settlement_Flag).disabled = true;
			}
		}
...
function UpdateCOB()
{
	var Sub_product= window.parent.getNGValue("Sub_Product");
	var EmploymentType = window.parent.getNGValue("EmploymentType");
	var update_result="";
	var stable = "ng_RLOS_CUSTEXPOSE_LoanDetails";
	var data_save_var = "Consider_For_Obligations";
	var value = "'false'";
	var swhere="";
	var key = "";
	if(activityName!='CA' )
		{
			key= "IntLiab_Loan_C_COB";
	 //swhere = "Consider_For_Obligations is Null and cifid in (select Main_Cif from ng_rlos_custexpose_LinkedICF with (nolock) where Linked_CIFs in (select CIF_ID from ng_RLOS_GR_FinacleCRMCustInfo with (nolock) where ChildMapping = (select ParentMapping from ng_RLOS_FinacleCRMCustInfo with (nolock) where wi_name = '"+winame+"')) and Relation='Joint Holder' and child_wi='"+winame+"')";
		}
	else
		{
			key= "IntLiab_Loan_P_COB";
		//swhere = "Consider_For_Obligations=Null and cifid in (select Main_Cif from ng_rlos_custexpose_LinkedICF where Linked_CIFs =(select Cif_Id from ng_RLOS_Customer where wi_name = '"+winame+"') and Relation='Joint Holder' and wi_name='"+winame+"')";
		}
		key=key + "_" + winame;
	if(!(Sub_product=="PA" && EmploymentType=="Self Employed"))
		{
	var params="key="+key+"&Name="+data_save_var+"&Value="+value;
	update_result=update_result+","+CallAjax("CustomUpdate_svt.jsp",params);
		}
...
function tb_row(row_num, val){
	...
	if(val==""){
		...
		for(var i=0;i<row_arr.length-1;i++){
			...
			else if(col_id_arr[3]=='Limit_Increase' && !(subProd=='LI' || subProd=='PU' || subProd=='PULI' ) || (((appType.indexOf('TOP')>-1) || (appType.indexOf('RES')>-1)) && ( activityName=="PL" ||  activityName=="CA"))){
			if(initiationSteps.indexOf(ws_name)==-1)
			{
				if(ws_name.indexOf('CAD_Analyst1')>-1 && activityName=="PL" ){
			...
			else if(col_id_arr[3].indexOf("Settlement_Flag")>-1 && (initiationSteps.indexOf(ws_name)>-1 || (ws_name.indexOf('CAD_Analyst1')>-1 && activityName=="PL")))
			{
			table_row = table_row +"<td>"+"<input id = '"+col_id_arr[3]+"_"+row_num+"' type = '"+ col_id_arr[1] +"' name = '"+col_id_arr[3]+"_"+row_num+"' "+col_id_arr[4]+" "+col_id_arr[5]+" onclick = 'LimitincreaseIndicatorLogic(this.id);disableOthers(this.id);calculateNetPayout(this.id);SettlementFlag(this.id);' >"+"</td>";
			}
			else{
...
function openPopup(id)
	{
		...
		//svt points start
		var url='/webdesktop/custom/CustomJSP/internal.jsp?Wi_Name='+winame+'&AgreementId='+agreementval+'&table='+tablename+'&WD_UID='+WD_UID;
