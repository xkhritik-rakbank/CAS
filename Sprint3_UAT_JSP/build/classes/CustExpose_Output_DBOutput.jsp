...
<%
	...
	//svt points start
	//String result = ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("result"), 1000, true) ;	
	...
	String input6 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("is_mobility"), 1000, true) );	
	String is_mobility = ESAPI.encoder().encodeForSQL(new OracleCodec(), input6!=null?input6:"");
	...
	String sessionId = ESAPI.encoder().encodeForSQL(new OracleCodec(), input7!=null?input7:"");
	 */	
	...
	String cust_type = ESAPI.encoder().encodeForSQL(new OracleCodec(), input8);
	//svt points end
	...
	String flagOp="";
		...
		{	
			...
			if("CAS-0000074887-PROCESS".equalsIgnoreCase(wi_name)){
				squery = "select OUTPUT_XML from "+process_tablename+" with (nolock) where MESSAGE_ID ='CAS166791890579837' and WI_NAME = '"+wi_name+"'";
			}
		...
		try
		{
			...
			if(parseXml.indexOf("<RequestType>")>-1)
			{
				...
				{	
					...
					{
						WriteLog("Inside internal exposure call");
						result_str=parseInternalExposure(returnType,parseXml,wrapperIP,wrapperPort,sessionId,cabinetName,wi_name,appServerType,prod,subprod,cifId,cust_type,CompanyCIF);
					}
					...
					{
						result_str=parseCollectionSummary(returnType,parseXml,wrapperIP,wrapperPort,sessionId,cabinetName,wi_name,appServerType,prod,subprod,cifId,cust_type,CompanyCIF);
					}
				}
...
%>
<%!
	public static String parseInternalExposure(String returnType,String parseXml,String wrapperIP,String wrapperPort,String sessionId,String cabinetName,String wi_name,String appServerType, String prod, String subprod, String cifId,String cust_type,String CompanyCIF)
	{
		WriteLog("Inside parse internal exposure call");
		String flag1="";
	    ...
	    try{
			...
			WriteLog("nList_loan:"+ nList_loan);
			...
			for(int i = 0 ; i<nList_loan.getLength();i++){
				...
				Document newXmlDocument = DocumentBuilderFactory.newInstance().newDocumentBuilder().newDocument();
				 ...
				 WriteLog("lsSerializer:"+ lsSerializer);
				...
				WriteLog("root:"+ root);
				 ...
				 cifId =  (n_parseXml.contains("<CustIdValue>")) ? n_parseXml.substring(n_parseXml.indexOf("<CustIdValue>")+"</CustIdValue>".length()-1,n_parseXml.indexOf("</CustIdValue>")):cifId;
					WriteLog("n_parseXml: "+n_parseXml);
	...
	public static String parseExternalExposure(String returnType,String parseXml,String wrapperIP,String wrapperPort,String sessionId,String cabinetName,String wi_name,String appServerType, String prod, String subprod, String cifId,String cust_type)
	{
		...
		subtag_single="AdditionalAccountInfo";
									...
									if(flag1.equalsIgnoreCase("true")){
										...
										flag1=commonParseProduct(parseXml,tagName,wi_name,returnType,sTableName,wrapperIP,wrapperPort,sessionId,cabinetName,appServerType,subTagName,prod,subprod,cifId,cust_type,subtag_single);
											if(flag1.equalsIgnoreCase("true")){
												...
												tagName="CaseDetails";
												subTagName = "";
												sTableName="ng_rlos_cust_extexpo_CaseDetails";
												flag1=commonParseProduct(parseXml,tagName,wi_name,returnType,sTableName,wrapperIP,wrapperPort,sessionId,cabinetName,appServerType,subTagName,prod,subprod,cifId,cust_type,subtag_single);
												}
	...
	public static String parseCollectionSummary(String returnType,String parseXml,String wrapperIP,String wrapperPort,String sessionId,String cabinetName,String wi_name,String appServerType, String prod, String subprod, String cifId,String cust_type,String CompanyCIF)
	{
		String tagName="";
		String subTagName="";
		String sTableName="";
		...
		subTagName = "KeyDt,AmountDtls,DelinquencyInfo";
		sTableName="ng_RLOS_CUSTEXPOSE_LoanDetails";
		flag1=commonParseProduct_collection(parseXml,tagName,wi_name,returnType,sTableName,wrapperIP,wrapperPort,sessionId,cabinetName,appServerType,subTagName,prod,subprod,cifId,subtag_single,cust_type,CompanyCIF);
		...
		String colValue="";
		  for (Map.Entry<String, String> entry : map.entrySet())
		  {
			  ...
			  if(sTableName.equalsIgnoreCase("ng_rlos_FinancialSummary_AvgBalanceDtls")){
				  ...
				  WriteLog( "Inside commonParseFinance for ng_rlos_FinancialSummary_LienDetails column name"+columnName);
				  WriteLog( "Inside commonParseFinance for ng_rlos_FinancialSummary_LienDetails column value"+columnValues);
				  WriteLog( "Inside commonParseFinance for ng_rlos_FinancialSummary_LienDetails where"+sWhere);
				}
			 ...
			 Map<String, String> map = tagValuesMap;
			  ...
			  for (Map.Entry<String, String> entry : map.entrySet())
			  {
				  ...
				  if(sTableName.equalsIgnoreCase("ng_RLOS_CUSTEXPOSE_CardDetails")){
					  ...
					  columnValues = valueArr[1]+",'"+wi_name+"','"+returnType+"','"+cifId+"','"+cust_type+"','"+BorrowingCustomer+"','"+ECRN+"'";
					   ...
					   sQry="Select count(*) as selectdata from "+sTableName+" where Wi_Name='"+wi_name+"' And CardEmbossNum = '"+entry.getKey()+"' And Liability_type ='Individual_CIF' ";
						...
						{
							parseLinkedCif(parseXml,sTableName,cifId,wi_name,entry.getKey(),cust_type,"Card",cabinetName,sessionId,wrapperIP,wrapperPort,appServerType);
						}
				  }
				  else if(sTableName.equalsIgnoreCase("ng_RLOS_CUSTEXPOSE_LoanDetails")){
					  ...
					  //change by saurabh on 2nd Feb for cust_type
						...
						if(parseXml.contains("<LinkedCIFs>"))
						{
							parseLinkedCif(parseXml,sTableName,cifId,wi_name,entry.getKey(),cust_type,"Loan",cabinetName,sessionId,wrapperIP,wrapperPort,appServerType);
						}
				  WriteLog( "sQry  loan sQry" + sQry);
					  ...
					  for(int arrlen=0;arrlen<columnName_arr.length;arrlen++){
						  ...
						  if("History".equalsIgnoreCase(columnName_arr[arrlen])){
								 ...
								 // WriteLog( "inside loan desc tag value" + columnValues_arr[arrlen]);
								  //String loan_desc = get_loanDesc(columnValues_arr[arrlen], cabinetName, sessionId, wrapperIP,wrapperPort, appServerType);
								  columnValues = columnValues.replaceFirst(columnValues_arr[arrlen], "'"+History+"'");
					  ...
					  WriteLog("inside ng_rlos_cust_extexpo_CardDetails");
					  String History = parseHistoryUtilization(parseXml, entry.getKey(), "CardDetails", "<History>", "</History>");
						...
						Utilization = Utilization.replace("\n", "").replace("\r", "");
						WriteLog("history tag:"+History);
						WriteLog("utilixation tag:"+Utilization);
					  ...
					  WriteLog("colname tag:"+columnName);
						WriteLog("colvalue tag:"+columnValues);
				  ...
				  //Changes Done to save data in NG_RLOS_CUSTEXPOSE_RecordDestribution table on 14th sept by Aman
					   ...
					   // Added By Pooja for utilization
					    String History = parseHistoryUtilization(parseXml, entry.getKey(), "AcctDetails", "<History>", "</History>");
						WriteLog("Before replace tag Pooja:"+History);
						History = History.replace("\n", "").replace("\r", "");
						WriteLog("after replace tag Pooja:"+History);
						String Utilization = parseHistoryUtilization(parseXml, entry.getKey(), "AcctDetails", "<Utilizations24Months>", "</Utilizations24Months>");
						WriteLog("utlization before tag Pooja:"+History);
						Utilization = Utilization.replace("\n", "").replace("\r", "");
						WriteLog("history tag Pooja:"+History);
						WriteLog("After replace utlization tag Pooja1:"+Utilization);
					   // end here
						  columnName = valueArr[0]+",Wi_Name,Request_Type,CifId";
						  columnValues = valueArr[1]+",'"+wi_name+"','"+returnType+"','"+cifId+"'";
						  String columnName_arr[] = columnName.split(",");
						  String columnValues_arr[] = columnValues.split(",");
						  WriteLog("After replace utlization tag Pooja6:"+columnName);
						  
						  for(int arrlen=0;arrlen<columnName_arr.length;arrlen++){
							  WriteLog("After replace utlization tag Pooja5:"+columnName_arr[arrlen]);
							  ...
							  // Added by Pooja
							 else if("History".equalsIgnoreCase(columnName_arr[arrlen])){
								  WriteLog( "inside loan desc tag name" + columnName_arr[arrlen]);
								  WriteLog( "inside loan desc tag value" + columnValues_arr[arrlen]);
								  //String loan_desc = get_loanDesc(columnValues_arr[arrlen], cabinetName, sessionId, wrapperIP,wrapperPort, appServerType);
								  columnValues = columnValues.replaceFirst(columnValues_arr[arrlen], "'"+History+"'");
							  ...
							  else if("Utilizations24Months".equalsIgnoreCase(columnName_arr[arrlen])){
									WriteLog( "inside loan desc tag name" + columnName_arr[arrlen]);
								  WriteLog( "inside loan desc tag value" + columnValues_arr[arrlen]);
								  //String loan_desc = get_loanDesc(columnValues_arr[arrlen], cabinetName, sessionId, wrapperIP,wrapperPort, appServerType);
								  columnValues = columnValues.replaceFirst(columnValues_arr[arrlen], "'"+Utilization+"'");
				  ...
				  //below changes Done to save AccountType in ng_RLOS_CUSTEXPOSE_AcctDetails table on 29th Dec by Disha
					...
					String isDirect = (parseXml.contains("<IsDirect>")) ? parseXml.substring(parseXml.indexOf("<IsDirect>")+"</IsDirect>".length()-1,parseXml.indexOf("</IsDirect>")):"";
					 ...
					 //String ODDesc = (parseXml.contains("<ODDesc>")) ? parseXml.substring(parseXml.indexOf("<ODDesc>")+"</ODDesc>".length()-1,parseXml.indexOf("</ODDesc>")):"";
						  columnName = valueArr[0]+",Wi_Name,Request_Type,CifId,CreditGrade,Account_Type,isDirect";
						  columnValues = valueArr[1]+",'"+wi_name+"','"+returnType+"','"+cifId+"','"+CreditGrade+"','"+cust_type+"','"+isDirect+"'";
						  sWhere="Wi_Name='"+wi_name+"' AND AcctId = '"+entry.getKey()+"' AND Account_Type = '"+cust_type+"' AND CifId='"+cifId+"'";
					...
					if(parseXml.contains("<LinkedCIFs>"))
					{
						parseLinkedCif(parseXml,sTableName,cifId,wi_name,entry.getKey(),cust_type,"Account",cabinetName,sessionId,wrapperIP,wrapperPort,appServerType);
					}
					WriteLog("inside account "+columnName );
					WriteLog("inside account "+columnValues);
					}
				  //Deepak 22 july 2019 new condition added to save custinfo
				  ...
				  //Added by Pooja 
					else if(sTableName.equalsIgnoreCase("ng_rlos_cust_extexpo_CaseDetails")){
	...
	public static void parseLinkedCif(String Xml,String TableName,String Main_CIF,String Wi_name, String Agreement_id,String Cust_Type,String Liability_type,String cabinetName,String sessionId,String wrapperIP,String wrapperPort,String appServerType)
	{
		          ...
		          //  System.out.println("\nCurrent Element :" + nNode.getNodeName());
		            ...
		            if (nNode.getNodeType() == Node.ELEMENT_NODE) 
		            {
		             ...
		             /*  String Linked_CIF= eElement.getElementsByTagName("CIFId").item(0).getTextContent();
		              ...
		              String SQuery = "select count(wi_name) as Select_Count from ng_rlos_custexpose_LinkedICF where Linked_CIFs='"+Linked_CIF1+"' and Relation='"+Relation1+"' and wi_name='"+Wi_name+"' and Main_Cif='"+Main_CIF+"' and AgreementId='"+Agreement_id+"'";
		              ...
		              WriteLog("Inside parse CIF:: ExecuteQuery_APSelect"+strInputXml);
						...
						if(totalretrieved==0)
						{
							...
							String columnValues="'"+Wi_name+"','"+Linked_CIF1+"','"+Relation1+"','"+Agreement_id+"','"+Main_CIF+"','"+Liability_type+"','"+Cust_Type+"'";
							strInputXml =	ExecuteQuery_APInsert(sTableName,columnName,columnValues,cabinetName,sessionId);
							//WriteLog( "strInputXml" + strInputXml);
	...
	public static String parseHistoryUtilization(String Xml,String Agreement_id,String Liability_type,String StartType, String EndType)
	{
		...
		try {
			...
			//String ParentTag= doc.getDocumentElement().getNodeName();
			NodeList nList;
			WriteLog("Before LoanDetails");
			 if ("LoanDetails".equalsIgnoreCase(Liability_type)) {
				nList = doc.getElementsByTagName("LoanDetails");
				Liabilityid = "AgreementId";
			} 
			else if("AcctDetails".equalsIgnoreCase(Liability_type)){
					WriteLog("Inside account details");
					nList = doc.getElementsByTagName("AcctDetails");
					WriteLog("Inside account details1111");
				   Liabilityid = "AcctId";
				   WriteLog("Inside account details222");
			}
			else {
				nList = doc.getElementsByTagName("CardDetails");
				Liabilityid = "CardEmbossNum";
	...
	public static String commonParseProduct_collection(String parseXml,String tagName,String wi_name,String returnType,String sTableName,String wrapperIP,String wrapperPort,String sessionId,String cabinetName,String appServerType, String subTagName,String prod,String subprod, String cifId,String subtag_single,String cust_type,String CompanyCIF)
	{
		...
		*/		  
				    //Changes Done by aman to save the data of collection summary
					 ...
					 String CreditGrade = (parseXml.contains("<CreditGrade>")) ? parseXml.substring(parseXml.indexOf("<CreditGrade>")+"</CreditGrade>".length()-1,parseXml.indexOf("</CreditGrade>")):"";
					//PCASP-2833 
					//String isDirect = (parseXml.contains("<IsDirect>")) ? parseXml.substring(parseXml.indexOf("<IsDirect>")+"</IsDirect>".length()-1,parseXml.indexOf("</IsDirect>")):"N";
		...
		WriteLog("subTagName jsp: commonParseParent: "+subTagName);
		 ...
		 Map<Integer, String> map = tagValuesMap;
		  ...
		  for (Map.Entry<Integer, String> entry : map.entrySet())
		  {
			 ...
			 WriteLog( "colArr" + colArr);
			 keyDtType = colArr[0];
			
			WriteLog( "keyDtType" + keyDtType);
			if(detailType.equalsIgnoreCase("LoanDetails"))
			{
				...
				if(returnType.equalsIgnoreCase("ExternalExposure"))
				{
				  ...
				  {
					 WriteLog( "Inside MAximum OverdueAmount Date function");
					sWhere="Wi_Name='"+wi_name+"' AND Request_Type='"+returnType+"' AND AgreementId = "+id_val;
					columnName = colArr[0];
					colValue = "'"+colArr[1]+"'";
			...
			if(detailType.equalsIgnoreCase("CardDetails"))
			{
				...
				if(returnType.equalsIgnoreCase("ExternalExposure"))
				{
				  if(keyDtType.equalsIgnoreCase("MaxOverDueAmountDate"))
				  {
					WriteLog( "Inside MaxOverdue amount Date card Details ");  
					sWhere="Wi_Name='"+wi_name+"' AND Request_Type='"+returnType+"' AND CardEmbossNum = "+id_val;
					columnName = colArr[0];
					colValue = "'"+colArr[1]+"'";
					WriteLog( "valueArr[2] sWhere" + sWhere);
			...
			if(detailType.equalsIgnoreCase("AcctDetails"))
			{
				...
				if(returnType.equalsIgnoreCase("ExternalExposure"))
				{
				  ...
				  {
					 WriteLog("Inside Maximum overdue Date account details ");
					sWhere="Wi_Name='"+wi_name+"' AND Request_Type='"+returnType+"' AND AcctId = "+id_val;
					columnName = colArr[0];
					colValue = "'"+colArr[1]+"'";
					WriteLog( "valueArr[2] sWhere" + sWhere);
...
public static String ExecuteQuery_APSelect(String sQry, String cabinetName, String sessionId)
 ...
 public static String getTagValue(String parseXml,String tagName,String subTagName)
	...
	public static void updateQuery(String sTableName,String columnName,String colValue,String sWhere,String cabinetName,String sessionId,String returnType,String wrapperIP,String wrapperPort, String appServerType,String cifId,String wi_name)
  {
	...
	WriteLog("sTableName: "+sTableName);
	WriteLog("columnName: "+columnName);
	WriteLog("colValue: "+colValue);
	...
	WriteLog( "strInputXml update " + strInputXml);
	  try 
		...
		row_updated = getTagValue(strOutputXml,tagNameU,subTagNameU_2);
		WriteLog("getTagValue select mainCode --> "+mainCode);
		WriteLog("getTagValue select row_updated --> "+row_updated);
