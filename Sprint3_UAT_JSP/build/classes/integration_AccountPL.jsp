...
<%
	...
	String activityName = ESAPI.encoder().encodeForSQL(new OracleCodec(), input3!=null?input3:"");
	...
	String parentWiName = ESAPI.encoder().encodeForSQL(new OracleCodec(), input4!=null?input4:"");
	...
	String WD_UID = ESAPI.encoder().encodeForSQL(new OracleCodec(), input6!=null?input6:"");
	...
	//var param_json = ($ESAPI.encoder().decodeForHTML(param_json_encode));
		...
		function integration_all (){
				...
				$.ajax({
					...
					data: { wi_name:parentWiName , req_name : req_name , activity : activityName , child_wi: wi_name } ,
					dataType: "text",
					async: true,
					success: function (response) {
				...
				var req_name_arr = req_name.split(',');
					for (var i=0;i<req_name_arr.length;i++)
					{
						...
						$.ajax({
							...
							data: { request_name: req_name_arr[i], is_mobility : "N", SeesionId:sessionId ,wi_name:wi_name ,activityName:activityName ,param_json:param_json,cifId:cif_no,header:'<%=headerOp.get("ACCOUNT_SUMMARY")%>',maincode:'<%=maincodeOp.get("ACCOUNT_SUMMARY")%>',footer:'<%=footerOp.get("ACCOUNT_SUMMARY")%>',parentTagName:'<%=parentTagNameOp.get("ACCOUNT_SUMMARY")%>'} ,
							dataType: "text",
							async: true,
							success: function (response) {
