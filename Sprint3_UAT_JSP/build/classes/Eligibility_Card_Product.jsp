...
//select distinct(groupindex) from PDBGroupMember where UserIndex=1 and GroupIndex in (1,10)
	...
	var pname="ECP";
	//alert(ws_name);
	  if(activityName=='Cr'|| activityName=='CDOB'|| activityName=='CreditCard'){
		activityName='CC';
	  }
	  else if(activityName=='CAS'){
		  activityName='CA';
	  }
...
function saveDate() {
		...
		for(var row_arr_count= 1;row_arr_count<=totalrowid;row_arr_count++){
				...
				for(var x= 0 ; x<data_save_var_arr.length ; x++){
					...
					key="ECP_P_"+document.getElementById(s_no).value;
					}
					else {
					//swhere = "Child_wi = '"+winame+"'  and s_no ='"+document.getElementById(s_no).value+"'";
					key="ECP_C_"+document.getElementById(s_no).value;
					}
...
function tb_header(){
	var subprod = window.parent.com.newgen.omniforms.formviewer.getLVWAT('cmplx_Product_cmplx_ProductGrid',0,2);//pcasp-2404
	var row_arr = WiDetails.split("~");
	var col_arr;
	var temp = "";
	var table_header ="";
	...
	for(var i=0;i<row_arr.length-1;i++){
		temp = row_arr[i];
		col_arr=temp.split("#");
		if(!(subprod=='SEC' && col_arr[3]=='Score_Grade'))//pcasp-2404
		...
		for(var i=0;i<row_arr.length-1;i++){
			...
			if((col_id_arr[1]=="text")){
				if(activityName=='CA'){ 
					...
					else if ( subprod=='SEC' && col_id_arr[3]=='Score_Grade'){//pcasp-2404
						table_row = table_row +"<td>"+"<input id = '"+col_id_arr[3]+"_"+row_num+"' type = 'hidden'  title='"+val_new[i]+"'  name = '"+col_id_arr[3]+"_"+row_num+"' value='"+val_new[i]+"' maxlength ='"+col_id_arr[2]+"' "+col_id_arr[5]+">"+"</td>";
