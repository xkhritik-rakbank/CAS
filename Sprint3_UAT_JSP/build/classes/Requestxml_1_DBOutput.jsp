...
<%
	...
	String wrapperPort = "";
	String cabinetName = "";
	String row_count = ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("row_count"), 1000, true);//request.getParameter("row_count"); 
	String CardNumber = ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("CardNumber"), 1000, true);//request.getParameter("CardNumber"); 
	...
	request_name = ESAPI.encoder().encodeForSQL(new OracleCodec(), input4!=null?input4:"");
	...
	String input5 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("wi_name"), 1000, true) );	
	 wi_name = ESAPI.encoder().encodeForSQL(new OracleCodec(), input5!=null?input5:"");
	...
	String input6 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("activityName"), 1000, true) );	
	 ws_name = ESAPI.encoder().encodeForSQL(new OracleCodec(), input6!=null?input6:"");	
	...
	String input7 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("cifId"), 1000, true) );	
	 cifId = ESAPI.encoder().encodeForSQL(new OracleCodec(), input7!=null?input7:"");
	...
	String input8 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("acc_id"), 1000, true) );	
	 acc_id = ESAPI.encoder().encodeForSQL(new OracleCodec(), input8!=null?input8:"");
	...
	Customer_type = ESAPI.encoder().encodeForSQL(new OracleCodec(), input9!=null?input9:"");
	//svt points end
	request_name = request_name.trim();
	String request_Arr[]=request_name.split(",");
	WriteLog("is_mobility --: "+is_mobility +" || sessionId--: "+sessionId+" || request_name--: "+request_name+" || wi_name--: "+wi_name+" || ws_name--: "+ws_name+" || cifId--: "+cifId+" || acc_id--: "+acc_id+" || Customer_type--: "+Customer_type);	
			 ...
			 //WriteLog("call_type ="+call_type+" Call_name="+Call_name+" form_control="+form_control+" parent_tag_name="+parent_tag_name+" xmltag_name="+xmltag_name+" is_repetitive="+is_repetitive+"default_val="+default_val );
				   ...
				   if (parent_tag!=null && !parent_tag.equalsIgnoreCase(""))
						...
						//WriteLog("inside 1st if int_xml: "+int_xml);
								...
								{
										String ContractType_sQuery = "select top 1 ReqProduct from ng_RLOS_GR_Product where Prod_WIname = '"+wi_name+"'";
										String ContractType_inputXML = "<?xml version='1.0'?><APSelectWithColumnNames_Input><Option>APSelectWithColumnNames</Option><Query>"
												+ ContractType_sQuery + "</Query><EngineName>" + cabinetName
												+ "</EngineName><SessionId>" + sessionId
												+ "</SessionId></APSelectWithColumnNames_Input>";
										
												WriteLog("outputXML ContractType_OutputXML-->"+ContractType_inputXML);
												String ContractType_OutputXML = "";
										try {
											ContractType_OutputXML = ngejclient_obj.makeCall(wrapperIP,
													wrapperPort, appServerType, ContractType_inputXML);
										} catch (NGException e) {
											e.printStackTrace();
										} catch (Exception ex) {
											ex.printStackTrace();
										...
										WriteLog("outputXML ContractType_OutputXML-->"+ContractType_OutputXML);
										XMLParser  xmlParserData_cont = new XMLParser();
										xmlParserData_cont.setInputXML((ContractType_OutputXML));
										String Process = "Personal Loan".equalsIgnoreCase(xmlParserData_cont.getValueOf("ReqProduct")) ? "PL" : "CC";
										WriteLog("outputXML Process-->"+Process);
										if ("PL".equalsIgnoreCase(Process)) {
											form_control_val = "16";
										} else {
									...
									if (tag_name.equalsIgnoreCase("DocRefNum")
											&& parent_tag.equalsIgnoreCase("DocDetails")
											&& form_control_val.equalsIgnoreCase("")) {
										if (xml_str.contains("</DocDetails>")) {
											xml_str = xml_str.substring(0,
													xml_str.lastIndexOf("</DocDetails>"));
											int_xml.put(parent_tag, xml_str);
										} else
											int_xml.remove(parent_tag);
									} else if (tag_name.equalsIgnoreCase("DocNum")
											&& parent_tag.equalsIgnoreCase("LegalDocInfo")
											&& form_control_val.equalsIgnoreCase("")) {
										if (xml_str.contains("</LegalDocInfo>")) {
											xml_str = xml_str.substring(0,
													xml_str.lastIndexOf("</LegalDocInfo>"));
											int_xml.put(parent_tag, xml_str);
										} else
											int_xml.remove(parent_tag);
									} else if (tag_name.equalsIgnoreCase("CIFIdValue")
											&& parent_tag.equalsIgnoreCase("CIFId")
											&& form_control_val.equalsIgnoreCase("")) {
								...
								if (!(form_control == null || form_control.equalsIgnoreCase("")
										|| form_control.equalsIgnoreCase("null")))
								//if(!(jsonObj.getString(form_control)==null || jsonObj.getString(form_control).equalsIgnoreCase("")||  jsonObj.getString(form_control).equalsIgnoreCase("null")))
								{
									...
									if (int_xml.containsKey(parent_tag)
											|| is_repetitive.equalsIgnoreCase("Y")) {
					...
					while (itr.hasNext()) {
						...
						if (final_xml.indexOf((entry.getKey())) > -1) {
							...
							final_xml = final_xml.insert(
									final_xml.indexOf("<" + entry.getKey() + ">") + entry.getKey().length() + 2,
									entry.getValue());
							itr.remove();
						}
				...
				if (!(socketServerIP.equalsIgnoreCase("") && socketServerIP.equalsIgnoreCase(null)
						&& socketServerPort.equals(null) && socketServerPort.equals(0))) {
					mqInputRequest = getMQInputXML(sessionId, cabinetName, wi_name, ws_name, userName,
							final_xml);
