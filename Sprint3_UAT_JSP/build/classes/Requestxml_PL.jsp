...
<%
	...
	String row_count = ""; 
	String CardNumber = ""; 
	String InterfaceTable="NG_Integration_PL_field_Mapping";
	String cardNo="";
	String userName="null";	
	String cif_id="";//request.getParameter("userName");
	...
	String is_mobility = ESAPI.encoder().encodeForSQL(new OracleCodec(), input1!=null?input1:"");
	
	String input2 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("request_name"), 1000, true) );
	String request_name = ESAPI.encoder().encodeForSQL(new OracleCodec(), input2!=null?input2:"");
	...
	String ws_name = ESAPI.encoder().encodeForSQL(new OracleCodec(), input4!=null?input4:"");
	
	String input5 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("cifId"), 1000, true) );
	String cifId = ESAPI.encoder().encodeForSQL(new OracleCodec(), input5!=null?input5:"");
	
	String input6 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("acc_id"), 1000, true) );
	String acc_id = ESAPI.encoder().encodeForSQL(new OracleCodec(), input6!=null?input6:"");
	
	String input7 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("Customer_Type"), 1000, true) );
	String Customer_type = ESAPI.encoder().encodeForSQL(new OracleCodec(), input7!=null?input7:"");
	
	String input8 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("SeesionId"), 1000, true) );
	String sessionId = ESAPI.encoder().encodeForSQL(new OracleCodec(), input8!=null?input8:"");
	request_name = request_name.trim();
	//svt points end
	String request_Arr[]=request_name.split(",");

	WriteLog("session id is"+sessionId);
	try{
			//String input10 = ""+Integer.parseInt(request.getParameter("row_count")); 
			//svt points start
			String row_count_encode = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("row_count"), 1000, true) );
			row_count = ESAPI.encoder().encodeForSQL(new OracleCodec(), row_count_encode!=null?row_count_encode:"");
			
			String CardNumber_encode = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("CardNumber"), 1000, true) );
			CardNumber = ESAPI.encoder().encodeForSQL(new OracleCodec(), CardNumber_encode!=null?CardNumber_encode:"");
			//svt points end
	...
	String jsonobj2 = request.getParameter("param_json");	
		...
		{//svt points start
			//String input14 = request.getParameter("cardNo");
			
		String cardNo_encode = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("cardNo"), 1000, true) );
			cardNo = ESAPI.encoder().encodeForSQL(new OracleCodec(), cardNo_encode);
			//socketServerPort=5555;
		}
		else if(request_name.contains("ACCOUNT_SUMMARY")){
			String input15 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("cifId"), 1000, true) );
			 cif_id = ESAPI.encoder().encodeForSQL(new OracleCodec(), input15);
		//svt points end
		...
		if(cifId!=null && !cifId.equalsIgnoreCase("")){
			...
			if(request_name.contains("CARD_SUMMARY"))
			{//svt points start
				cardNo = ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("cardNo"), 1000, true);//request.getParameter("cardNo");
				//svt points end
				jsonObj.put("cmplx_Customer_CIFNO",cardNo.trim());
			}
	   ...
	   //WriteLog("$$outputgGridtXML");
		try
		{
			...
			for(int i =0;i<request_Arr.length;i++){
				...
				{
					sQuery_header = "SELECT Header,Footer,parenttagname FROM NG_Integration with (nolock) where Call_name='FINANCIAL_SUMMARY'";
				}
					...
					sQuery_header = "SELECT Header,Footer,parenttagname FROM NG_Integration with (nolock) where Call_name='OTP_MANAGEMENT'";
				}
				else{
					sQuery_header = "SELECT Header,Footer,parenttagname FROM NG_Integration with (nolock) where Call_name='"+request_Arr[i]+"'";
				}
			...
			//svt points start
				String input16 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("maincode"), 1000, true) );
				mainCodeData = ESAPI.encoder().encodeForSQL(new OracleCodec(), input16);
			//svt points end
			}
				if(mainCodeData.equals("0"))
	       	{
				...
				//svt points start
					String input17 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("header"), 1000, true) );
					header = request.getParameter("header");
			
					String input18 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("footer"), 1000, true) );
					footer = request.getParameter("footer");//ESAPI.encoder().encodeForSQL(new OracleCodec(), input13);
			
					String input19 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("parentTagName"), 1000, true) );
					parentTagName = ESAPI.encoder().encodeForSQL(new OracleCodec(), input19);
					//svt points end
				}
				...
				{ 
					...
					else if (request_Arr[i].equalsIgnoreCase("OTP_Management"))
					{
						...
						sQuery = "SELECT call_type, Call_name,Mobility_form_control,parent_tag_name,xmltag_name,is_repetitive,default_val,data_format FROM NG_Integration_field_Mapping with (nolock) where Call_name='OTP_MANAGEMENT' and  Operation_name='GenerateOTP' and  Active = 'Y' ORDER BY tag_seq ASC";
					}
				...
				else
				{
					...
					{
						sQuery = "SELECT call_type, Call_name,form_control,parent_tag_name,xmltag_name,is_repetitive,default_val,data_format FROM "+InterfaceTable+" with (nolock) where Call_name='FINANCIAL_SUMMARY' and Operation_name='"+ request_Arr[i] +"' ORDER BY tag_seq ASC" ;
					}
					else
					{
						sQuery = "SELECT call_type, Call_name,form_control,parent_tag_name,xmltag_name,is_repetitive,default_val,data_format FROM "+InterfaceTable+" where Call_name='"+request_Arr[i]+"'ORDER BY tag_seq ASC" ;
					}								
				}
						   ...
						   if (parent_tag!=null && !parent_tag.equalsIgnoreCase(""))
                            {
                                ...
                                if(int_xml.containsKey(parent_tag))
                                {
                                    ...
                                    if(is_repetitive.equalsIgnoreCase("Y") && int_xml.containsKey(tag_name)){
										...
										else if (tag_name.equalsIgnoreCase("DurationOfAgreement") && "EXTERNALEXPOSURE".equalsIgnoreCase(Call_name) && wi_name.substring(0,2).equalsIgnoreCase("CC"))
										{
											xml_str = xml_str;
										}
										
										else if (tag_name.equalsIgnoreCase("InquiryPurpose") && "EXTERNALEXPOSURE".equalsIgnoreCase(Call_name))
										{
											
											if(wi_name.substring(0,2).equalsIgnoreCase("PL")){
												form_control_val = "16";
											}
											else{
...
<%!
	...
	private static String getMQInputXML(String sessionID, String cabinetName, String wi_name, String ws_name, String userName, StringBuffer final_xml) 
    {
		...
		String table_name="";
		if(wi_name.substring(0,2).equalsIgnoreCase("CC")||wi_name.substring(0,2).equalsIgnoreCase("Cr")){
			table_name="NG_CC_XMLLOG_HISTORY";
		}
		else if(wi_name.substring(0,wi_name.indexOf("-")).equalsIgnoreCase("CDOB")){
			table_name="NG_CDOB_XMLLOG_HISTORY";
		}
			else{
			table_name="NG_PL_XMLLOG_HISTORY";
		}	
		StringBuffer strBuff=new StringBuffer();
		strBuff.append("<APMQPUTGET_Input>");
		strBuff.append("<SessionId>"+sessionID+"</SessionId>");
		strBuff.append("<EngineName>"+cabinetName+"</EngineName>");
		strBuff.append("<XMLHISTORY_TABLENAME>"+table_name+"</XMLHISTORY_TABLENAME>");
