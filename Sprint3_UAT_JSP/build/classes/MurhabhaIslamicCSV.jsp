    ...
    <div class="shade1">
	 ...
	 <form name="DisplayWi" id="DisplayWi"   class="form-horizontal" style="margin-left:5%;width:80%">
		<div class="form-group">		
			...
			<%
			String SqlQuery="SELECT cc_wi_name AS wi_name,CIF_ID, CustomerName, Sub_Product, warranty.Card_Product,file_generated_name,warranty.status FROM  ng_cc_exttable  cc WITH(nolock) JOIN ng_rlos_Murabha_Warranty warranty WITH(nolock) ON Murhabha_WIName=cc_wi_name WHERE CURR_WSNAME='Disbursal' and cif_id is not null and   warranty.Card_Product is not null UNION ALL SELECT pl_wi_name,CIF_ID, CustomerName, Sub_Product, warranty.card_Product,file_generated_name,warranty.status AS wi_name FROM NG_PL_EXTTABLE pl WITH(nolock) JOIN ng_rlos_Murabha_Warranty warranty WITH(nolock) ON Murhabha_WIName=pl_wi_name WHERE delivery_status='W' and cif_id is not null   and warranty.Card_Product is not null UNION ALL SELECT cc_wi_name AS wi_name,CIF_ID, CustomerName, Sub_Product, warranty.Card_Product,file_generated_name,warranty.status FROM  ng_dob_exttable  cc WITH(nolock) JOIN ng_rlos_Murabha_Warranty warranty WITH(nolock) ON Murhabha_WIName=cc_wi_name WHERE CURR_WSNAME='Disbursal' and cif_id is not null and   warranty.Card_Product is not null";
				 ...
				 for (var i = 0; i < recordcount; i++) {
					 if(document.getElementById('mbCheckbox_'+i)!=null){
						 if(document.getElementById('mbCheckbox_'+i).checked==false && document.getElementById('FileName_'+i).innerHTML==''){
							...
							selectedRecords[i]="'"+wi_name+"','"+cardProduct+"'";
