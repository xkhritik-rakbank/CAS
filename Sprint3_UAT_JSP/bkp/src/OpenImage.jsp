...
<%
	...
	String wsname = ESAPI.encoder().encodeForSQL(new OracleCodec(), input2);
	...
	String input4 = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", request.getParameter("wi_name"), 1000, true) );
	String wi_name = ESAPI.encoder().encodeForSQL(new OracleCodec(), input4);
	...
	String cif_id = ESAPI.encoder().encodeForSQL(new OracleCodec(), input5);
	//svt points end
	...
	String filePath =System.getProperty("user.dir")+File.separatorChar+"installedApps"+File.separatorChar+"ANT2CASAPPS01Node01Cell"+File.separatorChar+"webdesktop_HF61.ear"+File.separatorChar+"webdesktop.war"+File.separatorChar+"CustomForms"+File.separatorChar+"RLOS";
	...
	public String getImages(String tempImagePath,String debt_acc_num)
	{
		...
		for (int i=0;i<imageArr.length;i++)
		{
			try
			{
				...
				//Changed By Ashish
				String sImagePath = debt_acc_num+"imageCreatedN"+i+".jpg";
...
//***********************************************************************************//	
	...
	var debt_acc_num_encode = '<%=debt_acc_num%>';
	var debt_acc_num = ($ESAPI.encoder().decodeForHTML(debt_acc_num_encode));
	//svt points end
...
function deleteImageFromServer (filePath,debt_acc_num,returnedSignatures)
{
	...
	//svt points start
	var url = 'DeleteSignFromServer.jsp?debt_acc_num='+debt_acc_num+"&returnedSignatures="+returnedSignatures+"&WD_UID="+WD_UID;
	var xhr;
	var ajaxResult;	
