...
<%
	...
	WriteLog("Integration jsp: value of WD_UID: "+WD_UID);
					...
					success: function (response){
						...
						if(last4=="true"||last4=="0000"){
							document.getElementById(eml_name).innerHTML="Sucessfull";
							 ...
							 if(aecb_rep_url){
							  ...
							  window.opener.com.newgen.omniforms.formviewer.setNGValueCustom('aecb_call_status','SUCCESS');
						...
						else if(flag_value[0]=="Failure" || flag_value[0]=="false")
						{
						 ...
						 if(document.getElementById('callname_'+trimStr(flag_value[1])).innerHTML.indexOf('External')>-1){
							...
							/*document.getElementById("retry_btn_"+trimStr(flag_value[1])).style.visibility="visible";
							window.returnValue = "FAIL";	
							window.opener.com.newgen.omniforms.formviewer.setNGValueCustom('aecb_call_status','FAIL');*/
						}
						check_complete();
					},
		...
		function Aecb_report_url(result){
			try{
				...
				if($RequestType.text()=='ExternalExposure'){
					...
					window.opener.com.newgen.omniforms.formviewer.setNGValueCustom('cmplx_Liability_New_Aecb_Report_Url',aecb_rep_url);
				}
			}
			catch(e){
			}
		...
		function SetDataOnForm(response)
		{
				...
				try{
					if($AvgBalanceDtls.length>0) {
						...
						window.opener.com.newgen.omniforms.formviewer.setNGValueCustom('cmplx_IncomeDetails_AvgBal',averageBal.toFixed(2)) ;
					}
						...
						window.opener.com.newgen.omniforms.formviewer.setNGValueCustom('cmplx_IncomeDetails_AvgCredTurnover',AvgCrTurnOver.toFixed(2));
						window.opener.com.newgen.omniforms.formviewer.setNGValueCustom('cmplx_IncomeDetails_CredTurnover',TotalCrAmt.toFixed(2));
					}
					
					if(window.opener.getNGValue('EmploymentType').indexOf('Salaried')>-1){
						if($SalTxnDetails.length>0){
							window.opener.com.newgen.omniforms.formviewer.setNGValueCustom('cmplx_IncomeDetails_SalaryDay',salcredDate.getDate());	
							//change by saurabh for PPG changes on 20/4/19
							window.opener.com.newgen.omniforms.formviewer.setNGValueCustom('cmplx_IncomeDetails_netSal1',salaries[arr[arr.length-1]]);
							window.opener.com.newgen.omniforms.formviewer.setNGValueCustom('cmplx_IncomeDetails_netSal2',salaries[arr[arr.length-2]]);
							window.opener.com.newgen.omniforms.formviewer.setNGValueCustom('cmplx_IncomeDetails_netSal3',salaries[arr[arr.length-3]]);
						}
					}
				}
				catch(e){
