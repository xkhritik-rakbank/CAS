...
<%
	...
	String input8 = request.getParameter("cust_type");
	...
	String prod = ESAPI.encoder().encodeForSQL(new OracleCodec(), input3);
	...
	String subprod = ESAPI.encoder().encodeForSQL(new OracleCodec(), input4);
	...
	String cifId = ESAPI.encoder().encodeForSQL(new OracleCodec(), input5);
	...
	String is_mobility = ESAPI.encoder().encodeForSQL(new OracleCodec(), input6);
	...
	String SeesionId = ESAPI.encoder().encodeForSQL(new OracleCodec(), input7);
	...
	try {
	    String input9 = ""+Integer.parseInt(request.getParameter("row_count")); 
		String row_count_encode = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", input9, 1000, true) );
	     row_count = ESAPI.encoder().encodeForSQL(new OracleCodec(), row_count_encode);
	 
        String input10 = ""+Integer.parseInt(request.getParameter("CardNumber"));
		String CardNumber_encode = ESAPI.encoder().encodeForHTML( ESAPI.validator().getValidSafeHTML("htmlInput", input10, 1000, true) );
		CardNumber = ESAPI.encoder().encodeForSQL(new OracleCodec(), CardNumber_encode);
	...
	{		
		if(result.indexOf("<MQ_RESPONSE_XML>")>-1)
		{
			...
			//WriteLog("$$outputXMLMsg1234 "+outputXMLMsg);
			cifId =cifId.trim();
			...
			//WriteLog("wrapperIP jsp: parseInternalExposure:row_count "+row_count+",result"+result+",CardNumber"+CardNumber);
	...
	public static String getOutputXMLValues(String parseXml,String appServerType,String wrapperIP,String wrapperPort,String sessionId,String cabinetName,String wi_name, String prod, String subprod, String cifId,String CardNumber,String cust_type)
	{
		...
		try
		{
			...
			if(parseXml.indexOf("<RequestType>")>-1)
			{
				...
				{	
					String errorQuery="SELECT isnull((SELECT alert FROM ng_MASTER_INTEGRATION_ERROR_CODE WHERE  error_code=:error_code),(SELECT alert FROM ng_MASTER_INTEGRATION_ERROR_CODE WHERE error_code='DEFAULT')) As Alert";
					params="error_code=="+returnCode;
					//WriteLog("@@@@@@@@@@@@@@  "+errorQuery);
					String strInputXml=ExecuteQuery_APSelectwithparam(errorQuery,params,cabinetName,sessionId);
				...
				if(returnType.equalsIgnoreCase("CollectionsSummary"))
				{
					...
					//WriteLog("cifId jsp: getOutputXMLValuesresult:Collections "+result_str);
				}
			}
			  ...
			  for (Map.Entry<String, String> entry : map.entrySet())
			  {
				  ...
				  if(sTableName.equalsIgnoreCase("ng_RLOS_CUSTEXPOSE_CardDetails")){
					  ...
					  columnValues = valueArr[1]+",'"+wi_name+"','"+returnType+"','"+cifId+"','"+cust_type+"','"+BorrowingCustomer+"','"+ECRN+"'";
					   ...
					   sQry="Select count(*) as selectdata from "+sTableName+" where Wi_Name=:Wi_Name And CardEmbossNum = :CardEmbossNum And Liability_type ='Individual_CIF' ";
					   params="Wi_Name=="+wi_name+"~~CardEmbossNum=="+entry.getKey();
						 //WriteLog( "sQry sQry" + sQry);
						if(cust_type.equalsIgnoreCase("Individual_CIF")) {
							companyUpdateQuery="Select count(*) as selectdata from "+sTableName+" where Wi_Name='"+wi_name+"' And CardEmbossNum = :CardEmbossNum And Liability_type ='Corporate_CIF'";
							params="Wi_Name=="+wi_name+"~~CardEmbossNum=="+entry.getKey();
						}
				  }
				  else if(sTableName.equalsIgnoreCase("ng_RLOS_CUSTEXPOSE_LoanDetails")){
					  ...
					  //change by saurabh on 2nd Feb for cust_type
						...
						if(cust_type.equalsIgnoreCase("Individual_CIF")) {
							companyUpdateQuery="Select count(*) as selectdata from "+sTableName+" where Wi_Name=:Wi_Name And AgreementId = :AgreementId And Liability_type ='Corporate_CIF'";
							params="Wi_Name=="+wi_name+"~~AgreementId=="+entry.getKey();
						}
				  ...
				  //below changes Done to save AccountType in ng_RLOS_CUSTEXPOSE_AcctDetails table on 29th Dec by Disha
					...
					sQry="Select count(*) as selectdata from NG_RLOS_ALOC_OFFLINE_DATA where CIF_ID =:CIF_ID";
					params="CIF_ID=="+cifId;
					//WriteLog( "sQry  loan sQry" + sQry);	  
				  }
					...
					//WriteLog( "strInputXml update " + strInputXml);
					  try 
						...
						if (!sQry.equalsIgnoreCase("")){
							strInputXml=ExecuteQuery_APSelectwithparam(sQry,params,cabinetName,sessionId);
							...
							strInputXml=ExecuteQuery_APSelectwithparam(companyUpdateQuery,params,cabinetName,sessionId);
			  ...
			  for (Map.Entry<String, String> entry : map.entrySet())
			  {
				  ...
				  if(sTableName.equalsIgnoreCase("ng_RLOS_CUSTEXPOSE_CardDetails")){
					...
					params="Wi_Name=="+wi_name+"~~CardEmbossNum=="+entry.getKey();
					  ...
					  sQry="Select count(*) as selectdata from "+sTableName+" where Wi_Name=:Wi_Name And AgreementId = :AgreementId And Liability_type ='Individual_CIF'";
					  params="Wi_Name=="+wi_name+"~~AgreementId=="+entry.getKey();
					  if(cust_type.equalsIgnoreCase("Individual_CIF")) {
							companyUpdateQuery="Select count(*) as selectdata from "+sTableName+" where Wi_Name=:Wi_Name And AgreementId = :AgreementId And Liability_type ='Corporate_CIF'";
							params="Wi_Name=="+wi_name+"~~AgreementId=="+entry.getKey();
		...
		*/		  
				    //Changes Done by aman to save the data of collection summary
					 ...
					 String CreditGrade = (parseXml.contains("<CreditGrade>")) ? parseXml.substring(parseXml.indexOf("<CreditGrade>")+"</CreditGrade>".length()-1,parseXml.indexOf("</CreditGrade>")):"";
						  ...
						  sWhere="Wi_Name='"+wi_name+"' AND AcctId = '"+entry.getKey()+"'";
						  sQry="Select count(*) as selectdata from "+sTableName+" where Wi_Name=:Wi_Name And AcctId = :AcctId And Account_Type ='Individual_CIF'";
						  params="Wi_Name=="+wi_name+"~~AcctId=="+entry.getKey();
				  }
				else{
				   sWhere="Wi_Name='"+wi_name+"' AND Request_Type='"+returnType+"'";
				  }
