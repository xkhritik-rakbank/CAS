	...
	String jsonobj2 = request.getParameter("param_json");	
	   ...
	   //WriteLog("$$outputgGridtXML");
		try
		{
			...
			mainCodeData = request.getParameter("maincode");
			}
				if(mainCodeData.equals("0"))
	       	{
	            ...
	            parentTagName = request.getParameter("parentTagName");
				}
				...
				else
				{
					...
					{
						sQuery = "SELECT call_type, Call_name,form_control,parent_tag_name,xmltag_name,is_repetitive,default_val,data_format FROM NG_Integration_PL_field_Mapping with (nolock) where Call_name='FINANCIAL_SUMMARY' and Operation_name='"+ request_Arr[i] +"' ORDER BY tag_seq ASC" ;
					}
					else
					{
				...
				*/
				WriteLog("saurabh socket detailsare: "+socketServerIP+" port:"+socketServerPort);
				if(!(socketServerIP.equalsIgnoreCase("") && socketServerIP.equalsIgnoreCase(null) && socketServerPort.equals(null) && socketServerPort.equals(0)))
								...
								//outputResponse= dummyResponse(request_Arr[i]);
									...
									WriteLog("saurabh socket mqrequest: "+mqInputRequest);
...
<%!
	...
	private static String getMQInputXML(String sessionID, String cabinetName, String wi_name, String ws_name, String userName, StringBuffer final_xml) 
    {
		...
		String table_name="";
		if(wi_name.substring(0,2).equals("CC") || wi_name.substring(0,2).equals("Cr")){
