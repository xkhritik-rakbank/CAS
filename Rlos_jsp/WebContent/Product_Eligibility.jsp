...
function fetchGridData()
{
	...
	var sQry="";
		...
		if(activityName=='CA'){//RLOS
			sQry="select S_no,RequestedLoan,RequestedAppType,CACCalculatedlimit,EligibleExposure,'',Decision,DeclinedReasonCode,ReferReasoncode,DeviationCodeRefer,DelegationAuthorithy FROM  "+table_name+" with (nolock) WHERE wi_name = '"+winame+"'";
		}
		else{
			 sQry="select S_no,RequestedLoan,RequestedAppType,CACCalculatedlimit,EligibleExposure,'',Decision,DeclinedReasonCode,ReferReasoncode,DeviationCodeRefer,DelegationAuthorithy FROM  "+table_name+" with (nolock) WHERE Child_wi = '"+winame+"'";
		}
			...
			if(col_id_arr[1]=="text"){
				if(activityName=='CA'){ 	
					if((row_num!=1)&&(i==5)){
						...
						table_row = table_row +"<td>"+"<input id = '"+col_id_arr[3]+"_"+row_num+"' type = '"+ col_id_arr[1] +"' name = '"+col_id_arr[3]+"_"+row_num+"' value='"+FinalLimit+"' onchange ='setFinalLimit(this.id);' maxlength ='"+col_id_arr[2]+"' "+col_id_arr[5]+">"+"</td>";
					}
					else{
