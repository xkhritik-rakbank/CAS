...
String cabinetName="";
		...
		WriteLog("appServerType while MarqueeNotepadSelect : "+appServerType);
		WriteLog("wrapperIP while MarqueeNotepadSelect : "+wrapperIP);
		WriteLog("wrapperPort while MarqueeNotepadSelect : "+wrapperPort);
		WriteLog("sessionId while MarqueeNotepadSelect : "+sessionId);
		WriteLog("cabinetName while MarqueeNotepadSelect : "+cabinetName);
			
		try
		{
			String procName = wi_name.substring(0,2);
			if(procName.equalsIgnoreCase("PL")){
			SqlQuery = "SELECT Parent_WIName,Current_ws FROM NG_PL_EXTTABLE WHERE PL_wi_name ='"+wi_name+"'";
			}
			else if(procName.equalsIgnoreCase("CC")){
			SqlQuery = "SELECT Parent_WIName,Current_ws FROM NG_CC_EXTTABLE where CC_Wi_Name ='"+wi_name+"'";
			}
			sInputXML = "<?xml version='1.0'?>"+
			 ...
			 WriteLog("sOutputXML while MarqueeNotepadSelect : "+sInputXML);
		...
		try
		{
			SqlQuery = "SELECT distinct NoteDesc,NoteCode FROM NG_RLOS_GR_Note1 WHERE Notepad_winame ='"+parent_Winame+"' and workstep = '"+activityName+"'";
