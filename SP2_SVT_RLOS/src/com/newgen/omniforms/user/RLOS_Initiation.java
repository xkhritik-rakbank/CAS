		...
		if(NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_M").equalsIgnoreCase(formObject.getNGValue("initiationChannel")) ){
			...
			if ("Customer".equalsIgnoreCase(pEvent.getSource().getName())) {
				...
				else
				{
					...
					LoadPickList("cmplx_IncomeDetails_CreditTurnoverFreq", "select '--Select--' union all select convert(varchar, description) from NG_MASTER_Frequency with (nolock)");
					LoadPickList("cmplx_IncomeDetails_AvgCredTurnoverFreq", "select '--Select--' union all select convert(varchar, description) from NG_MASTER_Frequency with (nolock)");
