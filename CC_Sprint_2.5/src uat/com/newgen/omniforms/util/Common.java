...
public Date convertToDate(String input, Locale locale) {
       ...
       try {
           if (!"".equalsIgnoreCase(input)) {
               ...
               DigitalOnBoarding.mLogger.info("Empty Field");
               return null;
           }

       } catch (Exception e1) {
           DigitalOnBoarding.mLogger.info("RLOSCommon convertToDate"+ "Exception:" + e1);
       }
       return date;
   }
...
//frameCollapsibleAdjusment(childFrameLRSP, parentFrameLRSP, childFrameToExpLRSP, childFrameToVisLRSP, fragSequenceLRSP, allframesLRSP, formObject);
       DigitalOnBoarding.mLogger.info("In Frame Collapsible Adjusment");
       int prevTop = formObject.getTop(childFrameCollapsible[0]);
       int prevHeight = 20;
       int flag = 0;
       DigitalOnBoarding.mLogger.info(childFrameCollapsible[0] + " prevTop " + prevTop);
       DigitalOnBoarding.mLogger.info("Child Frame To Expand " + childFrameToExpand);
       DigitalOnBoarding.mLogger.info("Child Frame To Visible " + childFrameToVis.length);
       ...
       if (childFrameToExpand.equals("") && childFrameToVis.length == 0) {
           for (int i = 0; i < childFrameCollapsible.length; i++) {
               if (formObject.isVisible(childFrameCollapsible[i])) {
                   DigitalOnBoarding.mLogger.info("Child Frame " + childFrameCollapsible[i] + " is Visible");
                   if (flag == 0) {
                       formObject.setTop(childFrameCollapsible[i], prevTop);
                       DigitalOnBoarding.mLogger.info("ChildFrame " + childFrameCollapsible[i] + " top setted");
                       flag = 1;

                   } else {
                       formObject.setTop(childFrameCollapsible[i], prevTop + prevHeight + 2);
                       DigitalOnBoarding.mLogger.info("ChildFrame " + childFrameCollapsible[i] + " moved up");
                   }

                   prevTop = formObject.getTop(childFrameCollapsible[i]);
                   //prevHeight = formObject.getHeight(childFrameCollapsible[i]) + 10;
                   DigitalOnBoarding.mLogger.info("ChildFrame " + childFrameCollapsible[i] + " prevTop " + prevTop + " prevHeight " + prevHeight);

               }
           }

           if (!parentFrame.equals("")) {
               DigitalOnBoarding.mLogger.info("parent frame" + parentFrame + " set to prevTop " + prevTop + " prevHeight " + prevHeight);
               formObject.setHeight(parentFrame, prevTop + prevHeight + 2);
               DigitalOnBoarding.mLogger.info("parent frame " + parentFrame + " height " + formObject.getHeight(parentFrame));
           }

       } else if (!childFrameToExpand.equals("") && childFrameToVis.length == 0) {
           DigitalOnBoarding.mLogger.info("In 1 else frameCollapsibleAdjusment");
           DigitalOnBoarding.mLogger.info("Frame To Expand " + childFrameToExpand);
           for (int i = 0; i < childFrameCollapsible.length; i++) {
               if (formObject.isVisible(childFrameCollapsible[i])) {
                   DigitalOnBoarding.mLogger.info("Child Frame " + childFrameCollapsible[i] + " is Visible");
                   if (flag == 0) {
           ...
           DigitalOnBoarding.mLogger.info("In 2 else frameCollapsibleAdjusment");

           for (int i = 0; i < childFrameCollapsible.length; i++) {
               if (formObject.isVisible(childFrameCollapsible[i])) {
                   DigitalOnBoarding.mLogger.info("ChildFrame " + childFrameCollapsible[i] + " is Visible");
                   if (flag == 0) {
                       formObject.setTop(childFrameCollapsible[i], prevTop);
                       flag = 1;
                   ...
                   DigitalOnBoarding.mLogger.info("ChildFrame " + childFrameCollapsible[i] + " is InVisible");
                   for (String childToVis : childFrameToVis) {
                       if (childFrameCollapsible[i].equalsIgnoreCase(childToVis)) {
                           ...
                           DigitalOnBoarding.mLogger.info("ChildFrame To Visible " + childToVis + " prevTop " + prevTop + " prevHeight " + prevHeight);
       ...
       if (allframes.length > 0 && frag.length > 0) {
           ...
           flag = 0;
           int fragHeight = 0;
           DigitalOnBoarding.mLogger.info(allframes[0] + " " + formObject.isVisible(allframes[0]) + " previousTop " + previousTop);
           for (int i = 0; i < (allframes.length) && j < (frag.length); i++, j++) {
               if (formObject.isVisible(allframes[i])) {
                   ...
                   DigitalOnBoarding.mLogger.info("fragHeight: " + fragHeight);
                   fragHeight += 5;
                   DigitalOnBoarding.mLogger.info("fragHeight: " + fragHeight);
                   ...
                   DigitalOnBoarding.mLogger.info("previousTop: " + previousTop);
                   DigitalOnBoarding.mLogger.info("previousHeight: " + previousHeight);
               }
           }
   ...
   public void LoadPickList(String controlname, String query) {
	      FormReference formObject = FormContext.getCurrentInstance()
	              .getFormReference();
	      DigitalOnBoarding.mLogger.info("LoadPickList Query for " + controlname + ":"
	              + query);
