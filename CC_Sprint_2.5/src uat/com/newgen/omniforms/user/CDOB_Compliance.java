...
public class CDOB_Compliance extends CDOB_Common implements FormListener
{
	/**
	 * 
	 */
	...
	public void formLoaded(FormEvent pEvent)
	{
		...
		makeSheetsInvisible(tabName, "6,8,9,10,11,12,14,15,16,17");//pcaf-392
	...
	public void submitFormStarted(FormEvent pEvent) throws ValidatorException {
		try{
			...
			formObject.setNGValue("Compliance_DeC", formObject.getNGValue("cmplx_DEC_Decision"));
			//Changed by aman for Decision Name change
			//below code added by nikhil for PCSP-546
			//changes done by nikhil for PCSP-546
			formObject.setNGValue("IsComplianceChild_Exists", "N");
			LoadReferGrid();
			saveIndecisionGrid();
	...
	private void fragment_loaded(FormEvent pEvent,FormReference formObject)
	{
		...
		DigitalOnBoarding.mLogger.info( "Compliance Step Fragment loaded event frame is: " + pEvent.getSource().getName());
			if ("Customer".equalsIgnoreCase(pEvent.getSource().getName())) {
				...
				*/
				...
				formObject.setLocked("IncomingDocNew_Frame1",true);
...
if (pEvent.getSource().getName().equalsIgnoreCase("Liability_New")) {
			...
			//else if ("DecisionHistory".equalsIgnoreCase(pEvent.getSource().getName()) && "DecisionHistory".equalsIgnoreCase(pEvent.getSource().getName())) {
				 	...
				 	DigitalOnBoarding.mLogger.info("***********Inside checker decision history");
					fragment_ALign("DecisionHistory_Decision_Label1,cmplx_DEC_VerificationRequired#DecisionHistory_Decision_Label3,cmplx_DEC_Decision#DecisionHistory_Label26,DecisionHistory_ReferTo#DecisionHistory_Label11,DecisionHistory_dec_reason_code#DecisionHistory_Decision_Label4,cmplx_DEC_Remarks#\n#DecisionHistory_ADD#DecisionHistory_Modify#DecisionHistory_Delete#\n#DecisionHistory_Decision_ListView1#\n#DecisionHistory_save","DecisionHistory");//\n for new line
             		DigitalOnBoarding.mLogger.info("***********Inside checker after fragment alignment decision history");
...
//++ Below Code added By Yash on Oct 14, 2017  to fix : "for loading the Part Match nationality " : Reported By Shashank on Oct 09, 2017++
			...
			//12th sept
				...
				try{
					...
					//change by saurabh for Compliance point 1. 4th Feb 19.
					formObject.setLocked("Compliance_Frame1",true);
					...
					DigitalOnBoarding.mLogger.info("value of world check "+n);
					DigitalOnBoarding.mLogger.info("value of compliance row count "+Compliance_row_count);
					//below code modified by nikhil 02/12/17 to add all rows of world check to compliance grid
					if(n>Compliance_row_count && Compliance_row_count==0)//sagarika for PCAS 2257
					{
						
						for(int i=0;i<n;i++)
						{ 
							String matchFound = formObject.getNGValue("cmplx_WorldCheck_WorldCheck_Grid",i,17);
							//SKLogger_CC.writeLog("CC value of world check row grid to get values","inside world check grid--- ");
							//change by saurabh for WorldCompliance point on 2nd Feb 19.
							if(matchFound.contains("true")){
								...
								DigitalOnBoarding.mLogger.info("Compliance Dob"+formObject.getNGValue("cmplx_WorldCheck_WorldCheck_Grid",i,2));
								...
								DigitalOnBoarding.mLogger.info("setting compliance Dob"+Dob);
								...
								DigitalOnBoarding.mLogger.info("setting compliance Entered_Date"+Entered_Date);
								...
								DigitalOnBoarding.mLogger.info("setting compliance"+Updated_Date);
								...
								DigitalOnBoarding.mLogger.info("setting compliance"+Match_Rank);
								...
								DigitalOnBoarding.mLogger.info("setting compliance"+Gender);
								...
								//change by saurabh for compliance point 2. 5th Feb 19.
								String furtherinfo = formObject.getNGValue("cmplx_WorldCheck_WorldCheck_Grid",i,36);
								DigitalOnBoarding.mLogger.info( "furtherinfo: "+furtherinfo);
								formObject.setNGValue("Compliance_FurtherInfo",furtherinfo);
								DigitalOnBoarding.mLogger.info( "furtherinfo: "+formObject.getNGValue("Compliance_FurtherInfo"));
					...
					//12th september
					DigitalOnBoarding.mLogger.info( "befor locking frame2");
					formObject.setLocked("Compliance_Frame2",true);
					formObject.setLocked("cmplx_Compliance_ComplianceRemarks",false);
					DigitalOnBoarding.mLogger.info( "after locking frame 2");

					//12th september
				}catch(Exception ex){
					DigitalOnBoarding.mLogger.info( "Exception in compliance condition frag load: "+printException(ex));
				}
			}
