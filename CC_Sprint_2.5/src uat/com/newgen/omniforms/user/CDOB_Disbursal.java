...
public class CDOB_Disbursal extends CDOB_Common implements FormListener
{
	/**
	 * 
	 */
	 ...
	 public void formLoaded(FormEvent pEvent)
	{
	     ...
	     makeSheetsInvisible(tabName, "6,8,9,10,11,13,14,15,16,17");//pcaf-392
	     FormReference formObject = FormContext.getCurrentInstance().getFormReference();
	     formObject.setNGValue("Mandatory_Frames", NGFUserResourceMgr_DigitalOnBoarding.getGlobalVar("Disbural_Frame_Name"));
	...
	public void formPopulated(FormEvent pEvent) 
	{
	     try{
	    	 FormReference formObject = FormContext.getCurrentInstance().getFormReference();
	    	 DigitalOnBoarding.mLogger.info("Inside CC_Disbursal CC");
	...
	public void submitFormCompleted(FormEvent arg0) throws ValidatorException {
		...
		try{
			...
			if(NGFUserResourceMgr_DigitalOnBoarding.getGlobalVar("CC_Submit_Desc").equalsIgnoreCase(formObject.getNGValue("Decision"))){
				List<String> objInput=new ArrayList<String>();
				objInput.add("Text:"+formObject.getWFWorkitemName());
				objInput.add("Text:"+"Approve");
				DigitalOnBoarding.mLogger.info("objInput args are: "+objInput);
	...
	public void submitFormStarted(FormEvent arg0) throws ValidatorException {
		...
		try{
			...
			else if("Reject".equalsIgnoreCase(formObject.getNGValue("cmplx_DEC_Decision")))//Deepak To insert Rejected application entry.
			{
			...
			DigitalOnBoarding.mLogger.info("Inside validator exception before throwing again.");
			throw new ValidatorException(new FacesMessage("Please generate and confirm Murabha File!!!!"));
		}
		catch(Exception e){
	...
	{
		DigitalOnBoarding.mLogger.info("fragemnt to be loaded is: "+pEvent.getSource().getName());
		if ("Customer".equalsIgnoreCase(pEvent.getSource().getName())) {
			...
			LoadView(pEvent.getSource().getName());
			DigitalOnBoarding.mLogger.info("Inside alternate contact details fragment (Disbursal) :"+formObject.getNGValue("Card_Dispatch_Option"));
			...
			formObject.setEnabled("AlternateContactDetails_SkywardNumber",true);
			formObject.setLocked("AlternateContactDetails_SkywardNumber",true);
		}
		...
		//COMMENTED BY ABHISHEK AS PER cc fsd
			...
			loadPicklist_Fatca();//added by saurabh1 for pcasf-784
		}
			...
			LoadDatainCardDetailsSecurityChq();
		}
		...
		//	formObject.setLocked("EMploymentDetails_Frame1", true);
         ...
         if(	NGFUserResourceMgr_DigitalOnBoarding.getGlobalVar("CC_BusinessTitaniumCard").equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,2)) || NGFUserResourceMgr_DigitalOnBoarding.getGlobalVar("CC_SelfEmployedCreditCard").equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,2))){
					if(	NGFUserResourceMgr_DigitalOnBoarding.getGlobalVar("CC_true").equalsIgnoreCase(formObject.getNGValue("cmplx_Customer_NTB"))){
						...
						DigitalOnBoarding.mLogger.info( "set visible FinacleCore_Frame8 if condition ");
						formObject.setVisible("FinacleCore_Frame8", true);
						DigitalOnBoarding.mLogger.info( "after set visible FinacleCore_Frame8 if condition ");
						formObject.setVisible("FinacleCore_Frame2", false);
						formObject.setVisible("FinacleCore_Frame3", false);
						DigitalOnBoarding.mLogger.info( "Inside fianacle core fragment if condition ");
					}
					else{
		...
		//	LoadPickList("PartMatch_nationality","select '--Select--' as description,'' as code union select convert(varchar, Description),code from ng_MASTER_Country with (nolock) order by code"); //Arun (10/10)
			...
			if(!"PU".equalsIgnoreCase(subProd) && !"LI".equalsIgnoreCase(subProd) && !"PULI".equalsIgnoreCase(subProd)){
				...
				if(framestate1 == 0){
					DigitalOnBoarding.mLogger.info("Incomedetails");
				}
				else {
					formObject.fetchFragment("IncomeDEtails", "IncomeDetails", "q_IncomeDetails");
					DigitalOnBoarding.mLogger.info("fetched income details");
			...
			*/	
				...
				if(!"Y".equals(formObject.getNGValue("cmplx_LimitInc_UpdateCustFlag"))){
					...
					DigitalOnBoarding.mLogger.info("before calling loadDataInLimitInc");
				loadDataInLimitInc();
				DigitalOnBoarding.mLogger.info("after calling loadDataInLimitInc");
			//}
			/*else{
				formObject.setLocked("Limit_Inc_Frame4",true);
			}*/
