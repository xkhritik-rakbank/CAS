...
public class CDOB_DDVT_Checker extends CDOB_Common implements FormListener
{
	/**
	 * 
	 */
	...
	public void formPopulated(FormEvent pEvent) 
	{
		FormReference formObject = FormContext.getCurrentInstance().getFormReference();
	     try{
	    	 DigitalOnBoarding.mLogger.info("Inside CC_DDVT CHECKER CC");
	...
	***********************************************************************************  */
	...
	public void saveFormStarted(FormEvent arg0) throws ValidatorException {
		...
		try{
			...
			DigitalOnBoarding.mLogger.info("Exception occured while setting Customer name & CIf in ext table at DDVT checker"+ e.getMessage());
		}
	...
	public void submitFormCompleted(FormEvent pEvent) throws ValidatorException {
		...
		DigitalOnBoarding.mLogger.info("Inside PL PROCESS ddvtchecker submitFormCompleted()" + pEvent.getSource()); 
		FormReference formObject = FormContext.getCurrentInstance().getFormReference();
...
CPV_REQUIRED
	...
	public void submitFormStarted(FormEvent arg0) throws ValidatorException {
		...
		try{
			...
			if(!NGFUserResourceMgr_DigitalOnBoarding.getGlobalVar("CC_SelfEmployed").equals(formObject.getNGValue("EmploymentType"))){
			...
			if("Y".equalsIgnoreCase(formObject.getNGValue("IS_Approve_Cif")) && "Approve".equalsIgnoreCase(formObject.getNGValue("cmplx_DEC_Decision"))){
				DigitalOnBoarding.mLogger.info("Submit form started of DDVT checker case of Approve Sub to CIF :: Winame: "+ formObject.getWFWorkitemName()+" user name: "+ formObject.getUserName());
	...
	private void fragment_loaded(FormEvent pEvent,FormReference formObject)
	{
		...
		//21/08/2017 it should be disabled added
				...
				DigitalOnBoarding.mLogger.info("inside locking of reference details");
				formObject.setLocked("Reference_Details_Frame1",true);
				
			}
			else if ("RejectEnq".equalsIgnoreCase(pEvent.getSource().getName())) {
				DigitalOnBoarding.mLogger.info("inside locking of reject enquiry");
				formObject.setLocked("RejectEnq_Frame1",true);
				
			}
			else if ("SalaryEnq".equalsIgnoreCase(pEvent.getSource().getName())) {
				DigitalOnBoarding.mLogger.info("inside locking of Salary Enquiry");
				formObject.setLocked("SalaryEnq_Frame1",true);
				
			}
			else if ("CreditCardEnq".equalsIgnoreCase(pEvent.getSource().getName())) {
				DigitalOnBoarding.mLogger.info("inside locking of Credit Card");
				...
				DigitalOnBoarding.mLogger.info("inside locking of LOS_Frame1");
				formObject.setLocked("LOS_Frame1",true);
				
			}
			else if ("ExternalBlackList".equalsIgnoreCase(pEvent.getSource().getName())) {
				DigitalOnBoarding.mLogger.info("inside locking of External Blacklist");
			...
			*/
			...
			//shifted here by akshay on 17/1/18
				...
				if(	"".equalsIgnoreCase(formObject.getNGValue("cmplx_Customer_NEP")) && 	NGFUserResourceMgr_DigitalOnBoarding.getGlobalVar("CC_false").equalsIgnoreCase(formObject.getNGValue("cmplx_Customer_NTB")))
					formObject.setVisible("IncomingDoc_UploadSig",false);
				else if(!"".equalsIgnoreCase(formObject.getNGValue("cmplx_Customer_NEP")) || 	NGFUserResourceMgr_DigitalOnBoarding.getGlobalVar("CC_true").equalsIgnoreCase(formObject.getNGValue("cmplx_Customer_NTB"))){
			...
			//ended by yash
				...
				DigitalOnBoarding.mLogger.info("Before adding in Multiple applicants Grid!!!");
				...
				try{
					...
					//if(formObject.getLVWRowCount("cmplx_DEC_MultipleApplicantsGrid")>0){
						...
						DigitalOnBoarding.mLogger.info("Applicant types in Grid is: "+grid_applicantType);
				...
				/*		if(!grid_applicantType.contains("Primary")){
						...
						if(!grid_applicantType.contains("Supplement")){
							int n=formObject.getLVWRowCount("SupplementCardDetails_cmplx_supplementGrid");
								...
								DigitalOnBoarding.mLogger.info("supplementary grid row count: "+n);
							for(int i=0;i<n;i++){
								subList.clear();
						...
						DigitalOnBoarding.mLogger.info("mylist 3: "+mylist);
						if(mylist.size()>0){
					...
					//}		
				}catch(Exception e){DigitalOnBoarding.mLogger.info(printException(e));}
