...
public class CDOB_DDVT_maker extends CDOB_Common implements FormListener
{
	/**
	 * 
	 */
...

	{
		...
		try{
			DigitalOnBoarding.mLogger.info("Inside CC_DDVT_maker CC");
			...
			formObject.setNGValue("Mandatory_Frames", NGFUserResourceMgr_DigitalOnBoarding.getGlobalVar("DDVT_Frame_Name"));
	...
	public void submitFormStarted(FormEvent arg0) throws ValidatorException {
		// TODO Auto-generated method stub
		FormReference formObject = FormContext.getCurrentInstance().getFormReference();
		DigitalOnBoarding.mLogger.info("Submit form started method start:: Winame: "+ formObject.getWFWorkitemName()+" user name: "+ formObject.getUserName());
		Check_selectedCard();
		/*Check_EFC_Limit();*/
		try{
			...
			DigitalOnBoarding.mLogger.info("Exception occured in set ext table data: "+e.getMessage());
		}
		...
		try{
			...
			DigitalOnBoarding.mLogger.info("Exception occured in set decision+fragment data save table data: "+e.getMessage());
		}
		
		try
		{
			DigitalOnBoarding.mLogger.info("CC val change "+ "befor incomingdoc");
			//IncomingDoc();
			validateStatusForSupplementary();
			DigitalOnBoarding.mLogger.info("CC val change "+ "after incomingdoc");
		}
		catch(Exception ex)
		{
			DigitalOnBoarding.mLogger.info(ex);
			printException(ex);
		}
		//--above code added by nikhil 11//11/17
		DigitalOnBoarding.mLogger.info("Submit form started method end:: Winame: "+ formObject.getWFWorkitemName()+" user name: "+ formObject.getUserName());
	...
	private void fragment_loaded(FormEvent pEvent,FormReference formObject)
	{
		if ("Customer".equalsIgnoreCase(pEvent.getSource().getName())) {
			...
			if(	NGFUserResourceMgr_DigitalOnBoarding.getGlobalVar("CC_true").equalsIgnoreCase(formObject.getNGValue("cmplx_Customer_CIFID_Available"))){
		...
		//	formObject.setLocked("AddressDetails_Frame1",true);
			...
			DigitalOnBoarding.mLogger.info("value of  AlternateContactDetails_AirArabiaIdentifier: + "+formObject.getNGValue("AlternateContactDetails_AirArabiaIdentifier"));
			...
			if("".equalsIgnoreCase(formObject.getNGValue("AlternateContactDetails_AirArabiaIdentifier"))){
				if(null!=formObject.getNGValue("cmplx_Customer_CIFNO") && !"".equals(formObject.getNGValue("cmplx_Customer_CIFNO"))){
					...
					DigitalOnBoarding.mLogger.info("RLOS COMMON"+" iNSIDE squeryair+ "+squeryair);
					...
					DigitalOnBoarding.mLogger.info("CIF is empty for Air Arabia logic to work");
				}
			...
			DigitalOnBoarding.mLogger.info( "Query to insert: card if not exist: "+sQuery_CardProduct);
			...
			DigitalOnBoarding.mLogger.info( "inside decision history fragment load");
          
			if(	NGFUserResourceMgr_DigitalOnBoarding.getGlobalVar("CC_Telesales_Init").equalsIgnoreCase(formObject.getNGValue("InitiationType")))
			{
				loadPicklist3();
			}
			else
			...
			//changed by nikhil increase decision reaon width
          ...
          //below code added by niukhil for PCAS-1346
			...
			if(NGFUserResourceMgr_DigitalOnBoarding.getGlobalVar("CC_false").equalsIgnoreCase(formObject.getNGValue("cmplx_Customer_NTB")))
				formObject.setEnabled("IncomingDoc_ViewSIgnature",true);
			else if(NGFUserResourceMgr_DigitalOnBoarding.getGlobalVar("CC_true").equalsIgnoreCase(formObject.getNGValue("cmplx_Customer_NTB"))){
			...
			if(	NGFUserResourceMgr_DigitalOnBoarding.getGlobalVar("CC_CreditCard").equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,1)) && "yes".equalsIgnoreCase(formObject.getNGValue("cmplx_CardDetails_Supplementary_Card"))){
		...
		/*	formObject.setLocked("cmplx_IncomeDetails_grossSal", true);
			...
			*/
			...
			DigitalOnBoarding.mLogger.info( "Fter Load Pick Company: 1");
			...
			DigitalOnBoarding.mLogger.info( "Fter Load Pick Company: 2");
			//below code added by nikhil 08/12/17
		...
		*/	loadPicklist_CompanyDet();
		DigitalOnBoarding.mLogger.info( "Fter Load Pick Company: ");
...
*/			
			...
			try{
				//partMatchValues();
				DigitalOnBoarding.mLogger.info("CC"+ "Inside partMatch frag loaded!!");
				String Fullname="";
				...
				if(framestate_Alt_Contact == 0){
					DigitalOnBoarding.mLogger.info("Alternate details alread fetched");
				}
				else {
					formObject.fetchFragment("Alt_Contact_container", "AltContactDetails", "q_AltContactDet");
					DigitalOnBoarding.mLogger.info("fetched Alternate Contact details");
				}
			...
			DigitalOnBoarding.mLogger.info("formObject.getNGValue('cmplx_Customer_FirstNAme')"+ formObject.getNGValue("cmplx_Customer_FirstNAme"));
			formObject.setNGValue("PartMatch_lname", formObject.getNGValue("cmplx_Customer_LastNAme"));
			DigitalOnBoarding.mLogger.info("formObject.getNGValue('cmplx_Customer_LastNAme')"+ formObject.getNGValue("cmplx_Customer_LastNAme"));
			...
			if(!"Y".equals(formObject.getNGValue("Is_PartMatchSearch"))){
				...
				if(NGFUserResourceMgr_DigitalOnBoarding.getGlobalVar("CC_BTC").equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,2)) || 	NGFUserResourceMgr_DigitalOnBoarding.getGlobalVar("CC_SE").equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,2))){
				...
				DigitalOnBoarding.mLogger.info("eXCEPTION"+ printException(e));	
			}
		...
		//	LoadPickList("cmplx_MOL_nationality", "select '--Select--'as description,'' as code  union select convert(varchar, description),code from ng_MASTER_Country with (nolock) order by Code");
			...
			if(	NGFUserResourceMgr_DigitalOnBoarding.getGlobalVar("CC_true").equalsIgnoreCase(formObject.getNGValue("cmplx_Liability_New_overrideIntLiab"))){
