	...
	public String GenerateXML(String callName, String Operation_name) {
		...
		try {
			...
			if (!OutputXML_header.isEmpty()) {
				...
				DigitalOnBoarding.mLogger.info("OutputXML"+ "OutputXML" + DB_List);
	...
	public String getCustAddress_details(String call_name){
		...
		try{
			...
			for (int i = 0; i<add_row_count;i++){
				...
				else if(Applicant_Name.split("-")[1].trim().replaceAll(" ","").equalsIgnoreCase(formObject.getNGValue("CC_Creation_CustomerName").trim().replaceAll(" ","")))
				{
		...
		try {
			...
			if (OutputXML.size()==0) 
			{
				...
				for (int i = 0; i < OutputXML.size(); i++) {
					...
					DigitalOnBoarding.mLogger.info("EnquiryDate :" + EnquiryDate);
					
					Date parseDateCC = new SimpleDateFormat("dd-MM-yyyy").parse(EnquiryDate);
					DigitalOnBoarding.mLogger.info("parseDateCC :" + parseDateCC);
					
					SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss");
					DigitalOnBoarding.mLogger.info("formatter :" + formatter);
					
					String enquire_date_format_CC = formatter.format(parseDateCC);
					DigitalOnBoarding.mLogger.info("enquire_date_format :" + enquire_date_format_CC);
					add_xml_str = add_xml_str + "<ExternalBureau>";
					add_xml_str = add_xml_str + "<applicant_id>" + CifId+ "</applicant_id>";
					add_xml_str = add_xml_str + "<bureauone_ref_no>"+refNo+"</bureauone_ref_no>";
					add_xml_str = add_xml_str + "<full_name>" + fullnm+ "</full_name>";
			...
			for (List<String> mylist : DB_List) {
				...
				{	*/
					...
					//added by akshay on 17/1/18 fpr proc 1595
						...
						String xml_str = int_xml.get(parent_tag);
						DigitalOnBoarding.mLogger.info("CDOB CIF update EStatementFlags form control"+ form_control+" form conrol val:+ " + formObject.getNGValue(form_control));
						xml_str = xml_str + "<" + tag_name + ">" + ("Yes".equalsIgnoreCase(formObject.getNGValue(form_control))||"Y".equalsIgnoreCase(formObject.getNGValue(form_control))?"Y":"N") + "</" + tag_name + ">";
						int_xml.put(parent_tag, xml_str);
					}
					else{
						int_xml = GenDefault_Input_DB(int_xml,recordFileMap,formObject,callName);
			...
			for (List<String> mylist : DB_List) {
				...
				//Deepak - 28july 2019 code added for CardRefNumber
					...
					try{
						...
						String squery="select top 1 ReqProduct from ng_master_cardProduct where CODE = '"+card_prod+"'";
				...
				else if( "PRIMARY".equalsIgnoreCase(formObject.getNGValue("cmplx_CCCreation_cmplx_CCCreationGrid",formObject.getSelectedIndex("cmplx_CCCreation_cmplx_CCCreationGrid"),12)) || ("SUPPLEMENT".equalsIgnoreCase(formObject.getNGValue("cmplx_CCCreation_cmplx_CCCreationGrid",formObject.getSelectedIndex("cmplx_CCCreation_cmplx_CCCreationGrid"),12)) && formObject.getNGValue("cmplx_Customer_PAssportNo").equalsIgnoreCase(formObject.getNGValue("cmplx_CCCreation_cmplx_CCCreationGrid",formObject.getSelectedIndex("cmplx_CCCreation_cmplx_CCCreationGrid"),13))  ))
				{
					...
					//changes by saurabh on 
					...
					else if("DispatchDetails".equalsIgnoreCase(tag_name)){
						//String xml_str = int_xml.get(parent_tag);
						String xml_str = int_xml.get(parent_tag);
						String Dispatch_details = formObject.getNGValue("AlternateContactDetails_CardDisp");
						Dispatch_details = "<DispatchMode>"+Dispatch_details+"</DispatchMode>";
						...
						xml_str =  xml_str+"<"+tag_name+">"+Dispatch_details+"</"+ tag_name+">"+Branch_details;
						...
						//elow code comented by nikhil for Card notification CR
							...
							String query_prdtype="select top 1 ReqProduct from ng_master_cardProduct where CODE = '"+card_prod+"' and Subproduct = '"+formObject.getNGValue("Sub_Product")+"'";
							DigitalOnBoarding.mLogger.info("CDOB Integration query_prdtype: + "+query_prdtype);
							List<List<String>> typeOfCP = formObject.getDataFromDataSource(query_prdtype);
						...
						tagValue=(formObject.getNGValue("cmplx_Customer_VIPFlag").equalsIgnoreCase("true")?"1":"0");
							...
							tagValue=(formObject.getNGValue("cmplx_Customer_RESIDENTNONRESIDENT").equalsIgnoreCase("Y")?"N":"Y");//790
						}	
					...
					else if(tag_name.equalsIgnoreCase("SalaryDate")&& (formObject.getNGValue("EmploymentType").contains("Salaried") || "S".equalsIgnoreCase(formObject.getNGValue("EmploymentType")))){
						//CreditCard.mLogger.info("inside 1st if inside customer update req1");
						String xml_str = int_xml.get(parent_tag);
						Calendar now = Calendar.getInstance();
						String month = "";
				...
				//Deepak Code commented for CR PCSP-475 to send value from master 
					...
					if(int_xml.containsKey(parent_tag))
					{
						try{
							...
							DigitalOnBoarding.mLogger.info("Profileserno query: "+ ProfileSerNo_Query);
...
//cmplx_RejectEnq_RejectEnqGrid
			...
			String cheqNo = formObject.getNGValue("PORTAL_REF_NUMBER"); //change done by sagarika as discussed.
	...
	public String getExtensions_details(boolean primary){
		...
		try{
			FormReference formObject = FormContext.getCurrentInstance().getFormReference();
			String card_prod = formObject.getNGValue("CC_Creation_Product");
			List<List<String>> typeOfCP = formObject.getDataFromDataSource("select top 1 ReqProduct from ng_master_cardProduct where CODE = '"+card_prod+"' and Subproduct = '"+formObject.getNGValue("Sub_Product")+"'");
				...
				if(entry.getKey().contains("F154")){
					...
					if(formObject.getLVWRowCount(lvName)>0){
	               	 ...
	               	 if(primary){
	               		 ...
	               		 DigitalOnBoarding.mLogger.info("CardShieldQuery: "+ CardShieldQuery);
	...
	public String getFATCA_details(FormReference formObject){
		...
		try{
			...
			for(int i=0;i<formObject.getLVWRowCount("cmplx_FATCA_cmplx_GR_FatcaDetails");i++){
				DigitalOnBoarding.mLogger.info("Sajan FaTCA "+formObject.getLVWRowCount("cmplx_FATCA_cmplx_GR_FatcaDetails"));
	...
	public String getSkywardNo(String skywardno){//change done for skyward call -production change
	...
	public String formatDateFromOnetoAnother(String date,String givenformat,String resultformat) {
		...
		{DigitalOnBoarding.mLogger.info("Exception in get History");
		...
		return add_xml_str;
			...
			public static String getCourtCase()
			{
				...
				DigitalOnBoarding.mLogger.info("inside getCourtCase Query: "+ Query);
				String add_xml_str = "";

				try
				{
					List<List<String>> OutputXML = formObject.getDataFromDataSource(Query);
					DigitalOnBoarding.mLogger.info("inside getCourtCase OutputXML: "+ OutputXML.toString());
					for (int i = 0; i < OutputXML.size(); i++) {
						...
						DigitalOnBoarding.mLogger.info("inside getCourtCase add_xml_str: "+ add_xml_str);
					}
				}
				catch(Exception ex)
				{DigitalOnBoarding.mLogger.info("Exception in get getCourtCase" + ex.getMessage());
