...
public class CDOB_Query extends CDOB_Common implements FormListener{
	...
	public void formLoaded(FormEvent pEvent)
	{
		...
		if("SalesCoordinator".equalsIgnoreCase(sActivityName)){
			makeSheetsInvisible(tabName, "1,6,7,10,11,12,13,14,15,16,17");//pcaf-392
       	}
        else if(NGFUserResourceMgr_DigitalOnBoarding.getGlobalVar("CC_CSM").equalsIgnoreCase(sActivityName)){
        	makeSheetsInvisible(tabName, "1,6,7,11,12,13,14,15,16,17");
       	}
        else if("DDVT_Maker".equalsIgnoreCase(sActivityName)){
        	makeSheetsInvisible(tabName, "1,6,11,12,13,14,15,16,17");	
       	}
        	...
        	makeSheetsInvisible(tabName, "1,6,7,11,12,13,14,15,17");
       	}
        else if("Sales_Approver".equalsIgnoreCase(sActivityName)){
        	makeSheetsInvisible(tabName, "1,6,7,11,12,13,14,15,16,17");
       	}
        else if("Dispatch".equalsIgnoreCase(sActivityName)){
        	makeSheetsInvisible(tabName, "1,6,7,11,12,13,14,15,16");
       	}
        ...
        else if("EFMS_Hold".equalsIgnoreCase(sActivityName)){
        	makeSheetsInvisible(tabName, "1,6,7,11,12,13,14,15,16,17");
        	formObject.setVisible("ExtLiability_Frame4",true);
        }
    	//added 11th dec 2017
	...
	public void formPopulated(FormEvent pEvent) 
	{
		try{
			...
			formObject.setVisible("Product_Frame1",true);//saurabh1 jira
			
			new CDOB_CommonCode().setFormHeader(pEvent);
            DigitalOnBoarding.mLogger.info("Inside Part_Match");            
	...
	*/}
	...
	private void fragment_loaded(FormEvent pEvent,FormReference formObject)
	{
		...
		//	String sActivityName=FormContext.getCurrentInstance().getFormConfig( ).getConfigElement("ActivityName"); 
			/*if("EFMS_Hold".equalsIgnoreCase(sActivityName))
			   {
				   formObject.fetchFragment("Internal_External_Liability", "Liability_New", "q_Liabilities");   
			   }*/
			  formObject.setVisible("ExtLiability_Frame4",true);  
			  formObject.setLocked("ExtLiability_Frame4", true);
			  DigitalOnBoarding.mLogger.info("sagarika@@@-liability");
			//formObject.fetchFragment("Internal_External_Liability", "Liability_New", "q_Liabilities");
		//	formObject.setLocked("ExtLiability_Frame1", true);
			//formObject.setLocked("ExtLiability_Frame4", true);ExtLiability_Frame4
			...
			formObject.setVisible("Liability_New_Label1",true);//ExtLiability_Limit ExtLiability_takeoverAMount 
			...
			formObject.setVisible("ExtLiability_Label19",true);
			formObject.setVisible("ExtLiability_Label22",true);
			formObject.setVisible("Liability_New_EMI",true);
			formObject.setVisible("Liability_New_Label9",true);
			if("CAD_Analyst1".equalsIgnoreCase(formObject.getWFActivityName()) || "CAD_Analyst2".equalsIgnoreCase(formObject.getWFActivityName()))
			{
				formObject.setVisible("Liability_New_Label1",false);//ExtLiability_Limit ExtLiability_takeoverAMount 
...
LoadPickList("cmplx_EligibilityAndProductInfo_RepayFreq", "select '--Select--' as description,'' as code  union select convert(varchar, description),code from NG_MASTER_frequency with (nolock) order by code");
		...
		//	fetchIncomingDocRepeater();
		...
		else if ("CRMCustInfo".equalsIgnoreCase(pEvent.getSource().getName())) {
			...
			DigitalOnBoarding.mLogger.info("finacle_ss");
			formObject.setVisible("FinacleCore_Frame6",true);
	...
	/*	else if ("ReferenceDetailVerification".equalsIgnoreCase(pEvent.getSource().getName())) {
		...
		//	formObject.setVisible("cmplx_SmartCheck_SmartCheckGrid",false);-saurabh1 for 796
			...
			formObject.setLocked("cmplx_SmartCheck1_SmartChkGrid_FCU",true);
			formObject.setEnabled("SmartCheck1_CreditRemarks",false);
			formObject.setEnabled("SmartCheck1_CPVRemarks",false);
			formObject.setEnabled("SmartCheck1_FCURemarks",false);
			DigitalOnBoarding.mLogger.info( "sagg-ss ");
			if("CPV".equalsIgnoreCase(pEvent.getSource().getName()))
			{
				DigitalOnBoarding.mLogger.info( "sagg ");
				formObject.setEnabled("SmartCheck1_Frame1",false);	
			}
			formObject.setEnabled("SmartCheck1_Frame1",false);
		}
