...
public class RLOS_Reject extends RLOSCommon implements FormListener
{
	...
	public void formPopulated(FormEvent pEvent) 
	{
		...
		/*	formObject.setNGValue("RM_Name",getDataFromSourceCodeMaster("supervisor_name"));
			formObject.setNGValue("BRANCH", getDataFromSourceCodeMaster("Branch"));			formObject.setNGValue("lbl_TL_Name_val",formObject.getNGValue("RM_Name"));
			*/
	...
	public void submitFormCompleted(FormEvent pEvent)
		...
		String reject_query="insert into ng_cas_rejected_table(CustomerName,CIF	,DeclinedDate,	Remarks	,AccountNo,	CRN	,CardProduct,PassportNo,	Mobile,	Employer,	RMName,	Reject_System,	UploadDate,	ECRN,	Product,	SubProduct	,Nationality	,DOB,	EmiratesID,	Customer_ref_No) select top 1 exttable.CUSTOMERNAME,exttable.CIF_ID,decision.dateLastChanged,decision.remarks,exttable.Account_Number,exttable.crn,exttable.CardProduct_Primary,exttable.PassportNo,exttable.MobileNo,exttable.Employer_Name,exttable.RM_Name,'CAS',sysdatetime(),exttable.ecrn,exttable.primaryProduct,exttable.Sub_Product,customer.Nationality,customer.dob,customer.EmirateID,SUBSTRING(exttable.winame,6,8) from NG_rlos_EXTTABLE exttable with(nolock) join ng_rlos_customer customer with(nolock) on exttable.winame=customer.wi_name join NG_RLOS_GR_DECISION decision with(nolock) on decision.dec_wi_name=exttable.winame  where winame='"+formObject.getWFWorkitemName()+"' order by decision.dateLastChanged desc";
		//RLOS.mLogger.info("Query to insert data in ng_cas_rejected_table: "+reject_query);
