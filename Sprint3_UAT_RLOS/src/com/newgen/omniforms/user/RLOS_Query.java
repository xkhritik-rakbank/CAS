...
//cmplx_EmploymentDetails_Designation
	...
	public void call_Frame_Expanded(ComponentEvent pEvent)
	{
		...
		else if ("Incomedetails".equalsIgnoreCase(pEvent.getSource().getName())) 
		{
			...
			else if ((formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,2)=="PA"))
			{
...
*/
	...
	public void call_Value_Changed(ComponentEvent pEvent)
	{
		...
		if ("SubProd".equalsIgnoreCase(pEvent.getSource().getName())){
			...
			LoadPickList("AppType", "select convert(text, description),code from ng_master_ApplicationType with (nolock) where subProdFlag='"+formObject.getNGValue("SubProd")+"' and isActive='Y' order by code");
