...
//CardDispatchToButton 
	...
	public String getDataFromSourceCodeMaster(String columnName){
		...
		//RLOS.mLogger.info( "result of fetch RMname query: "+result); Output_CPV_Waiver
	...
	//added by saurabh1 for enroll
	 public void enrollrewardValid() {
		...
		RLOS.mLogger.info("query produced is@sagarika:"+query);
		try{
			if(records!=null && records.size()>0 && records.get(0)!=null){
				...
				//	RLOS.mLogger.info("sssagarika"+records.get(0).get(0));
				...
				//	RLOS.mLogger.info("else sagarika"+records.get(0).get(0));
	 ...
	 public void SupplementaryenrollrewardValid() {
			...
			String query = "select count(*) from ng_rlos_IGR_Eligibility_CardLimit limit with (nolock) join ng_master_cardProduct mstr with (nolock) on limit.Card_Product=mstr.CODE where wi_name like '"+formObject.getWFWorkitemName()+"' and Cardproductselect='true' and mstr.DESCRIPTION like '%SKYWARDS%'";
			List<List<String>> records = formObject.getDataFromDataSource(query);
			RLOS.mLogger.info("squery produced is@sagarika:"+query);
			try{
				if(records!=null && records.size()>0 && records.get(0)!=null){
					RLOS.mLogger.info("records list is:"+records);
					if("0".equals(records.get(0).get(0))){
						//RLOS.mLogger.info("Supsssagarika"+records.get(0).get(0));
						formObject.setVisible("SupplementCardDetails_Label24", false);
						formObject.setVisible("SupplementCardDetails_Text3", false);
					}
					else{
						//RLOS.mLogger.info("else sagarika"+records.get(0).get(0));
						formObject.setVisible("SupplementCardDetails_Label24", true);
						formObject.setVisible("SupplementCardDetails_Text3", true);
					}
				}
			}catch(Exception ex){
				RLOS.mLogger.info( "Exception in AirArabiaValid Function"+printException(ex));
		...
		try{
			...
			if(tenore!=null && !"".equalsIgnoreCase(tenore) && !" ".equalsIgnoreCase(tenore)){
				...
				if(moratorium!=null && !"".equalsIgnoreCase(moratorium)){
					...
					formObject.setNGValue("cmplx_EligibilityAndProductInfo_Moratorium",0);
	...
	public void Eligibilityfields()
	{
		...
		if(NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_SE").equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,2)) || (NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_BTC").equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,2))) || (NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_SelfEmployed").equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,6)))){
			...
			if(!(NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_PersonalLoan").equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,1)))){
				...
				formObject.setVisible("cmplx_EligibilityAndProductInfo_LoanProcessingVATPercent",true);
				formObject.setVisible("ELigibiltyAndProductInfo_Label25",true);
				formObject.setVisible("cmplx_EligibilityAndProductInfo_InsuranceVATPercent",true);
				formObject.setVisible("ELigibiltyAndProductInfo_Label26",true);
				formObject.setVisible("cmplx_EligibilityAndProductInfo_InsuranceVat",true);
				formObject.setVisible("ELigibiltyAndProductInfo_Label27",true);
				formObject.setVisible("cmplx_EligibilityAndProductInfo_LoanProcessingVat",true);
		...
		{
			formObject.setNGValue("ELigibiltyAndProductInfo_Label19", "Processing fee");
			...
			formObject.setEnabled("cmplx_EligibilityAndProductInfo_LPF", true);
			formObject.setLocked("cmplx_EligibilityAndProductInfo_LPF", false);
			
			// condition to set by defalut value
			
			if ("".equalsIgnoreCase(formObject.getNGValue("cmplx_EligibilityAndProductInfo_LPF"))){
				
				formObject.setNGValue("cmplx_EligibilityAndProductInfo_LPF", "1");
			}
		}
	}
...

	{
		...
		LoadPickList("cmplx_Customer_Constitution","select distinct convert(varchar, Description),code from NG_MASTER_Constitution with (nolock)" );
	...
	public void setcustomer_enable(){
		...
		formObject.setEnabled("cmplx_Customer_Third_Nationaity_Applicable", true);
		...
		formObject.setEnabled("cmplx_Customer_Constitution", true);
		...
		//PCASI-1782
		if(formObject.getNGValue("cmplx_Customer_COuntryOFResidence")==null ||formObject.getNGValue("cmplx_Customer_COuntryOFResidence")=="")
			formObject.setNGValue("cmplx_Customer_COuntryOFResidence", "AE");
...
//CardProd
		if(NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_PersonalLoan").equalsIgnoreCase(ReqProd)){
			...
			if("Telesales_Init".equalsIgnoreCase(formObject.getWFActivityName())){
				LoadPickList("SubProd", " select '--Select--' as description,'' as code union select convert(varchar(50),description),code  from ng_master_Subproduct_CC with(nolock) where  code='"+NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_IM")+"' and  IsActive='Y' order by code");
...
//AppType
	...
	public void loadPicklist_Kyc()
	{
		...
		if("true".equalsIgnoreCase(formObject.getNGValue("cmplx_Customer_NTB"))){
			RLOS.mLogger.info("inside if ntb: kyc");
		...
		RLOS.mLogger.info("KYC_Combo1 :: "+formObject.getNGValue("KYC_Combo1") +" , NTB case :: "+formObject.getNGValue("cmplx_Customer_NTB"));
		RLOS.mLogger.info("KYC LoadPickList");
	...
	public void loadPicklist_ServiceRequest()
	{
		...
		LoadPickList("approvalCode", "select convert(varchar, description),code from NG_MASTER_ApprovalCode with (nolock) where IsActive = 'Y' order by code");
		...
		//LoadPickList("Account_No_for_AT", "select Acctid as Account_ID from  ng_RLOS_CUSTEXPOSE_AcctDetails where wi_name ='"+formObject.getWFWorkitemName()+"' and CifId='"+formObject.getNGValue("cmplx_Customer_CIFNO")+"' order by case when accttype='CURRENT ACCOUNT' then '1'  when accttype='FAST SAVER' then '2' else accttype end asc");	
		LoadPickList("Account_No_for_AT", "select '--Select--' as Account_ID union select Acctid as Account_ID from  ng_RLOS_CUSTEXPOSE_AcctDetails where wi_name ='"+formObject.getWFWorkitemName()+"' and CifId='"+formObject.getNGValue("cmplx_Customer_CIFNO")+"' order by Account_ID desc");	
	}
	...
	public void  setALOCListed(){
		...
		{
			if("false".equalsIgnoreCase(IncInCC) && "false".equalsIgnoreCase(INcInPL))
			{
	...
	public void employment_details_load(){
		...
		formObject.setNGValue("cmplx_EmploymentDetails_JobConfirmed", formObject.getNGValue("cmplx_Customer_Confirmed_in_Job")=="true"?"Yes":"No");
		...
		{
			formObject.setNGValue("cmplx_EmploymentDetails_Emp_Type","S");
		}
		else if("Pensioner".equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,6)))
		{
			formObject.setNGValue("cmplx_EmploymentDetails_Emp_Type","PENSIONER");
		}
		
		/*Code changes by saurabh for hiding lob for PL*/
		RLOS.mLogger.info( "bandana:"+formObject.getNGValue("cmplx_Product_cmplx_ProductGrid", 0, 1));
		...
		/*Code changes by saurabh ends*/
		lobCheck(formObject);//PCASP-1823
	}
	public void loadPicklist4()    
	{
		...
		loadPicklist_TargetSegmentCode();
		//Added by Shivang for loading Marketing code for Telesales Queue
		LoadPickList("cmplx_EmploymentDetails_marketingcode","select '--Select--' as description,'' as code union select convert(varchar, Description),code from NG_MASTER_MarketingCode with (nolock) where isActive='Y' and subproduct = 'IM' order by code");	
	...
	public void Eligibility_Hide()
	{
		...
		formObject.setVisible("cmplx_EligibilityAndProductInfo_InterestRate",false); 
		formObject.setVisible("cmplx_EligibilityAndProductInfo_EMI",false);
		formObject.setVisible("cmplx_EligibilityAndProductInfo_promo_interest_rate",false);
		formObject.setVisible("ELigibiltyAndProductInfo_Label28",false);
		formObject.setVisible("cmplx_EligibilityAndProductInfo_Tenor",false);
		formObject.setVisible("cmplx_EligibilityAndProductInfo_NetPayout",false);
		formObject.setVisible("cmplx_EligibilityAndProductInfo_RepayFreq",false);
	...
	public String maskNumber(String Acc, String mask)
	{ //hritik
		RLOS.mLogger.info("Inside maskNum");
		int index=0;
		StringBuilder masked = new StringBuilder();
		RLOS.mLogger.info( "Inside maskNum");
		for(int i=0; i<mask.length(); i++){
			char c= mask.charAt(i);
			RLOS.mLogger.info( "Inside loop : "+c);
			if(c == '#'){
				masked.append(Acc.charAt(index));
				index++;
			}
			else if (c == 'x'){
				masked.append(c);
	...
	public void fetchIncomingDocRepeaterNew(){
		...
		try {
			...
			//	query="SELECT distinct doctype,mandatory,Designation FROM ng_rlos_DocTable WHERE ((ProductName = '"+requested_product+"' and SubProductName = '"+requested_subproduct+"') or ProductName='All') and Active = 'Y' and (Designation='"+targetsegCode+"' or Designation='"+empTypeDoc+"' or Designation='All') and (Nationality='"+nationality+"' or Nationality='All') and (minor_flag='"+minor_flag+"' or minor_flag='N')  ORDER BY Mandatory desc,DocType";
				...
				RLOS.mLogger.info("query while we fetch documents"+query);
	...
	public void parse_cif_eligibility(String output,String operation_name)
	{
		try
		{
			...
			if("Supplementary_Card_Details".equalsIgnoreCase(operation_name)){
				...
				formObject.setNGValue("cif",Guarantor.get("CustId")==null?"":Guarantor.get("CustId"));
	...
	public void save_cif_data(Map<Integer, HashMap<String, String>> cusDetails ,int prim_cif){
		try{
			...
			while(itr.hasNext()){
				...
				RLOS.mLogger.info( "inside save_cif_data methos Cif_data: "+Cif_data );
	...
	// Deepak Change to calculate EMI start
		...
		{       
			...
			try{
				RLOS.mLogger.info( "inside getEMI");
	...
	{      
		String EMI_val="";
		try{
			RLOS.mLogger.info( "inside getEMI_new");
			...
			RLOS.mLogger.info( "mqIP: "+ mqIP+ " mqPort_str: "+ mqPort_str+" mqPort: " + mqPort);
			
			
			RLOS.mLogger.info( "before makeSocketCall_EMI");
			EMI_val = makeSocketCall_EMI(loanAmount,rate,(int)Math.round(tenureMonths),Disb_date,First_emi_date,mqIP,mqPort);
			RLOS.mLogger.info( "after makeSocketCall_EMI EMI_val: "+EMI_val);
	...
	public String makeSocketCall_EMI(double loanAmount,double rate,int months,String disbursement_Date,String firstEMI_Date, String mqIP, int mqPort )
	{
			...
			RLOS.mLogger.info( "inside makeSocketCall_EMI socketParams: "+socketParams);
				...
				if("Y".equalsIgnoreCase(Staff_cif)){
					...
					try{
						...
						if(mobileNo!=null && !"".equalsIgnoreCase(mobileNo)){
							formObject.setEnabled("OTP_Mobile_NO", true);
	...
	public void loadDatainLiengrid(){
		try{
			FormReference formObject = FormContext.getCurrentInstance().getFormReference();
			String query = "select AcctId,LienAmount,isnull(LienReasonCode,''),Wi_Name from ng_rlos_FinancialSummary_LienDetails with (nolock) where Wi_Name = '"+formObject.getWFWorkitemName()+"'";
	...
	public void getDataFromALOC(FormReference formObject2, String corpName) {
		try{
			...
			String query = "select INCLUDED_IN_PL_ALOC,INCLUDED_IN_CC_ALOC,INDUSTRY_SECTOR,INDUSTRY_MACRO,INDUSTRY_MICRO,NAME_OF_FREEZONE_AUTHORITY,EMPLOYER_CATEGORY_PL,COMPANY_STATUS_CC,COMPANY_STATUS_PL,CONSTITUTION,NAME_OF_AUTHORIZED_PERSON_FOR_ISSUING_SC_STL_PAYSLIP from NG_RLOS_ALOC_OFFLINE_DATA with(nolock) where EMPLOYER_CODE = '"+EmpCode+"'";
			...
			if(result!=null && !result.isEmpty())  //if(result!=null && result.size()>0)
			{
				...
				formObject2.setNGValue("cmplx_EmploymentDetails_Auth_Desig", result.get(0).get(10));
			...
			if(!"".equals(subproduct) && !"".equals(reqProd)){
				if(NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_PersonalLoan").equalsIgnoreCase(reqProd)){
					...
					if(appCategory!=null &&  "BAU".equalsIgnoreCase(appCategory)){
						...
						RLOS.mLogger.info("sagarika_TargetSegmentCode()1");
					...
					if("EKTMC-EXPAT".equalsIgnoreCase(formObject.getNGValue("CardProduct_Primary"))||"EKTMC-UAE".equalsIgnoreCase(formObject.getNGValue("CardProduct_Primary"))||"EKWEC-EXPAT".equalsIgnoreCase(formObject.getNGValue("CardProduct_Primary"))||"EKWEC-UAE".equalsIgnoreCase(formObject.getNGValue("CardProduct_Primary"))||"EKWEI-EXPAT".equalsIgnoreCase(formObject.getNGValue("CardProduct_Primary")))
							{
						RLOS.mLogger.info("sagarika_TargetSegmentCode()");
	...
	public void checkforCurrentAccounts()
	{
		FormReference formObject = FormContext.getCurrentInstance().getFormReference();
		try{
			String query = "select distinct acctid from ng_RLOS_CUSTEXPOSE_AcctDetails with (nolock) where Wi_Name = '"+formObject.getWFWorkitemName()+"' and AcctType not like '%CHARGE RECEIVABLE%'  and AcctStat='Active'";//query modified by akshay on 5/7/18 for proc 11633
	...
	public void openDemographicTabs()
	{
		...
		if(!formObject.isVisible("CustomerConsent_Frame1")){
			formObject.fetchFragment("Customer_Consents_container", "CustomerConsent", "q_customerConsents");
			formObject.setNGFrameState("Customer_Consents_container", 0);
			//RLOS.mLogger.info("fetched reference details");
		}
		if(formObject.isVisible("SupplementCardDetails_Frame1")){
			adjustFrameTops("Address_Details_container,Alt_Contact_container,CardDetails_container,Supplementary_Container,FATCA_container,KYC_container,OECD_container,ReferenceDetails_container,Customer_Consents_container");
		}
		else{
			adjustFrameTops("Address_Details_container,Alt_Contact_container,CardDetails_container,FATCA_container,KYC_container,OECD_container,ReferenceDetails_container,Customer_Consents_container");
		}
	}
	...
	public void setDataInMultipleAppGrid(){
		...
		try{
			...
			if(formObject.isVisible("GuarantorDetails_Frame1")==false && NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_PersonalLoan").equalsIgnoreCase(formObject.getNGValue("PrimaryProduct")) && !"".equals(formObject.getNGValue("cmplx_Customer_age"))  && isCustomerMinor(formObject))
			{
	...
	public void fetch_cardProductMasters(FormReference formObject)
	{
		...
		if(NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_Salariedpensioner").equalsIgnoreCase(empType) || NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_Salaried").equalsIgnoreCase(empType) || "Pensioner".equalsIgnoreCase(empType)){
			...
			query1="select '--Select--' as Description,'' as Code union select convert(varchar, Description),code from ng_master_cardProduct with (nolock) where Reqproduct='"+TypeOfProd+"' and SUBproduct = '"+subprod+"'  and VIPFlag='"+VIPFlag+"' and Nationality='AE' and EmployerCategory='Self Employed' and isActive='Y' order by code";	
			query2="select '--Select--' as Description,'' as Code union select convert(varchar, Description),code from ng_master_cardProduct with (nolock) where Reqproduct='"+TypeOfProd+"' and SUBproduct = '"+subprod+"'  and VIPFlag='"+VIPFlag+"' and Nationality ='ALL' and EmployerCategory='Self Employed' and isActive='Y'  order by code"; //Arun (05/12/17) modified to load the corret masters if nationality is null
	...
	public void Check_EFC_Limit()
	{
		...
		try
		{
			...
			if(!result.isEmpty())
			{
				if(Double.parseDouble(result.get(0).get(0))>FinalLimit && !"SEC".equals(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,2)))
				{
	...
	public void Update_Office_Address()
	{
		...
		{
			if(formObject.getLVWRowCount("cmplx_CompanyDetails_cmplx_CompanyGrid")>1)
			{
				Employer_Name=formObject.getNGValue("cmplx_CompanyDetails_cmplx_CompanyGrid",1,4);
			}
		}
		else
		{
			Employer_Name=formObject.getNGValue("cmplx_EmploymentDetails_EmpName")==null?"":formObject.getNGValue("cmplx_EmploymentDetails_EmpName");
		}
			...
			if("false".equalsIgnoreCase(ntb_cust)&& !"".equalsIgnoreCase(Cif)){
				...
				if(!result.isEmpty()){
					...
					if(!result_Str.contains("primary")){
						...
						RLOS.mLogger.info(" inside supplementry_to_PrimaryCheck 1 :"+formObject.getNGValue("cmplx_Customer_CIFNO"));
						formObject.setNGValue("cmplx_Customer_CIFNO","");
						RLOS.mLogger.info(" inside supplementry_to_PrimaryCheck 2 :"+formObject.getNGValue("cmplx_Customer_CIFNO"));
	...
	//Method added by Deepak for product dedupe check
			        ...
			        if((NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_TOPN").equalsIgnoreCase(AppType))||(NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_TOPE").equalsIgnoreCase(AppType))||
			          ...
			          //  RLOS.mLogger.info("RLOS Comman: Product dedupe will not run as SubProd: "+SubProd+" AppType: "+AppType);
			           ...
			           // RLOS.mLogger.info("objOutput args is: " + (String) objOutput.get(0));
			            ...
			            if (formObject.getNGValue("dedupecheck").equalsIgnoreCase("Y") && !"IM".equalsIgnoreCase(SubProd)){
		...
		//below code added by nikhil for Bscore Changes
		public void Customer_Bscore(String CIF,FormReference formObject)
		{
			List<List<String>> Result=null;
			try
			{
				String Query = "select top 1 B_Score from ng_master_cif_bscore with (nolock) where CIF_No='"+CIF+"'";
				Result = formObject.getDataFromDataSource(Query);
				if(!Result.isEmpty())
				{
					formObject.setNGValue("cmplx_Customer_BScore", Result.get(0).get(0));
				}
				else
				{
					formObject.setNGValue("cmplx_Customer_BScore", "");
				}
			}
			catch (Exception ex)
			{
				formObject.setNGValue("cmplx_Customer_BScore", "");
				RLOS.mLogger.info("Exception in Customer_Bscore data"+ex.getMessage());
			}
		}
			public void Set_score_range()
		{
			FormReference formObject = FormContext.getCurrentInstance().getFormReference();
			activeCourtCasesValidation();
			RLOS.mLogger.info("fetch_Liability_frag : Activity Name::"+formObject.getWFActivityName()+" WI No. ::"+formObject.getWFWorkitemName());
			if(formObject.getWFActivityName().equalsIgnoreCase("Branch_Init") || "Paperbased_Init".equalsIgnoreCase(formObject.getWFActivityName())){
				
				String refNo = "";
				String aecbScore = "";
				String range = "";
				try{
					String query = "select ReferenceNo,AECB_Score,Range from NG_rlos_custexpose_Derived where Wi_Name = '"+formObject.getWFWorkitemName()+"' and Request_Type='ExternalExposure'";
					List<List<String>> list = formObject.getDataFromDataSource(query);
					RLOS.mLogger.info("Size of List:: "+list.size());
					if(list.size()>0){
						refNo= list.get(0).get(0);
						aecbScore =  list.get(0).get(1);
						range=  list.get(0).get(2);
					}
					//RLOS.mLogger.info("Reference No:: "+refNo+ " AECB score ::"+aecbScore+ " Range ::"+range);
				...
				{
					RLOS.mLogger.info("Exception occurred while fetching data"+ex.getMessage());
					printException(ex);
				}
			}
			...
			public void promorate()
			{ //hritik CR promorate
				try{
				
				FormReference formObject = FormContext.getCurrentInstance().getFormReference();
				RLOS.mLogger.info("Inside promo_rate : hritik");
				String category = formObject.getNGValue("cmplx_EmploymentDetails_CurrEmployer");
				String score = formObject.getNGValue("cmplx_Liability_New_AECBScore");
				int score_s= Integer.parseInt(score);
				String salary = formObject.getNGValue("cmplx_IncomeDetails_totSal").replace(",","");
				double s_1 = Double.parseDouble(salary);
				int S_2 = (int)s_1;
				RLOS.mLogger.info("Inside promorate: category: "+category);
				RLOS.mLogger.info("Score: "+score);
				RLOS.mLogger.info("salary: "+salary);
				String expat = formObject.getNGValue("cmplx_Customer_Nationality");
				String loantype = formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,4);
				String ces = formObject.getNGValue("cmplx_EmploymentDetails_CESflag");
				RLOS.mLogger.info("expat: "+expat);
				RLOS.mLogger.info("loantype: "+loantype);
				RLOS.mLogger.info("ces: "+ces);
				
				//expat on nation and category on expat
				if(expat.equalsIgnoreCase("AE")){
					expat="N";
					if(category.equalsIgnoreCase("CA") || category.equalsIgnoreCase("CAP")){
						category="A";
					}
					else if(category.equalsIgnoreCase("CB")){
						category="B";
					}
					else if(category.equalsIgnoreCase("CC")){
						category="C";
					}
					RLOS.mLogger.info("category if else:"+category);
				}
				else{
					expat="Y";
					if(category.equalsIgnoreCase("CA") || category.equalsIgnoreCase("CAP") || 
					category.equalsIgnoreCase("CB") || category.equalsIgnoreCase("CC")){
						category="Non-CNOL";		
					}
				}
				RLOS.mLogger.info("expat if else:"+expat);
				RLOS.mLogger.info("category if else:"+category);
				
				//category for cnol
				if(category.equalsIgnoreCase("CN")){
					category="CNOL";
					RLOS.mLogger.info("category if else:"+category);
				}
		
				//ces flag
				if(ces.equalsIgnoreCase("true")){
					ces="Y";
				}
				else{
					ces="N";
				}
				RLOS.mLogger.info("ces if else:"+ces);
			
				//loan type
				if(loantype.contains("NEW")){
					loantype="New";
				}
				else if(loantype.contains("TKO")){
					loantype="Takeover";
				}
				else if(loantype.contains("TOP")){
					loantype="Top-Up";
				}
				else{
					loantype="All";
				}
				RLOS.mLogger.info("loantype if else:"+loantype);
				
				if(category.equalsIgnoreCase("CNOL")){
				String Q_1="select interest_rate from NG_MASTER_promo_rate where "+S_2+">=MinSalary and "+S_2+" < maxsalary and category = '"+category+"' and expat='"+expat+"' and ces='"+ces+"' and loan_type ='All' and ("+score_s+"> minaecb and "+score_s+"<=maxaceb) ";
				List<List<String>> res=formObject.getNGDataFromDataCache(Q_1);
				RLOS.mLogger.info("Q1:  "+Q_1);
				if(!res.isEmpty())
				{
					RLOS.mLogger.info("Q1: res:"+res.get(0).get(0));
					formObject.setNGValue("cmplx_EligibilityAndProductInfo_promo_interest_rate",res.get(0).get(0));
				}
				}
				
				if(expat.equalsIgnoreCase("Y") && !(category.equalsIgnoreCase("CNOL"))){
				String Q_2="select interest_rate from NG_MASTER_promo_rate where ("+S_2+">=MinSalary and "+S_2+"<MaxSalary) and ("+score_s+"> minaecb and "+score_s+"<=maxaceb) and expat='"+expat+"' and ces='"+ces+"' and loan_type='"+loantype+"' and category ='"+category+"' union select interest_rate from NG_MASTER_promo_rate where ("+S_2+">=MinSalary and "+S_2+"<MaxSalary) and ("+score_s+"> minaecb and "+score_s+"<=maxaceb) and expat='"+expat+"' and category ='"+category+"' and ces='"+ces+"' and loan_type='All'";
				List<List<String>> res_2=formObject.getNGDataFromDataCache(Q_2);
				RLOS.mLogger.info("Q2:  "+Q_2);
				if(!res_2.isEmpty())
				{
					RLOS.mLogger.info("Q2: res:"+res_2.get(0).get(0));
					formObject.setNGValue("cmplx_EligibilityAndProductInfo_promo_interest_rate",res_2.get(0).get(0));
				}
				}
				
				if(expat.equalsIgnoreCase("N")){
				String Q_3="select interest_rate from NG_MASTER_promo_rate where ("+S_2+">=MinSalary and "+S_2+"<MaxSalary) and ("+score_s+"> minaecb and "+score_s+"<=maxaceb) and expat='"+expat+"' and category ='"+category+"' and loan_type='"+loantype+"' and  ces='"+ces+"' union select interest_rate from NG_MASTER_promo_rate where ("+S_2+">=MinSalary and "+S_2+"<MaxSalary) and ("+score_s+"> minaecb and "+score_s+"<=maxaceb) and expat='"+expat+"' and  category ='"+category+"' and loan_type='"+loantype+"' and ces='All'";
				List<List<String>> res_3=formObject.getNGDataFromDataCache(Q_3);
				RLOS.mLogger.info("Q3:  "+Q_3);
				if(!res_3.isEmpty())
				{
					RLOS.mLogger.info("Q3: res:"+res_3.get(0).get(0));
					formObject.setNGValue("cmplx_EligibilityAndProductInfo_promo_interest_rate",res_3.get(0).get(0));
				}
				}
				}
				catch(Exception e){
			...
			public int getnoactivecards()
			{
				FormReference formObject = FormContext.getCurrentInstance().getFormReference();
				int count=0;
				try
				{
					String Query = "select count(Wi_Name) from ng_RLOS_CUSTEXPOSE_CardDetails where Wi_Name ='"+formObject.getWFWorkitemName()+"' and CardStatus not in ('C','CLSB','CLSC')";
					List<List<String>> list = formObject.getDataFromDataSource(Query);
					RLOS.mLogger.info("Size of List:: "+list.size());
					if(list.size()>0){
						count=Integer.parseInt(list.get(0).get(0));
					}
					RLOS.mLogger.info("Size of List::getnoactivecards "+count);
				}
				catch(Exception ex)
				{
					RLOS.mLogger.info("Exception occurred while fetching data"+ex.getMessage());
			...
			try
			{
				String Query = "select count(Acctid)  from ng_RLOS_CUSTEXPOSE_AcctDetails where wi_name ='"+formObject.getWFWorkitemName()+"' and CifId='"+formObject.getNGValue("cmplx_Customer_CIFNO")+"'";
				List<List<String>> list = formObject.getDataFromDataSource(Query);
				RLOS.mLogger.info("Size of List:: "+list.size());
				if(list.size()>0){
					count=Integer.parseInt(list.get(0).get(0));
			...
			if(count>0)
			{
				if("Self Employed".equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid", 0, 6)))
				{
					formObject.setVisible("IncomeDetails_Label32", true);
					formObject.setVisible("cmplx_IncomeDetails_Account_self_num", true);
				}
				else
				{
					formObject.setVisible("IncomeDetails_Label31", true);
					formObject.setVisible("cmplx_IncomeDetails_Account_num_sal", true);
					
				}
				LoadPickList("cmplx_IncomeDetails_Account_num_sal", "select Acctid as Account_ID from  ng_RLOS_CUSTEXPOSE_AcctDetails where wi_name ='"+formObject.getWFWorkitemName()+"' and CifId='"+formObject.getNGValue("cmplx_Customer_CIFNO")+"' order by case when accttype='CURRENT ACCOUNT' then '1'  when accttype='FAST SAVER' then '2' else accttype end asc");
				LoadPickList("cmplx_IncomeDetails_Account_self_num", "select Acctid as Account_ID from  ng_RLOS_CUSTEXPOSE_AcctDetails where  wi_name ='"+formObject.getWFWorkitemName()+"' and CifId='"+formObject.getNGValue("cmplx_Customer_CIFNO")+"' order by case when accttype='CURRENT ACCOUNT' then '1'  when accttype='FAST SAVER' then '2' else accttype end asc");
				
			}
				
			}
			//method created for PL
			public boolean isCustomerMinor(FormReference formObject) {
					try
					{
						...
						if(Age >= 18.06 && Age<=21.06 && formObject.getNGValue("cmplx_Customer_Nationality").equalsIgnoreCase("AE")) 
			                return true;
					...
					{
						RLOS.mLogger.info("Exception in isCustomerMinor"+ex.getMessage());
						return false;
					}
				}
			public void lobCheck(FormReference formObject){
				...
				if(("Salaried".equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,6))||"SAL".equals(subProd)) && 
					...
					if((formObject.getNGValue("cmplx_EmploymentDetails_IncInCC").equalsIgnoreCase("false") && formObject.getNGValue("cmplx_EmploymentDetails_IncInPL").equalsIgnoreCase("false"))
						...
						//RLOS.mLogger.info("@Shivang ::Employment type :: "+formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,6));
			...
			public void Checkcustomersectionfields(FormReference formObject)
			{
				String Employment = formObject.getNGValue("EmploymentType");
				if("Self Employed".equalsIgnoreCase(Employment))
				{
					formObject.setVisible("Customer_Label35", false);
					formObject.setVisible("cmplx_Customer_Basic_Sal", false);
					formObject.setVisible("cmplx_Customer_Confirmed_in_Job", false);
					
				}
				else
				{
					formObject.setVisible("Customer_Label35", true);
					formObject.setVisible("cmplx_Customer_Basic_Sal", true);
					formObject.setVisible("cmplx_Customer_Confirmed_in_Job", true);
				}
			}
			public void aecbAndReferenceDetail()
			 {
				 ...
				 if(formObject.getWFActivityName().equalsIgnoreCase("Branch_Init") || "Paperbased_Init".equalsIgnoreCase(formObject.getWFActivityName())){
						...
						try{
							...
							RLOS.mLogger.info("Reference No:: "+refNo_Comp+ " AECB score ::"+aecbScore_Comp+ " Range ::"+range_Comp);
				 ...
				 if("SE".equalsIgnoreCase(subProd)||("BTC".equalsIgnoreCase(subProd)&& appType!="SMES")){
			...
			public void MinorFieldVisibility(FormReference formObject){
				formObject.setVisible("Customer_Label40",true);
				formObject.setVisible("Customer_Label42",true);
				...
				formObject.setLocked("eidExpiry",false);
				formObject.setLocked("idIssueDate",false);
				formObject.setLocked("visaExpiry",false);
				formObject.setLocked("GuarantorDetails_VisaISsueDate",false);
				formObject.setLocked("dob",false);
				formObject.setLocked("passExpiry",false);
				formObject.setLocked("GuarantorDetails_PassportIssueDate",false);
			}
			public void setGuarantor_enable(){
				
			}
			// JS ->  Java for risk mgmnt 23.3.22 
			public void Fatca_Enable(FormReference formObject)
			{
				RLOS.mLogger.info("Fatca_Enable : ");
				formObject.setEnabled("cmplx_FATCA_IdDoc", true);
				formObject.setEnabled("cmplx_FATCA_DecForInd", true);
				formObject.setEnabled("cmplx_FATCA_w8form", true);
				formObject.setEnabled("cmplx_FATCA_w9form", true);
				formObject.setEnabled("cmplx_FATCA_lossofnationality", true);
				formObject.setEnabled("cmplx_FATCA_TINNo", true);
				formObject.setEnabled("cmplx_FATCA_SignedDate", true);
				formObject.setEnabled("cmplx_FATCA_ExpiryDate", true);
				formObject.setEnabled("cmplx_FATCA_Category", true);
				formObject.setEnabled("cmplx_FATCA_ListedReason", true);
				formObject.setEnabled("cmplx_FATCA_SelectedReason", true);
				formObject.setEnabled("cmplx_FATCA_ControllingPersonUSRel", true);
			}
			
			public void SetEnableCustomer(FormReference formObject)
			{
				RLOS.mLogger.info("SetEnableCustomer : ");
				formObject.setEnabled("cmplx_Customer_VIPFlag", true);
				formObject.setEnabled("cmplx_Customer_SecNAtionApplicable", true);
			}
			
			public void SetdisableCustomer(FormReference formObject)
			{
				RLOS.mLogger.info("SetdisableCustomer : ");
				formObject.setEnabled("cmplx_Customer_VIPFlag", false);
				formObject.setEnabled("cmplx_Customer_SecNAtionApplicable", false);
			}
			
			public void Liability_Save_Check(FormReference formObject)
			{
				RLOS.mLogger.info("Liability_Save_Check : "+formObject.getNGValue("cmplx_Liability_New_AECBconsentAvail"));
				if(formObject.getNGValue("cmplx_Liability_New_AECBconsentAvail").equalsIgnoreCase("false"))
				 {
			...
			public void paper_init_customer_fields(FormReference formObject)
			{
				formObject.setNGValue("cmplx_Customer_CardNotAvailable", "true");
				...
				formObject.setLocked("Validate_OTP_Btn", false);
				formObject.setLocked("OTP_No", false);
				formObject.setLocked("FetchDetails", false);
				formObject.setNGValue("cmplx_Customer_IsGenuine", "false");					
			...
			//	setLocked("cmplx_Customer_CIFNO",false);
				...
				formObject.setLocked("cmplx_Customer_DOb",false);
				//setLocked("cmplx_Customer_Basic_Sal",false);
				formObject.setLocked("cmplx_Customer_gender",false);
				//setLocked("Designation_button1",false);
				formObject.setLocked("cmplx_Customer_Title",false);
				//setLocked("cmplx_Customer_Email_ID",false);
				//setLocked("cmplx_Customer_Designation",false);
	           	//added by nikhil 29/11 
				formObject.setEnabled("cmplx_Customer_DOb",true);
				//setEnabled("cmplx_Customer_DOb",true); cmplx_Customer_Title
				...
				formObject.setNGValue("cmplx_Customer_card_id_available","false");
				formObject.setLocked("cmplx_Customer_CIFNO",true);
				...
				formObject.setLocked("cmplx_Customer_EmirateIDExpiry",false);
				formObject.setLocked("cmplx_Customer_IdIssueDate",false);
				//setEnabled("cmplx_Customer_SecNAtionApplicable",true);
				formObject.setLocked("cmplx_Customer_VIsaExpiry",false);
				...
				if("Paperbased_Init".equalsIgnoreCase(formObject.getWFActivityName()))
				{
    	...
    	try{
			...
			if(application_type.contains("TOP")){
				String outBal = "select liab.TotalOutstandingAmt from ng_RLOS_CUSTEXPOSE_LoanDetails liab " +
						" with (nolock) where liab.wi_name = '"+formObject.getWFWorkitemName()+"' and " +
				...
				RLOS.mLogger.info("outstanding list is: "+record);
				if(record.size()>0){
					float bal=0;
					float lim=0;
 ...
 // validation  for court cases by deepanshu 
    private void activeCourtCasesValidation() {
		...
		try{
			...
			RLOS.mLogger.info("inside activeCourtCasesValidation query to extract: "+courtCasesQuery);
			List<List<String>> result=formObject.getDataFromDataSource(courtCasesQuery);
			RLOS.mLogger.info("inside activeCourtCasesValidation result to extract: "+result.toString());
			if(!result.isEmpty()){
			...
			RLOS.mLogger.info( "Exception in activeCourtCasesValidation"+printException(ex));
		}
