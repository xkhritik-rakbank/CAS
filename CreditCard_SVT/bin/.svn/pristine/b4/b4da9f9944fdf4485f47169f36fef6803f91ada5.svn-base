	...
	public void FrameExpandEvent(ComponentEvent pEvent){
		...
		try{
			...
			if ("FATCA".equalsIgnoreCase(pEvent.getSource().getName())) 
			{
				...
				formObject.setNGValue("cmplx_FATCA_TypeOFRelation","Reason/Type of Relation");
				try
				{
					throw new ValidatorException(new CustomExceptionHandler("Error Message", "Text1","EventType", hm));
				}
			...
			if ("Finacle_Core".equalsIgnoreCase(pEvent.getSource().getName())) {
				...
				try{
					...
					if(formObject.getLVWRowCount("cmplx_FinacleCore_FinaclecoreGrid")==0){
					 ...
					 query="select AcctType,'',AcctId,AcctNm,AccountOpenDate,AcctStat,'',AvailableBalance,EffectiveAvailableBalance,CreditGrade,'',child_wi, AcctSegment,AcctSubSegment from ng_RLOS_CUSTEXPOSE_AcctDetails with (nolock) where Child_Wi='"+formObject.getWFWorkitemName()+"' and AcctStat !='CLOSED'";
					...
					String checkSubquery = "(select count(*) from ng_RLOS_CUSTEXPOSE_AcctDetails with (nolock) where AcctId = AcctId and CifId=CifId and Child_Wi='"+formObject.getWFWorkitemName()+"')>1";
					...
					query="select distinct case when "+checkSubquery+" then 'Individual_CIF' else "+elseSubquery+" end ,AcctId,returntype,returnAmount,returnNumber,returnDate,retReasonCode,instrumentdate,returnType,'','','','','','' from ng_rlos_FinancialSummary_ReturnsDtls with (nolock) where Child_Wi='"+formObject.getWFWorkitemName()+"'";
...
/*				//Tanshu Aggarwal Disbursal work 14/08/2017
	...
	public void autopopulateValues(FormReference formObject) {
		...
		for(int i=0;i<adressrowcount;i++){
			...
			String pref = formObject.getNGValue("cmplx_AddressDetails_cmplx_AddressGrid",i,11);
			if("true".equalsIgnoreCase(pref)){
	...
	public void autopopulateValuesFCU(FormReference formObject) {
		...
		try
		{
			...
			{
			List<List<String>> db=formObject.getNGDataFromDataCache("select Description from NG_MASTER_emirate with (nolock) where Code='"+emirate+"'") ;
			if(db!=null && db.get(0)!=null && db.get(0).get(0)!=null){
				emirate= db.get(0).get(0); 
				CreditCard.mLogger.info("fcu @ emirate"+emirate);
			}
	...
	public void mouse_clicked(FormEvent pEvent) 
	{
		...
		*/
		...
		if("CardDetails_save".equalsIgnoreCase(pEvent.getSource().getName())){
			if("DDVT_Maker".equals(formObject.getWFActivityName())){
		...
		else if("cmplx_FATCA_cmplx_GR_FatcaDetails".equalsIgnoreCase(pEvent.getSource().getName()))
		{
			...
			CreditCard.mLogger.info("cmplx_FATCA_SelectedReason"+listedReasonList);
			formObject.addItem("cmplx_FATCA_SelectedReason", listedReasonList);
			CreditCard.mLogger.info("cmplx_FATCA_ListedReason"+selectedReasonList);
		...
		/*else if ("EMploymentDetails_Button2".equalsIgnoreCase(pEvent.getSource().getName())) {
			...
			formObject.saveFragment("EmploymentDetails");
			throw new ValidatorException(new CustomExceptionHandler("", "EMploymentDetails_Save#Employment Details Saved !","", hm));
		}
		else if("AuthorisedSignDetails_Button1".equalsIgnoreCase(pEvent.getSource().getName())){
...
hm.clear();
		...
		else if("Customer_Button1".equalsIgnoreCase(pEvent.getSource().getName()))
		{
			...
			// changed by abhishek start for populating cutomer info grid 22may2017
				...
				adjustFrameTops("Finacle_CRM_CustomerInformation,External_Blacklist,Finacle_Core,MOL,World_Check,Reject_Enquiry,Salary_Enquiry,CreditCard_Enq,Case_hist,LOS");
...
*/		else if ("CardDetails_add".equalsIgnoreCase(pEvent.getSource().getName())){
		...
		// added by abhishek on 17 august 2017//change by prabhakar drop-4 point-3
			...
			CreditCard.mLogger.info( "Inside FATCA_Button1 "+hiddenText);
		...
		//change by prabhakar drop-4 point-3
		...
		else if("DecisionHistory_Button2".equalsIgnoreCase(pEvent.getSource().getName()))
		{
			...
			if("true".equalsIgnoreCase(NTB_flag) || "true".equalsIgnoreCase(NEP_flag)||"".equalsIgnoreCase(CIF_no)){
				...
				if("Primary".equalsIgnoreCase(formObject.getNGValue("cmplx_DEC_MultipleApplicantsGrid",formObject.getSelectedIndex("cmplx_DEC_MultipleApplicantsGrid"), 0))){
			...
			else if("Supplement".equalsIgnoreCase(formObject.getNGValue("cmplx_DEC_MultipleApplicantsGrid",formObject.getSelectedIndex("cmplx_DEC_MultipleApplicantsGrid"), 0))){
...
/*			if("Self Employed".equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,6)) && formObject.getNGValue("cmplx_CompanyDetails_cmplx_CompanyGrid",0,0)!=null ){
		...
		else  if("CC_Creation_Update_Customer".equalsIgnoreCase(pEvent.getSource().getName()) || "Limit_Inc_UpdCust".equalsIgnoreCase(pEvent.getSource().getName())){
		...
		public void value_changed(FormEvent pEvent)
		{
			...
			if ("cmplx_DEC_Decision".equalsIgnoreCase(pEvent.getSource().getName())) {
				...
				if(formObject.getNGValue("cmplx_DEC_Decision").equalsIgnoreCase("REFER"))
		        {
					...
					LoadPickList("DecisionHistory_ReferTo", " select referTo from ng_MASTER_Decision with (nolock) where ProcessName='CreditCard' and WorkstepName='"+formObject.getWFActivityName()+"' and decision='"+formObject.getNGValue("cmplx_DEC_Decision")+"'");
		        }
