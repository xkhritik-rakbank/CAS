...
public class CC_Integration_Input extends Common implements java.io.Serializable{
		...
		try{
			...
			for (int i = 0; i<add_row_count;i++){
				...
				if (call_name.equalsIgnoreCase("NEW_CUSTOMER_REQ")){
					...
					if(Applicant_Name.split("-")[1].equalsIgnoreCase(formObject.getNGValue("cmplx_DEC_MultipleApplicantsGrid",formObject.getSelectedIndex("cmplx_DEC_MultipleApplicantsGrid"),1)))
					{
				...
				*/}
				else if(Applicant_Name.split("-")[1].equalsIgnoreCase(formObject.getNGValue("CC_Creation_CustomerName")))
				{
	...
	private Map<String, String> NEW_CUSTOMER_Custom(List<List<String>> DB_List,FormReference formObject,String callName) {
		...
		try{
			for (List<String> mylist : DB_List) {
				...
				if( "SUPPLEMENT".equalsIgnoreCase(common.MultipleAppGridSelectedRow("MultipleApp_AppType"))){	
					if("PersonDetails".equalsIgnoreCase(tag_name)){
						...
						String xml_str = int_xml.get(parent_tag);
						String ph_no="";
						 if( "SUPPLEMENT".equalsIgnoreCase(common.MultipleAppGridSelectedRow("MultipleApp_AppType")) && formObject.getLVWRowCount("SupplementCardDetails_cmplx_supplementGrid")>0){ 
							for(int i=0;i<formObject.getLVWRowCount("SupplementCardDetails_cmplx_supplementGrid");i++){
								if(common.MultipleAppGridSelectedRow("MultipleApp_AppPass").equals(formObject.getNGValue("SupplementCardDetails_cmplx_supplementGrid",i,3))){
			...
			for (List<String> mylist : DB_List) {
				...
				if("ReferenceDetails".equalsIgnoreCase(tag_name)){
					...
					if(int_xml.containsKey(parent_tag))
					{
						...
						List<List<String>> record = formObject.getNGDataFromDataCache("select top 1 MAST_PROF_PRIM from ng_master_cardProduct where CODE ='"+formObject.getNGValue("CC_Creation_Product")+"'");
						...
						int_xml.put(parent_tag, xml_str);
							...
							CreditCard.mLogger.info("Exception in aqdding profileserno "+ new CC_Common().printException(ex));
						}
					}
	...
	public String getcontact_details(){
		...
		try{
			FormReference formObject = FormContext.getCurrentInstance().getFormReference();
			if("PRIMARY".equals(formObject.getNGValue("cmplx_CCCreation_cmplx_CCCreationGrid",formObject.getSelectedIndex("cmplx_CCCreation_cmplx_CCCreationGrid"),12)))
			{
