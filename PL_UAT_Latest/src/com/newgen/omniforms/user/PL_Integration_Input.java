...
public class PL_Integration_Input implements java.io.Serializable{
	...
	private String fin_call_name = "Customer_details, Customer_eligibility,NEW_CUSTOMER_REQ,new_account_req,DEDUP_SUMMARY,CUSTOMER_UPDATE_REQ,NEW_CREDITCARD_REQ,CARD_NOTIFICATION";
	private String capitalExceptionsTags = "EmploymentStatus,EmploymentType";
  ...
  /*commented by sagarika due to dupliacte condition
	...
	public void Integration_fragName(String callName)
	{
		try{
			...
			if(("CUSTOMER_DETAILS").equalsIgnoreCase(callName))
			{
				...
				call_framename="Loan_Disbursal,DecisionHistoryContainer";
			}
				...
				call_framename="CC_Creation,DecisionHistoryContainer";
			}
	...
	public String getCustAddress_details(String call_name){
		...
		try{
			...
			for (int i = 0; i<add_row_count;i++){
				...
				if("NEW_CUSTOMER_REQ".equalsIgnoreCase(call_name) && Applicant_Name.split("-")[1].equalsIgnoreCase(plcommonObj.MultipleAppGridSelectedRow("MultipleApp_AppName")))
				{	
					...
					if("Primary".equalsIgnoreCase(cust_type)){
						if(Applicant_Name.split("-")[0].trim().replaceAll(" ","").equalsIgnoreCase("P")&& Applicant_Name.split("-")[1].trim().replaceAll(" ","").equals(formObject.getNGValue("cmplx_DEC_MultipleApplicantsGrid",formObject.getSelectedIndex("cmplx_DEC_MultipleApplicantsGrid"),1).trim().replaceAll(" ","")))
						{
						...
						if(Applicant_Name.split("-")[0].trim().replaceAll(" ","").equalsIgnoreCase("S") && Applicant_Name.split("-")[1].trim().replaceAll(" ","").equals(formObject.getNGValue("cmplx_DEC_MultipleApplicantsGrid",formObject.getSelectedIndex("cmplx_DEC_MultipleApplicantsGrid"),1).trim().replaceAll(" ","")) &&
								Applicant_Name.split("-")[2].trim().replaceAll(" ","").equals(formObject.getNGValue("cmplx_DEC_MultipleApplicantsGrid",formObject.getSelectedIndex("cmplx_DEC_MultipleApplicantsGrid"),2).trim().replaceAll(" ","")))
						{
							add_xml_str = add_xml_str + "<AddressDetails><AddressType>"+Address_type+"</AddressType><AddrPrefFlag>"+preferrd+"</AddrPrefFlag><PrefFormat>STRUCTURED_FORMAT</PrefFormat>";
							add_xml_str = add_xml_str + "<AddrLine1>"+flat_Villa+"</AddrLine1>";
							add_xml_str = add_xml_str + "<AddrLine2>"+Building_name+"</AddrLine2>";
					...
					if(Applicant_Name.split("-")[1].equalsIgnoreCase(plcommonObj.MultipleAppGridSelectedRow("MultipleApp_AppName")) || Applicant_Name.split("-")[1].equals(formObject.getNGValue("cmplx_CCCreation_cmplx_CCCreationGrid",formObject.getSelectedIndex("cmplx_CCCreation_cmplx_CCCreationGrid"),4)))
				...
				else if(Applicant_Name.split("-")[1].equals(formObject.getNGValue("cmplx_CCCreation_cmplx_CCCreationGrid",formObject.getSelectedIndex("cmplx_CCCreation_cmplx_CCCreationGrid"),4)))
				{
					...
					//10/08/2017 added for the respective call
					  ...
					  //Deepak changes added on 24 june for card creation from CC code
		...
		try{
			for (int i = 0; i<OutputXML.size();i++){
				...
				if (!(OutputXML.get(i).get(73) == null || OutputXML.get(i).get(73).equals(""))) {
					amt_paid_last6mnths = OutputXML.get(i).get(73).toString();
				}
				if (!(OutputXML.get(i).get(74) == null || OutputXML.get(i).get(74).equals(""))) {
					interest_rate = OutputXML.get(i).get(74).toString();
				}
				...
				if(cifId!=null && !"".equalsIgnoreCase(cifId) && !"null".equalsIgnoreCase(cifId)){
					...
					/*f ("Credit Card".equalsIgnoreCase(ReqProd)){
	...
	***********************************************************************************  */
	...
	public String InternalBureauPipelineProducts(){
		...
		PersonalLoanS.mLogger.info("RLOSCommon"+ "Internal liab tag Cration: "+ add_xml_str);
			...
			PersonalLoanS.mLogger.info("RLOSCommon"+ "Exception occurred in InternalBureauPipelineProducts()");
			PLCommon.printException(e);
		}
		return add_xml_str;
	}
	...
	public String ExternalBureauManualAddIndividualProducts(){
		...
		if (Man_liab_row_count !=0){
			for (int i = 0; i<Man_liab_row_count;i++){
				...
				add_xml_str = add_xml_str + "<cac_indicator>"+cac_Indicator+"</cac_indicator>";
				add_xml_str = add_xml_str + "<qc_emi>"+Qc_Emi+"</qc_emi>";
				add_xml_str = add_xml_str + "<qc_amount>"+Qc_amt+"</qc_amount><company_flag>N</company_flag><take_over_indicator>"+Take_over_Indicator+"</take_over_indicator><consider_for_obligation>"+consider_for_obligation+"</consider_for_obligation><duplicate_flag>N</duplicate_flag></ExternalBureauIndividualProducts>";
	...
	***********************************************************************************  */
	...
	public String ExternalBureauPipelineProducts(){
		...
		FormReference formObject = FormContext.getCurrentInstance().getFormReference();
		String sQuery = "select AgreementId,ProviderNo,LoanType,LoanDesc,CustRoleType,Datelastupdated,TotalAmt,TotalNoOfInstalments,'' as col1,NoOfDaysInPipeline,isnull(Consider_For_Obligations,'true'),case when IsDuplicate= '1' then 'Y' else 'N' end from ng_rlos_cust_extexpo_LoanDetails with (nolock) where child_wi  =  '"+formObject.getWFWorkitemName()+"' and LoanStat = 'Pipeline' union select CardEmbossNum,ProviderNo,CardType,CardTypeDesc,CustRoleType,LastUpdateDate,'' as col2,NoOfInstallments,TotalAmount,NoOfDaysInPipeLine,isnull(Consider_For_Obligations,'true'),case when IsDuplicate= '1' then 'Y' else 'N' end  from ng_rlos_cust_extexpo_CardDetails with (nolock) where child_wi  =  '"+formObject.getWFWorkitemName()+"' and cardstatus = 'Pipeline'";
	...
	***********************************************************************************  */
	...
	public String getcontact_details(){
		...
		try{
			FormReference formObject = FormContext.getCurrentInstance().getFormReference();
			//Contactdetails_xml_str=Contactdetails_xml_str+"<PhoneNumber>"+formObject.getNGValue("cmplx_Customer_MobNo")+"</PhoneNumber><PhnPrefFlag>Y</PhnPrefFlag></PhnDet></ContactDetails>";
			...
			if(!formObject.getNGValue("AlternateContactDetails_RESIDENCENO").equals(""))
				{
					Contactdetails_xml_str=Contactdetails_xml_str+"<ContactDetails><PrefModeOfContact>Phone</PrefModeOfContact><PhnDet><PhoneType>HOMEPH1</PhoneType><PhoneNumber>"+formObject.getNGValue("AlternateContactDetails_RESIDENCENO")+"</PhoneNumber><PhnPrefFlag>N</PhnPrefFlag></PhnDet></ContactDetails>";
				}
				...
				if(!formObject.getNGValue("AlternateContactDetails_HOMECOUNTRYNO").equals("")){
					Contactdetails_xml_str = Contactdetails_xml_str+"<ContactDetails><PrefModeOfContact>Phone</PrefModeOfContact><PhnDet><PhoneType>OverseasPhone</PhoneType><PhoneNumber>"+formObject.getNGValue("AlternateContactDetails_HOMECOUNTRYNO")+"</PhoneNumber><PhnPrefFlag>N</PhnPrefFlag></PhnDet></ContactDetails>";
				}
				...
				if("RejectedDetails".equalsIgnoreCase(tag_name)){
					...
					if(int_xml.containsKey(parent_tag))
					{
						...
						if (count != 0){
							...
							ref_relationship=formObject.getNGValue("cmplx_ReferenceDetails_cmplx_ReferenceGrid",0,2);
				...
				PersonalLoanS.mLogger.info("CC_Integration_Input"+" CARD SERVICE custom function parent_tag: "+parent_tag+" tag_name: "+tag_name);
				 ...
				 if(tag_name.equalsIgnoreCase("ResidencyStatus")){
					...
					if(int_xml.containsKey(parent_tag))
					{
						...
						String emptype = formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,5);
						if("Primary".equalsIgnoreCase(Operation_name)){
					...
					xml_str = "<"+tag_name+">"+Date+"</"+ tag_name+">";
	...
	public String getSuppCardnotifdata(String tag_name){
		...
		if("EmploymentType".equalsIgnoreCase(tag_name) || "Designation".equalsIgnoreCase(tag_name) || "MarketingCode".equalsIgnoreCase(tag_name) || "Occupation".equalsIgnoreCase(tag_name)){
			...
			//CreditCard.mLogger.info("RLOS COMMON"+ " before adding guarantor EmploymentType " + xml_str);
				...
				else if("Designation".equalsIgnoreCase(tag_name) || "Occupation".equalsIgnoreCase(tag_name)){
					return Occupation;
				}
		...
		try{
			for (List<String> mylist : DB_List) {
				...
				if("ReferenceDetails".equalsIgnoreCase(tag_name)){
					...
					if(int_xml.containsKey(parent_tag))
					{
						...
						List<List<String>> record = formObject.getNGDataFromDataCache("select top 1 MAST_PROF_PRIM from ng_master_cardProduct with(nolock) where CODE ='"+formObject.getNGValue("CC_Creation_Product")+"'");
			...
			else if( "SUPPLEMENT".equalsIgnoreCase(formObject.getNGValue("cmplx_CCCreation_cmplx_CCCreationGrid",formObject.getSelectedIndex("cmplx_CCCreation_cmplx_CCCreationGrid"),12)))
			{
				...
				if("CIFId".equalsIgnoreCase(tag_name) ){
					...
					PersonalLoanS.mLogger.info("inside supplement tag name IsNTB: "+tag_name);
					String xml_str = int_xml.get(parent_tag);
					String tagValue="";
					PersonalLoanS.mLogger.info("PL COMMON  before adding isNTB:  "+xml_str);
					tagValue = new PLCommon().fetchIsNTBval("Supplement");
					//tagValue=(tagValue.equalsIgnoreCase("true")?"-1":"0");
					//if(!tagValue.equals("")){
						xml_str = xml_str + "<"+tag_name+">"+tagValue+"</"+ tag_name+">";
					//}
						int_xml.put(parent_tag, xml_str);
						PersonalLoanS.mLogger.info("PL COMMON  after adding isNTB:  "+xml_str);
				}
				else if("Extensions".equalsIgnoreCase(tag_name)){
					//CreditCard.mLogger.info("inside 1st if"+" inside ReferenceDetails for Card NOTIF");
					if(int_xml.containsKey(parent_tag))
...
public String getOutWtthMessageID(String message_ID){
	...
	public String getReference_details(){
		...
		try{
			...
			for (int i = 0; i<ref_row_count;i++){
				...
				String ref_relation=formObject.getNGValue("cmplx_ReferenceDetails_cmplx_ReferenceGrid", i, 2);//2
	...
	public String getExtensions_details(boolean primary){
		...
		try{
			FormReference formObject = FormContext.getCurrentInstance().getFormReference();
			Map<String,String> desctoFormCtrl = new HashMap<String, String>();
/*			desctoFormCtrl.put("PS:Card:10002","AlternateContactDetails_CardDisp");
			desctoFormCtrl.put("PS:Card:10010","CC_Creation_CRN");
			desctoFormCtrl.put("P:Card:10044","cmplx_CC_Loan_cmplx_btc");
			desctoFormCtrl.put("P:Card:10048","cmplx_CC_Loan_VPSPAckage");
			desctoFormCtrl.put("P:Card:10049","cmplx_CC_Loan_VPSStatus");
			desctoFormCtrl.put("P:Card:10052","cmplx_CardDetails_CompanyEmbossing_name");
			desctoFormCtrl.put("PS:Account:21017","cmplx_CardDetails_cmplx_CardCRNDetails");
			desctoFormCtrl.put("PS:Account:21026","");
			desctoFormCtrl.put("PS:Account:21073","AlternateContactDetails_eStatementFlag");*/
			...
			desctoFormCtrl.put("PS:Account:F162","A 21017 :cmplx_CardDetails_cmplx_CardCRNDetails");
			//desctoFormCtrl.put("PS:Account:F163","A 21026 :");
			//desctoFormCtrl.put("PS:Account:F164","A 707 :Account Primary Agreement Start Date");
			//desctoFormCtrl.put("PS:Account:F165","A 705 :Account  Primary Agreement Commodity Certificate No");
			...
			for(Map.Entry<String, String> entry : desctoFormCtrl.entrySet()){
				if(entry.getKey().contains("F154")){
					...
					if(formObject.getLVWRowCount(lvName)>0){
						for(int i=0;i<formObject.getLVWRowCount(lvName);i++){
							if("BT".equalsIgnoreCase(formObject.getNGValue(lvName,i,0))){
								ext_xml_str += addExtensionValue(entry.getKey(),entry.getValue(),"Y"); 
								break;
							}
						}
						continue;
				...
				else if(entry.getKey().contains("F162")){
					if(formObject.getNGValue("CC_Creation_Product").contains("KALYAN")){
						...
						if(formObject.getLVWRowCount(lvName)>0){
							for(int i=0;i<formObject.getLVWRowCount(lvName);i++){
								if(formObject.getNGValue(lvName,i,0).equals(cardprod) && formObject.getNGValue(lvName,i,10).equals(passport)){
									ext_xml_str += addExtensionValue(entry.getKey(),entry.getValue(),formObject.getNGValue(lvName,i,8));
									break;
								}
							}
						}
					...
					if(emptype.equalsIgnoreCase("Self Employed")){
						...
						if(rowCount>0){
							for(int i=0;i<rowCount;i++){
								if("Business".equalsIgnoreCase(formObject.getNGValue("cmplx_CompanyDetails_cmplx_CompanyGrid", i,1))){
									mktCode = formObject.getNGValue("cmplx_CompanyDetails_cmplx_CompanyGrid",i,28);
									ext_xml_str += addExtensionValue(entry.getKey(),entry.getValue(),mktCode);
									break;
								}
							}
						}
					...
					//	value = ("True".equalsIgnoreCase(value) || "Yes".equalsIgnoreCase(value))?"Y":"N";
						...
						if("PS".equals(PorS) || ("P".equals(PorS) && primary)){
							...
							ext_xml_str += addExtensionValue(entry.getKey(),entry.getValue(),value);
	...
	public String addExtensionValue(String mapKey,String mapValue, String value){
		try{
			return "<Extensions><Type>"+mapKey.split(":")[1]+"</Type><Number>"+mapKey.split(":")[2]+"</Number><Value>"+mapValue.split(":")[0]+ value+"</Value></Extensions>";
		}
		catch(Exception e){
			PersonalLoanS.mLogger.info("Exception occured in addExtensionValue: "+e.getMessage());
			PLCommon.printException(e);
			return"";
		}
		...
		try{
			for (List<String> mylist : DB_List) {
				...
				if("CCIFID".equalsIgnoreCase(tag_name) || "CIFId".equalsIgnoreCase(tag_name)){
					PersonalLoanS.mLogger.info("inside 1st if inside customer update req");
					String xml_str = int_xml.get(parent_tag);
					if(formObject.isVisible("cmplx_CCCreation_cmplx_CCCreationGrid") && "SUPPLEMENT".equalsIgnoreCase(formObject.getNGValue("cmplx_CCCreation_cmplx_CCCreationGrid",formObject.getSelectedIndex("cmplx_CCCreation_cmplx_CCCreationGrid"),12)))
					{
						xml_str =xml_str+ "<"+tag_name+">"+formObject.getNGValue("cmplx_CCCreation_cmplx_CCCreationGrid",formObject.getSelectedIndex("cmplx_CCCreation_cmplx_CCCreationGrid"),3)+"</"+ tag_name+">";
					}
					else if("GUARANTOR".equalsIgnoreCase(plcommonObj.MultipleAppGridSelectedRow("MultipleApp_AppType")) || "Primary".equalsIgnoreCase(plcommonObj.MultipleAppGridSelectedRow("MultipleApp_AppType")))
					{	
						xml_str =xml_str+ "<"+tag_name+">"+plcommonObj.MultipleAppGridSelectedRow("MultipleApp_AppCIF")+"</"+ tag_name+">";
					}
