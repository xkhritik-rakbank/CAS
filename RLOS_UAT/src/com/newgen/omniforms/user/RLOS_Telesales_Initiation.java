			...
			try
			{
				...
				case VALUE_CHANGED:
					...
					if (pEvent.getSource().getName().equalsIgnoreCase("SubProd")){
						...
						if(req_prod.equals("Personal Loan")){
							LoadPickList("AppType", " select '--Select--' as description,'' as code union select convert(varchar, description),code from ng_master_ApplicationType with (nolock) where code='TOPN' or code='TOPE' order by code");
						}
						else{
