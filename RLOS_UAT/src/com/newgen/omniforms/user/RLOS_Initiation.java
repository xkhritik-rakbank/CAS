...
public class RLOS_Initiation extends RLOSCommon implements FormListener
{
	...
	public void formPopulated(FormEvent pEvent) {
		...
		try{
			...
			formObject.setNGValue("Intro_Date",formObject.getNGValue("Created_Date"));// changed fromm createddate to created_date.
			...
			if(init_Channel!=null && init_Channel.equalsIgnoreCase(""))
			...
			if(formObject.getNGValue("empType").contains("Salaried")){
				...
				formObject.setNGValue("lbl_Comp_Name_val",formObject.getNGValue("COmpany_Name"));//modified by akshay on 14/10/17
		...
		try
		{
			...
			case FRAME_EXPANDED:
				...
				if(framestate0 == 0){
					...
					int framestate8=formObject.getNGFrameState("IncomingDocuments");
					if(framestate8==0){
						SKLogger.writeLog("RLOS Initation","Incoming documents already fetched.");
					}
					else{
                ...
                {                           
                    ...
                    SKLogger.writeLog("RLOSCommon", "Inside Liability_container:"); 
					...
					if(framestatecomp == 1){
						...
						formObject.setTop("AuthorisedSignatoryDetails", formObject.getTop("CompanyDetails")+formObject.getHeight("CompanyDetails")+25);
						 ...
						 if(formObject.getNGValue("Subproduct_productGrid").equalsIgnoreCase("Se")/* && formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",i,9).equalsIgnoreCase("Primary")*/){
								formObject.setVisible("CompanyDetails_Label27", true);
								formObject.setVisible("EOW", true);//Arun (22/10)
								formObject.setVisible("CompanyDetails_Label29", true);
								formObject.setVisible("headOffice", true); //Arun (22/10)
                   ...
                   SKLogger.writeLog("RLOSCommon", "OUTSIDE Liability_container:"); 
			...
			case MOUSE_CLICKED:
...

				{
					...
					Eligibilityfields();
					formObject.setNGFocus("Add");//set focus to the same button which was clicked
				}	
					...
					formObject.setNGFrameState("CompanyDetails", 1);
					formObject.setNGFrameState("CardDetails", 1);
					formObject.setNGFocus("Delete");//set focus to the same button which was clicked
				}
					...
					SKLogger.writeLog("RLOS", "Inside add button: "+formObject.getNGValue("IncomeDetails_wi_name"));
...

				{
					...
					formObject.setNGValue("OECD_noTinReason","--Select--"); //Arun (23/10)
...
*/
				...
				else if(pEvent.getSource().getName().equalsIgnoreCase("Customer_Button1"))
				{
					...
					if(ReturnCode.equalsIgnoreCase("0000")){
						...
						alert_msg = "Customer Eligibility failed.";
						formObject.setNGValue("Is_Customer_Eligibility","N");
						formObject.RaiseEvent("WFSave");
					}
					SKLogger.writeLog("RLOS value of Customer Details",formObject.getNGValue("Is_Customer_Eligibility"));
				...
				//Fetch Details call in Supplementary details tab by saurabh on 11th September.
					...
					SKLogger.writeLog("RLOS_Initiation ", "EID value for Entity Details for Supplementary Cards: "+EmiratesID);
					...
					if( EmiratesID!=null && !EmiratesID.equalsIgnoreCase("")){
						...
						ReturnCode =  (outputResponse.contains("<ReturnCode>")) ? outputResponse.substring(outputResponse.indexOf("<ReturnCode>")+"</ReturnCode>".length()-1,outputResponse.indexOf("</ReturnCode>")):"";
						SKLogger.writeLog("RLOS value of ReturnCode for Supplementary Cards: ",ReturnCode);
						if(ReturnCode.equalsIgnoreCase("0000")){
						...
						SKLogger.writeLog("RLOS value of Primary Cif",primaryCif);
					}
					if(!isEntityDetailsSuccess || (primaryCif==null || primaryCif.equalsIgnoreCase(""))){
						...
						SKLogger.writeLog("RLOS value of ReturnDesc for Supplementary Cards: ","Customer Eligibility");
						ReturnCode =  (outputResponse.contains("<ReturnCode>")) ? outputResponse.substring(outputResponse.indexOf("<ReturnCode>")+"</ReturnCode>".length()-1,outputResponse.indexOf("</ReturnCode>")):"";
						SKLogger.writeLog("RLOS value of ReturnCode for Supplementary Cards: ",ReturnCode);
						if(ReturnCode.equalsIgnoreCase("0000") )
						{
				...
				else   if(pEvent.getSource().getName().equalsIgnoreCase("AuthorisedSignDetails_FetchDetails")){
...
*/					if(ReturnCode.equalsIgnoreCase("0000") ){
						...
						alert_msg="Company CIF fetch is successful";
						...
						//	String Date1=BusinessIncDate;
							...
							if(estbDate!=null&&!estbDate.equalsIgnoreCase("")){
								...
								SKLogger.writeLog("RLOS initiation new estbDate ",formObject.getNGValue("estbDate"));
							}
							if(TLExpiry!=null&&!TLExpiry.equalsIgnoreCase("")){
								formObject.setNGValue("TLExpiry",sdf2.format(sdf1.parse(TLExpiry)),false);
								SKLogger.writeLog("RLOS initiation new TLExpiry ",formObject.getNGValue("TLExpiry"));
							}
						}
						catch(Exception ex){
						...
						alert_msg="Company CIF fetch is failed";
					...
					if(NTB_flag.equalsIgnoreCase("true") || NEP_flag.equalsIgnoreCase("true")||CIF_no.equalsIgnoreCase("")){
						...
						if(ReturnCode.equalsIgnoreCase("0000")){
							...
							if(ReturnCode.equalsIgnoreCase("0000") ){
								...
								alert_msg="Create customer and Create account is successful.";
							}
							else{
					...
					else
					{
						...
						if((formObject.getNGValue("cmplx_Customer_NTB").equals("false") && countCurrentAccount==0)){
							...
							if(ReturnCode.equalsIgnoreCase("0000") ){
								valueSetCustomer(outputResponse , "");    
								formObject.setNGValue("Is_Account_Create","Y");
								alert_msg="Account Creation is successful!!";
							}
							else{
								formObject.setNGValue("Is_Account_Create","N");
							}
...
*/
				...
				{		popupFlag="Y";
						...
						if (ReqProd.equalsIgnoreCase("Credit Card")){
							outputResponse = GenerateXML("DECTECH","CreditCard");
							SKLogger.writeLog("$$After Generatexml for dectech call..outputResponse is : "+outputResponse,"");
							...
							outputResponse = GenerateXML("DECTECH","PersonalLoan");
							SKLogger.writeLog("$$After Generatexml for dectech call..outputResponse is : "+outputResponse,"");
							SKLogger.writeLog("$$After Generatexml for dectech call..outputResponse ASDASDASDASD : "+outputResponse,"");
						...
						try{
							...
							SKLogger.writeLog("Exception Occured in RLOS Iniitation : "," Exception in EMI Generation");
						}
				 ...
				 //Added for Dectech Call on Self Employed Case
					...
					{		popupFlag="Y";
							...
							//Change Done to fetch Income Details
							String ReqProd=formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,1);
							SKLogger.writeLog("$$After ReqProd for dectech call..outputResponse is : "+ReqProd,"");
							if ((formObject.getNGValue("Winame") != null) || formObject.getNGValue("Winame").equalsIgnoreCase("")){
								formObject.setNGValue("Winame", formObject.getWFWorkitemName());
							}
							if (ReqProd.equalsIgnoreCase("Credit Card")){
								...
								SystemErrorCode =  (outputResponse.contains("<SystemErrorCode>")) ? outputResponse.substring(outputResponse.indexOf("<SystemErrorCode>")+"</SystemErrorCode>".length()-1,outputResponse.indexOf("</SystemErrorCode>")):"Error";
								SKLogger.writeLog("RLOS value of ReturnCode",SystemErrorCode);
								SKLogger.writeLog("RLOS value of outputreas",ReturnCode);
								if(SystemErrorCode.equalsIgnoreCase("")){
									valueSetCustomer(outputResponse , "");   
									alert_msg="Decision engine integration successful";
									SKLogger.writeLog("after value set customer for dectech call","");
				...
				/*else if(pEvent.getSource().getName().equalsIgnoreCase("AuthorisedSignDetails_Save")){
					formObject.saveFragment("AuthorisedSignDetails");
					popupFlag = "Y";
					alert_msg="Authorized Signatory Details Saved";
					throw new ValidatorException(new FacesMessage(alert_msg));
				}*/
				...
				else if(pEvent.getSource().getName().equalsIgnoreCase("CC_Loan_DDS_save")){
					formObject.saveFragment("CC_Loan_container");
					popupFlag = "Y";
				...
				else if(pEvent.getSource().getName().equalsIgnoreCase("CC_Loan_SI_save") ){
					formObject.saveFragment("CC_Loan_container");
					popupFlag = "Y";
				...
				else if(pEvent.getSource().getName().equalsIgnoreCase("CC_Loan_RVC_Save")){
					formObject.saveFragment("CC_Loan_container");
					popupFlag = "Y";
				...
				// ended by abhishek as per rlos FSD
				 ...
				 Notepad_grid();
					...
					SKLogger.writeLog("RLOS Initiation", " No condition reached in mouse click event");
				}
				break;	
				...
				if(formObject.getNGValue("initiationChannel").equalsIgnoreCase("M") ){
					...
					//	formObject.setNGValue("cmplx_Customer_CustomerCategory", "Normal");
						...
						try{
							...
							if (formObject.getNGValue("cmplx_Customer_NTB").equalsIgnoreCase("true")){
								setcustomer_enable();
								SKLogger.writeLog("Check for cmplx_Customer_NTB","asd");
							}
						...
						if(!formObject.getNGValue("cmplx_IncomeDetails_Accomodation").equals("Yes"))//added by akshay on 16/10/17 for point 6 in PL_NTB sheet-By default Value of accomodation should be disabled 
					...
					if((formObject.getNGValue("cmplx_Product_cmplx_ProductGrid", 0, 6).equalsIgnoreCase("Self Employed"))||(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid", 0, 6).equalsIgnoreCase("SE"))){
						...
						String customerName = formObject.getNGValue("cmplx_Customer_FIrstNAme")+" "+formObject.getNGValue("cmplx_Customer_MiddleName")+" "+formObject.getNGValue("cmplx_Customer_LAstNAme");
                            if(customerName!=null && !customerName.equalsIgnoreCase("") && !customerName.equalsIgnoreCase(" ") && formObject.getLVWRowCount("cmplx_CompanyDetails_cmplx_CompanyGrid")==0){
						...
						//loadpicklist uncommented by Saurabh on 12th Oct.
						...
						SKLogger.writeLog("RLOS","Saurabh COMPANY FRAGMENT LOADED"+formObject.getNGValue("PrimaryProduct"));
						if(formObject.getNGValue("PrimaryProduct").equalsIgnoreCase("Credit Card")){
					...
					if (pEvent.getSource().getName().equalsIgnoreCase("DecisionHistory")) {
						...
						if((formObject.getNGValue("cmplx_Customer_NTB").equals("true") || AccountCount.get(0).get(0).equals("0")) && formObject.getNGValue("cmplx_Product_cmplx_ProductGrid", 0, 1).equals("Personal Loan")){
						...
						{
							SKLogger.writeLog("RLOS", "Exception occurred in Decision History tab:"+e.getMessage()+printException(e));
						}
			...
			case VALUE_CHANGED:
				...
				if (pEvent.getSource().getName().equalsIgnoreCase("ApplicationCategory")){
					SKLogger.writeLog("RLOS val change ", "Value of dob is:"+formObject.getNGValue("ApplicationCategory"));
					String subproduct = formObject.getNGValue("cmplx_Product_cmplx_ProductGrid", 0, 2);
				...
				if (pEvent.getSource().getName().equalsIgnoreCase("TLIssueDate")){
					//SKLogger.writeLog("RLOS val change ", "Value of TLIssueDate is:"+formObject.getNGValue("TLIssueDate"));
					getAge(formObject.getNGValue("TLIssueDate"),"lob");
				}
				//code added for LOB
				if (pEvent.getSource().getName().equalsIgnoreCase("indusSector")){
					
					LoadPickList("indusMAcro", "select convert(varchar, macro) from NG_MASTER_EmpIndusMacroAndMicro with (nolock) where IndustrySector='"+formObject.getNGValue("indusSector")+"' and IsActive='Y'");
				}

				if (pEvent.getSource().getName().equalsIgnoreCase("indusMAcro")){
					LoadPickList("indusMicro", "select convert(varchar, micro) from NG_MASTER_EmpIndusMacroAndMicro with (nolock) where Macro='"+formObject.getNGValue("indusMAcro")+"' and IsActive='Y'");
				}
				...
				if (pEvent.getSource().getName().equalsIgnoreCase("cmplx_EligibilityAndProductInfo_FirstRepayDate")){
					SKLogger.writeLog("RLOS val change ", "Value of dob is:"+formObject.getNGValue("cmplx_EligibilityAndProductInfo_MaturityDate"));
					String AgeAtMAturity=getYearsDifference(formObject,"cmplx_Customer_DOb","cmplx_EligibilityAndProductInfo_MaturityDate");
					SKLogger.writeLog("RLOS val change ", "Value of dob is:"+AgeAtMAturity);
				...
				if (pEvent.getSource().getName().equalsIgnoreCase("SubProd")){
					...
					if (req_prod.equalsIgnoreCase("Credit Card")){
						if(subprod.equalsIgnoreCase("PU")||subprod.equalsIgnoreCase("PULI"))
						{
				...
				if (pEvent.getSource().getName().equalsIgnoreCase("cmplx_EmploymentDetails_EmpIndusSector")){
					...
					LoadPickList("cmplx_EmploymentDetails_Indus_Macro", "select * from (select '--Select--' as macro union select distinct (convert(varchar(100), macro)) from NG_MASTER_EmpIndusMacroAndMicro  with (nolock) where IndustrySector='"+formObject.getNGValue("cmplx_EmploymentDetails_EmpIndusSector")+"' and IsActive='Y')t order by case when t.macro = '--Select--' then 0 else 1 end"); //Arun (30/10) to load masters --Select-- at top
					//LoadPickList("cmplx_EmploymentDetails_Indus_Macro", "select '--Select--' select distinct(convert(varchar(100), macro)) from NG_MASTER_EmpIndusMacroAndMicro with (nolock) where IndustrySector='"+formObject.getNGValue("cmplx_EmploymentDetails_EmpIndusSector")+"' and IsActive='Y'");
				}
				...
				if (pEvent.getSource().getName().equalsIgnoreCase("cmplx_EmploymentDetails_Indus_Macro")){
					...
					//LoadPickList("cmplx_EmploymentDetails_Indus_Micro", "select '--Select--' select convert(varchar, micro) from NG_MASTER_EmpIndusMacroAndMicro with (nolock) where Macro='"+formObject.getNGValue("cmplx_EmploymentDetails_Indus_Macro")+"' and IndustrySector='"+formObject.getNGValue("cmplx_EmploymentDetails_EmpIndusSector")+"' and IsActive='Y'");
				}
	...
	public String makeInputForGrid(String customerName) {
		...
		{			
            ListView objListView = ( ListView )pComp;
 			...
 			Column c=(Column)(pComp.getChildren().get(0));
		SKLogger.writeLog("name of 0th column: ",c.getName());
		SKLogger.writeLog("columns: ", columns+"");
		for(int i=0;i<columns;i++){
			 c=(Column)(pComp.getChildren().get(i));
			if(c.getName().equals("Applicant Name"))
				temp+= "<SubItem>"+customerName+"</SubItem>";
			else if(c.getName().equals("Applicant Category"))
				temp+="<SubItem>Authorised signatory</SubItem>";
			else if(c.getName().equals("Applicant Type"))
				temp+="<SubItem>Primary</SubItem>";
		//Loop changed from 21 to 26 by Saurabh on 12th Oct.  
			else if(c.getName().equals("wi_name")){
		...
		{
			SKLogger.writeLog("Inside makeinputForGrid(): exception occurred", e.getMessage()+printException(e));
		}
		return temp;
	}
	public void printMapContents(Map<String,Object> abcd){
		for (Map.Entry<String, Object> entry : abcd.entrySet())
		{
			SKLogger.writeLog("attribute: ", entry.getKey() + "/" + entry.getValue()+"");
		}
	}
	...
	public void saveFormStarted(FormEvent pEvent) 
	{
		...
		formObject.setNGValue("Employer_Name", formObject.getNGValue("cmplx_EmploymentDetails_EmpName"));//added By Akshay on 16/9/17 to set header
		formObject.setNGValue("COmpany_Name", formObject.getNGValue("cmplx_CompanyDetails_cmplx_CompanyGrid",1,4));//modified By Akshay on 14/10/17 to set header
		formObject.setNGValue("Tl_No", formObject.getNGValue("cmplx_CompanyDetails_cmplx_CompanyGrid",1,5));//added By Akshay on 16/9/17 to set value i ext table column
		formObject.setNGValue("PassportNo",formObject.getNGValue("cmplx_Customer_PAssportNo"));
		formObject.setNGValue("MobileNo",formObject.getNGValue("cmplx_Customer_MobNo"));
		if(formObject.getNGValue("cmplx_Customer_MiddleName").equals(""))
