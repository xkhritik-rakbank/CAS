	...
	***********************************************************************************  */
	...
	public void fragment_loaded(ComponentEvent pEvent){
		...
		// ended by yash
			...
			//hritik - pCASI - 3801
			PersonalLoanS.mLogger.info("before DisbM Moratorium");
			Moratorium();
			PersonalLoanS.mLogger.info("After DisbM Moratorium");
		...
		// disha FSD
			...
			/*//added for CR - PCASSI - 3154 - start
			try{
			PersonalLoanS.mLogger.info("Load pick list - note Code and Broker Code - start");
			formObject.setVisible("NotepadDetails_notecode", true);
			formObject.setLocked("NotepadDetails_notecode", false);
			formObject.setEnabled("NotepadDetails_notecode", true);
			formObject.setVisible("NotepadDetails_BrokerCode", true);
			formObject.setVisible("NotepadDetails_Label4", true);
			formObject.setVisible("NotepadDetails_Label12", true);
			LoadPickList("NotepadDetails_notecode", "select '--Select--' as description,'' as Code union select convert(varchar, Description),Code from NG_MASTER_NoteCode with (nolock) order by Code");
			LoadPickList("NotepadDetails_BrokerCode", "select '--Select--' as description,'' as Code union select convert(varchar, Description),Code from NG_MASTER_BrokerCode with (nolock) order by Code");
			PersonalLoanS.mLogger.info("Load pick list - note Code and Broker Code - end");
			}
			catch(Exception e){
				PersonalLoanS.mLogger.info("Load pick list - note Code and Broker Code - exception :: "+e);
			}
			//added for CR - PCASSI - 3154 - end
*/			
			notepad_load();
