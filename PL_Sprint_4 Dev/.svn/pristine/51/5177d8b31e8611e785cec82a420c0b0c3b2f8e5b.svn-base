...
public class PL_Reject_Queue extends PLCommon implements FormListener
{
	...
	***********************************************************************************  */
	...
	public void submitFormCompleted(FormEvent arg0) throws ValidatorException {
		...
		//Bandana: Sequence of customer id and CIF exchanged in query for PCASI-3426
		String reject_query="insert into ng_cas_rejected_table(CIF,Customer_Name,Declined_Date,Remarks,Account_No,CRN,Card_Product,Passport_No,Mobile,Employer,RM_Name,Reject_System,Upload_Date,ECRN,Product,Sub_Product,Nationality,DOB,Emirates_ID,Customer_ref_No) select distinct exttable.CUSTOMERNAME,exttable.CIF_ID,decision.dateLastChanged,decision.remarks,exttable.Account_Number,crnGrid.crn,exttable.Card_Product,exttable.PassportNo,exttable.MobileNo,exttable.Employer_Name,exttable.RM_Name,'CAS',sysdatetime(),exttable.ecrn, exttable.Product,exttable.Sub_Product,customer.Nationality,customer.dob,customer.EmirateID,'"+ref_no+"' from NG_PL_EXTTABLE exttable join ng_rlos_customer customer on exttable.PL_wi_name=customer.wi_name join NG_RLOS_GR_DECISION decision  on decision.dec_wi_name=exttable.PL_wi_name and decision.workstepName = 'Reject_queue' left join ng_rlos_gr_cardDetailsCRN crnGrid on crnGrid.CRN_winame=exttable.PL_wi_name and applicantType = 'Primary' where PL_wi_name='"+formObject.getWFWorkitemName()+"' order by decision.dateLastChanged desc";
