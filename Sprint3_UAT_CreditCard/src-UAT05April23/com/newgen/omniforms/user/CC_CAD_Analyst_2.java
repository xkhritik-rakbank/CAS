...
/*			if("N".equalsIgnoreCase(formObject.getNGValue("CPV_REQUIRED"))){
           ...
           String wi_name = formObject.getWFWorkitemName();
			...
			CreditCard.mLogger.info("outside office check at form pop");
	...
	public void setExposureGridData() {
		...
		float totalOut = 0;
		float totalEMI = 0;
		//Changed by shivang for PCASP-2611,PCASP-2983
		String query =	"select LoanType as Scheme,Liability_type,AgreementId as Agreement,isnull(TotalOutstandingAmt,isnull(OutstandingAmt,0)) as OutBal,isnull(PaymentsAmt,0) as EMI,Consider_For_Obligations  from ng_RLOS_CUSTEXPOSE_LoanDetails with (nolock) where Child_Wi = '"+wi_name+"' and LoanStat not in ('Pipeline','CAS-Pipeline','C','CLOSED') and isnull(ProviderNo,'') != 'B01' union select CardType as Scheme,Liability_type,CardEmbossNum as Agreement,isnull(TotalOutstandingAmt,isnull(OutstandingAmt,0)) as OutBal,isnull(PaymentsAmount,0) as EMI,Consider_For_Obligations  from ng_RLOS_CUSTEXPOSE_CardDetails with (nolock) where Child_Wi = '"+wi_name+"' and CardStatus not in ('Pipeline','CAS-Pipeline','C') and custroleType!='Secondary'  union select LoanType as Scheme,Liability_type,AgreementId as Agreement,isnull(OutstandingAmt,'0') as OutBal,isnull(PaymentsAmt,'0') as EMI,Consider_For_Obligations from ng_rlos_cust_extexpo_LoanDetails with (nolock) where Child_Wi = '"+wi_name+"' and isnull(ProviderNo,'') != 'B01' and LoanStat not in  ('Pipeline','Closed') union select CardType as Scheme,Liability_type,CardEmbossNum as Agreement,isnull(CurrentBalance,'0') as OutBal,isnull(PaymentsAmount,0) as EMI,Consider_For_Obligations from ng_rlos_cust_extexpo_CardDetails with (nolock) where Child_Wi = '"+wi_name+"' and isnull(ProviderNo,'') != 'B01' and CardStatus not in  ('Pipeline','Closed') and custRoleType!='Secondary' union select (select Description from NG_MASTER_contract_type with (nolock) where code = TypeOfContract) as Scheme,'' as Liability_type,'' as Agreement,isnull(OutstandingAmt,0) as OutBal,isnull(cast(EMI as float),0),Consider_For_Obligations  from ng_rlos_gr_LiabilityAddition with (nolock) where LiabilityAddition_wiName = '"+wi_name+"' union select accttype as scheme,case when CifId=(select CompanyCIF from NG_RLOS_GR_CompanyDetails where comp_winame='"+wi_name+"' AND applicantCategory='Business') then 'Corporate_CIF' else 'Individual_CIF' end,acctId,isnull(outstandingbalance,0),isnull(paymentsamount,0),Consider_For_Obligations from ng_rlos_cust_extexpo_AccountDetails with (nolock) where  child_wi = '"+wi_name+"' and isnull(ProviderNo,'') != 'B01' and AcctStat not in  ('Pipeline','Closed') union select 'Overdraft' as scheme,account_type,AcctId,isnull(sanctionlimit,0),0,consider_for_obligations from ng_RLOS_CUSTEXPOSE_AcctDetails with (nolock) where Child_Wi= '"+wi_name+"' and  ODType != '' and CustRoleType='Main'";
	...
	public void submitFormCompleted(FormEvent arg0) throws ValidatorException {
		...
		try{
			...
			FormReference formObject = FormContext.getCurrentInstance().getFormReference();
			 ...
			 //Deepak changes done for PCAS-3523 ng_RLOS_CaseExpiry
		...
		CreditCard.mLogger.info("Inside CC_CAD2-->frag loaded() for "+pEvent.getSource().getName());
				...
				//added by yash for CC FSD
					...
					*/formObject.setLocked("IncomeDetails_Text2",true);
					...
					//Added by shivanshi on 23-09-2021
					CreditCard.mLogger.info( "Sub Product Value" + formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,2).equalsIgnoreCase("IM"));
					CreditCard.mLogger.info( "Product type" + formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,6).equalsIgnoreCase("Salaried"));
					if(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,2).equalsIgnoreCase("IM")){
						CreditCard.mLogger.info( "Inside if IM");
				...
				//added by yash for CC FSD
					...
					formObject.setLocked("CompanyDetails_Frame1" ,true);//PCASP-2848
		               formObject.setLocked("CompanyDetails_Frame2", true);

					formObject.setLocked("CompanyDetails_Frame3" ,true);//PCASP-2848

					formObject.setLocked("CompanyDetails_Save" ,true);//PCASP-2848
					...
					//pcasp-2856
					if(subProd.equalsIgnoreCase("SE")){
						formObject.setVisible("CompanyDetails_Label5", false);
					...
					else if(subProd.equalsIgnoreCase("BTC")){
						formObject.setVisible("CompanyDetails_Label5", false);
						formObject.setVisible("PromotionCode", false);
						formObject.setVisible("CompanyDetails_Label29",false);
						formObject.setVisible("headOffice", false);
					...
					//Added by shivanshi on 23-09-2021
					CreditCard.mLogger.info( "Sub Product Value Company Details" + formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,2).equalsIgnoreCase("IM"));
					CreditCard.mLogger.info( "Product type Company Details" + formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,6).equalsIgnoreCase("Salaried"));
					if(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,2).equalsIgnoreCase("IM")){
						CreditCard.mLogger.info( "Inside if IM Company Details");
						formObject.setLocked("cmplx_IncomeDetails_DurationOfBanking", true);
					}
					//Added by shivanshi on 23-09-2021
				...
				//added by yash for CC FSD
					...
					if("IM".equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid", 0, 2)))
					{
						...
						formObject.setLocked("cmplx_EligibilityAndProductInfo_Max_threshold_exposure", true); // added by deepanshu for Im policy
						formObject.setEnabled("cmplx_EligibilityAndProductInfo_Max_threshold_exposure",false);
					}
				}
			...
			formObject.setVisible("FinacleCore_Button10",true);
			formObject.setVisible("FinacleCore_Button11",true);
			formObject.setNGValue("cmplx_FinacleCore_avg_credit_6month", "-");
			formObject.setNGValue("cmplx_FinacleCore_total_avg_last_16", "-");
			//InwardTT_date
			//loadPicklist_Address();
			...
			for(int i=0;i<formObject.getLVWRowCount(Lien_Grid);i++)
			{
				if("true".equals(formObject.getNGValue(Lien_Grid,i,7)))
				{
					...
					if((formObject.getNGValue("Application_Type").equalsIgnoreCase("RELT")||formObject.getNGValue("Application_Type").equalsIgnoreCase("RELTN")) || ("Self Employed".equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,6)) && "true".equalsIgnoreCase(formObject.getNGValue("cmplx_CompanyDetails_cmplx_CompanyGrid",1,41))))
					{
						...
						formObject.setVisible("FinacleCore_Label35", true);
					}
					else
					{
						...
						formObject.setVisible("FinacleCore_Label35", false);
					}
				/*	formObject.setEnabled("FinacleCore_CheckBox4",false);
					formObject.setLocked("FinacleCore_CheckBox4",true);
					String finaclecore_grid="cmplx_FinacleCore_FinaclecoreGrid";
					Float fd_sum=0f;
					for(int i=0;i<formObject.getLVWRowCount(finaclecore_grid);i++)
					{
						if(NGFUserResourceMgr_CreditCard.getGlobalVar("CC_FinacleCore_AccType").contains(formObject.getNGValue(finaclecore_grid,i,0)))
						{
							//formObject.setNGValue("FinacleCore_CheckBox4","true");
							//coomented so that the value is not set to true ideally
							formObject.setEnabled("FinacleCore_CheckBox4",true);
							formObject.setLocked("FinacleCore_CheckBox4",false);

							formObject.setNGValue(finaclecore_grid,i,15,"true");
							fd_sum=fd_sum+ Float.parseFloat(formObject.getNGValue(finaclecore_grid,i,8));
						}
						else
						{
							formObject.setNGValue("FinacleCore_CheckBox4","false");
							formObject.setNGValue(finaclecore_grid,i,15,"false");
							formObject.setEnabled("FinacleCore_CheckBox4",false);
							formObject.setLocked("FinacleCore_CheckBox4",false);
						}
					...
					//formObject.setLocked("FinacleCore_Frame9", true); commented by jahnavi for new nbc view change
					
					formObject.setEnabled("FinacleCore_Frame10",true);
					formObject.setLocked("FinacleCore_Frame10",false);
					}
					else
					{
						formObject.setEnabled("FinacleCore_Frame10",false);
					...
					formObject.setLocked("FinacleCore_Button10", false);
					formObject.setEnabled("FinacleCore_Button10", true);
					formObject.setLocked("FinacleCore_acc_modify", false);
					formObject.setEnabled("FinacleCore_acc_modify", true);
