		...
		try{
			...
			for (int i = 0; i<add_row_count;i++){
				...
				if (call_name.equalsIgnoreCase("NEW_CUSTOMER_REQ")){
					...
					//Deepak code change to handle address details when Supp address is not captured -- 10Nov22
				...
				/*else if(NGFUserResourceMgr_CreditCard.getGlobalVar("Services").contains(formObject.getNGValue("Sub_Product"))
						|| Applicant_Name.split("-")[0].trim().replaceAll(" ","").equalsIgnoreCase(formObject.getNGValue("CC_Creation_CustomerName").trim().replaceAll(" ","")))
				*/	
					else if(NGFUserResourceMgr_CreditCard.getGlobalVar("Services").contains(formObject.getNGValue("Sub_Product"))||	Applicant_Name.split("-")[0].trim().replaceAll(" ","").equalsIgnoreCase("P"))
				{
					...
					if(call_name.equalsIgnoreCase("CUSTOMER_UPDATE_REQ")){
						...
						if (Address_type.equalsIgnoreCase("Mailing")|| Address_type.equalsIgnoreCase("Home") ){
							...
							if("0".equalsIgnoreCase(fetchAddressFlagVal())){
								...
								//Deepak code change to not send PO Box in Prime call as per Ritu - 10Nov22
		...
		try {
			...
			if (OutputXML.size()==0) 
			{
				...
				for (int i = 0; i < OutputXML.size(); i++) {
					...
					String enquire_date_format_CC = EnquiryDate;
					...
					try{
						...
						enquire_date_format_CC = formatter.format(parseDateCC);
						CreditCard.mLogger.info("MaxOverdueAmountDateFormat :" + enquire_date_format_CC);
					}
					catch(Exception e){
						CreditCard.mLogger.info("Exception occured in conversion of MaxOverDueAmountDate :" + e.getMessage());
					}
					...
					add_xml_str = add_xml_str + "<AECB_Enquiry_date>"+enquire_date_format_CC+"</AECB_Enquiry_date>";
			...
			public String calcResidenceSince(FormReference formObject){
				...
				try{
					for(int i=0;i<size;i++){
						...
						if("P".equalsIgnoreCase(formObject.getNGValue("cmplx_AddressDetails_cmplx_AddressGrid",i,13).split("-")[0].trim().replaceAll(" ","")))
						{
