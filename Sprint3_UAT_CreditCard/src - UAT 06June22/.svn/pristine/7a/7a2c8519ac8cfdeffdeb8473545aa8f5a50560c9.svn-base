		...
		try {
			...
			if (OutputXML.size()==0) 
			{
				...
				for (int i = 0; i < OutputXML.size(); i++) {
					...
					add_xml_str = add_xml_str + "<aecb_score>"+aecb_score+"</aecb_score>";
					add_xml_str = add_xml_str + "<range>"+range+"</range>";
					add_xml_str = add_xml_str + "<AECB_Enquiry_date>"+EnquiryDate+"</AECB_Enquiry_date>";
	...
	Description                         : Function to Generate Input XML for DB INPUT
			...
			public String getExtensions_details(boolean primary){
				...
				desctoFormCtrl.put("PS:Card:F170","C 10103:Y");
                    ...
                    if(primary){
						//Extension 10053 & 10054 are changed from A to C based on change request via Ritu.
                        desctoFormCtrl.put("PS:Account:F169","C 10053:AlternateContactDetails_MobileNo1");
                        desctoFormCtrl.put("PS:Account:F170","C 10054:AlternateContactDetails_Email1");
