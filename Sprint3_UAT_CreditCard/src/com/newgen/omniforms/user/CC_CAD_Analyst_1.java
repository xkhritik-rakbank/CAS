...
public class CC_CAD_Analyst_1 extends CC_Common implements FormListener
{
	...
	public void submitFormStarted(FormEvent arg0) throws ValidatorException 
	{
		...
		try{
			...
			if("Refer".equalsIgnoreCase(formObject.getNGValue("cmplx_DEC_Decision")) && "DDVT_Maker".equalsIgnoreCase(formObject.getNGValue("DecisionHistory_ReferTo") ))
			{
				formObject.setNGValue("q_hold1",1);
				formObject.setNGValue("VAR_INT3",1);
				CreditCard.mLogger.info("This is refer decision check for cad1");
			...
			{
				CreditCard.mLogger.info("if refer in submit form sttrt where condition of refer/reject");
			...
			CreditCard.mLogger.info("load in 1st try");
		/*	if("Refer".equalsIgnoreCase(formObject.getNGValue("cmplx_DEC_Decision")) && (!"DDVT_Maker".equalsIgnoreCase(formObject.getNGValue("DecisionHistory_ReferTo"))) ||(!"Smart_CPV".equalsIgnoreCase(formObject.getNGValue("DecisionHistory_ReferTo"))) ){
			LoadReferGrid();
			} */
		}catch(Exception ex){
			...
			try{
				...
				CreditCard.mLogger.info("load in finally try");
				saveIndecisionGrid();
				CreditCard.mLogger.info("Cmplx dec: "+formObject.getNGValue("cmplx_DEC_Decision"));
				CreditCard.mLogger.info("Cmplx refer dec: "+formObject.getNGValue("DecisionHistory_ReferTo"));
				if("Refer".equalsIgnoreCase(formObject.getNGValue("cmplx_DEC_Decision")) && !("DDVT_Maker".equalsIgnoreCase(formObject.getNGValue("DecisionHistory_ReferTo")) || "Smart CPV".equalsIgnoreCase(formObject.getNGValue("DecisionHistory_ReferTo"))))
				{
					CreditCard.mLogger.info("Inside load refer");
					LoadReferGrid();	
				}//by jahnavi for cpv changes
			}
			catch(Exception e){
				CreditCard.mLogger.info("Exception occured in submit form started method" +e.getMessage());
				CreditCard.mLogger.info("Exception occured in submit form started method"+e.getStackTrace());
			}
		}
	...
	/// Added by Rajan for PCASP-2123
		...
		CreditCard.mLogger.info("Inside if condition to setvisible agg expo: CAd::");
		CreditCard.mLogger.info("Inside if condition to setvisible agg expo:event name:"+pEvent.getSource().getName());
		...
		formObject.setLocked("cmplx_Liability_New_AggrExposure",true);
			...
			if("Self Employed".equalsIgnoreCase( formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,6)))
			{
				...
				// Pooja
				if(formObject.getNGValue("cmplx_CompanyDetails_cmplx_CompanyGrid",0,6).equalsIgnoreCase("BELT") ){
			...
			if(("N".equalsIgnoreCase(formObject.getNGValue("Corporate_Aecb")) &&  !apptype.equalsIgnoreCase("BELT"))){
			
				formObject.setLocked("Liability_New_Button1", true);
			}
			else if ("Y".equalsIgnoreCase(formObject.getNGValue("Corporate_Aecb")) || apptype.equalsIgnoreCase("BELT"))
			{
				formObject.setLocked("Liability_New_Button1", false);
				formObject.setEnabled("Liability_New_Button1", true);
			}
		...
		//added by yash for CC FSD
			...
			// changes by Pooja
			formObject.setVisible("FinacleCore_Button10",false);
			...
			formObject.setVisible("FinacleCore_Modifycfo", true);
				...
				for(int i=0;i<formObject.getLVWRowCount(finaclecore_grid);i++)
				{
					//if(NGFUserResourceMgr_CreditCard.getGlobalVar("CC_FinacleCore_AccType").contains(formObject.getNGValue(finaclecore_grid,i,0)))
					 if(formObject.getNGValue(finaclecore_grid,i,15).equalsIgnoreCase("true"))
						
					{
						//formObject.setNGValue("FinacleCore_CheckBox4","true");
						//commented by jahnavi so that initialky checkbox is not true only the value in grid is true
						CreditCard.mLogger.info("cfo value"+formObject.getNGValue(finaclecore_grid,i,15));
