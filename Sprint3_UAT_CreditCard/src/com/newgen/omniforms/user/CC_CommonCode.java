...
public class CC_CommonCode extends CC_Common {
	...
	public void FrameExpandEvent(ComponentEvent pEvent){
		...
		try{
			...
			CreditCard.mLogger.info("Inside CC_CommonCode-->FrameExpandEvent()"+pEvent.getSource().getName());
			...
			if ("IncomeDEtails".equalsIgnoreCase(pEvent.getSource().getName())) 
			{
				...
				if("DDVT_Maker".equalsIgnoreCase(formObject.getWFActivityName())){
					...
					// Added Pooja for statemnt cycle enable at ddvt
					formObject.setEnabled("cmplx_IncomeDetails_StatementCycle", true);
					formObject.setLocked("cmplx_IncomeDetails_StatementCycle", false);
			...
			if ("Internal_External_Liability".equalsIgnoreCase(pEvent.getSource().getName())) {
				...
				if(formObject.getWFActivityName().equalsIgnoreCase("CAD_Analyst1") || formObject.getWFActivityName().equalsIgnoreCase("Cad_Analyst2")){
					...
					try{
						...
						// added by deepanshu for copurt cases 
						activeCourtCasesValidation();
						//pcasp-603
				...
				if(applType.equalsIgnoreCase("BELT") || employmentType.equalsIgnoreCase("Self Employed")){
				...
				// Added By Pooja
				if("CAD_Analyst1".equalsIgnoreCase(formObject.getWFActivityName()) && "Self Employed".equalsIgnoreCase( formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,6)))
				{
			...
			//added by gaurav
			CreditCard.mLogger.info("fetched before Customer_Consents");

			if ("Customer_Consents".equalsIgnoreCase(pEvent.getSource().getName())) 
			{
				CreditCard.mLogger.info("fetchedCustomer_Consents");
				hm.put("Customer_Consents","Clicked");
				formObject.fetchFragment("Customer_Consents", "CustomerConsent", "q_customerConsents");
				CreditCard.mLogger.info("fetched after Customer_Consents");
			...
			if ("Card_Details".equalsIgnoreCase(pEvent.getSource().getName())) {
				...
				if(formObject.getLVWRowCount("cmplx_Product_cmplx_ProductGrid")>0){
					...
					if("IM".equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid", 0, 2))){
						formObject.setLocked("cmplx_CardDetails_SelfSupp_required", true);
						formObject.setEnabled("cmplx_CardDetails_SelfSupp_required", false);
						formObject.setNGValue("cmplx_CardDetails_SelfSupp_required", "No");
					}
				}
			...
			CreditCard.mLogger.info("Inside Original_validation");
			if ("Original_validation".equalsIgnoreCase(pEvent.getSource().getName())) {
				CreditCard.mLogger.info("Inside2 Original_validation");
	...
	// function for validation of court cases by deepanshu
	private void activeCourtCasesValidation() {
		// TODO Auto-generated method stub
		try{
			FormReference formObject = FormContext.getCurrentInstance().getFormReference();
			String courtCasesQuery = "select count(reference_number) as reference_number  from ng_rlos_cust_extexpo_CaseDetails with(nolock)where child_wi ='"+ formObject.getWFWorkitemName()+"'";
			int courtCases_count=0;
			CreditCard.mLogger.info("inside activeCourtCasesValidation query to extract: "+courtCasesQuery);
			List<List<String>> result=formObject.getDataFromDataSource(courtCasesQuery);
			CreditCard.mLogger.info("inside activeCourtCasesValidation result to extract: "+result.toString());
			if(!result.isEmpty()){
				courtCases_count=Integer.parseInt(result.get(0).get(0));
			}
			if(courtCases_count > 0){
				formObject.setNGValue("cmplx_Liability_New_ActiveCourtCases", "Y");
				formObject.setNGValue("cmplx_Liability_New_NoOfActiveCourtCases", courtCases_count);
			}else{
				formObject.setNGValue("cmplx_Liability_New_ActiveCourtCases", "N");
				formObject.setNGValue("cmplx_Liability_New_NoOfActiveCourtCases", courtCases_count);
			}
		}
		catch(Exception ex){
			CreditCard.mLogger.info( "Exception in activeCourtCasesValidation"+printException(ex));
		}
...
Description                         : auto populate Vlaues Buss Verification       
	...
	public void mouse_clicked(FormEvent pEvent) 
	{
		...
		try{
			CreditCard.mLogger.info("CC_CommonCode"+ "Inside mouse_clicked: " + pEvent.getSource().getName());
			...
			//Added by shivang for pCASP-2007
				...
				if(selected_row==-1)
				{
					formObject.setLocked("CardDetails_Button1", false);
				}				
			}
			//ended by akshay
			...
			//Added by shivang
				...
				String Emp_Type = formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,6);
				String appCatg = formObject.getNGValue("cmplx_CompanyDetails_cmplx_CompanyGrid",1,21);
				//Deepak Newly added condition removed for pre-approved and salaried
				/*if( !("Salaried".equalsIgnoreCase(Emp_Type) ||  "SEC".equals(Sub_Prod) || "BPA".equals(Sub_Prod) || "true".equalsIgnoreCase(formObject.getNGValue("cmplx_Customer_VIPFlag")) ||
						("PA".equalsIgnoreCase(Sub_Prod) && ("RFL".equals(App_type) || "RFLP".equals(App_type) || "MOR".equals(App_type))) ||
						("BTC".equalsIgnoreCase(Sub_Prod) && "SMES".equals(App_type)) || ("SE".equalsIgnoreCase(Sub_Prod) && "S".equalsIgnoreCase(appCatg)) ))
				{*/
				//Deepak condition changed based on the prodution issue reported by Afrin. production ticket 220303194898 
				if( !("Salaried".equalsIgnoreCase(Emp_Type) ||  "SEC".equals(Sub_Prod) || "BPA".equals(Sub_Prod) || "true".equalsIgnoreCase(formObject.getNGValue("cmplx_Customer_VIPFlag")) ||
						("PA".equalsIgnoreCase(Sub_Prod) && ("RFL".equals(App_type) || "RFLP".equals(App_type) || "MOR".equals(App_type))) ||
						("BTC".equalsIgnoreCase(Sub_Prod) && "SMES".equals(App_type)) || ("SE".equalsIgnoreCase(Sub_Prod) && "S".equalsIgnoreCase(appCatg)) ) || ("PA".equalsIgnoreCase(Sub_Prod) && "Salaried".equalsIgnoreCase(Emp_Type) && !"MOR".equalsIgnoreCase(App_type)))
				{
					formObject.setNGValue("Data_refresh_trigger", "Y");
					formObject.setEnabled("FinacleCore_Data_Refresh", true);
					formObject.setNGFocus("Finacle_Core");
			...
			else if("SupplementCardDetails_Add".equalsIgnoreCase(pEvent.getSource().getName()))
			{
				...
				formObject.setNGValue("SupplementCardDetails_Status","Active");
			...
			else if("SupplementCardDetails_Modify".equalsIgnoreCase(pEvent.getSource().getName()))
			{
				...
				if("".equalsIgnoreCase(formObject.getNGValue("SupplementCardDetails_CIF")) && !"".equalsIgnoreCase(formObject.getNGValue("SupplementCardDetails_linked_product"))){
					CreditCard.mLogger.info("Inside the condition to set linked product as blank for passport: " + formObject.getNGValue("SupplementCardDetails_passportNo"));
					formObject.setNGValue("SupplementCardDetails_linked_product","");
				}
				formObject.setNGValue("SupplementCardDetails_Status","Active");
			...
			{
				String selected_pass = formObject.getNGValue("SupplementCardDetails_passportNo");
				deleteMultipleAppData(selected_pass);
				int selectedRow = formObject.getSelectedIndex("SupplementCardDetails_cmplx_supplementGrid");
				String passport_no_supp=formObject.getNGValue("SupplementCardDetails_cmplx_supplementGrid",selectedRow,3);
				int rowcount=formObject.getLVWRowCount("cmplx_CardDetails_cmplx_CardCRNDetails");
				int del_index=0;
				for(int j=0;j<rowcount;j++)
				{	
					String pno=	formObject.getNGValue("cmplx_CardDetails_cmplx_CardCRNDetails",j,10);
					if(passport_no_supp.equalsIgnoreCase(pno) && "Supplement".equalsIgnoreCase(formObject.getNGValue("cmplx_CardDetails_cmplx_CardCRNDetails",j,9)))
					{	CreditCard.mLogger.info( "Inside delete row by selecting manually 1"+passport_no_supp+formObject.getNGValue("cmplx_CardDetails_cmplx_CardCRNDetails",j,9));

					del_index=j;
					}
				}
				formObject.setSelectedIndex("cmplx_CardDetails_cmplx_CardCRNDetails", del_index);
				CreditCard.mLogger.info( "Inside delete row by selecting manually"+del_index);
		...
		if("ExtLiability_Button4".equalsIgnoreCase(pEvent.getSource().getName()))
		{
			...
			//TODO apply condition for im deepanshu 
...
hm.clear();
				...
				if( EmiratesID!=null && !"".equalsIgnoreCase(EmiratesID)){
					...
					if("0000".equalsIgnoreCase(ReturnCode)){
						...
						String Query_suppCif = "select COUNT(cif) as CIF_count from CAPS_MAIN_MIG_DATA with(nolock) where ECRN in (select  distinct ECRN from CAPS_MAIN_MIG_DATA with(nolock) where CIF ='"+formObject.getNGValue("cmplx_Customer_CIFNo")+"') and CIF = '"+primaryCif+"' and substring(crn,8,2)!='00';";
				...
				if(!isEntityDetailsSuccess || (primaryCif==null || "".equalsIgnoreCase(primaryCif))){
					...
					if("0000".equalsIgnoreCase(ReturnCode) )
					{
						if(outputResponse.contains("<SearchType>Internal</SearchType>")){	
							primaryCif = (outputResponse.contains("<CustId>")) ? outputResponse.substring(outputResponse.indexOf("<CustId>")+"</CustId>".length()-1,outputResponse.indexOf("</CustId>")):"";
							...
							if("Y".equals(NTBFlag)){
								...
								String Query_suppCif = "select COUNT(cif) as CIF_count from CAPS_MAIN_MIG_DATA with(nolock) where ECRN in (select  distinct ECRN from CAPS_MAIN_MIG_DATA with(nolock) where CIF ='"+formObject.getNGValue("cmplx_Customer_CIFNo")+"') and CIF = '"+primaryCif+"' and substring(crn,8,2)!='00';";
								CreditCard.mLogger.info("supp card Query_suppCif: "+ Query_suppCif);
								List<List<String>> result = formObject.getNGDataFromDataCache(Query_suppCif);
								if(result.get(0).get(0)!=null && Integer.parseInt(result.get(0).get(0))>0){
									...
									CreditCard.mLogger.info("supp card cif if 2");
									alert_msg = fetch_cust_details_supplementary();
								}
								else{
									alert_msg = NGFUserResourceMgr_CreditCard.getAlert("VAL013");
									CreditCard.mLogger.info("supp card cif else 2");
									formObject.setNGValue("SupplementCardDetails_CIF","");
									CreditCard.mLogger.info("supp card cif else 2 SupplementCardDetails_CIF:"+formObject.getNGValue("SupplementCardDetails_CIF"));
									formObject.setNGValue("SupplementCardDetails_linked_product","");
									formObject.setNGValue("Supplementary_CIFNO","");
									formObject.setLocked("SupplementCardDetails_FetchDetails", true);
								}
		...
		else if("CC_Creation_Card_Services".equalsIgnoreCase(pEvent.getSource().getName()))
		{
			...
			// ended by yash
				 ...
				 //++Added by sarthak for prime4upg CARDLOADBRIDGE
				 if("Conventional".equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid", 0, 0)) ||
						 "Islamic".equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid", 0, 0))){
					 CreditCard.mLogger.info(""+"Card Creation Call triggered Inside Conventional");

					 String inputXml="";
					 StringBuilder prime4changes_xml;
					 ...
					 if (("true".equals(formObject.getNGValue("cmplx_Customer_NTB"))) && "Primary".equalsIgnoreCase(App_type)) {
						 
						 CreditCard.mLogger.info(""+"Card Creation Call triggered For primary");

						 if ("0".equalsIgnoreCase(new CC_Integration_Input().fetchIsNTBval("Primary"))){
							 CreditCard.mLogger.info(""+"Card Creation Call triggered For  existing primary");

							 inputXml = primeCustomerExisting(formObject); 
						 }else{
							 CreditCard.mLogger.info(""+"Card Creation Call triggered For NTB primary");

							 inputXml = primeCustomerNew(formObject);
						 }
						
						 
						 
						 
						 if(inputXml.indexOf("<EE_EAI_MESSAGE>")>-1)
						 {
							 inputXml=inputXml.substring(inputXml.indexOf("<EE_EAI_MESSAGE>"),inputXml.indexOf("</EE_EAI_MESSAGE>")+17);	
						 }

						 CreditCard.mLogger.info("Prime 4 XML :"+ inputXml);

						 prime4changes_xml = new StringBuilder("");
						 prime4changes_xml = prime4changes_xml.append(inputXml);

						 CreditCard.mLogger.info("Input request :"+ prime4changes_xml);

						 outputResponse = new CC_Integration_Input().MQ_connection_response(prime4changes_xml);

						 CreditCard.mLogger.info("Response: "+ outputResponse);
						 ...
						 if(NGFUserResourceMgr_CreditCard.getGlobalVar("CC_integrationsuccess").equalsIgnoreCase(ReturnCode)){
							 CreditCard.mLogger.info("Card Created");
							 alert_msg = "Card Creation Successful";
							 ...
							 CreditCard.mLogger.info("Failure in prime 4 changes");
							 alert_msg =  "Failure in prime 4 changes";
							 throw new ValidatorException(new FacesMessage(alert_msg));
						 } 
					 }
					 ...
					 if ("SUPPLEMENT".equalsIgnoreCase(App_type)){
						 ...
						 for (int i = 0; i<add_row_count;i++){
							 String AppType = formObject.getNGValue("cmplx_CCCreation_cmplx_CCCreationGrid", i, 12); //0
							 
							 if("Primary".equalsIgnoreCase(AppType)){
								 CustomerName = formObject.getNGValue("cmplx_CCCreation_cmplx_CCCreationGrid", i, 4);
								 CIF_Number = formObject.getNGValue("cmplx_CCCreation_cmplx_CCCreationGrid", i, 3);
								 break;
							 }
						 }
						 
						 String SupCustomerName = formObject.getNGValue("CC_Creation_CustomerName");
						 String SupCIF_Number = formObject.getNGValue("CC_Creation_CIF");
						 
						 CreditCard.mLogger.info("Primary customer Name -- rubi:"+ CustomerName);
						 CreditCard.mLogger.info("Primary CIF No -- rubi"+ CIF_Number);
						 
						 CreditCard.mLogger.info("Primary Supplementary customer Name -- rubi:"+ SupCustomerName);
						 CreditCard.mLogger.info("Primary Supplementary CIF No -- rubi"+ SupCIF_Number);
						 
						 if(CustomerName.equalsIgnoreCase(SupCustomerName)&& CIF_Number.equalsIgnoreCase(SupCIF_Number)){
							 inputXml = selfSupplementryCard(formObject);
						 }else{
							 inputXml = SupplementryCard(formObject);
						 }
					 ...
					 else if ("0".equalsIgnoreCase(new CC_Integration_Input().fetchIsNTBval("Primary"))) {
						 
						 //prime add for existing customer
						 inputXml = primeCustomerExisting(formObject);
				 ...
				 //Limit Increase
//				 if("Conventional".equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid", 0, 0))){
//					 //Limit increate changes by rubi 
//
//
//					 String cardInstallment_xml = limitIncrease(formObject);
//
//					 CreditCard.mLogger.info("Card Installment XML :" +cardInstallment_xml);
//
//					 StringBuilder creditCard_xml = new StringBuilder("");
//					 creditCard_xml = creditCard_xml.append(cardInstallment_xml);
//					 CreditCard.mLogger.info("Input request :"+ creditCard_xml);
//
//
//					 String response = new CC_Integration_Input().MQ_connection_response(creditCard_xml);
//
//					 CreditCard.mLogger.info("Input Response: "+ response);
//
//					 ReturnCode =  response.contains("<ReturnCode>") ? response.substring(outputResponse.indexOf("<ReturnCode>")+"</ReturnCode>".length()-1,response.indexOf("</ReturnCode>")):"";
//					 CreditCard.mLogger.info("Conventional value of ReturnCode"+ReturnCode);
//					 ReturnDesc =  (outputResponse.contains("<ReturnDesc>")) ? outputResponse.substring(outputResponse.indexOf("<ReturnDesc>")+"</ReturnDesc>".length()-1,outputResponse.indexOf("</ReturnDesc>")):"";    
//					 CreditCard.mLogger.info(ReturnDesc);
//					 CreditCard.mLogger.info("Return code"+NGFUserResourceMgr_CreditCard.getGlobalVar("CC_integrationsuccess"));
//
//					 if(NGFUserResourceMgr_CreditCard.getGlobalVar("CC_integrationsuccess").equalsIgnoreCase(ReturnCode)){
//						 CreditCard.mLogger.info("Success in Limit Increase");
//						 alert_msg = "Success in Limit Increase";
//						 throw new ValidatorException(new FacesMessage(alert_msg));
//					 } 
//					 else{
//						 CreditCard.mLogger.info("Failure in Limit Increase");
//						 alert_msg =  "Failure in Limit Increase";
//						 throw new ValidatorException(new FacesMessage(alert_msg));
//
//					 }
//				 }

				 //Card Details (Primary / supp)
//				 String Succesflag= formObject.getNGValue("cmplx_CCCreation_cmplx_CCCreationGrid",formObject.getNGListIndex("cmplx_CCCreation_cmplx_CCCreationGrid"),8);
//				 CreditCard.mLogger.info("SuccessFlag "+Succesflag);
//				 if(!("Y".equalsIgnoreCase(Succesflag) && Succesflag.equalsIgnoreCase(null))){
//
//					 if("SUPPLEMENT".equalsIgnoreCase(formObject.getNGValue("cmplx_CCCreation_cmplx_CCCreationGrid",formObject.getSelectedIndex("cmplx_CCCreation_cmplx_CCCreationGrid"),12)) && !formObject.getNGValue("cmplx_Customer_PAssportNo").equalsIgnoreCase(formObject.getNGValue("cmplx_CCCreation_cmplx_CCCreationGrid",formObject.getSelectedIndex("cmplx_CCCreation_cmplx_CCCreationGrid"),13)))
//					 {
//						 outputResponse = genX.GenerateXML("CARD_DETAILS","SUPPLEMENT");
//						 CreditCard.mLogger.info("Output response Supplement "+outputResponse);
//
//					 }
//					 else{
//						 outputResponse = genX.GenerateXML("CARD_DETAILS","PRIMARY");
//						 CreditCard.mLogger.info("Output response primary "+outputResponse);
//					 }
//
//					 ReturnCode = outputResponse.contains("<ReturnCode>") ? outputResponse.substring(outputResponse.indexOf("<ReturnCode>")+"</ReturnCode>".length()-1,outputResponse.indexOf("</ReturnCode>")):"";
//					 CreditCard.mLogger.info("Card Details value of ReturnCode"+ReturnCode);
//
//					 if (NGFUserResourceMgr_CreditCard.getGlobalVar("CC_integrationsuccess").equalsIgnoreCase(ReturnCode)) {
//
//						 CreditCard.mLogger.info("Success in Card Details");
//						 alert_msg = "Success in Card Details";
//						 throw new ValidatorException(new FacesMessage(alert_msg));
//					 }
//					 else {
//						 CreditCard.mLogger.info("Failure in Card Details");
//						 alert_msg = "Failure in Card Details";
//						 throw new ValidatorException(new FacesMessage(alert_msg));
//
//					 }
//				 }

				 //				 //Card Installment Details
				 //				 if("Conventional".equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid", 0, 0))){
				 //					 String cardInstallment_xml = CardInstallmentDet(formObject);
				 //					 
				 //					 CreditCard.mLogger.info("Card Installment XML :" +cardInstallment_xml);
				 //					 
				 //					 StringBuilder creditCard_xml = new StringBuilder("");
	...
	public void value_changed(FormEvent pEvent)
	{
		...
		try{
			...
			// code by deepanshu prashar
			// if (formObject.getWFActivityName().equalsIgnoreCase("Telesales_Agent")){
			else if (("cmplx_EligibilityAndProductInfo_InterestRate".equalsIgnoreCase(pEvent.getSource().getName())|| "cmplx_EligibilityAndProductInfo_Tenor".equalsIgnoreCase(pEvent.getSource().getName())|| "cmplx_EligibilityAndProductInfo_FinalLimit".equalsIgnoreCase(pEvent.getSource().getName())) && formObject.getWFActivityName().equalsIgnoreCase("Telesales_Agent")){
				try{
					double LoanAmount=Double.parseDouble(formObject.getNGValue("cmplx_EligibilityAndProductInfo_FinalLimit")==null||formObject.getNGValue("cmplx_EligibilityAndProductInfo_FinalLimit").equalsIgnoreCase("")?"0":formObject.getNGValue("cmplx_EligibilityAndProductInfo_FinalLimit"));
					double tenor=Double.parseDouble(formObject.getNGValue("cmplx_EligibilityAndProductInfo_Tenor")==null||formObject.getNGValue("cmplx_EligibilityAndProductInfo_Tenor").equalsIgnoreCase("")?"0":formObject.getNGValue("cmplx_EligibilityAndProductInfo_Tenor"));
					double RateofInt=Double.parseDouble(formObject.getNGValue("cmplx_EligibilityAndProductInfo_InterestRate")==null||formObject.getNGValue("cmplx_EligibilityAndProductInfo_InterestRate").equalsIgnoreCase("")?"0":formObject.getNGValue("cmplx_EligibilityAndProductInfo_InterestRate"));
	...
	public void framecollapsed(FormEvent pEvent)
	{
		...
		{
			CreditCard.mLogger.info("Inside if condition Frame Collapsed");
			adjustFrameTops("Card_Details,Supplementary_Cont,FATCA,KYC,OECD,Reference_Details,Customer_Consents_container");
		}
	...
	public void LockFragmentsOnLoad(ComponentEvent pEvent)
	{
		...
		if ("Customer".equalsIgnoreCase(pEvent.getSource().getName())) {
			...
			String Activityname=formObject.getWFActivityName();
			if(Activityname.equalsIgnoreCase("Original_Validation")&& !"IM".equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,2)) )
			{
		...
		{			      
			...
			if((formObject.getNGValue("Application_Type").equalsIgnoreCase("RELT")||formObject.getNGValue("Application_Type").equalsIgnoreCase("RELTN")) || (("Self Employed".equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,6)) && "true".equalsIgnoreCase(formObject.getNGValue("cmplx_CompanyDetails_cmplx_CompanyGrid",1,41)))))
			{
	...
	// Code added by Sarthak
	// for Card Load bridge (Limit increase)

	public String limitIncrease(FormReference formObject) {

		String limitInc = "";
		try {

			limitInc= ReadRequestXML("Card_Installment.txt");

			CreditCard.mLogger.info("Initial XML------>\n"+ limitInc);
			Map<String, String> commonMap = new LinkedHashMap<String, String>();

			String CC_Creation_CombinedLimit = formObject.getNGValue("CC_Creation_CombinedLimit");
			String CC_Creation_CRN = formObject.getNGValue("CC_Creation_CRN");
			String ref_type1 = "SERNO";

			commonMap.put("$Ref_Id$", CC_Creation_CRN);
			commonMap.put("$Ref_type$", ref_type1);
			commonMap.put("$Max_agg$", CC_Creation_CombinedLimit);

			CreditCard.mLogger.info("customer map---> " +commonMap);
			limitInc=primeXml("customer", commonMap, limitInc);
			commonMap.clear();
			CreditCard.mLogger.info("Final input Xml :"+limitInc);
			
			
			//customer
			String CC_Creation_ECRN = formObject.getNGValue("CC_Creation_ECRN");

			//account
			
			// refId always CRN of prime card by rubi
			commonMap.put("$acc_number$", CC_Creation_ECRN);

			String CC_Creation_CardLimit = formObject.getNGValue("CC_Creation_CardLimit");
			commonMap.put("$creditLimit$", CC_Creation_CardLimit);

			String CC_Creation_templateId = formObject.getNGValue("CC_Creation_templateId");
			commonMap.put("$templateCode$", CC_Creation_templateId);
			CreditCard.mLogger.info("account map: " +commonMap);
			limitInc=primeXml("account", commonMap, limitInc);
			commonMap.clear();
			CreditCard.mLogger.info("XML after account :\n"+limitInc);

			//person		
			String cmplx_Customer_CIFNo = formObject.getNGValue("cmplx_Customer_CIFNo");
			cmplx_Customer_CIFNo="C"+cmplx_Customer_CIFNo;
			commonMap.put("$custIdNumber$", cmplx_Customer_CIFNo);

			String cmplx_Customer_LastNAme = formObject.getNGValue("cmplx_Customer_LastNAme");
			commonMap.put("$lastName$", cmplx_Customer_LastNAme);
			
			CreditCard.mLogger.info("person map---> " +commonMap);
			limitInc=primeXml("person", commonMap, limitInc);
			commonMap.clear();
			CreditCard.mLogger.info("XML after person :\n"+limitInc);

			//address
			String address = formObject.getNGValue("AddressDetails_addtype");
			String AlternateContactDetails_MobileNo1 = formObject.getNGValue("AlternateContactDetails_MobileNo1");
			String AlternateContactDetails_MobNo2 = formObject.getNGValue("AlternateContactDetails_MobNo2");
			String AlternateContactDetails_HomeCOuntryNo = formObject.getNGValue("AlternateContactDetails_HomeCOuntryNo");
			String AlternateContactDetails_Email1 = formObject.getNGValue("AlternateContactDetails_Email1");
			...
			for (int i = 0; i<add_row_count;i++){
				...
				String addresstpe = formObject.getNGValue("cmplx_AddressDetails_cmplx_AddressGrid", i, 0); //0
				...
				String PrefferedAddr=formObject.getNGValue("cmplx_AddressDetails_cmplx_AddressGrid", i, 10);//9
				String Applicant_Name=formObject.getNGValue("cmplx_AddressDetails_cmplx_AddressGrid", i,13);

				String location = "";
				String Address_type = "";//

				if(PrefferedAddr.equalsIgnoreCase("true")){
					location = "STMT";
					Address_type = "11010";
				}else{
					location = "OTHER";
				...
				{
					CreditCard.mLogger.info("address Type rubi: " +addresstpe);

					if("STMT".equalsIgnoreCase( location)){
						add_xml_str = add_xml_str + "<address refType=\"PERSONCUSTID\" refId=\""+cmplx_Customer_CIFNo+"\" refAddressType=\"11010\"></address>";
						
					}
				}
			}

			commonMap.put("<Straddress></Straddress>", add_xml_str);

			CreditCard.mLogger.info("address map: " +commonMap);
			CreditCard.mLogger.info("address map---rubi: " +add_xml_str);

			limitInc=primeXml("address", commonMap, limitInc);
			commonMap.clear();

			CreditCard.mLogger.info("XML after address :\n"+limitInc);

			//card
			String primaryCard="0";
			String CC_Creation_Product= formObject.getNGValue("CC_Creation_Product");
			
			commonMap.put("$primaryCard$", primaryCard);
			commonMap.put("$productName$", CC_Creation_Product);

			String cmplx_CardDetails_cardEmbossing_name = formObject.getNGValue("cmplx_CardDetails_cardEmbossing_name");
			commonMap.put("$embName$",cmplx_CardDetails_cardEmbossing_name);

			String card_templateCode = "";
			String subProd = formObject.getNGValue("cmplx_Product_cmplx_ProductGrid", 0, 2);
			CreditCard.mLogger.info("subProd :-"+ subProd);	
			String EmpType = formObject.getNGValue("cmplx_Product_cmplx_ProductGrid", 0, 6);
			CreditCard.mLogger.info("EmpType :-"+ EmpType);

			String card_tempCode = "select MAST_PROF_PRIM  from ng_master_cardproduct where CODE = '"+ CC_Creation_Product +"' and Subproduct = '"+subProd+"' and EmployerCategory = '"+EmpType+"'";
			CreditCard.mLogger.info("Template code :-"+ card_templateCode);	

			List<List<String>> card_list=formObject.getDataFromDataSource(card_tempCode);
			CreditCard.mLogger.info("Card List :-"+card_list.size());
			if (card_list!=null) {
				CreditCard.mLogger.info("Card List size :-"+card_list.size());
				for (int i = 0; i < card_list.size(); i++) {	
					card_templateCode = card_list.get(i).get(0);
					CreditCard.mLogger.info("Template code :-"+ card_templateCode);	
				}
			}
			commonMap.put("$card_templateCode$", card_templateCode);

			CreditCard.mLogger.info("card map---> " +commonMap);
			limitInc=primeXml("card", commonMap, limitInc);
			commonMap.clear();
			CreditCard.mLogger.info("XML after card :\n"+limitInc);

			//riskDomain
			commonMap.put("$creditLimit$", CC_Creation_CardLimit);
			limitInc=primeXml("riskDomain", commonMap, limitInc);
			CreditCard.mLogger.info("XML after riskDomain ------>"+limitInc);
			commonMap.clear();

						
			String value=CC_Creation_CRN;
			commonMap.put("$value$", value);

			CreditCard.mLogger.info("Extension map :"+commonMap);
			limitInc = primeXml("extension", commonMap, limitInc);
			commonMap.clear();
			...
			CreditCard.mLogger.debug("Card Installment XML Failure: "+ error);
		}
		return limitInc;
	...
	//New card req added by sarthak

	//common function to read request xml by rubi
	public String ReadRequestXML(String FileName){

		try{
			String fileLocation = new StringBuffer().append(
					System.getProperty("user.dir")).append(
							System.getProperty("file.separator")).append("Prime4changes")
							.append(System.getProperty("file.separator")).append(
									FileName).toString();

			BufferedReader sbf = new BufferedReader(
					new FileReader(fileLocation));
			CreditCard.mLogger.info("File Location:"+fileLocation);
			...
			CreditCard.mLogger.debug(" XML Failure: "+ e);
		}
	...
	public String primeCustomerNew(FormReference formObject) {

		String inputXml = "";
		try {

			inputXml = ReadRequestXML("primeCustomerNew.txt");
			...
			String Id = ProcessInstanceId.split("-")[1];
			commonMap.put("$id$", Id);

			String DSA_name = "";
			String name = "select DSA_Name FROM NG_CC_EXTTABLE with (nolock) WHERE CC_Wi_Name = '"+ ProcessInstanceId +"'";
			List<List<String>> list=formObject.getDataFromDataSource(name);
			if (list!=null) {
				for (int i = 0; i < list.size(); i++) {
					DSA_name = list.get(i).get(0);
					CreditCard.mLogger.info("Name from ext Table "+ DSA_name);		
				}
			}
			commonMap.put("$createdBy$",DSA_name);
			CreditCard.mLogger.info("Application map: " +commonMap);
			inputXml=primeXml("application", commonMap , inputXml);
			commonMap.clear();
			CreditCard.mLogger.info("XML after application :\n"+inputXml);

			// customer
			String CC_Creation_ECRN = formObject.getNGValue("CC_Creation_ECRN");
			commonMap.put("$cust_number$", CC_Creation_ECRN);

			String CC_Creation_CustomerName = formObject.getNGValue("CC_Creation_CustomerName");
			commonMap.put("$name$", CC_Creation_CustomerName);

			String CC_Creation_Product = formObject.getNGValue("CC_Creation_Product");
			//		commonMap.put("$productName$" , CC_Creation_Product);

			String CC_Creation_CombinedLimit = formObject.getNGValue("CC_Creation_CombinedLimit");
			commonMap.put("$maxAggcl$", CC_Creation_CombinedLimit);

			CreditCard.mLogger.info("customer map: " +commonMap);
			inputXml=primeXml("customer", commonMap , inputXml);
			commonMap.clear();
			CreditCard.mLogger.info("XML after customer :\n"+inputXml);

			// account
			String CC_Creation_CRN = formObject.getNGValue("CC_Creation_CRN");
			commonMap.put("$acc_number$", CC_Creation_CRN);
			...
			if(result.length()==1)
				...
				result="-2"+result;
			commonMap.put("$stmtDateInc$", result);

			String CC_Creation_templateId = formObject.getNGValue("CC_Creation_templateId");
			commonMap.put("$templateCode$", CC_Creation_templateId);
			CreditCard.mLogger.info("account map: " +commonMap);
			inputXml=primeXml("account", commonMap, inputXml);
			commonMap.clear();
			CreditCard.mLogger.info("XML after account :\n"+inputXml);

			//person
			String cmplx_Customer_Title = formObject.getNGValue("cmplx_Customer_Title");
			commonMap.put("$title$", cmplx_Customer_Title);

			String cmplx_Customer_LastNAme = formObject.getNGValue("cmplx_Customer_LastNAme");
			commonMap.put("$lastName$", cmplx_Customer_LastNAme);

			String cmplx_Customer_FirstNAme = formObject.getNGValue("cmplx_Customer_FirstNAme");
			commonMap.put("$firstName$", cmplx_Customer_FirstNAme);

			String cmplx_Customer_MiddleNAme = formObject.getNGValue("cmplx_Customer_MiddleNAme");
			commonMap.put("$midName$", cmplx_Customer_MiddleNAme);

			String cmplx_Customer_MotherName = formObject.getNGValue("cmplx_Customer_MotherName");
			commonMap.put("$motherName$", cmplx_Customer_MotherName);

			String cmplx_Customer_DOb = formObject.getNGValue("cmplx_Customer_DOb");
			SimpleDateFormat formatter = new SimpleDateFormat("dd/MM/yyyy");
			Date date = formatter.parse(cmplx_Customer_DOb);
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
			String new_Date = sdf.format(date);
			commonMap.put("$dob$", new_Date);

			String cmplx_Customer_gender = formObject.getNGValue("cmplx_Customer_gender");
			commonMap.put("$sex$",cmplx_Customer_gender);

			String cmplx_Customer_MAritalStatus = formObject.getNGValue("cmplx_Customer_MAritalStatus");
			commonMap.put("$maritalStatus$", cmplx_Customer_MAritalStatus);

			String cmplx_Customer_PAssportNo = formObject.getNGValue("cmplx_Customer_PAssportNo");
			commonMap.put("$ssnumber$", cmplx_Customer_PAssportNo);

			String vipFlag=(formObject.getNGValue("cmplx_Customer_VIPFlag").equalsIgnoreCase("true")?"1":"0");
			commonMap.put("$personType$", vipFlag);

			//Add "C" before CIF Number added by rubi
			String CC_Creation_CIF = formObject.getNGValue("CC_Creation_CIF");
			CC_Creation_CIF = "C"+CC_Creation_CIF;
			commonMap.put("$custIdNumber$", CC_Creation_CIF);

			CreditCard.mLogger.info("Person map: " +commonMap);
			inputXml=primeXml("person", commonMap, inputXml);
			commonMap.clear();
			CreditCard.mLogger.info("XML after person :\n"+inputXml);
...
//					if(!"Home".equalsIgnoreCase( addresstpe)){
//						add_xml_str = add_xml_str + "<address addressType=\""+Address_type+"\" location=\""+location+"\" title=\""+cmplx_Customer_Title;
//						add_xml_str = add_xml_str + "\" address1=\""+flat_Villa+"\" address2=\""+Building_name+"\" address3=\""+street_name+"\" address4=\""+Landmard+"\" city=\""+city+
//						"\" country=\""+country+"\" zip=\""+Po_Box+"\" tel1=\""+AlternateContactDetails_MobileNo1+"\" tel2=\""+AlternateContactDetails_MobNo2+
//						"\" mobile=\""+AlternateContactDetails_HomeCOuntryNo+"\" email=\""+AlternateContactDetails_Email1+"\" respId=\"6\"></address>";
//					}
					if(!"Home".equalsIgnoreCase( addresstpe)){
						String tel1="";
						if("Office".equalsIgnoreCase( addresstpe)){
							tel1 = formObject.getNGValue("AlternateContactDetails_OfficeNo");
						}
						else {
							tel1 = formObject.getNGValue("AlternateContactDetails_ResidenceNo");
						}
						add_xml_str = add_xml_str + "<address addressType=\""+Address_type+"\" location=\""+location+"\" title=\""+cmplx_Customer_Title;
						add_xml_str = add_xml_str + "\" address1=\""+flat_Villa+"\" address2=\""+Building_name+"\" address3=\""+street_name+"\" address4=\""+Landmard+"\" city=\""+city+
						"\" country=\""+country+"\" zip=\""+Po_Box+"\" tel1=\""+tel1+"\" tel2=\""+AlternateContactDetails_MobNo2+
						"\" mobile=\""+AlternateContactDetails_MobileNo1+"\" email=\""+AlternateContactDetails_Email1+"\" respId=\"6\"></address>";
					}
				}
			...
			inputXml=primeXml("address", commonMap, inputXml);
			commonMap.clear();
			CreditCard.mLogger.info("XML after address :\n"+inputXml);

			//card
			String primaryCard="1";
			commonMap.put("$primaryCard$", primaryCard);
			commonMap.put("$productName$", CC_Creation_Product);
			...
			CreditCard.mLogger.info("card map---> " +commonMap);
			inputXml=primeXml("card", commonMap, inputXml);
			commonMap.clear();
			CreditCard.mLogger.info("XML after card :\n"+inputXml);


			//extension		
			String newXml = new CC_Integration_Input().getExtensions_details(true);
			
			String cardExt = new CC_Integration_Input().getCardExtensions_details(true);
			

			CreditCard.mLogger.info("Extensions from get extension method :"+newXml);
			/*String[] new_XML = newXml.split("</Extensions>");           

			String acc_extension = "";
			String card_extension="";
			Integer respId = 1;
			int count = 0;
			int countC=0;
			for (int i = 0; i < new_XML.length ; i++) 
			{
				String new_ext = new_XML[i].contains("<Value>")? new_XML[i].substring(new_XML[i].indexOf("<Value>")+7, new_XML[i].indexOf("</Value>")) : "";
				//System.out.println(new_ext);
				String[] ext_space = new_ext.split(" ");

				String fieldNoValue = ext_space[1];
				if (ext_space.length>2) {
					fieldNoValue += ext_space[2];
				}

				if ("A".equalsIgnoreCase(ext_space[0]) &&  count==0) {
					String extension="";
					CreditCard.mLogger.info("Value of count "+count);
					extension ="<extension fieldNo=$fieldNo$ value=$value$ respId=$respId$></extension>";
					//				if( count == 0){
					//					extension ="<extension fieldNo=$fieldNo$ value=$value$ respId=$respId$></extension>\n";
					//				}
					//				else{
					//					extension ="\t\t\t\t\t\t<extension fieldNo=$fieldNo$ value=$value$ respId=$respId$></extension>\n";
					//				}
					acc_extension+=extension.replace("$fieldNo$", "\""+fieldNoValue.substring(0, 5)+"\"")
					.replace("$value$","\""+ fieldNoValue.substring(5)+"\"")
					.replace("$respId$", "\""+respId.toString()+"\"");

					CreditCard.mLogger.info("Account ext :"+acc_extension);			 
					// System.out.println(commonMap);
					respId++;
					count+=1;
				}

				if ("C".equalsIgnoreCase(ext_space[0])&& countC ==0) {
					String extension="";
					CreditCard.mLogger.info("Value of count "+count);
					extension ="<extension fieldNo=$fieldNo$ value=$value$ respId=$respId$></extension>";
					//				if(countC==0)
					...
					//				else
					//					extension ="\t\t\t\t\t\t<extension fieldNo=$fieldNo$ value=$value$ respId=$respId$></extension>\n";
					card_extension+=extension.replace("$fieldNo$", "\""+fieldNoValue.substring(0, 5)+"\"")
					.replace("$value$","\""+ fieldNoValue.substring(5)+"\"")
					.replace("$respId$", "\""+respId.toString()+"\"");

					CreditCard.mLogger.info("Card ext : "+card_extension);
			...
			CreditCard.mLogger.info("Account ext :"+newXml);	
			CreditCard.mLogger.info("Card ext : "+cardExt);
			commonMap.put("<StrAccountextension>", newXml);
			commonMap.put("<StrCardextension>", cardExt);
			//System.out.println(commonMap);

			CreditCard.mLogger.info("Extension map :"+commonMap);
			inputXml = primeXml("extension", commonMap, inputXml);
			commonMap.clear();

			CreditCard.mLogger.info("Final input Xml :"+inputXml);

		} catch (Exception e) {

			CreditCard.mLogger.info("Exception in commonMapPrime Method "+e.getMessage());
			String alert_msg = "Exception in commonMapPrime";
			throw new ValidatorException(new FacesMessage(alert_msg));
		}
		return inputXml;
	...
	public String selfSupplementryCard(FormReference formObject){

		String inputXml="";

		try {

			inputXml= ReadRequestXML("Cu(E)Ac(E).txt");
			...
			String ProcessInstanceId = formObject.getWFWorkitemName();
			String id = ProcessInstanceId.split("-")[1];
			CreditCard.mLogger.info("ProcessInstanceId:"+ProcessInstanceId +" ,id:"+id);
			commonMap.put("$id$", id);
			...
			CreditCard.mLogger.info("DSA Name from ext Table "+ DSA_name);
			commonMap.put("$createdBy$",DSA_name);
			CreditCard.mLogger.info("Application map---> " +commonMap);
			inputXml=primeXml("application", commonMap, inputXml);
			CreditCard.mLogger.info("XML after application ------>"+inputXml);
			commonMap.clear();
			...
			for (int i = 0; i<add_row_count;i++){
				...
				if(Applicant_Name.split("-")[0].trim().replaceAll(" ","").equalsIgnoreCase("P"))//&& Applicant_Name.split("-")[1].trim().replaceAll(" ","").equalsIgnoreCase(formObject.getNGValue("cmplx_DEC_MultipleApplicantsGrid",formObject.getSelectedIndex("cmplx_DEC_MultipleApplicantsGrid"),1).trim().replaceAll(" ","")))
				{
					...
					if("STMT".equalsIgnoreCase( location)){
						add_xml_str = add_xml_str + "<address refType=\"PERSONCUSTID\" refId=\""+cmplx_Customer_CIFNo+"\" refAddressType=\"STMT\"></address>";
			...
			//riskDomain
			commonMap.put("$creditLimit$", CC_Creation_CardLimit);
			inputXml=primeXml("riskDomain", commonMap, inputXml);
			CreditCard.mLogger.info("XML after riskDomain ------>"+inputXml);
			commonMap.clear();
		...
		//	CreditCard.mLogger.info("Account ext :"+newXml);	
			CreditCard.mLogger.info("Card ext : "+cardExt);
	//		commonMap.put("<StrAccountextension>", newXml);
			commonMap.put("<StrCardextension></StrCardextension>", cardExt);
			//System.out.println(commonMap);
			...
			CreditCard.mLogger.info("Account ext :"+acc_extension);	
			CreditCard.mLogger.info("Card ext : "+card_extension);
			commonMap.put("<StrAccountextension>", acc_extension);
			commonMap.put("<StrCardextension>", card_extension);*/
			//System.out.println(commonMap);
...
//			commonMap.clear();
			...
			CreditCard.mLogger.info("Exception in CommonMapPrime3 Method"+e.getMessage());
			String alert_msg = "Exception in CommonMapPrime3 Method";
			throw new ValidatorException(new FacesMessage(alert_msg));
		}
	...
	public String SupplementryCard(FormReference formObject){
		...
		try {
			...
			inputXml= ReadRequestXML("supplementary.txt");
...
//			commonMap.clear();
	...
	public String primeCustomerExisting(FormReference formObject){

		String inputXml="";

		try {
			
			inputXml= ReadRequestXML("Pe(E)Ad(E).txt");
...
//			commonMap.clear();
		...
		//		commonMap.put("<StrAccountextension>", newXml);
			...
			CreditCard.mLogger.info("Exception in commonMapPrime2 Method "+e.getMessage());
			String alert_msg = "Exception in commonMapPrime2";
			throw new ValidatorException(new FacesMessage(alert_msg));
		}
	...
	public String primeAccountExisting(FormReference formObject){
		...
		try {
			...
			//		String ext = new CC_Integration_Input().getExtensions_details(true);
			//		String ext_list[] =ext.split("</Extensions>");
			//		
			//		for (String i : ext_list) {
			//			
			//			String respId="4";
			//			String ext_val = (i.contains("<Value>"))?i.substring(i.indexOf("<Value>")+"</Value>".length()-1,i.indexOf("</Value>")):"";
			//			CreditCard.mLogger.info("Values :"+ext_val);
			//			
			//			String ext_fieldNo = ext_val.substring(2,7);
			//			String ext_value = ext_val.substring(7);
			//			
			//			inputXml = inputXml.concat("<extension fieldNo="+ext_fieldNo+" value="+ext_value+" respId="+respId+">");
			//					
			//		}
			//		commonMap.put("fieldNo", "200088885");
			//		commonMap.put("value", "0");
			//		commonMap.put("respId", "37");
			//		commonMap.put("entityVersion", "0");
			//		CreditCard.mLogger.info("Accounts extension map---> " +commonMap);
			//
			//		inputXml=primeXml("extension", commonMap, inputXml);
			//		commonMap.clear();
			...
			String AddressDetails_addtype = formObject.getNGValue("AddressDetails_addtype");
			commonMap.put("$addressType$", AddressDetails_addtype);

			String AddressDetails_house = formObject.getNGValue("AddressDetails_house");
			commonMap.put("$address1$", AddressDetails_house);

			String AddressDetails_buildname = formObject.getNGValue("AddressDetails_buildname");
			commonMap.put("$address2$", AddressDetails_buildname);

			String AddressDetails_street = formObject.getNGValue("AddressDetails_street");
			commonMap.put("$address3$", AddressDetails_street);

			String AddressDetails_landmark = formObject.getNGValue("AddressDetails_landmark");
			commonMap.put("$address4$", AddressDetails_landmark);

			String AddressDetails_city = formObject.getNGValue("AddressDetails_city");
			commonMap.put("$city$", AddressDetails_city);

			String AddressDetails_country = formObject.getNGValue("AddressDetails_country");
			commonMap.put("$country$", AddressDetails_country);

			String AddressDetails_pobox = formObject.getNGValue("AddressDetails_pobox");
			commonMap.put("$zip$", AddressDetails_pobox);

			String AlternateContactDetails_MobileNo1 = formObject.getNGValue("AlternateContactDetails_MobileNo1");
			commonMap.put("$tel1$", AlternateContactDetails_MobileNo1);

			String AlternateContactDetails_MobNo2 = formObject.getNGValue("AlternateContactDetails_MobNo2");
			commonMap.put("$tel2$", AlternateContactDetails_MobNo2);

			String AlternateContactDetails_HomeCOuntryNo = formObject.getNGValue("AlternateContactDetails_HomeCOuntryNo");
			commonMap.put("$mobile$", AlternateContactDetails_HomeCOuntryNo);

			String AlternateContactDetails_Email1 = formObject.getNGValue("AlternateContactDetails_Email1");
			commonMap.put("$email$", AlternateContactDetails_Email1);
			...
			//		commonMap.put("fieldNo", "200088885");
			//		commonMap.put("value", "0");
			//		inputXml=commonMap.put("entityVersion", "0");
			//		CreditCard.mLogger.info("card extension map---> " +commonMap);
			//
			//		inputXml=primeXml("extension", commonMap, inputXml);
			//		commonMap.clear();
		...
		for (int i = 0; i < new_XML.length ; i++) 
		{
			...
			if ("A".equalsIgnoreCase(ext_space[0])) {
				String extension ="<extension fieldNo=$fieldNo$ value=$value$ respId=$respId$></extension>\n";
				acc_extension+=extension.replace("$fieldNo$", "\""+fieldNoValue.substring(0, 5)+"\"")
				 						.replace("$value$","\""+ fieldNoValue.substring(5)+"\"")
				 						.replace("$respId$", "\""+respId.toString()+"\"");

				//System.out.println(acc_extension);

				// System.out.println(commonMap);
				respId++;
			}

			if ("C".equalsIgnoreCase(ext_space[0])) {
				String extension ="<extension fieldNo=$fieldNo$ value=$value$ respId=$respId$></extension>\n";
				card_extension+=extension.replace("$fieldNo$", "\""+fieldNoValue.substring(0, 5)+"\"")
				 						.replace("$value$","\""+ fieldNoValue.substring(5)+"\"")
				 						.replace("$respId$", "\""+respId.toString()+"\"");

				//System.out.println(card_extension);	
	...
	public String primeXml (String type , Map<String, String> map,String strXml){

		try {
			if(type.equalsIgnoreCase("application") || type.equalsIgnoreCase("account") 
					|| type.equalsIgnoreCase("extension") || type.equalsIgnoreCase("person") 
					|| type.equalsIgnoreCase("address")|| type.equalsIgnoreCase("customer") || type.equalsIgnoreCase("card")){

				for (Map.Entry<String, String> m : map.entrySet()) {
					String key = m.getKey();
					if (!("".equalsIgnoreCase(key)||key==null)) {
						strXml = strXml.replace(m.getKey(), m.getValue());
					} 
					//				else {
					//					strXml = strXml.replace(key, "").replace(key.substring(1, key.length()-1)+"=\"\"", "");
					//				}	
				}
			}

		} catch (Exception e) {
			CreditCard.mLogger.info("Exception in PrimeXml");
		}
		return strXml;
	}
}
