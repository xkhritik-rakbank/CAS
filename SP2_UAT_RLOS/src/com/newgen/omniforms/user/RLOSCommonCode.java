	...
	public void call_Frame_Expanded(ComponentEvent pEvent)
	{
		...
		{				
			...
			if("S".equalsIgnoreCase(formObject.getNGValue("cmplx_EmploymentDetails_ApplicationCateg"))){
				...
				//Sagarika change for Falcon BAU
...
if("Primary".equalsIgnoreCase(formObject.getNGValue("cmplx_DecisionHistory_MultipleApplicantsGrid",formObject.getSelectedIndex("cmplx_DecisionHistory_MultipleApplicantsGrid"), 0))){
			...
			{	
				...
				try
				{
					...
					String AecbHistQuery = "select COUNT(AECBHistMonthCnt) from  (select cast(isnull(AECBHistMonthCnt,'0') as int) as AECBHistMonthCnt,CustRoleType as CustRoleType,CardType as Liability_type,ProviderNo as ProviderNo  from ng_rlos_cust_extexpo_CardDetails with (nolock) where  Wi_Name  = '"+Wi_Name+"'  union all   select cast(isnull(AECBHistMonthCnt,'0') as int) as AECBHistMonthCnt,CustRoleType as CustRoleType,LoanType as Liability_type,ProviderNo as ProviderNo from ng_rlos_cust_extexpo_LoanDetails with (nolock) where Wi_Name  = '"+Wi_Name+"'  union all   select cast(isnull(AECBHistMonthCnt,'0') as int) as AECBHistMonthCnt,CustRoleType as CustRoleType,ServiceID as Liability_type,ProviderNo as ProviderNo from ng_rlos_cust_extexpo_ServicesDetails with (nolock) where Wi_Name  = '"+Wi_Name+"'  ) as ext_expo where Liability_type not in ( '85','99','Communication Services','TelCo-Mobile Prepaid','104','51','Cheque Purchase','Cheque Book Issuance','101','Current/Saving Account with negative Balance')   and ProviderNo<>'B01'   and CustRoleType in ('A','Main Contract Holder')";
					//String  add_xml_str ="";					
