	...
	public void FrameExpandEvent(ComponentEvent pEvent){
		...
		try{
			...
			if ("Internal_External_Liability".equalsIgnoreCase(pEvent.getSource().getName())) {
				...
				if(("CAD_Analyst1").equalsIgnoreCase(activity)){
			...
			if ("Employment_Verification".equalsIgnoreCase(pEvent.getSource().getName())) {
				...
				if(!"FCU".equalsIgnoreCase(activityName))//sagarika
				{
			...
			if ("FPU_GRID".equalsIgnoreCase(pEvent.getSource().getName())) {
				...
				if(!"FCU".equalsIgnoreCase(formObject.getWFActivityName()))
				{
				...
				for(int i=0; i<10;i++){
					...
					if(WorkstepName.equalsIgnoreCase("FCU") && (formObject.getNGValue("DecisionHistory_Decision_ListView1,i,3").equalsIgnoreCase("Positive")||formObject.getNGValue("DecisionHistory_Decision_ListView1,i,3").equalsIgnoreCase("Negative")))
					{
	...
	public void setFormHeader(FormEvent pEvent){
		...
		try{
			...
			List<String> activity_openFragmentsonLoad = Arrays.asList("Hold_CPV","HR","Interest_Rate_Approval","Manager","Original_Validation","Post_Disbursal","Rejected_Application"
					...
					,"Compliance","CPV_Analyst","CPV","CSM_Review","RM_Review",NGFUserResourceMgr_CreditCard.getGlobalVar("CC_CSM"),"DDVT_Checker","Disbursal","FCU","Fulfillment_RM","CAD_Analyst1","Cad_Analyst2","DDVT_Maker","Archival","CustomerHold","Credit_Analyst_Hold");//Changed by aman for PCSP-477
...
hm.clear();
		...
		else if("CC_Creation_Card_Services".equalsIgnoreCase(pEvent.getSource().getName()))
		{
			...
			//CODE ADDED BY BANDANA FOR PCASP-6
				 ...
				 int rowCount = 0;
				 for(int i=0;i<rowCount;i++){
						if("Y".equalsIgnoreCase(formObject.getNGValue("cmplx_FinacleCore_cmplx_gr_investment",i,13))){
							aum = aum+ Double.parseDouble(formObject.getNGValue("cmplx_FinacleCore_cmplx_gr_investment",i,8));
						}
					}
				 if (aum !=0){
				 formObject.setNGValue("FinacleCore_Investment_TotalAUM" , aum);
				 }
				}
	...
	public  void AddInAuthorisedGrid(FormReference formObject, String customerName) {
		...
		CreditCard.mLogger.info("inside AddInAuthorisedGrid:: "+formObject.getWFWorkitemName());
