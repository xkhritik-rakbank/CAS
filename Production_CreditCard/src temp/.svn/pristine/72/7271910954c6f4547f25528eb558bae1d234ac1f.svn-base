		...
		try {
			...
			if (OutputXML.size()==0) 
			{
				...
				for (int i = 0; i < OutputXML.size(); i++) {
					...
					String refNo = formObject.getNGValue("cmplx_Liability_New_ReferenceNo");
					CreditCard.mLogger.info( "aecb_score :"+aecb_score+" range :: "+range+" refNo:: "+refNo);
					if (!(OutputXML.get(i).get(0) == null || OutputXML.get(i)	.get(0).equals(""))) {CifId = OutputXML.get(i).get(0).toString();
					}
	...
	private Map<String, String> CARD_NOTIFICATION_Custom(List<List<String>> DB_List,FormReference formObject,String callName,String Operation_name) {
		...
		try{
			for (List<String> mylist : DB_List) {
				...
				//Added By Shivang for pcasp-1821
				else if("AcctInfo".equalsIgnoreCase(tag_name)){
					String emptype = formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,6);
					...
					xml_str = xml_str + "<AcctInfo>";
					xml_str = xml_str + getAccountInfo();
					xml_str = xml_str + "</AcctInfo>";
					int_xml.put(parent_tag, xml_str);
	...
	public String fetchAddressFlagVal() {
		try{
			...
			if(Cust_type.equalsIgnoreCase("Primary")){
				String query = "select count(distinct ECRN) from ng_RLOS_CUSTEXPOSE_CustInfo with (nolock) where Child_Wi = '"+formObject.getWFWorkitemName()+"' and CifId= '"+selectCIF+"' and ecrn !='' and ecrn is not null and ecrn not in (select ECRN from ng_cas_rejected_table where CIF_ID= '"+selectCIF+"' and CRN=ECRN)";
				...
				try{
					if(!selfEmpcase){
						...
						try{
							...
							if(primaryApp){
								Empdetails_xml_str += "<Designation>"+formObject.getNGValue("cmplx_EmploymentDetails_Designation")+"</Designation>";
							}
							else{
								Empdetails_xml_str += "<Designation>"+getSuppCardnotifdata("Designation")+"</Designation>";
							}
