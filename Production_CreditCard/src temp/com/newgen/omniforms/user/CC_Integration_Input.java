...
public class CC_Integration_Input extends Common implements java.io.Serializable{
			...
			for (List<String> mylist : DB_List) {
				...
				//tag added by saurabh on 8th Feb 19 for default value points.
					...
					try{
						...
						String branchUserQuery = "select DSA_Name as Introduce_By from NG_CC_EXTTABLE where CC_Wi_Name='"+formObject.getWFWorkitemName()+"'";
						...
						try{
							...
							if(card_product==null|| card_product.equals("")){
								...
								if(tag_name.equalsIgnoreCase("InterestProfile")){
									...
									intrest_Query="select code from NG_MASTER_FeeProfile with (nolock) where Product = '"+card_product+"'";
								}
		...
		try{
			for (List<String> mylist : DB_List) {
				...
				/*else if ("current_emp_catogery".equalsIgnoreCase(tag_name)) {
					...
					String fd_amount=formObject.getNGValue("cmplx_FinacleCore_Final_FD_Amount");	
					String xml_str =  int_xml.get(parent_tag);
	...
	{
		FormReference formObject = FormContext.getCurrentInstance().getFormReference();
		String Query ="select CardEmbossNum,History from ng_rlos_cust_extexpo_CardDetails where child_wi='"+formObject.getWFWorkitemName()+"' and (History is not null or History!='') union all select AgreementId,History from ng_rlos_cust_extexpo_LoanDetails where child_wi='"+formObject.getWFWorkitemName()+"' and (History is not null or History!='')";
		String add_xml_str = "";
		try
		{
