...
public class RLOSCommon extends Common implements Serializable
{
	...
	public void loadPicklist3()
	{
		...
		if("RESCE".equalsIgnoreCase(formObject.getNGValue("initiationChannel"))){
		 ...
		 query="select '--Select--' union all select  convert(varchar,Decision) from NG_MASTER_Decision with (nolock) where ProcessName='RLOS' and workstepName='"+formObject.getWFActivityName()+"' AND Initiation_Type = ''";
	}
	...
	public void Fields_ApplicationType_Employment()
	{
		...
		FormReference formObject = FormContext.getCurrentInstance().getFormReference();
		if(NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_RESCE").equalsIgnoreCase(formObject.getNGValue("Application_Type")) || NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_RESCN").equalsIgnoreCase(formObject.getNGValue("Application_Type"))){
	...
	public void AddPrimaryData()
	{
		...
		if("RESCE".equalsIgnoreCase(AppType) || "RESCN".equalsIgnoreCase(AppType)){//added by akshay on 17/1/18 for reschedule/Collection case
	...
	public void loadInDecGrid()
	{
		...
		try
		{
			...
			String query="select FORMAT(datelastchanged,'dd-MM-yyyy hh:mm'),userName,workstepName,Decision,remarks,dec_wi_name,Entry_date from ng_rlos_gr_Decision with (nolock) where wi_nAme='"+ParentWI_Name+"' or wi_nAme='"+formObject.getWFWorkitemName()+"'";
				...
				if("Y".equalsIgnoreCase(Staff_cif)){
					...
					try{
						...
						if(framestate0 == 0){
							...
							formObject.setTop("Supplementary_Container",formObject.getTop("CardDetails_container")+formObject.getHeight("CardDetails_container")+40);
							formObject.setTop("FATCA", formObject.getTop("CardDetails")+formObject.getHeight("CardDetails_container")+50);
							formObject.setTop("KYC", formObject.getTop("FATCA")+formObject.getHeight("FATCA")+50);
							formObject.setTop("OECD", formObject.getTop("KYC")+formObject.getHeight("KYC")+50);
						...
						if(framestate2 == 0)
						{
							...
							formObject.setTop("FATCA", formObject.getTop("CardDetails")+formObject.getHeight("CardDetails")+50);
							formObject.setTop("KYC", formObject.getTop("FATCA")+formObject.getHeight("FATCA")+50);
							formObject.setTop("OECD", formObject.getTop("KYC")+formObject.getHeight("KYC")+50);
						...
						if(framestate3 == 0){
							...
							if (!"LI".equalsIgnoreCase(formObject.getNGValue("Subproduct_productGrid"))){
	...
	public void adjustFrameTops(String frameList){
		...
		for(int i=1;i<framelist_array.length;i++){
			RLOS.mLogger.info("Frame state for "+framelist_array[i]+": "+formObject.getNGFrameState(framelist_array[i]));
