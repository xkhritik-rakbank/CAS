...
var timeDiff=Math.abs(maturityDate.getTime()-doB.getTime());
var dateDiff=Math.ceil(timeDiff/(1000*3600*24));
setNGValueCustom("cmplx_EligibilityAndProductInfo_ageAtMaturity",dateDiff);
}
	...
	function SetValueCustomer_ReadCard(){
		...
		$.ajax({
			...
			success : function(data, textStatus, jQxhr) {
					...
					setNGValueCustom('cmplx_Customer_MobNo',dataobj.workMobileNumber);
  ...
  function checkCardGenuineStatus() {
	var userData = {  
	   "appCode":"CAS",
	   "recordReference":"132456",
	   "useCase":"Credeit Card Application",
	   "userId":"Deepak"
	}
	...
	var ajaxReq = $.ajax({
		...
		success : function(data, textStatus, jQxhr) {
			...
			if(data.cardGenuineCheckStatus) {
		...
		error : function(jqXhr, textStatus, errorThrown) {
			...
			if(jqXhr.readyState==0){
				...
				var Erroobj = JSON.parse(jqXhr.responseText);	
				alert('Card Not Genuine: '+ Erroobj.errorDesc);
			}
		}
	});
}
  ...
  function SetValueCustomer_CheckEligibility(){
	...
	//	setNGValueCustom("cmplx_EligibilityAndProductInfo_FinalLimit","4");
		...
		setNGValueCustom('cmplx_EligibilityAndProductInfo_ageAtMaturity',parseInt(totalage));
 ...
 function SetValueCustomer_FetchDetails(){
	...
	{
		var fields="cmplx_Customer_Title,cmplx_Customer_ResidentNonResident,cmplx_Customer_gender,cmplx_Customer_age,cmplx_Customer_MotherName,cmplx_Customer_EmirateIDExpiry,cmplx_Customer_IdIssueDate,cmplx_Customer_VisaNo,cmplx_Customer_PassPortExpiry,cmplx_Customer_SecNationality,cmplx_Customer_MAritalStatus,cmplx_Customer_COuntryOFResidence,cmplx_Customer_EMirateOfVisa,cmplx_Customer_EmirateOfResidence,cmplx_Customer_yearsInUAE,cmplx_Customer_CustomerCategory,cmplx_Customer_GCCNational,cmplx_Customer_PassportIssueDate,cmplx_Customer_VisaIssuedate"; 
		var field_array=fields.split(",");
		for(var i=0;i<field_array.length;i++)
