...
if(!checkFor_SupplemntGrid()){
	...
	var ntb_case=false;
		...
		for(var i=0;i<row_count;i++)
			...
			if(ntb_case && getNGValue('cmplx_Customer_NTB')==true){
		...
		if(getNGValue('is_cc_waiver_require')!='Y' && getNGValue('Sub_Product')!='PU'){//changed from LI to PU by akshay on 3/5/18 for proc 8978
			...
			if(SecurityGridCount==0 && getNGValue('Sub_Product')!='LI'){
	...
	else if(activityName=='Smart_CPV')
	{
		...
		if(n==1)
		{
			if(getLVWAT("cmplx_SmartCheck_SmartCheckGrid",0,2)=='')
			{
				showAlert('cmplx_Compliance_cmplx_gr_compliance',alerts_String_Map['CC144']);
				return false;
			}				
		}	
...
//below code commented by akshay on 23/11/18
	...
	*/
	...
	if(activityName=='FCU' || activityName=='Compliance' || activityName=='DSA_CSO_Review' || activityName=='CPV_Analyst'){
...
//Changes Done by aman for preferred choice
	function checkPrefferedChoice_1(){
		...
		for(var i=0; i<gridRowCount;i++)
		{
			...
			for(var i=0; i<gridRowCount;i++){
				...
				if(getLVWAT('cmplx_AddressDetails_cmplx_AddressGrid',i,10) == 'true' && preferedCustomers.indexOf(getLVWAT('cmplx_AddressDetails_cmplx_AddressGrid',i,13))==-1){
						//prefferedFlag = true;
						//added by prabhakar
						allPreffered++;
	...
	if(isFieldFilled('cmplx_Customer_yearsInUAE')==false){
	...
	function validateReferGrid()
	{
		...
		if(n>0){
			for(var i=0;i<n;i++){
				...
				else if(activity=='FCU'){activity='FPU';}
...
function checkFor_SupplemntGrid()
	...
	var listViewName = 'SupplementCardDetails_cmplx_supplementGrid';
		...
		for(var i=0;i<getLVWRowCount(listViewName);i++){
			...
			else if(getLVWAT(listViewName,i,37)=='' )//ID Issue date
			{
			...
			else if(getLVWAT(listViewName,i,23)=='' )//ID Issue date
			{
				showAlert('SupplementCardDetails_Frame1','Please add Visa Expiry Date');
				return false;
			}
			else if(getLVWAT(listViewName,i,22)=='' )//ID Issue date
			{
...
function validate_time(id)
{
  ...
  var re=/^([0-9]|0[0-9]|1[0-9]|2[0-3]):[0-5][0-9]$/;
   if(re.test(str))
    ...
    {
        showAlert('',alerts_String_Map['CC283']);
       return false;
    }
}	