...
//done by nikhil for part match view button
	...
	|| pId=='AddressDetails_Button1_View' || pId=='Designation_button1'|| pId=='Designation_button2'|| pId=='Button_City_View'|| pId=='Button_State_View'|| pId=='ButtonOECD_State_View' || pId=='CardDispatchToButton_View'|| pId=='ButtonOECD_State_View' || pId=='Nationality_Button1_View')
{
return true;
}
if(pId=='Designation_button1_View' || pId=='Designation_button2_View')
{
return true;
}
	//sagarika for PCSP-470
	if(pId=='NotepadDetails_Add1')
	{
		...
		if(activityName == 'DDVT_Maker'){
			if(pId=='ProductContainer' || pId=='IncomeDetails' || pId=='EligibilityAndProductInformation' || pId=='Address_Details_container' || pId=='Alt_Contact_container' ||  pId=='Card_Details' ||  pId=='Supplementary_Cont' ||  pId=='FATCA' ||  pId=='KYC' ||  pId=='OECD' || pId=='Details' || pId=='Finacle_CRM_Incidents' || pId=='Finacle_CRM_CustomerInformation'  || pId=='MOL' || pId=='World_Check' || pId=='Reject_Enquiry' || pId=='Salary_Enquiry' || pId=='Frame4' || pId=='DecisionHistory' || pId == 'PartMatch_Search' || pId == 'Reference_Details'||pId=='External_Blacklist' || pId=='PostDisbursal'){
				...
				var patmatch_flag = getNGValue('cmplx_PartMatch_partmatch_flag');
					...
					else if(patmatch_flag == '' || patmatch_flag == null){
...
// Done by aman for CR 1312 
		...
		// sagarika
				...
				var Mobile_no=getNGValue('Cust_ver_sp2_Text11');
		...
		if(mob_No.length>=13 && mob_No.length<=15){
			showAlert(pId,alerts_String_Map['VAL385']);
			return false;
		}	
		...
		//sagarika
		 ...
		 {  
		 //alert("clicked");
		 //below code changed by nikhil to return false 
		 if(!validate_time('cmplx_fieldvisit_sp2_field_v_time'))
		 {
			return false;
		 }
		 if(!validate_time('cmplx_fieldvisit_sp2_field_visit_rec_time'))
		 {
		 	return false;
		 }
...
if(getNGValue(pId)=="Mismatch")
{
 ...
 com.newgen.omniforms.formviewer.setLockedCustom("cmplx_emp_ver_sp2_salary_break_remarks",true);
	...
	var mob1 = getNGValue(pId);
		if((mob1=='Yes' || mob1=='No' || mob1=='Mismatch') && getNGValue('cmplx_CustDetailVerification_mobno2_ver')==''){
	...
	var mob1 = getNGValue(pId);
		if((mob1=='Yes' || mob1=='No' || mob1=='Mismatch') && getNGValue('cmplx_CustDetailVerification_email2_ver')==''){
	...
	var mob2 = getNGValue(pId);
		if((mob2=='Yes' || mob2=='No' || mob2=='Mismatch') && getNGValue('cmplx_CustDetailVerification_email1_ver')==''){
		...
		if(getNGValue('cmplx_CardDetails_Supplementary_Card')=='Yes')
			{
				...
				setTop('Supplementary_Cont',parseInt(getTop('Card_Details'))+parseInt(getHeight('Card_Details'))+30+'px');
...
//changed by nikhil
	...
	else if (pId =='PartMatch_mno1')	
	{
		...
		var mob_No="00971"+Mobile_no;
		setNGValueCustom('PartMatch_mno1',mob_No);
	}
...
//below code changes by nikhil for wrong alert on change 6/12
