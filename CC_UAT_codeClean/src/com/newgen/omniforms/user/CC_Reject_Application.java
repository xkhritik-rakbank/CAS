...
public class CC_Reject_Application extends CC_Common implements FormListener
{
	...
	public void formLoaded(FormEvent pEvent)
	{
		...
		SKLogger_CC.writeLog("CC Reject Application", "Inside formLoaded()" + pEvent.getSource().getName());
	...
	{
        try{
        	SKLogger_CC.writeLog("CC Reject Application","Inside CC Reject Application CC");
           new CC_CommonCode().setFormHeader(pEvent);
        }catch(Exception e)
        {
            SKLogger_CC.writeLog("CC Reject Application", "Exception:"+e.getMessage());
        }
    }
	public void eventDispatched(ComponentEvent pEvent) throws ValidatorException {
		String alert_msg="";
		SKLogger_CC.writeLog("Inside CC Reject Application eventDispatched()", "EventName:" + pEvent.getType() + "#ControlName#" + pEvent.getSource().getName());
	  formObject =FormContext.getCurrentInstance().getFormReference();

      switch(pEvent.getType()) {

          case FRAME_EXPANDED:
				SKLogger_CC.writeLog(" In CC Reject Application eventDispatched()", "EventName:" + pEvent.getType() + "#ControlName#" + pEvent.getSource().getName());
...
/*							LoadPickList("cmplx_CompanyDetails_cmplx_CompanyGrid_AppType", "select '--Select--' union select convert(varchar, description) from NG_MASTER_ApplicantType with (nolock)");
			...
			case MOUSE_CLICKED:
				...
				if(pEvent.getSource().getName().equalsIgnoreCase("Customer_save")){
					SKLogger_CC.writeLog("CC Reject Application", "Inside Customer_save button: ");
					...
					SKLogger_CC.writeLog(" In CC Reject Application eventDispatched()", "EventName:" + pEvent.getType() + "#ControlName#" + pEvent.getSource().getName());
					 if (pEvent.getSource().getName().equalsIgnoreCase("cmplx_DEC_Decision")) {
						 if(formObject.getWFActivityName().equalsIgnoreCase("CAD_Analyst1"))	
						 {
							...
							SKLogger_CC.writeLog(" In CC Reject Application VALChanged---New Value of CAD_dec is: ", formObject.getNGValue("cmplx_DEC_Decision"));
							...
							SKLogger_CC.writeLog(" In CC Reject Application VALChanged---New Value of decision is: ", formObject.getNGValue("cmplx_DEC_Decision"));
						 	  }
					 	}
	...
	public void initialize() {
		SKLogger_CC.writeLog("PersonnalLoanS>  CC Reject Application", "Inside PL PROCESS initialize()" );
		  formObject =FormContext.getCurrentInstance().getFormReference();
	...
	public void saveFormCompleted(FormEvent pEvent) throws ValidatorException {
		SKLogger_CC.writeLog("PersonnalLoanS>  CC Reject Application", "Inside PL PROCESS saveFormCompleted()" + pEvent.getSource());
		  formObject =FormContext.getCurrentInstance().getFormReference();
	...
	public void saveFormStarted(FormEvent pEvent) throws ValidatorException {
		  formObject =FormContext.getCurrentInstance().getFormReference();
		SKLogger_CC.writeLog("PersonnalLoanS>  CC Reject Application", "Inside PL PROCESS saveFormStarted()" + pEvent.getSource());
	...
	public void submitFormCompleted(FormEvent pEvent) throws ValidatorException {
		SKLogger_CC.writeLog("PersonnalLoanS>  CC Reject Application", "Inside PL PROCESS submitFormCompleted()" + pEvent.getSource());
		
	}

	
	public void submitFormStarted(FormEvent pEvent) throws ValidatorException {
		SKLogger_CC.writeLog("PersonnalLoanS>  CC Reject Application", "Inside PL PROCESS submitFormStarted()" + pEvent.getSource());
