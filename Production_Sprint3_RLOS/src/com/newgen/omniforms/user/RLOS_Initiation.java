...
//cmplx_IncomeDetails_AccomodationValue
public class RLOS_Initiation extends RLOSCommon implements FormListener
{
	...
	public static int validate_count = 0;
...
branch
	...
	public void formPopulated(FormEvent pEvent) {
		...
		RLOS.mLogger.info("@Shivang :: Inside form Populated ");
		otp_count=0;//SVT point
		validate_count=0;
		try{
		...
		// added by Pooja for customer consent
			...
			formObject.setEnabled("cmplx_Customer_RM_TL_NAME", true);	//PCASI - 2694
			 ...
			 formObject.setLocked("cmplx_Customer_RM_TL_NAME", false); //PCASI - 2694
			 
			 formObject.setEnabled("cmplx_Customer_DSA_Code", true);	//PCASI - 3222
			 formObject.setEnabled("Customer_Button2", true);	//PCASI - 3222
			 formObject.setLocked("cmplx_Customer_DSA_Code", false); //PCASI - 3222
			 ...
			 formObject.setLocked("cmplx_Customer_Sourcing_Branch_Code", false);//PCASI - 3589 by Alok
	...
	public void call_Fragement_Loaded(ComponentEvent pEvent)
	{
		...
		if(NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_M").equalsIgnoreCase(formObject.getNGValue("initiationChannel")) ){
			...
			/*	formObject.setLocked("cmplx_Customer_Title",false);
				...
				formObject.setLocked("cmplx_Customer_tot_fd_amt",false);
				...
				//Added by shivang for PCASP-2052
				if(EID!=""){
				...
				if("Branch_Init".equalsIgnoreCase(formObject.getWFActivityName()))
				{
				...
				if("PA".equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid", 0, 2)) || "Self Employed".equalsIgnoreCase(EmploymentType))
				{
...
*/
				...
				try{
					...
					if((NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_SelfEmployed").equalsIgnoreCase(formObject.getNGValue("EmploymentType")))||(NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_SE").equalsIgnoreCase(formObject.getNGValue("EmploymentType")))){
						...
						formObject.setNGValue("cmplx_Liability_New_AECBCompanyconsentAvail", formObject.getNGValue("cmplx_Customer_Aecb_Consent_Recieved"));
						...
						if (subprod !=null && application_type!=null &&( "SE".equalsIgnoreCase(subprod) && "BAU".equalsIgnoreCase(application_type))
								|| "BTC".equalsIgnoreCase(subprod)
								|| ("PA".equalsIgnoreCase(subprod) && "RELT"!=application_type && "RELTN"!=application_type )){
							formObject.setVisible("Liability_New_Label27",true);
							formObject.setVisible("cmplx_Liability_New_NoOfBorrowing_rel",true);
							formObject.setVisible("Liability_New_Label10",true);
							formObject.setVisible("cmplx_Liability_New_NoofotherbankCompany",true);
						}
						if (subprod !=null && "BTC".equalsIgnoreCase(subprod)){
						...
						formObject.setVisible("Liability_New_Label27",false);
						formObject.setVisible("cmplx_Liability_New_NoOfBorrowing_rel",false);
						//change by saurabh on 8th Nov 2017.
				...
				if(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,2).equalsIgnoreCase("BPA")){
					...
					LoadPickList("appType", "select '--Select--' as description,'' as code union select convert(varchar, description),code from NG_MASTER_ApplicantType with (nolock) where isActive='Y' order by code");
					...
					*/
					...
					formObject.setEnabled("eff_lob_date", true); //PCASP-2722
					formObject.setLocked("eff_lob_date", false); //PCASP-2722
				}catch(Exception ex){
				...
				if("Telesales_Init".equalsIgnoreCase(formObject.getWFActivityName()) && "IM".equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid", 0, 2))){
					formObject.setVisible("MarketingCode", true);
					formObject.setVisible("cmplx_EmploymentDetails_marketingcode", true);
					...
					formObject.setVisible("EMploymentDetails_Label27", true);
					formObject.setVisible("cmplx_EmploymentDetails_DepartmentName", true);
				}
				else{
					//Deepak Changes for IM APP form new fields 06-July-22
					formObject.setVisible("EMploymentDetails_Label27", false);
					formObject.setVisible("cmplx_EmploymentDetails_DepartmentName", false);
				}
				...
				int prd_rowCount=formObject.getLVWRowCount("cmplx_Product_cmplx_ProductGrid");
				if(prd_rowCount>0 && !(formObject.getNGValue("cmplx_EligibilityAndProductInfo_Tenor") != null && formObject.getNGValue("cmplx_EligibilityAndProductInfo_Tenor")!="")){
				...
				formObject.setLocked("cmplx_EligibilityAndProductInfo_LoanProcessingVATPercent",true);//by shweta for VAT fields
				formObject.setLocked("cmplx_EligibilityAndProductInfo_InsuranceVATPercent",true);
			...
			//	LoadPickList("AlternateContactDetails_carddispatch", "select '--Select--' as description,'' as code union all select convert(varchar,description),code from NG_MASTER_Dispatch order by code ");// Load picklist added by aman to load the picklist in card dispatch to					
				...
				try{
					...
					}//cmplx_CardDetails_CompanyEmbossingName
				...
				LoadPickList("relationship", "select '--Select--' as description,'' as code union select convert(varchar, description),code from NG_MASTER_Relationship with (nolock) where isSupplementary='Y' order by code");
				...
				if(formObject.getNGValue("nationality").equalsIgnoreCase("AE")){
			...
			// Added By Pooja
			else if ("CustomerConsent".equalsIgnoreCase(pEvent.getSource().getName()))
			{
				...
				RLOS.mLogger.info("ResidentNonResident"+ formObject.getNGValue("cmplx_Customer_ResidentNonResident"));
				...
				RLOS.mLogger.info("CTR"+ formObject.getNGValue("OECD_CountryTaxResidence"));
			}
			...
			if ("CustomerConsent".equalsIgnoreCase(pEvent.getSource().getName())) {
	...
	public void saveFormStarted(FormEvent pEvent) 
	{
		...
		try{
			String Account  = formObject.getNGValue("Account_Number");
			RLOS.mLogger.info("Account NUmber : " + Account);
			String mask_acc = maskNumber(Account,"xxxxxxx######");
			RLOS.mLogger.info("outside masked  :"+mask_acc);
			formObject.setNGValue("mask_acc_no",mask_acc);
			RLOS.mLogger.info("outside after masked  :"+mask_acc);
			}
			catch(Exception e){
				RLOS.mLogger.info("Exception occured in account set: "+ e.getMessage());
			}
		formObject.setNGValue("Created_By",formObject.getUserName());
		...
		if(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,6).equalsIgnoreCase("Salaried")){
			...
			formObject.setNGValue("COmpany_Name", formObject.getNGValue("cmplx_EmploymentDetails_EmpName"));//added By Akshay on 16/9/17 to set header
		}
		else if(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,6).equalsIgnoreCase("Self Employed")){
		formObject.setNGValue("Employer_Name", formObject.getNGValue("cmplx_CompanyDetails_cmplx_CompanyGrid",1,4));//added By Akshay on 16/9/17 to set header
	...
	throws ValidatorException {
		...
		{
			if(!(formObject.getNGValue("cmplx_EligibilityAndProductInfo_InterestRate").equals(formObject.getNGValue("cmplx_EligibilityAndProductInfo_promo_interest_rate"))))
				{
	...
	/*	//hritik 2694 
		if(formObject.getNGValue("cmplx_Customer_RM_TL_NAME")=="" || formObject.getNGValue("cmplx_Customer_RM_TL_NAME")==null ){
			String alert_msg="Please select the RM/sTL Name.";
		...
		if("IM".equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,2)))
		{
			...
			{
				String alert_msg="Customer Does not have a active card, Not applicable for IM!";
				throw new ValidatorException(new FacesMessage(alert_msg));
			}
		}
		//Added by Shivang to handle the blank age for PACSP-2118
		if(formObject.getNGValue("cmplx_Customer_age")=="" || formObject.getNGValue("cmplx_Customer_age")==null){
			String alert_msg="Customer Age is blank, Please Re-select Customer DOB!";
			throw new ValidatorException(new FacesMessage(alert_msg));
		}
		...
		if("RESC".equalsIgnoreCase(formObject.getNGValue("Application_Type")) || "RESN".equalsIgnoreCase(formObject.getNGValue("Application_Type")) || "RESR".equalsIgnoreCase(formObject.getNGValue("Application_Type")))
			...
			//Changed by shivang for PCASP-2431 on 27th May
			if(formObject.getNGValue("initiationChannel")==""){
				formObject.setNGValue("initiationChannel","Branch_Init");//added by akshay on 17/4/18	
			}
			else{
				formObject.setNGValue("initiationChannel",formObject.getNGValue("initiationChannel"));
			}
		...
		//Commented for PL Process Need to apply condition here for PL 
		//by shweta added cc waiver and product condition for PL cases
		if(formObject.getNGValue("Product_Type").contains(NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_CreditCard")))
		{
			Check_EFC_Limit();
		} 
		//Comment End here 
