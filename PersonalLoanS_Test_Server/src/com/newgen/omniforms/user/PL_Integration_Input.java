...
//								PersonalLoanS.mLogger.info("RLOS"+"itr_value: Key: "+ entry.getKey()+" Value: "+entry.getValue());
  ...
  /*commented by sagarika due to dupliacte condition
	...
	***********************************************************************************  */
				...
				if(formObject.getLVWRowCount(lvName)>0){
					for(int i=0;i<formObject.getLVWRowCount(lvName);i++){
						...
						if("ICCS".equalsIgnoreCase(formObject.getNGValue(lvName,i,1))){
							if(monthsDiff(formObject.getNGValue(lvName,i,4))<=3){
								ICCS_3++;
							}
							else if(monthsDiff(formObject.getNGValue(lvName,i,4))<=6 && monthsDiff(formObject.getNGValue(lvName,i,4))>3){
								ICCS_6++;
							}
						}
	...
	public int monthsDiff(String diffDate){
		try{
			...
			for (List<String> mylist : OutputXML) {
				...
				if("RejectedDetails".equalsIgnoreCase(tag_name)){
					...
					if(!formObject.isVisible("FinacleCore_Frame6")){
						...
						if(formObject.getLVWRowCount("cmplx_FinacleCore_DDSgrid")>0){
							for(int i=0;i<formObject.getLVWRowCount("cmplx_FinacleCore_DDSgrid");i++){
								if("ICCS".equalsIgnoreCase(formObject.getNGValue("cmplx_FinacleCore_DDSgrid",i,1)) && monthsDiff(formObject.getNGValue("cmplx_FinacleCore_DDSgrid",i,4))<=3){
									count++;
								}
							}
						}
					...
					if(!formObject.isVisible("FinacleCore_Frame6")){
						...
						if(formObject.getLVWRowCount("cmplx_FinacleCore_DDSgrid")>0){
							for(int i=0;i<formObject.getLVWRowCount("cmplx_FinacleCore_DDSgrid");i++){
								if("DDS".equalsIgnoreCase(formObject.getNGValue("cmplx_FinacleCore_DDSgrid",i,1)) && monthsDiff(formObject.getNGValue("cmplx_FinacleCore_DDSgrid",i,4))<=3){
									count++;
								}
							}
						}
		...
		try{
			if(!selfEmpcase){
				...
				try{
					...
					Empdetails_xml_str += "<SalaryTransferFlag>"+formObject.getNGValue("cmplx_IncomeDetails_SalaryXferToBank")+"</SalaryTransferFlag>";
