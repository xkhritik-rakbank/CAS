	...
	public void call_Fragement_Loaded(ComponentEvent pEvent)
	{
		...
		if(NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_M").equalsIgnoreCase(formObject.getNGValue("initiationChannel")) ){
			...
			if ("Customer".equalsIgnoreCase(pEvent.getSource().getName())) {
				...
				if(NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_RESCE").equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid", 0, 4))){
...
*/
				...
				String queryTakeOver="select Sum(isNull(convert(float,Take_Amount),0)) from ng_rlos_cust_extexpo_LoanDetails where   Wi_Name='"+formObject.getWFWorkitemName()+"'";
	...
	public void submitFormStarted(FormEvent pEvent)
		...
		//formObject.setNGValue("cmplx_DecisionHistory_Remarks","");--commented by akshay
		int itemIndex=formObject.getWFFolderId();
		formObject.setNGValue("NewApplicationNo", itemIndex);
		formObject.setNGValue("ApplicationRefNo", itemIndex);
