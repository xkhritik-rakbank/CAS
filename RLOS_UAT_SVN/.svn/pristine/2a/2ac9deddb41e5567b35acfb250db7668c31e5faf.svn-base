...
branch
	...
	public void formPopulated(FormEvent pEvent) {
		...
		try{
			...
			String Query="select branch from NG_MASTER_SourceCode where UserId='"+formObject.getUserName()+"'";
	...
	public void call_Fragement_Loaded(ComponentEvent pEvent)
	{
		...
		if(NGFUserResourceMgr_RLOS.getGlobalVar("RLOS_M").equalsIgnoreCase(formObject.getNGValue("initiationChannel")) ){
			...
			if ("Customer".equalsIgnoreCase(pEvent.getSource().getName())) {
				...
				formObject.setEnabled("cmplx_Customer_card_id_available",false);//temp Changed to false for Sprint 1
				...
				LoadPickList("ReqProd", "select '--Select--' union select convert(varchar, description) from NG_MASTER_RequestedProduct with (nolock) where processname='"+formObject.getWFProcessName()+"' and isActive='Y'");//changed by akshay on 3/1/17
	...
	throws ValidatorException {
		...
		try{
			...
			formObject.saveFragment("DecisionHistoryContainer");//added by akshay on 18/9/18
