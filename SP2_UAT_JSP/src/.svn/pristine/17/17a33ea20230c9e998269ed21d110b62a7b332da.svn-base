...
<%
	...
	try{
		String sQuery1 = null,sQuery2 = null,sQuery_Common =null,sQuery_AecbHistory=null,sQuery_Sum=null;
		
		sQuery_Common = "select isNull(Total_Exposure,'-') as Total_Exposure,isNull(WorstCurrentPaymentDelay,'-') as WorstCurrentPaymentDelay,isNull(Nof_Records,'-') as Nof_Records, isNull(TotalOutstanding,'-') as TotalOutstanding,isNull(TotalOverdue,'-') as TotalOverdue,isNull(NoOfContracts,'-')as NoOfContracts,(SELECT count(*) FROM ng_rlos_FinancialSummary_ReturnsDtls with (nolock) WHERE CAST(returnDate AS datetime) between GETDATE()-90 and GETDATE() and returntype='ICCS' and  Child_Wi='"+Wi_Name+"') as no_of_chq_ret_3months,(SELECT count(*) FROM ng_rlos_FinancialSummary_ReturnsDtls with (nolock) WHERE CAST(returnDate AS datetime) between GETDATE()-180 and GETDATE() and returntype='ICCS' and  Child_Wi='"+Wi_Name+"') as no_of_chq_ret_6months,(SELECT count(*) FROM ng_rlos_FinancialSummary_ReturnsDtls with (nolock) WHERE CAST(returnDate AS datetime) between GETDATE()-90 and GETDATE() and returntype='DDS' and  Child_Wi='"+Wi_Name+"') as no_of_DDS_ret_3months,(SELECT count(*) FROM ng_rlos_FinancialSummary_ReturnsDtls with (nolock) WHERE CAST(returnDate AS datetime) between GETDATE()-180 and GETDATE() and returntype='DDS' and  Child_Wi='"+Wi_Name+"') as no_of_DDS_ret_6months,AggrExposure as  AggrExposure from  NG_RLOS_CUSTEXPOSE_Derived der with (nolock) INNER JOIN ng_RLOS_Liability_New liab with (nolock) ON (der.Child_Wi=liab.wi_name) where Child_Wi='"+Wi_Name+"' and Request_Type = 'CollectionsSummary'";
		//++below code added by nikhil for PCAS-1298 CR
		...
		sQuery_Sum = "select Sum(Total_liability) as Total_liability,cast(sum(cast(TotalOustsatnding as real)) as numeric(36,2)) as TotalOustsatnding,cast(sum(cast(TotalOverdue as real)) as numeric(36,2))  as TotalOverdue from ( select COUNT(child_wi) as Total_liability,SUM(CAST(ISNULL(OutstandingAmt,'0') AS FLOAT)) AS TotalOustsatnding,SUM(CAST(ISNULL(OverdueAmt,'0') AS FLOAT)) AS TotalOverdue  from ng_RLOS_CUSTEXPOSE_CardDetails with (nolock) where  Child_wi  = '"	+ Wi_Name+ "'  and CardStatus='A' union all select COUNT(child_wi) as Total_liability,SUM(CAST(ISNULL(TotalOutstandingAmt,'0') AS FLOAT)) AS TotalOustsatnding,SUM(CAST(ISNULL(OverdueAmt,'0') AS FLOAT)) AS TotalOverdue  from ng_RLOS_CUSTEXPOSE_LoanDetails with (nolock) where Child_wi  = '"+ Wi_Name + "' and (LoanStat='A'or LoanStat='ACTIVE')  union all select COUNT(child_wi) as Total_liability,SUM(CAST('0' AS FLOAT)) AS TotalOustsatnding,SUM(CAST('0' AS FLOAT)) AS TotalOverdue  from ng_RLOS_CUSTEXPOSE_AcctDetails with (nolock) where Child_wi  = '"+ Wi_Name + "' and AcctStat='ACTIVE' and ODType!='') as Int_expo";
...
<fieldset>
  ...
  <table cellspacing="10" cellpadding="5" id = 'table1'>
	...
	<tr>
		...
		<td style="padding:0 15px 0 15px;"><input type="text" name="full_name"  disabled value="<%=Sum_Total_Out%>" ></td> 
		<td style="padding:0 15px 0 15px;"><input type="text" name="full_name"  disabled value='<%=Sum_Total_OverDue%>' ></td>
	    <td style="padding:0 15px 0 15px;"><input type="text" name="full_name"  disabled value=<%=NoOfContracts%> ></td> 
	    <td style="padding:0 15px 0 15px;"><input type="text" name="full_name"  disabled value=<%="-"%> ></td> 
	</tr>
	...
	<tr>
		...
		<td style="padding:0 15px 0 15px;"><input type="text" name="full_name" disabled value='<%=Sum_Total_No%>'></td>
...
<fieldset>
  ...
  <table cellspacing="10" cellpadding="5" id = 'table2'>
	...
	<tr>
		...
		<td style="padding:0 15px 0 15px;"><input type="text" name="full_name"   disabled value="<%=MobAecb%>" ></td>
	...
	<tr>
	     <td style="padding:0 15px 0 15px;"><input type="text" name="full_name" value ='<%=Internal_WriteOff_Check%>' disabled></td>
		  <td style="padding:0 15px 0 15px;"><input type="text" name="full_name" value ='<%=DPD_60_in_last_12_months%>' disabled></td>
	</tr>
