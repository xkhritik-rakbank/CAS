 ...
 function cifIntegration(req_name,response)
		...
		function check_complete(){
			...
			if(document.getElementById("dyn_tbl_row")!=undefined){
					...
					if(return_flag!='Incomplete' && ( (req_name.indexOf('ExternalExposure')>-1) && NTB=='true')){
						document.getElementById("continue_btn").style.visibility="hidden";
						document.getElementById("Close_btn").style.visibility="visible";
						}
		...
		function continue_next(){
			...
			if(req_name.indexOf('InternalExposure')>-1 && !window.opener.document.getElementById("cmplx_Customer_NTB").checked){
			...
			else if(req_name.indexOf('CARD_INSTALLMENT_DETAILS')>-1 && !window.opener.document.getElementById("cmplx_Customer_NTB").checked){
				req_name="";
			}
			else{
				document.getElementById("Close_btn").style.visibility="visible";
		...
		function final_close(){
			...
			if(req_name.indexOf("ExternalExposure")==-1)
				{
