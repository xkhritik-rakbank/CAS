...
<%@ page import="com.newgen.dmsapi.DMSXmlList,com.newgen.dmsapi.DMSXmlResponse"%>
...
<%!
	...
	public String readTextFile(String fileName,String filePath,String cabinetName,String wrapperIP,String wrapperPort,String appServerType,String sessionId) 
	{
		...
		String totalrecords="";
				...
				if(strLine.substring(0, 1).equalsIgnoreCase("T")){					
					...
					if(columnName.length==values.length){
						for(int i=0;i<values.length;i++){
							mymap.put(columnName[i],values[i]);
						}
						...
						Warranty_CRNNo_map.put(mymap.get("Murhabha_WIName")+","+mymap.get("Card_Product"),mymap.get("CRN_no"));
			...
			sQuery="SELECT  (murhabha_winame+','+card_product) AS WIname_CardProduct,amount,crn_no FROM ng_rlos_Murabha_Warranty with(nolock) WHERE File_Generated_Name='"+fileName.split("\\.")[0]+"'";
			...
			if(mainCode.equalsIgnoreCase("0")){
				...
				for(int i=0;i<Integer.parseInt(totalrecords);i++)
				{
					...
					WIname_CardProduct=objXmlParser.getValueOf("WIname_CardProduct");
					if(Warranty_CRNNo_map.containsKey(WIname_CardProduct)){
						WriteLog("Warranty_CRNNo_map: "+Warranty_CRNNo_map.get(WIname_CardProduct));
						if(!Purchase_CRNNo.equalsIgnoreCase(Warranty_CRNNo_map.get(WIname_CardProduct))){
				...
				if(mainCode.equalsIgnoreCase("0")){
					sQuery="select * from ng_rlos_Murabha_Warranty_temp with(nolock)";
