...
<%
	...
	{	
	 ...
	 prod = ESAPI.encoder().encodeForSQL(new OracleCodec(), input7);
	 ...
	 subprod = ESAPI.encoder().encodeForSQL(new OracleCodec(), input8);
	...
	var parentWiName = ($ESAPI.encoder().decodeForHTML(parentWiName_encode));
		...
		function integ( btn_id ) {
			...
			var finalxmlResponse = null;
				$.ajax({
					...
					data: { wi_name:parentWiName,  child_wi: wi_name, WD_UID : WD_UID} ,
					dataType: "text",
					async: true,
		...
		function set_result_val(result,cardNo)
		{
			$.ajax({
					...
					data: { result: result, wi_name : wi_name ,accNo:cardNo,parentWiName:parentWiName, WD_UID : WD_UID},
					dataType: "text",
					async: true,
					success: function (response) {
				...
				var req_name_arr = req_name.split(',');
					for (var i=0;i<req_name_arr.length;i++)
					{
						...
						$.ajax({
							...
							data: { request_name: req_name_arr[i], is_mobility : "N", SeesionId:sessionId ,wi_name:wi_name ,activityName:activityName ,param_json:param_json, cardNo:cardNo,header:'<%=headerOp.get("CARD_SUMMARY")%>',maincode:'<%=maincodeOp.get("CARD_SUMMARY")%>',footer:'<%=footerOp.get("CARD_SUMMARY")%>',parentTagName:'<%=parentTagNameOp.get("CARD_SUMMARY")%>', WD_UID : WD_UID} ,
							dataType: "text",
							async: true,
							success: function (response) {
