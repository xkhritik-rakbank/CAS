...
function saveDate() {
		...
		for(var row_arr_count= 1;row_arr_count<=totalrowid;row_arr_count++){
				...
				//changed by nikhil for PCSP-883
				if(document.getElementById(CAC_Indicator).checked==true && document.getElementById(avg_utilization).value=='' && activityName!='CA' )
				{
				//change by saurabh for PCSP-123
				//below code for PCSP-683
					window.parent.showAlert('ExtLiability_IFrame_external','Please fill Average utilization in External Liabilities section with Provider Number :'+document.getElementById(Provider_Number).innerHTML);
...
function openPopup(id)
	...
	var url='/webdesktop/custom/CustomJSP/external.jsp?Wi_Name='+winame+'&AgreementId='+agreementval+'&table='+tablename+'&WD_UID='+WD_UID;
