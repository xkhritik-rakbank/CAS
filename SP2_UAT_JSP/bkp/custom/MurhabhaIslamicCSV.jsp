...
function call_Generate_CSV()
{
	...
	if(Object.keys(checkAllRecords).length>0){
		...
		$.ajax({
					...
					async: false,
					success: function (response) {
						if(trimStr(response)=='CSV file already generated for this wi!!' || trimStr(response)=='No Workitem to process' || trimStr(response).includes("invalid") || trimStr(response).includes("error"))
						{
