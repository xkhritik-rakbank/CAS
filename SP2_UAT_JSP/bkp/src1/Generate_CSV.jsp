				...
				if(!total_records.equals("")){
					...
					if (status.equalsIgnoreCase("Success"))
					{
						for(int i=0;i<Wi_Name_array.length;i++)	
						{
							...
							sInputXML=ExecuteQuery_APUpdate(tableName,"File_Generated_Name,File_Generated_Date,Login_User_id,status,Trade_DATE,Settlement_Date,Warranty_Expiry_Date","'"+File_Generated_Name+"','"+timestamp+"','"+userName+"','File Generated','"+Trade_DATE+"','"+Trade_DATE+"','"+Warranty_Expiry_Date+"'",sWhere,cabinetName,sessionId);
...
public static String getTagValue(String parseXml,String tagName, String subTagName)
    {
		...
		String attributeList[];
		 ...
		 {
			 attributeList=entry.getValue().split("~");
			 columnName= attributeList[0];
			 colValue= attributeList[1];
					
			 WriteLog("commonParse columnName set: "+columnName);
			 WriteLog("commonParse colValue set: "+colValue);
			 
		 }
			    
	    WriteLog("commonParse attribute set: ");
		return colValue;
...
public  String Generate_File(String stotalOutputXML,String columnList,String fileName,String userName)
{
	...
	try{		
		...
		{		
			...
			List<String> result=parseXML(sOutputXML.replaceAll("\\s",""));	//to remove spaces from xml
