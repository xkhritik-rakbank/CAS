...
public class CC_DDVT_maker extends CC_Common implements FormListener
{
	...
	private void fragment_loaded(FormEvent pEvent,FormReference formObject)
	{
		...
		//	formObject.setLocked("AddressDetails_Frame1",true);
			...
			//++below code added by nikhil to fix ddvt issues
			formObject.setTop("DecisionHistory_save", 290);
			formObject.setTop("DecisionHistory_ReferTo", 25);
			formObject.setLeft("DecisionHistory_ReferTo", 750);
			formObject.setLeft("DecisionHistory_Label1", 750);
		
		*/
		//Arun commented 17/12/17
		...
		/*else if ("CardDetails".equalsIgnoreCase(pEvent.getSource().getName())) {
			...
			// added by abhishek as per CC FSD
			if(NGFUserResourceMgr_CreditCard.getResourceString_ccIfConditions("CC_Salaried").equalsIgnoreCase(formObject.getNGValue("cmplx_Product_cmplx_ProductGrid",0,6))){
		...
		}--commented by akshay on 11/9/17---pids dont exist*/
