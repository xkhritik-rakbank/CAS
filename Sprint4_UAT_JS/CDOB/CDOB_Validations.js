...
if(!checkFor_SupplemntGrid()){
	...
	//below code addded for JIRa Income/employment check
		...
		else if(desc=='Refer to Smart CPV')
		{
		 ...
		 //validation for customer detail verification CPV changes 16-04 Changed on 17/8/21 - CPV Changes Req 1 (hidden)
...
//below code commented by akshay on 23/11/18
	...
	*/
	...
	if(activityName=='FCU'|| activityName=='DSA_CSO_Review' || activityName=='CPV_Analyst' || activityName=='RM_Review'){
...
function Income_Save_Check()
	...
	if(Principal_Approval=='Y' && activityName=='CSM')
	{
		...
		if(ftsReceived=='Y'){
			if(isFieldFilled('cmplx_IncomeDetails_AvgNetSal')==false)
			{
				showAlert('cmplx_IncomeDetails_netSal1','Average Net Salary cannot be null if FTS is received');
				return false;
			}
