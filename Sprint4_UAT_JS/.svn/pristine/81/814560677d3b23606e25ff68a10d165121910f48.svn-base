...
function formPopulated()
{
	...
	if(processName=='RLOS'){
		...
		load_PropertyFile();//added by akshay on 27/10/17 for loading property file
			...
			/*if(getNGValue('PrimaryProduct')!='Credit Card' && getNGValue('PrimaryProduct')!='Personal Loan'){
...
*/		
	 ...
	 else if (processName == 'DigitalOnBoarding')
	 {
...
function eventTabClick(tabid,sheetid)
{
    var pname = window.parent.strprocessname;
    var activityname=window.parent.stractivityName;
	//added below for pcasf-460 by saurabh1
	if (pname == 'PersonalLoanS' || pname == 'CreditCard' || pname =='DigitalOnBoarding'){
